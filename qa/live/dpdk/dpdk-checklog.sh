...
# Install trap early so all temp files are cleaned up on any exit path.
TMPFILES=()
...
SURI_EXIT=0
# $SECONDS is a bash builtin: total seconds since shell start (does not wrap at 60).
T0=$SECONDS
while [ $((SECONDS - T0)) -lt "$TIMEOUT_SEC" ]; do
