 ...
 *
 * \author Marc Norton
 * \author Preston Lewis <preston.lewis@baesystems.com>
 * \author Marcos Rodriguez <marcos.rodriguez@baesystems.com>
 *
...
static unsigned getdword(unsigned  char * p ) {
    unsigned w;
...
/*
  ...
  returns:
    ...
    > 0 peoffset, header processed
    offset : mz offset

    wp     : pescan data structure
    buf    : buffer to scan
    nbuf   : bytes in buf
...
int pescan( peattrib_t * wp, unsigned char * buf, int nbuf, int debug ) {
    ...
    if( (unsigned) wp->peoffset + 3 >= nleft ) {
        wp->trunc=1;
        return PE_INDETERMINATE;
    }

    // Verify PE
    if( (mz[wp->peoffset]!='P') || (mz[wp->peoffset+1]!='E') ||
            (mz[wp->peoffset+2]!=0) || (mz[wp->peoffset+3]!=0))
        return PE_NOT_PE;
    ...
    //nuff buffer for exp/imp tables ??
