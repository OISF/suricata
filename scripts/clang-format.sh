    ...
    if [ "${changes}" != "no modified files to format" -a \
        ...
        if [ $quiet -ne 1 ]; then
            ...
            if [ $from_make -eq 1 ]; then
                ...
                Error "Use ${italic}make style-rewrite-branch${normal} or ${italic}make style-branch${normal} to fix formatting"
            else
...
echo "Found clang-format version: $clang_format_version"
