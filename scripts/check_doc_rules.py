...
and prints normalized Suricata rules.
"""
...
def _collect_container_body(lines: List[str], start_idx: int) -> Tuple[str, int]:
    container_indent = _indent_width(lines[start_idx])
    body_lines: List[str] = []
    i = start_idx + 1
    ...
    while i < len(lines):
        ...
        if _indent_width(line) <= container_indent:
            break
...
def extract_rules_from_rst(path: Path) -> Iterator[Tuple[str, int]]:
    ...
    while i < len(lines):
        if lines[i].strip() == ".. container:: example-rule":
            block_text, i = _collect_container_body(lines, i)
            if block_text:
                cleaned = _clean_rule_text(block_text)
                yield cleaned, i
            continue
        i += 1
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Extract Suricata rules from doc RST example-rule containers."
    )
    parser.add_argument(
        "doc_path",
        nargs="?",
