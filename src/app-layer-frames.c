...
{
    SCLogDebug("[%s] %p: frame: %p type %u id %" PRIi64 " flags %02x rel_offset:%d, len:%" PRIi64
               ", events:%u %u/%u/%u/%u",
            prefix, frames, frame, frame->type, frame->id, frame->flags, frame->rel_offset,
        ...
        const uint64_t frame_start, const int64_t len, int dir, uint8_t frame_type)
{
    ...
    const uint64_t frame_start_rel = frame_start - stream_slice->offset;
#ifdef DEBUG
        ...
        const Frame *frame, const uint64_t abs_offset, const uint64_t abs_right_edge)
{
    ...
    if ((uint64_t)frame_right_edge <= abs_right_edge) {
        SCLogDebug("frame %p is done", frame);
        return true;
    }
    return false;
}
...
// TODO GAPS
// TODO limit to ACK'd data in IDS mode
static void FramePrune(Frames *frames, TcpStream *stream)
{
    const uint64_t abs_offset = STREAM_BASE_OFFSET(stream) + (uint64_t)frames->progress_rel;
    const uint8_t *_data;
    uint32_t data_len;

    if (StreamingBufferGetDataAtOffset(&stream->sb, &_data, &data_len, abs_offset) == 0) {
        return;
    }
    ...
    for (uint16_t i = 0; i < frames->cnt; i++) {
        if (i < FRAMES_STATIC_CNT) {
            ...
            if (FrameIsDone(frame, abs_offset, abs_offset + (uint64_t)data_len)) {
                // remove by not incrementing 'x'
                SCLogDebug("removing %p", frame);
                FrameClean(frame);
            } else {
