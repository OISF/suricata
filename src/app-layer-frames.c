...
static void FrameDebug(const char *prefix, const Frames *frames, const Frame *frame)
{
    ...
    const char *type_name =
            frames ? AppLayerParserGetFrameNameById(frames->ipproto, frames->alproto, frame->type)
                   : "<unknown>";
    SCLogDebug("[%s] %p: frame: %p type %u/%s id %" PRIi64 " flags %02x rel_offset:%d, len:%" PRIi64
               ", events:%u %u/%u/%u/%u",
...
static Frame *FrameNew(Frames *frames, uint32_t rel_offset, int64_t len)
{
    BUG_ON(frames == NULL);
...
{
    //    const int64_t base_offset = STREAM_BASE_OFFSET(stream);
    const int64_t app_progress = STREAM_APP_PROGRESS(stream);
    BUG_ON(base_offset > app_progress);

    const int64_t frame_offset = base_offset + (int64_t)frame->rel_offset;
...
static int FrameSlide(const char *ds, Frames *frames, const TcpStream *stream, const uint32_t slide)
{
    ...
    for (uint16_t i = 0; i < frames->cnt; i++) {
        if (i < FRAMES_STATIC_CNT) {
            ...
            if (frame->len >= 0 &&
                    frame->rel_offset + frame->len <= (int32_t)slide) { // TODO check seems off
            ...
            if (frame->len >= 0 && frame->rel_offset + frame->len <= (int32_t)slide) {
                // remove by not incrementing 'x'
                SCLogDebug("removing %p id %" PRIi64, frame, frame->id);
                FrameClean(frame);
                removed++;
        ...
        const uint8_t *frame_start, const int64_t len, int dir, uint8_t frame_type)
{
    ...
    uint64_t abs_frame_offset = stream_slice->offset + (uint64_t)ptr_offset;
    uint64_t rel_offset = abs_frame_offset - STREAM_BASE_OFFSET(stream);

    Frame *r = FrameNew(frames, (uint32_t)rel_offset, len);
    if (r != NULL) {
        r->type = frame_type;
    }
    return r;
        ...
        const uint32_t frame_start_rel, const int64_t len, int dir, uint8_t frame_type)
{
    ...
    uint32_t rel_offset = frame_base_offset;
#ifdef DEBUG
    const char *type_name = AppLayerParserGetFrameNameById(f->proto, f->alproto, frame_type);
    SCLogDebug("flow %p direction %s frame offset %u rel_offset %u (abs %" PRIu64
