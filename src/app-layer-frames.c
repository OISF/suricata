...
Frame *AppLayerFrameNew(Flow *f, const StreamSlice *stream_slice, const uint8_t *frame_start,
        const uint32_t len, int dir, uint8_t frame_type)
{
    ...
    if (f->protoctx == NULL)
        return NULL;
    if (frame_start < stream_slice->input || frame_start >= stream_slice->input + stream_slice->input_len)
        return NULL;
#endif
...
Frame *AppLayerFrameNew2(Flow *f, const StreamSlice *stream_slice, const uint32_t frame_start_rel,
        const uint32_t len, int dir, uint8_t frame_type)
{
    /* workarounds for many (unit|fuzz)tests not handling TCP data properly */
#if defined(UNITTESTS) || defined(FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION)
