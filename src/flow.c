...
#define SET_DEFAULTS(p, n, e, c, b, ne, ee, ce, be)     \
    ...
    if (flow_timeouts != NULL) {
        ...
        if (proto != NULL) {
            ...
            if (new != NULL &&
                ByteParseStringUint32(&configval, 10, strlen(new), new) > 0) {

                    flow_timeouts_normal[FLOW_PROTO_DEFAULT].new_timeout = configval;
            }
            if (established != NULL &&
                ByteParseStringUint32(&configval, 10, strlen(established),
                                        established) > 0) {

                flow_timeouts_normal[FLOW_PROTO_DEFAULT].est_timeout = configval;
            }
            if (closed != NULL &&
                ByteParseStringUint32(&configval, 10, strlen(closed),
                                        closed) > 0) {

                flow_timeouts_normal[FLOW_PROTO_DEFAULT].closed_timeout = configval;
            }
            if (bypassed != NULL &&
                    ByteParseStringUint32(&configval, 10,
                                            strlen(bypassed),
                                            bypassed) > 0) {
            ...
            if (emergency_new != NULL &&
                ByteParseStringUint32(&configval, 10, strlen(emergency_new),
                                        emergency_new) > 0) {
