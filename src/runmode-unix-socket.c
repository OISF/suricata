...
TmEcode UnixSocketSetMemcap(json_t *cmd, json_t* answer, void *data)
{
    ...
    if (strcmp("global", memcapstr) == 0) {
        if (!GlobalMemcapEnabled()) {
            json_object_set_new(
                    answer, "message", json_string("Cannot update: global memcap disabled"));
            return TM_ECODE_FAILED;
        }
        if (value == 0) {
            json_object_set_new(answer, "message",
    ...
    if (GlobalMemcapEnabled()) {
        for (i = 0; (memcap = MemcapListGetElement(i)); i++) {
            if (strcmp(memcap->name, "global") != 0 && strcmp(memcap->name, memcapstr) != 0 &&
                    memcap->GetFunc) {
        ...
        if (global && value < memcapsum) {
            ...
            snprintf(message, sizeof(message),
                    "global memcap specified is less "
                    "than the memory used: %s. Please try increasing it.",
                    memuse);
            json_object_set_new(answer, "message", json_string(message));
            return TM_ECODE_FAILED;
        } else if (value + memcapsum > GlobalMemcapGetValue()) {
            json_object_set_new(answer, "message", json_string("global memcap limit reached"));
            return TM_ECODE_FAILED;
        }
    }
        ...
        if (strcmp(memcap->name, memcapstr) == 0 && memcap->SetFunc) {
            ...
            if (updated) {
                snprintf(message, sizeof(message), "memcap value for '%s' updated: %" PRIu64 " %s",
                        memcap->name, value, (value == 0) ? "(unlimited)" : "");
                json_object_set_new(answer, "message", json_string(message));
                return TM_ECODE_OK;
            } else {
                if (value == 0) {
                    ...
                    if (memcap->GetMemuseFunc()) {
                        ...
                        snprintf(message, sizeof(message),
                                "%s memcap value specified for '%s' is less than the memory in "
                                "use: %s",
