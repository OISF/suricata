        ...
        const uint8_t *data, uint32_t data_len, uint64_t txid, bstr *rawvalue, HtpTxUserData *htud)
{
    ...
    if (s->file_range == NULL) {
        ...
        DEBUG_VALIDATE_BUG_ON(SC_ATOMIC_GET(file_range_container->hdata->use_cnt) > (uint32_t)INT_MAX);
    ...
    /* in some cases we don't take a reference, so decr use cnt */
        ...
        SCLogDebug("container %p use_cnt %u", s->file_range, SC_ATOMIC_GET(s->file_range->container->hdata->use_cnt));
        DEBUG_VALIDATE_BUG_ON(SC_ATOMIC_GET(s->file_range->container->hdata->use_cnt) > (uint32_t)INT_MAX);
    }
