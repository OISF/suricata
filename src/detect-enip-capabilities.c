...
{
    uint16_t value;
    if (!rs_enip_get_capabilities(txv, &value))
        SCReturnInt(0);
...
void DetectEnipCapabilitiesRegister(void)
{
    ...
    DetectAppLayerInspectEngineRegister2("enip.capabilities", ALPROTO_ENIP, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.capabilities", ALPROTO_ENIP, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectGenericList, NULL);
