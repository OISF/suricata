        ...
        const DetectEngineCtx *de_ctx, const Signature *s, const SignatureInitDataBuffer *b)
{
    ...
    bool has_must_error = false;
    bool has_must_succeed = false;
    ...
    if (only_absent && has_other) {
        ...
        SCLogError("signature with absent: or_else/error_or/must_succeed expects other keywords to "
                   "test on such as content");
        return false;
    }
    if (has_must_error && has_other) {
        SCLogError("absent: must_error cannot be combined with other keywords");
        return false;
    }
    if (has_error_or || has_or_else_only || has_must_error || has_must_succeed) {
        ...
        if (map) {
            ...
            if ((has_error_or || has_must_error || has_must_succeed) && !found_can_fail) {
                SCLogError("absent: error_or/must_error/must_succeed requires a transform that can "
                           "fail (e.g. from_base64)");
                return false;
            }
        ...
        } else if (has_error_or || has_must_error || has_must_succeed) {
            SCLogError("absent: error_or/must_error/must_succeed requires a transform that can "
                       "fail (e.g. from_base64)");
            return false;
        }
