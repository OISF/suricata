...
static int DetectEngineInspectIkeKeyExchangePayloadLengthGeneric(ThreadVars *tv,
        DetectEngineCtx *de_ctx, DetectEngineThreadCtx *det_ctx, const Signature *s,
        const SigMatchData *smd, Flow *f, uint8_t flags, void *alstate, void *txv, uint64_t tx_id);
...
void DetectIkeKeyExchangePayloadLengthRegister(void)
{
    ...
    sigmatch_table[DETECT_AL_IKE_KEY_EXCHANGE_PAYLOAD_LENGTH].desc =
            "match IKEv1 key exchange payload length";
    ...
    DetectAppLayerInspectEngineRegister("ikev1.key_exchange_payload_length", ALPROTO_IKE,
            SIG_FLAG_TOSERVER, 1, DetectEngineInspectIkeKeyExchangePayloadLengthGeneric);

    DetectAppLayerInspectEngineRegister("ikev1.key_exchange_payload_length", ALPROTO_IKE,
            SIG_FLAG_TOCLIENT, 1, DetectEngineInspectIkeKeyExchangePayloadLengthGeneric);

    g_ike_key_exch_payload_length_buffer_id =
            DetectBufferTypeGetByName("ikev1.key_exchange_payload_length");
        ...
        const SigMatchData *smd, Flow *f, uint8_t flags, void *alstate, void *txv, uint64_t tx_id)
{
    return DetectEngineInspectGenericList(
            tv, de_ctx, det_ctx, s, smd, f, flags, alstate, txv, tx_id);
}
