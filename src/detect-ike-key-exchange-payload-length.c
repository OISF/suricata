...
/**
 *   [ikev1.key_exchange_payload_length]:[=|<|>|<=|>=]<length>;
 */
...
static int DetectEngineInspectIkeKeyExchangePayloadLengthGeneric(ThreadVars *tv,
                                                   DetectEngineCtx *de_ctx, DetectEngineThreadCtx *det_ctx,
                                                   const Signature *s, const SigMatchData *smd,
                                                   Flow *f, uint8_t flags, void *alstate,
                                                   void *txv, uint64_t tx_id);

static int DetectIkeKeyExchangePayloadLengthMatch (DetectEngineThreadCtx *, Flow *,
...
{
    sigmatch_table[DETECT_AL_IKE_KEY_EXCHANGE_PAYLOAD_LENGTH].name = "ikev1.key_exchange_payload_length";
    sigmatch_table[DETECT_AL_IKE_KEY_EXCHANGE_PAYLOAD_LENGTH].desc = "match IKEv1 key exchange payload length";
    sigmatch_table[DETECT_AL_IKE_KEY_EXCHANGE_PAYLOAD_LENGTH].url = "/rules/ike-keywords.html#ike-key-exchange-payload-length";
    sigmatch_table[DETECT_AL_IKE_KEY_EXCHANGE_PAYLOAD_LENGTH].AppLayerTxMatch = DetectIkeKeyExchangePayloadLengthMatch;
    sigmatch_table[DETECT_AL_IKE_KEY_EXCHANGE_PAYLOAD_LENGTH].Setup = DetectIkeKeyExchangePayloadLengthSetup;
    ...
    DetectAppLayerInspectEngineRegister("ikev1.key_exchange_payload_length",
                                        ALPROTO_IKE, SIG_FLAG_TOSERVER, 3,
                                        DetectEngineInspectIkeKeyExchangePayloadLengthGeneric);

    DetectAppLayerInspectEngineRegister("ikev1.key_exchange_payload_length",
                                        ALPROTO_IKE, SIG_FLAG_TOCLIENT, 4,
                                        DetectEngineInspectIkeKeyExchangePayloadLengthGeneric);

    g_ike_key_exch_payload_length_buffer_id = DetectBufferTypeGetByName("ikev1.key_exchange_payload_length");
 ...
 * \retval 1 match.
 */
static int DetectIkeKeyExchangePayloadLengthMatch (DetectEngineThreadCtx *det_ctx,
...
/**
 * \brief Function to add the parsed IKE key exchange payload length query into the current signature.
 *
 ...
 * \retval -1 on Failure.
 */
static int DetectIkeKeyExchangePayloadLengthSetup (DetectEngineCtx *de_ctx, Signature *s, const char *rawstr)
{
