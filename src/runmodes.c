...
void RunModeDispatch(int runmode, const char *custom_mode,
    const char *source_plugin_name, const char *source_plugin_args)
{
...
#endif
            case RUNMODE_PLUGIN: {
                SourcePlugin *plugin = SCPluginFindSourceByName(source_plugin_name);
                if (plugin == NULL) {
                    FatalError(SC_ERR_PLUGIN, "No source plugin found with name %s",
                            source_plugin_name);
                }
                plugin->Init(RUNMODE_PLUGIN, TMM_RECEIVEPLUGIN, TMM_DECODEPLUGIN);
