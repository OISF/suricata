...
#define TCP_GET_WSCALE(p)                                                                          \
    (TCP_HAS_WSCALE((p)) ? (((*(uint8_t *)(p)->tcpvars.ws.data) <= TCP_WSCALE_MAX)                 \
        ...
        const uint16_t *shdr, const uint16_t *pkt, uint16_t tlen, uint16_t init)
{
    ...
    csum += shdr[0] + shdr[1] + shdr[2] + shdr[3] + shdr[4] + shdr[5] + shdr[6] + shdr[7] +
            shdr[8] + shdr[9] + shdr[10] + shdr[11] + shdr[12] + shdr[13] + shdr[14] + shdr[15] +
            htons(6) + htons(tlen);
