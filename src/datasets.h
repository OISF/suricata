 ...
 *
 * A dataset is a named in-memory set of values (string, md5,
 * sha256, ipv4 or ipv6) that rules match against with the
 * `dataset` and `datarep` keywords. Sets can be preloaded from
 * disk, persisted on exit, and reloaded at runtime.
 * Implementation lives in datasets.c.
 */
...
/** Reputation value attached to a dataset entry. Opaque to
 *  bindgen; passed and returned via the *wRep functions and
 *  DataRepResultType. */
 ...
 *  call dispatches on set->type to a per-type worker in
 *  datasets.c. If the value already exists the existing entry is
 *  reused. Per-set locking is internal; callers do not touch the
 *  global set list.
 *  @{ */
 ...
 *  \retval -2 data_len wrong for the set's type (never from the
 *             string worker)
 *  \note The IPv6 worker here also accepts a 4-byte buffer,
 *        stored IPv4-mapped. SCDatasetAddwRep is stricter and
 *        requires exactly 16.
 */
 ...
 *  \retval  0 already present; the stored entry (and its existing
 *             rep) is left untouched and the incoming rep is
 *             discarded. First writer wins; reps are never merged.
 *  \retval -1 error (NULL set, hash failure, unknown type)
 *  \retval -2 data_len wrong for the set's type (never from the
 *             string worker)
 ...
 *
 *  Creates each declared set and loads its file. Call once at
 *  startup, after dropping privileges. Fatal config errors call
 *  FatalError() and exit.
 *  \retval 0 on success
 */
 ...
 *
 *  Marks the current sets hidden and stages replacements, so
 *  in-flight lookups can finish against the old versions. Must
 *  be paired with DatasetPostReloadCleanup().
 */
...
/** In-memory dataset.
 *
 *  Sets are linked in a global list guarded by the dataset lock
 *  (DatasetLock/DatasetUnlock); code that walks the list directly
 *  must hold it. Value storage lives in the backing
 *  ::THashTableContext, which is set up at creation with
 *  type-specific hash, compare, set and free callbacks. */
...
/** \brief Map a textual type name to ::DatasetTypes.
 *
 ...
 *
 *  Takes ownership of \p set on success; on failure the caller
 *  still owns it and must clean up. Caller must hold the dataset
 *  lock.
 *
 *  \retval  0 ok
 ...
 *
 *  Zeroes the struct and assigns a unique id. The result has no
 *  type, no name, no hash, and is not in the global list; caller
 *  fills the rest in.
 *
 ...
 *  says "caller must hold the dataset lock", must do so inside a
 *  Lock/Unlock pair. Plain (non-recursive) mutex: taking it twice
 *  from the same thread deadlocks. Does not protect the contents
 *  of individual sets; per-set sync lives in the backing hash.
 */
void DatasetLock(void);

/** \brief Release the global dataset lock. One Unlock per
 *         successful Lock, from the same thread. */
void DatasetUnlock(void);

/** \brief Look up a set by name in the global list.
 *
 *  Linear scan; does not check the returned set's type. Caller
 *  must hold the dataset lock.
 */
Dataset *DatasetSearchByName(const char *name);

/** \brief Look up a set by name and type without creating it.
 *
 *  A name hit with a different type is treated as no match.
 *  Takes and releases the dataset lock internally; caller must
 *  NOT already hold it.
 */
 ...
 *
 *  The standard entry point used by the rule parser for the
 *  `dataset` and `datarep` keywords. Takes and releases the
 *  dataset lock internally; caller must NOT already hold it.
 *
 ...
 *  \p save / \p load (if provided) are checked against the stored
 *  values and a mismatch is a hard error.
 *
 ...
 *
 *  Also fills \p memcap and \p hashsize with configured defaults
 ...
 *  \retval  0 found but currently busy (in use); left in place,
 *             caller may retry
 *  \retval -1 not found, OR NULL set, OR unknown type. Unlike
 *             the add family, "not present" is reported as -1,
 *             not 0, and the three conditions are
 *             indistinguishable from the return code alone.
 *  \retval -2 data_len wrong for the set's type (never from the
 *             string worker)
 *  \note IPv6 worker requires exactly 16 bytes here, like
 *        SCDatasetAddwRep and unlike SCDatasetAdd.
 */
 ...
 *  \retval -1 NULL set, unknown type, OR data_len mismatch (the
 *             string worker has no length check)
 *  \note IPv6 worker accepts 4 or 16 bytes; a 4-byte buffer
 *        matches stored entries whose trailing 12 bytes are zero,
 *        matching SCDatasetAdd's permissive behaviour.
 */
int DatasetLookup(Dataset *set, const uint8_t *data, const uint32_t data_len);

/** \brief Look up a value and return the stored reputation.
 *  \param rep present for symmetry with SCDatasetAddwRep. Does
 *             NOT select which entry matches, but the string
 *             worker dereferences it; must not be NULL.
 *  \return On hit: .found true, .rep is the reputation stored
 *          with the entry (not the input rep). On miss, NULL
 *          set, unknown type, or data_len mismatch:
 *          { .found=false, .rep=0 } — errors and misses are
 ...
 *         honouring `datasets.defaults.*` YAML overrides.
 *
 *  The two are handled asymmetrically:
 *
 *  \param memcap   If the YAML key is absent, *memcap is left
 *                  as-is; caller must preinitialise it. If
 *                  present but unparseable, *memcap is set to 0
 *                  and a warning is logged. The 0 is a sentinel
 *                  meaning "no default available".
 *  \param hashsize Always initialised to DATASETS_HASHSIZE_DEFAULT
 *                  before the YAML lookup. A parseable key
 *                  replaces the default; unparseable restores it
 *                  and logs a warning. Safe to pass uninitialised.
 */
 ...
 *         IPv6 storage form.
 *
 *  Family is inferred from a ':' in \p line. IPv4 input (or IPv6
 *  in ::ffff:a.b.c.d form) is written with the four IPv4 bytes
 *  in the low positions of \p in6 and the rest zero. This is NOT
 *  a standard IPv4-mapped IPv6 address, it's Suricata's internal
 *  layout, and matches what SCDatasetAdd's IPv4 and
 ...
 *
 *  Add / lookup / remove given a textual value. Parse \p string
 *  according to set->type and dispatch to the binary variant.
 *  Callers that already hold the native binary form should
 *  prefer those variants directly.
 *
 ...
 *              DatasetParseIpv6String. An IPv4 spelling is
 *              accepted even for an IPv6 set and stored in the
 *              internal 4-byte-in-low-position layout.
 *
 ...
 *    worker; the parse layer only hands it a correctly-sized
 *    buffer.
 *
 ...
 *  \retval  1 found and removed
 *  \retval  0 found but busy; left in place, caller may retry
