...
#define SC_JA4_LOG_RAW_BUFSIZE 4096

static void JsonTlsLogSCJA4(JsonBuilder *js, SSLState *ssl_state)
{
    if (ssl_state->client_connp.ja4 != NULL) {
        uint8_t buffer[36],                        /* JA4 hash has 36 characters */
                buffer_r[SC_JA4_LOG_RAW_BUFSIZE],  /* arbitrary size for output */
                buffer_ro[SC_JA4_LOG_RAW_BUFSIZE]; /* arbitrary size for output */
        SCJA4GetHash(ssl_state->client_connp.ja4, (uint8_t(*)[36])buffer, buffer_r,
                SC_JA4_LOG_RAW_BUFSIZE, buffer_ro, SC_JA4_LOG_RAW_BUFSIZE);
        jb_open_object(js, "ja4");

        jb_set_string_from_bytes(js, "ja4", buffer, 36);
        jb_set_string(js, "ja4_r", (const char *)buffer_r);
        jb_set_string(js, "ja4_ro", (const char *)buffer_ro);
