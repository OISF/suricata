...
AppProto AppLayerRegisterParser(const struct AppLayerParser *p)
{
    ...
    switch (p->ip_proto) {
        ...
        case IPPROTO_UDP:
            ...
            break;
        default:
            SCLogDebug("Unknown or unsupported ip_proto field in parser");
            return ALPROTO_UNKNOWN;
    };

    /* Check if protocol detection is enabled. If it does not exist in
     * the configuration file then it will be enabled by default. */
    if (AppLayerProtoDetectConfProtoDetectionEnabled(ip_proto_str, p->name)) {
    ...
    if (AppLayerParserConfParserEnabled(ip_proto_str, p->name))
    {
