...
static int DetectFrameSetup(DetectEngineCtx *de_ctx, Signature *s, const char *str)
{
    ...
    if (dot == NULL) {
        SCLogError(SC_ERR_INVALID_RULE_ARGUMENT,
                "'frame' invalid argument: need <app proto>.<frame name>");
        return -1;
    }
    *dot++ = '\0';
    const char *val = dot;
    ...
    int raw_frame_type = AppLayerParserGetFrameIdByName(IPPROTO_TCP, alproto, val);
    if (raw_frame_type < 0) {
        SCLogError(
                SC_ERR_INVALID_RULE_ARGUMENT, "unknown frame '%s' for protocol '%s'", val, proto);
        return -1;
    }
    ...
    const int buffer_id = DetectEngineBufferTypeRegisterWithFrameEngines(
            de_ctx, str, SIG_FLAG_TOSERVER | SIG_FLAG_TOCLIENT, alproto, frame_type);
    if (buffer_id < 0)
        return -1;
...
{
    sigmatch_table[DETECT_RECORD].name = "frame";
    sigmatch_table[DETECT_RECORD].desc = "sticky buffer for inspecting app-layer frames";
    sigmatch_table[DETECT_RECORD].Setup = DetectFrameSetup;
    sigmatch_table[DETECT_RECORD].flags = SIGMATCH_INFO_STICKY_BUFFER;
#ifdef UNITTESTS
    sigmatch_table[DETECT_RECORD].RegisterTests = DetectFrameRegisterTests;
#endif
}
