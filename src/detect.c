...
static inline void EngineAnalysisFPPopulate(DetectEngineCtx *de_ctx,
                                            SigGroupHead *sgh,
                                            Signature *s, SigMatch *mpm_sm)
{
    s->mpm_sm = mpm_sm;
    ...
    while(fgets(line + offset, (int)sizeof(line) - offset, fp) != NULL) {
        ...
        if (sig != NULL) {
            if (rule_engine_analysis_set) {
                ...
                sh.match_array = SCMalloc(1 * sizeof(Signature));
                if (sh.match_array == NULL)
                    exit(EXIT_FAILURE);
                sh.match_array[0] = sig;
                sh.sig_cnt++;
                PatternMatchPreparePopulateMpm(de_ctx, &sh, EngineAnalysisFPPopulate);
                EngineAnalysisRules(sig, line);
                SCFree(sh.match_array);
            }
...
int SigLoadSignatures(DetectEngineCtx *de_ctx, char *sig_file, int sig_file_exclusive)
{
    ...
    if (engine_analysis) {
        ...
        if (fp_engine_analysis_set) {
            SCLogInfo("rules-fast-pattern now a deprecated option in our "
                      "and would be eventually phased out.  Fast pattern "
                      "analysis for rules would now be enabled as a part of "
                      "rules analysis.");
        }
