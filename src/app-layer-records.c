...
{
    SCLogDebug("[%s] %p: record: %p type %u flags %02x rel_offset:%u, len:%u, events:%u %u/%u/%u/%u",
...
static Record *RecordNew(Records *recs, uint32_t rel_offset, int32_t len)
{
    ...
    if (recs->cnt < RECORDS_STATIC_CNT) {
        ...
        assert(recs->dyn_size == 0);
        assert(recs->cnt == RECORDS_STATIC_CNT);
        ...
        recs->cnt++;
        assert(recs->cnt == RECORDS_STATIC_CNT + 1);
        ...
        Flow *f, const uint8_t *rec_start, const uint32_t len, int dir, uint8_t rec_type)
{
    ...
    /* workarounds for many unittests not handling TCP data properly */
#ifdef UNITTESTS
