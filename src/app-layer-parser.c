                        ...
                        uint8_t flags, const uint8_t *input, uint32_t input_len)
{
    ...
    if (input_len > 0 || (flags & STREAM_EOF)) {
        /* invoke the parser */
        AppLayerReturn res = p->Parser[direction](f, alstate, pstate,
                input, input_len,
        ...
        if (res.status < 0)
        {
            ...
            if (ssn != NULL) {
                SCLogDebug("direction %d/%s", direction, (flags & STREAM_TOSERVER) ? "toserver":"toclient");
                assert(res.consumed <= input_len);
                if (direction == 0) {
 ...
 */
static AppLayerReturn TestProtocolParser(Flow *f, void *test_state, AppLayerParserState *pstate,
