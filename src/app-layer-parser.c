                        ...
                        uint8_t flags, const uint8_t *input, uint32_t input_len)
{
    ...
    if (input_len > 0 || (flags & STREAM_EOF)) {
        Setup(f, flags & (STREAM_TOSERVER | STREAM_TOCLIENT), input, input_len, &app_stream);
        /* invoke the parser */
        AppLayerResult res = p->Parser[direction](f, alstate, pstate, app_stream, input, input_len,
