...
{
    return al_proto_table[alproto].StateGetProgress(state, direction);
}

uint64_t AppLayerGetNoOfTxs(uint16_t alproto, void *alstate)
{
    return al_proto_table[alproto].StateGetNoOfTxs(alstate);
}
...
{
    return al_proto_table[alproto].StateProgressDoneValue(direction);
}
...
{
    al_proto_table[alproto].StateGetProgress = StateGetProgress;
}

void AppLayerRegisterGetNoOfTxs(uint16_t alproto,
                                uint64_t (*StateGetNoOfTxs)(void *alstate))
{
    al_proto_table[alproto].StateGetNoOfTxs = StateGetNoOfTxs;
}
...
{
    al_proto_table[alproto].StateProgressDoneValue = StateProgressDoneValue;
}
...
void AppLayerTransactionUpdateInspectId(Flow *f, uint8_t flags)
{
    ...
    uint64_t total_txs = AppLayerGetNoOfTxs(f->alproto, f->alstate);
    uint64_t idx = AppLayerTransactionGetInspectId(f, flags);
    int state_done_progress = AppLayerGetProgressDoneValue(f->alproto, direction);
    void *tx;
    int tx_updated_by = 0;
    int state_progress;
    ...
    for (; idx < total_txs; idx++, tx_updated_by++) {
        ...
        state_progress = AppLayerGetProgress(f->alproto, tx, direction);
