                        ...
                        uint8_t flags, const uint8_t *input, uint32_t input_len)
{
    ...
    if (input_len > 0 || (flags & STREAM_EOF)) {
        Setup(f, flags & (STREAM_TOSERVER | STREAM_TOCLIENT), input, input_len, &stream_slice);
        /* invoke the parser */
        AppLayerResult res = p->Parser[direction](f, alstate, pstate, stream_slice, input, input_len,
                alp_tctx->alproto_local_storage[f->protomap][alproto], flags);
        if (res.status < 0) {
            goto error;
