...
next:
        if (!ires.has_next) {
            /* if there are no further transactions, we must have freed
             * an older tx while newer ones are already gone. In this case
             * we update the "new_min" to the highest known id.
             * Do this only if we didn't skip some in-progress tx before
             * the one just freed. */
            SCLogDebug("no next: cur tx i %"PRIu64", total %"PRIu64, i, total_txs);
            if (!skipped && i < total_txs) {
                new_min = total_txs;
