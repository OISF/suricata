...
static inline uint32_t FlowGetHash(const Packet *p)
{
    ...
    if (p->ip4h != NULL) {
        if (p->tcph != NULL || p->udph != NULL) {
            ...
            /* g_vlan_mask zeros the vlan_id if we don't use it for tracking. */
...
static inline bool cmp_addrs(const uint32_t addr1[4], const uint32_t addr2[4])
{
    return addr1[0] == addr2[0] && addr1[1] == addr2[1] &&
        addr1[2] == addr2[2] && addr1[3] == addr2[3];
}

static inline bool cmp_addrs_and_ports(const uint32_t src1[4],
    ...
    Port dst_port2)
{
    ...
    return (cmp_addrs(src1, src2) && cmp_addrs(dst1, dst2) &&
        src_port1 == src_port2 && dst_port1 == dst_port2) ||
        (cmp_addrs(src1, dst2) && cmp_addrs(dst1, src2) &&
        src_port1 == dst_port2 && dst_port1 == src_port2);
}

static inline bool cmp_vlan_ids(const uint16_t vlan_id1[2], const uint16_t vlan_id2[2])
{
    /* g_vlan_mask zeros the vlan values if we do not use them for tracking. */
...
static inline bool cmp_flow_packet(const Flow *f, const Packet *p)
{
    ...
    return cmp_addrs_and_ports(f_src, f_dst, f->sp, f->dp, p_src, p_dst, p->sp,
        p->dp) && f->proto == p->proto &&
        f->recursion_level == p->recursion_level &&
        cmp_vlan_ids(f->vlan_id, p->vlan_id);
}

static inline bool cmp_flow_key(const Flow *f, const FlowKey *k)
{
    const uint32_t *f_src = f->src.address.address_un_data32;
    const uint32_t *f_dst = f->dst.address.address_un_data32;
    const uint32_t *k_src = k->src.address.address_un_data32;
    const uint32_t *k_dst = k->dst.address.address_un_data32;
    return cmp_addrs_and_ports(f_src, f_dst, f->sp, f->dp, k_src, k_dst, k->sp,
        k->dp) && f->proto == k->proto &&
        f->recursion_level == k->recursion_level &&
        cmp_vlan_ids(f->vlan_id, k->vlan_id);
}

static inline bool cmp_addrs_and_icmp_types(const uint32_t src1[4],
...
static inline bool cmp_flow_icmp_packet(const Flow *f, const Packet *p)
{
    const uint32_t *f_src = f->src.address.address_un_data32;
    const uint32_t *f_dst = f->dst.address.address_un_data32;
    const uint32_t *p_src = p->src.address.address_un_data32;
    const uint32_t *p_dst = p->dst.address.address_un_data32;
    return cmp_addrs_and_icmp_types(f_src, f_dst, f->icmp_s.type,
...
static inline int FlowCompareICMPv4(Flow *f, const Packet *p)
{
    if (ICMPV4_DEST_UNREACH_IS_VALID(p)) {
        ...
        return cmp_flow_icmp_packet(f, p);
    }
