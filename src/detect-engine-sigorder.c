                         ...
                         SCSigOrderFunc *cmp_func_list)
{
    ...
    if ((ActionOrderVal(sw1->sig->action) == ActionOrderVal(sw2->sig->action)) &&
            !(sw1->sig->init_data->firewall_rule || sw2->sig->init_data->firewall_rule) &&
        ...
        void *graph, SCSigSignatureWrapper *sw, uint32_t max_fb_id)
{
    ...
    while (tmp != NULL) {
        ...
        if (node < 0) {
            FatalErrorOnInit("Couldn't create or get a node in the graph for sid: %d", s->id);
        }
    ...
    for (uint32_t x = 1; x < array_size; x++) {
        for (uint32_t i = 0; i < fba.array[x].isset_iids_idx; i++) {
            ...
            int64_t to = SCGetOrCreateNodeGraph(
                    graph, fba.array[x].isset_iids[i].iid, fba.array[x].isset_iids[i].sid);
            if (to < 0) {
                FatalErrorOnInit("Couldn't create or get a node in the graph for sid: %d",
            ...
            for (uint32_t y = 0; y < fba.array[x].set_iids_idx; y++) {
                ...
                int64_t from = SCGetOrCreateNodeGraph(
                        graph, fba.array[x].set_iids[y].iid, fba.array[x].set_iids[y].sid);
                if (from < 0) {
                    FatalErrorOnInit("Couldn't create or get a node in the graph for sid: %d",
                ...
                if (SCCreateNodeEdgeDirectedGraph(
                            graph, (uint32_t)from, (uint32_t)to, DETECT_FLOWBITS_CMD_SET, x) < 0) {
    ...
    for (uint32_t x = 1; x < array_size; x++) {
        for (uint32_t i = 0; i < fba.array[x].isnotset_iids_idx; i++) {
            int64_t to = SCGetOrCreateNodeGraph(
                    graph, fba.array[x].isnotset_iids[i].iid, fba.array[x].isnotset_iids[i].sid);
            if (to < 0) {
                FatalErrorOnInit("Couldn't create or get a node in the graph for sid: %d",
            ...
            for (uint32_t y = 0; y < fba.array[x].unset_iids_idx; y++) {
                int64_t from = SCGetOrCreateNodeGraph(
                        graph, fba.array[x].unset_iids[y].iid, fba.array[x].unset_iids[y].sid);
                if (from < 0) {
                    FatalErrorOnInit("Couldn't create or get a node in the graph for sid: %d",
                            fba.array[x].unset_iids[y].sid);
                }
                SCLogDebug("added node: %ld", from);
                if (SCCreateNodeEdgeDirectedGraph(graph, (uint32_t)from, (uint32_t)to,
                            DETECT_FLOWBITS_CMD_UNSET, x) < 0) {
                    if (rule_engine_analysis_set) {
                        SCLogFlowbitsGraph(graph);
                    }
                    FatalErrorOnInit(
        ...
        SCSigSignatureWrapper *head, uint32_t max_fb_id, uint8_t max_cycle_resolution)
{
    ...
    SCLogInfo("Signatures with flowbits found");
    ...
    while (tmp != NULL) {
        ++sig_cnt;
        /* The dependency-resolved order overrides any explicit priority for
         * SET_READ flowbit rules. Warn once per such rule that set a priority. */
        if (tmp->user[DETECT_SIGORDER_FLOWBITS] == DETECT_FLOWBITS_TYPE_SET_READ &&
                (tmp->sig->init_data->init_flags & SIG_FLAG_INIT_PRIO_EXPLICIT)) {
            SCLogWarning("sid %u: explicitly set priority is overridden by flowbits "
    ...
    if (CreateGraphFromFlowbitAnalyzer(graph, head, max_fb_id) < 0) {
        FatalErrorOnInit("Could not create Flowbit Analyzer Graph");
    }
    ...
    if (fb_sigw_list_start) {
        ...
        if (tmp == NULL) {
            ...
            if (SC_ATOMIC_GET(engine_stage) == SURICATA_INIT) {
                FatalErrorOnInit(
                        "Flowbits have circular dependencies that cannot be met at runtime");
            } else {
                SCLogWarning("Applying the existing ruleset as flowbit dependencies could not be "
                             "resolved");
            }
        } else {
            fb_sigw_list_start = tmp;
        }
    }
