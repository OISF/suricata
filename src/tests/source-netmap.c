...
/* point a slot at buffer buf_idx, set its length and flags, and fill the
 * buffer with a byte so we can check the copy order. */
...
/* one slot, no fragments. stays as is and keeps pointing into the ring so
...
/* two fragments whose buffers sit next to each other. the whole frame must be
 * rebuilt in the coalesce buffer. */
...
/* two fragments whose buffers are far apart. this is what the old code got
 * wrong. both slots must still be copied in order. */
...
/* a two slot packet that wraps around the end of the ring (slot 3 then slot
 * 0). the walk has to follow the wrap and rebuild the frame. */
...
/* an empty leading fragment must not break the copy of the next one. */
...
/* an oversize frame with fragments after the overflow point must be dropped
 * whole, so the trailing slots are not read back as a bogus packet. */
...
/* a chain that never ends must be dropped once every slot we own has been
 * looked at, and the ring must still move on so the reader cannot spin. */
