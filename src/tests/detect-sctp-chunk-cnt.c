...
static int DetectSCTPChunkCntParseTest01(void)
{
    DetectEngineCtx *de_ctx = DetectEngineCtxInit();
    FAIL_IF_NULL(de_ctx);

    FAIL_IF_NULL(DetectEngineAppendSig(de_ctx, "alert sctp any any -> any any "
                                               "(msg:\"test\"; sctp.chunk_cnt:2; sid:1;)"));
...
{
    UtRegisterTest("DetectSCTPChunkCntParseTest01", DetectSCTPChunkCntParseTest01);
}
