...
#else /* We do have NAPATECH support */
...
/* These constants are used to mask out color values matching protocols identified by the hardware */
...
int NapatechSetPortmap(int port, int peer) {
	...
	//SCLogInfo("Napatech network segment pairing between ports %d and %d.", port, peer);
	return 1;
}
...
static int ProgramFlow(Packet *p, int is_inline)
{
	...
	flow_match.gfi = 0; /* Do not generate FlowInfo records */
	...
	status = NT_FlowWrite(*phFlowStream, &flow_match, 200);
...
#ifdef NAPATECH_ENABLE_BYPASS
		SCLogInfo("--- BypssCB - Total: %ld,  UDP: %ld,  TCP: %ld,  Unhandled: %ld",
