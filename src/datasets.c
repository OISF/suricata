...
static int DatasetLoadIPv6(Dataset *set)
{
    ...
    while (fgets(line, (int)sizeof(line), fp) != NULL) {
        ...
        if (r == NULL) {
            ...
            struct in_addr in;
            if (inet_pton(AF_INET6, line, &in) != 1)
                FatalError(SC_ERR_FATAL, "dataset data parse failed %s/%s: %s", set->name,
                        set->load, line);

            if (DatasetAdd(set, (const uint8_t *)&in.s_addr, 16) < 0)
                FatalError(SC_ERR_FATAL, "dataset data add failed %s/%s", set->name, set->load);
            cnt++;
            ...
            *r = '\0';
            ...
            SCLogDebug("rep v:%u", rep.value);
            if (DatasetAddwRep(set, (const uint8_t *)&in.s_addr, 16, &rep) < 0)
                FatalError(SC_ERR_FATAL, "dataset data add failed %s/%s", set->name, set->load);
