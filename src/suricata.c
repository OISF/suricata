...
static TmEcode ParseCommandLine(int argc, char** argv, SCInstance *suri)
{
    ...
    while ((opt = getopt_long(argc, argv, short_opts, long_opts, &option_index)) != -1) {
        ...
        case 0:
            ...
            } else if (strcmp((long_opts[option_index]).name , "simulate-applayer-error-at-offset-ts") == 0) {
#ifdef DEBUG
                BUG_ON(optarg == NULL);
                uint64_t offset = 0;
...
#else
                SCLogNotice("--simulate-applayer-error-at-offset-ts only available when --enable-debug is compiled in");
#endif
            } else if (strcmp((long_opts[option_index]).name , "simulate-applayer-error-at-offset-tc") == 0) {
#ifdef DEBUG
                BUG_ON(optarg == NULL);
                uint64_t offset = 0;
                if (ParseSizeStringU64(optarg, &offset) < 0) {
...
#else
                SCLogNotice("--simulate-applayer-error-at-offset-tc only available when --enable-debug is compiled in");
#endif
...
#else
                SCLogNotice("--simulate-packet-loss only available when --enable-debug is compiled in");
#endif
...
#else
                SCLogNotice("--simulate-packet-tcp-memcap only available when --enable-debug is compiled in");
#endif
            }
            break;
        case 'c':
