...
#ifdef HAVE_DPDK
    printf("\t--dpdk                               : run in dpdk mode, no value select interfaces from suricata.yaml\n");
#endif
#ifdef HAVE_AF_PACKET
...
static TmEcode ParseInterfacesList(const int runmode, char *pcap_dev)
{
    ...
    if (runmode == RUNMODE_PCAP_DEV) {
        ...
        if (strlen(pcap_dev)) {
            ...
            if (ret == 0) {
                SCLogError(SC_ERR_INITIALIZATION, "No interface found in config for %s",
                        iface_selector);
                SCReturnInt(TM_ECODE_FAILED);
            }
        }
...
static int ParseCommandLineDpdk(SCInstance *suri, const char *in_arg)
{
    ...
    if (suri->run_mode == RUNMODE_UNKNOWN) {
        ...
        if (in_arg) {
            ...
            SCLogInfo("Multiple dpdk options without interface on each is useless");
        }
    } else {
        SCLogError(SC_ERR_MULTIPLE_RUN_MODE, "more than one run mode "
                                             "has been specified");
