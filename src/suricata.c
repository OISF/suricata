...
static int RunmodeSetCurrent(RunModesList *runmodes, enum RunModes run_mode,
                              int increment)
{
...
static int RunmodeIsSet(const RunModesList *runmodes,
...
static int ParseCommandLineAfpacket(SCInstance *suri, const char *optarg)
{
    ...
    if (RunmodeIsUnknown(runmodes) && !RunmodeGetNumber(runmodes)) {
        if (!RunmodeIsSet(runmodes, RUNMODE_AFP_DEV)) {
            RunmodeSetCurrent(runmodes, RUNMODE_AFP_DEV, 0);
        } else {
...
static int ParseCommandLinePcapLive(SCInstance *suri, const char *optarg)
{
    ...
    if (RunmodeIsUnknown(runmodes) && !RunmodeGetNumber(runmodes)) {
        RunmodeSetCurrent(runmodes, RUNMODE_PCAP_DEV, 0);
        if (optarg) {
...
static TmEcode ParseCommandLine(int argc, char** argv, SCInstance *suri)
{
    ...
    while ((opt = getopt_long(argc, argv, short_opts, long_opts, &option_index)) != -1) {
        ...
        case 0:
...
#ifdef HAVE_PFRING
                RunmodeSetCurrent(runmodes, RUNMODE_PFRING, 0);
                if (optarg != NULL) {
                    memset(suri->pcap_dev, 0, sizeof(suri->pcap_dev));
            ...
            else if (strcmp((long_opts[option_index]).name , "af-packet") == 0)
            {
...
#ifdef HAVE_NETMAP
                if (RunmodeIsUnknown(runmodes) && !RunmodeGetNumber(runmodes)) {
                    RunmodeSetCurrent(runmodes, RUNMODE_NETMAP, 0);
                    if (optarg) {
                ...
                if (RunmodeIsUnknown(runmodes)) {
                    if (!RunmodeSetCurrent(runmodes, RUNMODE_NFLOG, 1)) {
                ...
                if (RunmodeIsUnknown(runmodes) && !RunmodeGetNumber(runmodes)) {
                    RunmodeSetCurrent(runmodes, RUNMODE_UNIX_SOCKET, 0);
                    if (optarg) {
...
#ifdef UNITTESTS
                RunmodeSetCurrent(runmodes, RUNMODE_LIST_UNITTEST, 0);
#else
                ...
                RunmodeSetCurrent(runmodes, RUNMODE_LIST_RUNMODES, 0);
                return TM_ECODE_OK;
                ...
                RunmodeSetCurrent(runmodes, RUNMODE_INSTALL_SERVICE, 0);
                return TM_ECODE_OK;
            }
            else if(strcmp((long_opts[option_index]).name, "service-remove") == 0) {
                RunmodeSetCurrent(runmodes, RUNMODE_REMOVE_SERVICE, 0);
                return TM_ECODE_OK;
            }
            else if(strcmp((long_opts[option_index]).name, "service-change-params") == 0) {
                RunmodeSetCurrent(runmodes, RUNMODE_CHANGE_SERVICE_PARAMS, 0);
                return TM_ECODE_OK;
            }
#endif /* OS_WIN32 */
                ...
                RunmodeSetCurrent(runmodes, RUNMODE_ERF_FILE, 0);
...
#ifdef HAVE_DAG
                if (RunmodeIsUnknown(runmodes) && !RunmodeGetNumber(runmodes)) {
                    RunmodeSetCurrent(runmodes, RUNMODE_DAG, 0);
                }
                else if (RunmodeGetPrimary(runmodes) != RUNMODE_DAG) {
                    SCLogError(SC_ERR_MULTIPLE_RUN_MODE,
...
#ifdef HAVE_NAPATECH
            RunmodeSetCurrent(runmodes, RUNMODE_NAPATECH, 0);
#else
            ...
            return TM_ECODE_FAILED;
                ...
                RunmodeSetCurrent(runmodes, RUNMODE_PRINT_BUILDINFO, 0);
                return TM_ECODE_OK;
            }
#ifdef HAVE_MPIPE
            else if(strcmp((long_opts[option_index]).name , "mpipe") == 0) {
                if (RunmodeIsUnknown(runmodes) && !RunmodeGetNumber(runmodes)) {
                    RunmodeSetCurrent(runmodes, RUNMODE_TILERA_MPIPE, 0);
                    if (optarg != NULL) {
                        memset(suri->pcap_dev, 0, sizeof(suri->pcap_dev));
                        strlcpy(suri->pcap_dev, optarg,
                                ((strlen(optarg) < sizeof(suri->pcap_dev)) ?
...
#endif /* OS_WIN32 */
        case 'h':
            RunmodeSetCurrent(runmodes, RUNMODE_PRINT_USAGE, 0);
            return TM_ECODE_OK;
        case 'i':
            if (optarg == NULL) {
...
#ifdef NFQ
            if (RunmodeIsUnknown(runmodes)) {
                if (!RunmodeIsSet(runmodes, RUNMODE_NFQ)) {
                    if (!RunmodeSetCurrent(runmodes, RUNMODE_NFQ, 1)) {
                        SCLogError(SC_ERR_MULTIPLE_RUN_MODE,
                                   "Maximum number of runmodes reached");
                        usage(argv[0]);
                        return TM_ECODE_FAILED;
...
#ifdef IPFW
            if (RunmodeIsUnknown(runmodes) && !RunmodeGetNumber(runmodes)) {
                RunmodeSetCurrent(runmodes, RUNMODE_IPFW, 0);
                EngineModeSetIPS();
        ...
        case 'r':
            if (RunmodeIsUnknown(runmodes) && !RunmodeGetNumber(runmodes)) {
                RunmodeSetCurrent(runmodes, RUNMODE_PCAP_FILE, 0);
            } else {
                SCLogError(SC_ERR_MULTIPLE_RUN_MODE, "more than one run mode "
                                                     "has been specified");
                usage(argv[0]);
...
#ifdef UNITTESTS
            if (RunmodeIsUnknown(runmodes) && !RunmodeGetNumber(runmodes)) {
                RunmodeSetCurrent(runmodes, RUNMODE_UNITTEST, 0);
            } else {
        ...
        case 'U':
            ...
            break;
        case 'V':
            RunmodeSetCurrent(runmodes, RUNMODE_PRINT_VERSION, 0);
            return TM_ECODE_OK;
        case 'F':
            if (optarg == NULL) {
    ...
    if (list_unittests)
        RunmodeSetCurrent(runmodes, RUNMODE_LIST_UNITTEST, 0);
    if (dump_config)
        RunmodeSetCurrent(runmodes, RUNMODE_DUMP_CONFIG, 0);
    if (conf_test)
        RunmodeSetCurrent(runmodes, RUNMODE_CONF_TEST, 0);
    if (engine_analysis)
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < RunmodeGetNumber(&suri.runmodeslist); i++) {
        if (RunmodeGetCurrent(&suri.runmodeslist, i) != RUNMODE_UNKNOWN) {
            RunModeDispatch(RunmodeGetCurrent(&suri.runmodeslist, i), suri.runmode_custom_mode);
        }
    }
