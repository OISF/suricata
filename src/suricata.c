...
static void SCInstanceInit(SCInstance *suri, const char *progname)
{
    ...
    suri->runmodes.run_mode[0] = RUNMODE_UNKNOWN;
    suri->runmodes.run_mode[1] = RUNMODE_UNKNOWN;
    suri->runmodes.pos = 0;
...
{
#ifdef HAVE_AF_PACKET
    RunmodesSlots *runmodes = &suri->runmodes;
    if (RunmodesSlotsRunmodeIsUnknown(runmodes)) {
        if (!RunmodesSlotsRunmodeIsInSlot(runmodes, RUNMODE_AFP_DEV)) {
            RunmodesSlotsSetRunmode(runmodes, RUNMODE_AFP_DEV);
            if (in_arg) {
...
static int ParseCommandLinePcapLive(SCInstance *suri, const char *in_arg)
{
    ...
    if (RunmodesSlotsRunmodeIsUnknown(runmodes)) {
        RunmodesSlotsSetRunmode(runmodes, RUNMODE_PCAP_DEV);
        if (in_arg) {
            LiveRegisterDeviceName(suri->pcap_dev, RUNMODE_PCAP_DEV);
        }
    } else if (RunmodesSlotsRunmodeIsInSlot(runmodes, RUNMODE_PCAP_DEV)) {
#ifdef OS_WIN32
...
static TmEcode ParseCommandLine(int argc, char** argv, SCInstance *suri)
{
    ...
    while ((opt = getopt_long(argc, argv, short_opts, long_opts, &option_index)) != -1) {
        ...
        case 0:
...
#ifdef HAVE_PFRING
                RunmodesSlotsSetRunmode(runmodes, RUNMODE_PFRING);
                if (optarg != NULL) {
                    memset(suri->pcap_dev, 0, sizeof(suri->pcap_dev));
            ...
            else if (strcmp((long_opts[option_index]).name , "af-packet") == 0)
            {
...
#ifdef HAVE_NETMAP
                if (RunmodesSlotsRunmodeIsUnknown(runmodes)) {
                    RunmodesSlotsSetRunmode(runmodes, RUNMODE_NETMAP);
                    if (optarg) {
                ...
                } else if (RunmodesSlotsRunmodeIsInSlot(runmodes, RUNMODE_NETMAP)) {
                    if (optarg) {
                        LiveRegisterDeviceName(optarg, RUNMODE_NETMAP);
                    } else {
...
#ifdef HAVE_NFLOG
                if (RunmodesSlotsRunmodeIsUnknown(runmodes) && !RunmodesSlotsRunmodeIsInSlot(runmodes, RUNMODE_NFLOG)) {
                    if (RunmodesSlotsMaxSlotsReached(runmodes)) {
                        SCLogError(SC_ERR_MULTIPLE_RUN_MODE,
                        "Maximum number of runmodes reached");
                        PrintUsage(argv[0]);
                        return TM_ECODE_FAILED;
                    }
                    RunmodesSlotsSetRunmode(runmodes, RUNMODE_NFLOG);
                ...
                if (RunmodesSlotsRunmodeIsUnknown(runmodes)) {
                    RunmodesSlotsSetRunmode(runmodes, RUNMODE_UNIX_SOCKET);
                    if (optarg) {
...
#ifdef UNITTESTS
                RunmodesSlotsSetRunmode(runmodes, RUNMODE_LIST_UNITTEST);
#else
                fprintf(stderr, "ERROR: Unit tests not enabled. Make sure to pass --enable-unittests to configure when building.\n");
                return TM_ECODE_FAILED;
#endif /* UNITTESTS */
            } else if (strcmp((long_opts[option_index]).name, "list-runmodes") == 0) {
                RunmodesSlotsSetRunmode(runmodes, RUNMODE_LIST_RUNMODES);
                return TM_ECODE_OK;
                ...
                RunmodesSlotSetRunmode(runmodes, RUNMODE_INSTALL_SERVICE);
                return TM_ECODE_OK;
            }
            else if(strcmp((long_opts[option_index]).name, "service-remove") == 0) {
                RunmodesSlotSetRunmode(runmodes, RUNMODE_REMOVE_SERVICE);
                return TM_ECODE_OK;
            }
            else if(strcmp((long_opts[option_index]).name, "service-change-params") == 0) {
                RunmodesSlotSetRunmode(runmodes, RUNMODE_CHANGE_SERVICE_PARAMS);
                return TM_ECODE_OK;
            }
#endif /* OS_WIN32 */
                ...
                RunmodesSlotsSetRunmode(runmodes, RUNMODE_ERF_FILE);
...
#ifdef HAVE_DAG
                if (RunmodesSlotsRunmodeIsUnknown(runmodes)) {
                    RunmodesSlotsSetRunmode(runmodes, RUNMODE_DAG);
                }
                else if (RunmodeGetFirstSlot(runmodes) != RUNMODE_DAG) {
                    SCLogError(SC_ERR_MULTIPLE_RUN_MODE,
...
#ifdef HAVE_NAPATECH
            RunmodesSlotsSetRunmode(runmodes, RUNMODE_NAPATECH);
#else
            ...
            return TM_ECODE_FAILED;
                ...
                RunmodesSlotsSetRunmode(runmodes, RUNMODE_PRINT_BUILDINFO);
                return TM_ECODE_OK;
            }
#ifdef HAVE_MPIPE
            else if(strcmp((long_opts[option_index]).name , "mpipe") == 0) {
                if (RunmodesSlotsRunmodeIsUnknown(runmodes)) {
                    RunmodesSlotsSetRunmode(runmodes, RUNMODE_TILERA_MPIPE);
                    if (optarg != NULL) {
                        memset(suri->pcap_dev, 0, sizeof(suri->pcap_dev));
                        strlcpy(suri->pcap_dev, optarg,
                                ((strlen(optarg) < sizeof(suri->pcap_dev)) ?
...
#ifdef WINDIVERT
                if (RunmodesSlotsRunmodeIsUnknown(runmodes)) {
                    RunmodesSlotsSetRunmode(runmodes, RUNMODE_WINDIVERT);
                    if (WinDivertRegisterQueue(true, optarg) == -1) {
                        exit(EXIT_FAILURE);
                    }
                } else if (RunmodesSlotsRunmodeIsInSlot(runmodes, RUNMODE_WINDIVERT)) {
                    if (WinDivertRegisterQueue(true, optarg) == -1) {
                        exit(EXIT_FAILURE);
                    }
                } else {
...
#endif /* OS_WIN32 */
        case 'h':
            RunmodesSlotsSetRunmode(runmodes, RUNMODE_PRINT_USAGE);
            return TM_ECODE_OK;
        case 'i':
            if (optarg == NULL) {
        ...
        case 'q':
#ifdef NFQ
            if (RunmodesSlotsRunmodeIsUnknown(runmodes) && !RunmodesSlotsRunmodeIsInSlot(runmodes, RUNMODE_NFQ)) {
                if (RunmodesSlotsMaxSlotsReached(runmodes)) {
                    SCLogError(SC_ERR_MULTIPLE_RUN_MODE,
                               "Maximum number of runmodes reached");
                    PrintUsage(argv[0]);
                ...
                RunmodesSlotsSetRunmode(runmodes, RUNMODE_NFQ);  
                //EngineModeSetIPS();
                if (NFQRegisterQueue(optarg) == -1)
                    return TM_ECODE_FAILED;
            } else if (RunmodesSlotsRunmodeIsInSlot(runmodes, RUNMODE_NFQ)) {
                if (NFQRegisterQueue(optarg) == -1)
                    return TM_ECODE_FAILED;
            } else {
                SCLogError(SC_ERR_MULTIPLE_RUN_MODE, "more than one run mode "
...
#ifdef IPFW
            if (RunmodesSlotsRunmodeIsUnknown(runmodes)) {
                RunmodesSlotsSetRunmode(runmodes, RUNMODE_IPFW);
                EngineModeSetIPS();
                if (IPFWRegisterQueue(optarg) == -1)
                    return TM_ECODE_FAILED;
            } else if (RunmodesSlotsRunmodeIsInSlot(runmodes, RUNMODE_IPFW)) {
                if (IPFWRegisterQueue(optarg) == -1)
                    return TM_ECODE_FAILED;
            } else {
                SCLogError(SC_ERR_MULTIPLE_RUN_MODE, "more than one run mode "
        ...
        case 'r':
            if (RunmodesSlotsRunmodeIsUnknown(runmodes)) {
                RunmodesSlotsSetRunmode(runmodes, RUNMODE_PCAP_FILE);
            } else {
                SCLogError(SC_ERR_MULTIPLE_RUN_MODE, "more than one run mode "
                                                     "has been specified");
                PrintUsage(argv[0]);
...
#ifdef UNITTESTS
            if (RunmodesSlotsRunmodeIsUnknown(runmodes)) {
                RunmodesSlotsSetRunmode(runmodes, RUNMODE_UNITTEST);
            } else {
        ...
        case 'U':
            ...
            break;
        case 'V':
            RunmodesSlotsSetRunmode(runmodes, RUNMODE_PRINT_VERSION);
            return TM_ECODE_OK;
        case 'F':
            if (optarg == NULL) {
    ...
    if ((RunmodesSlotsGetFirstSlot(&suri->runmodes) == RUNMODE_UNIX_SOCKET) && set_log_directory) {
    ...
    suri->aux_runmodes = suri->runmodes;

    if (list_app_layer_protocols)
        RunmodesSlotsSetRunmode(runmodes, RUNMODE_LIST_APP_LAYERS);
    if (list_keywords)
        RunmodesSlotsSetRunmode(runmodes, RUNMODE_LIST_KEYWORDS);
    if (list_unittests)
        RunmodesSlotsSetRunmode(runmodes, RUNMODE_LIST_UNITTEST);
    if (dump_config)
        RunmodesSlotsSetRunmode(runmodes, RUNMODE_DUMP_CONFIG);
    if (conf_test)
        RunmodesSlotsSetRunmode(runmodes, RUNMODE_CONF_TEST);
    if (engine_analysis)
        RunmodesSlotsSetRunmode(runmodes, RUNMODE_ENGINE_ANALYSIS);

    suri->offline = IsRunModeOffline(RunmodesSlotsGetFirstSlot(runmodes));
...
static int StartInternalRunMode(SCInstance *suri, int argc, char **argv)
{
    ...
    switch(RunmodesSlotsGetFirstSlot(&suri->runmodes)) {
...
static int FinalizeRunMode(SCInstance *suri, char **argv)
{
    ...
    runmodesslots = suri->runmodes;

    if (!CheckValidDaemonModes(suri->daemon, RunmodesSlotsGetFirstSlot(&suri->runmodes))) {
        return TM_ECODE_FAILED;
    }
...
static void PostConfLoadedDetectSetup(SCInstance *suri)
{
    ...
    if (!suri->disabled_detect) {
        ...
        if (suri->delayed_detect && RunmodesSlotsGetFirstSlot(&suri->runmodes) != RUNMODE_CONF_TEST) {
            de_ctx = DetectEngineCtxInitStubForDD();
        } else if (mt_enabled && !default_tenant && RunmodesSlotsGetFirstSlot(&suri->runmodes) != RUNMODE_CONF_TEST) {
        ...
        if (de_ctx->type == DETECT_ENGINE_TYPE_NORMAL) {
            if (LoadSignatures(de_ctx, suri) != TM_ECODE_OK)
                exit(EXIT_FAILURE);
            if (RunmodesSlotsGetFirstSlot(&suri->runmodes) == RUNMODE_ENGINE_ANALYSIS) {
                exit(EXIT_SUCCESS);
            }
        }
...
#ifdef HAVE_AF_PACKET
    if (RunmodesSlotsGetFirstSlot(&suri->runmodes) == RUNMODE_AFP_DEV ||
        RunmodesSlotsGetSecondSlot(&suri->runmodes) == RUNMODE_AFP_DEV) {
        if (AFPRunModeIsIPS()) {
...
#endif
#ifdef HAVE_NETMAP
    if (RunmodesSlotsGetFirstSlot(&suri->runmodes) == RUNMODE_NETMAP ||
        RunmodesSlotsGetSecondSlot(&suri->runmodes) == RUNMODE_NETMAP) {
...
static int PostConfLoadedSetup(SCInstance *suri)
{
...
#ifdef NFQ
    if (RunmodesSlotsRunmodeIsInSlot(&suri->runmodes, RUNMODE_NFQ))
        NFQInitConfig(FALSE);
#endif
    ...
    if (RunmodesSlotsRunmodeIsInSlot(&suri->runmodes, RUNMODE_ENGINE_ANALYSIS)) {
...
#ifdef HAVE_NSS
    if (RunmodesSlotsRunmodeIsInSlot(&suri->runmodes, RUNMODE_CONF_TEST)) {
    ...
    PreRunInit(RunmodesSlotsGetFirstSlot(&suri->runmodes));
...
int main(int argc, char **argv)
{
    ...
    if (RunmodesSlotsRunmodeIsInSlot(&suricata.runmodes, RUNMODE_DUMP_CONFIG)) {
        ConfDump();
        exit(EXIT_SUCCESS);
    }
    ...
    int i;
    for (i = 0; i < RunmodesSlotsGetSlotsCount(&suricata.aux_runmodes); i++) {
      if (ParseInterfacesList(RunmodesSlotsGetRunmode(&suricata.aux_runmodes, i), suricata.pcap_dev) != TM_ECODE_OK) {
          exit(EXIT_FAILURE);
      }
    }
    ...
    PreRunPostPrivsDropInit(RunmodesSlotsGetFirstSlot(&suricata.runmodes));

    PostConfLoadedDetectSetup(&suricata);

    if (RunmodesSlotsRunmodeIsInSlot(&suricata.runmodes, RUNMODE_CONF_TEST)){
    ...
    RunModeDispatch(RunmodesSlotsGetFirstSlot(&suricata.runmodes), suricata.runmode_custom_mode);
    for (i = 1; i < RunmodesSlotsGetSlotsCount(&suricata.runmodes); i++) {
        if (RunmodesSlotsGetRunmode(&suricata.runmodes, i) != RUNMODE_UNKNOWN) {
            RunModeDispatch(RunmodesSlotsGetRunmode(&suricata.runmodes, i), suricata.runmode_custom_mode);
        }
    }
    ...
    PostRunDeinit(RunmodesSlotsGetFirstSlot(&suricata.runmodes), &suricata.start_time);
