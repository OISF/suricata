...
/** Run mode selected */
RunModesList runmodeslist = { {RUNMODE_UNKNOWN, RUNMODE_UNKNOWN}, 0, 0};
...
{
    return runmodeslist.run_mode[index];
}

static int RunmodeIsAlreadyRan(RunModesList *runmodes, int run_mode)
{
    int i;
...
static void SCInstanceInit(SCInstance *suri)
{
    ...
    suri->runmodeslist.enable_mixed_mode = 0;
    ...
    while ((opt = getopt_long(argc, argv, short_opts, long_opts, &option_index)) != -1) {
        ...
        case 0:
            if (strcmp((long_opts[option_index]).name , "pfring") == 0 ||
                ...
                if (runmodes->run_mode[runmodes->runmodes_cnt] == RUNMODE_UNKNOWN) {
                    runmodes->run_mode[runmodes->runmodes_cnt] = RUNMODE_UNKNOWN;
                    if (optarg) {
        ...
        case 'i':
            ...
            if (runmodes->run_mode[runmodes->runmodes_cnt] == RUNMODE_UNKNOWN) {
                if (runmodes->enable_mixed_mode) {
                    SCLogError(SC_ERR_PCAP_TRANSLATE, "You can't use the mixed mode with this runmode.");
                    usage(argv[0]);
                    return TM_ECODE_FAILED;
                }
                runmodes->run_mode[runmodes->runmodes_cnt] = RUNMODE_PCAP_DEV;
...
int StartInternalRunMode(SCInstance *suri, int argc, char **argv)
{
    ...
    switch(suri->runmodeslist.run_mode[0]) {
...
static int ConfigGetCaptureValue(SCInstance *suri)
{
    ...
    if ((ConfGet("default-packet-size", &temp_default_packet_size)) != 1) {
        switch (suri->runmodeslist.run_mode[suri->runmodeslist.runmodes_cnt]) {
...
static int PostConfLoadedSetup(SCInstance *suri)
{
...
#ifdef NFQ
    if (suri->runmodeslist.run_mode[0] == RUNMODE_NFQ || suri->runmodeslist.run_mode[1] == RUNMODE_NFQ)
        NFQInitConfig(FALSE);
#endif

    /* Load the Host-OS lookup. */
    SCHInfoLoadFromConfig();
    if (suri->runmodeslist.run_mode[0] != RUNMODE_UNIX_SOCKET) {
        DefragInit();
    }

    if (suri->runmodeslist.run_mode[0] == RUNMODE_ENGINE_ANALYSIS) {
    ...
    SigParsePrepare();
#ifdef PROFILING
    if (runmodes->run_mode[0] != RUNMODE_UNIX_SOCKET) {
...
int main(int argc, char **argv)
{
    ...
    if (suri.runmodeslist.run_mode[0] == RUNMODE_UNITTEST)
    ...
    if (!CheckValidDaemonModes(suri.daemon, suri.runmodeslist.run_mode[0])) {
        exit(EXIT_FAILURE);
    }
    ...
    if (runmodeslist.run_mode[0] != RUNMODE_UNIX_SOCKET) {
        StatsInit();
    }
    ...
    if (suri.runmodeslist.run_mode[0] == RUNMODE_DUMP_CONFIG) {
        ConfDump();
        exit(EXIT_SUCCESS);
    }
    ...
    if (ParseInterfacesList(suri.runmodeslist.run_mode[0], suri.pcap_dev) != TM_ECODE_OK) {
        exit(EXIT_FAILURE);
    }

    if(suri.runmodeslist.run_mode[0] == RUNMODE_CONF_TEST){
        SCLogNotice("Configuration provided was successfully loaded. Exiting.");
        exit(EXIT_SUCCESS);
    }

    RunModeDispatch(suri.runmodeslist.run_mode[0], suri.runmode_custom_mode);
    int i;
    for (i = 1; i < suri.runmodeslist.runmodes_cnt; i++) {
