...
void usage(const char *progname)
{
    ...
    printf("\t                                     : odp.interface      - option to specify current interface\n");
    printf("\t                                     : odp.threads        - option to specify maximum number of ODP workers\n");
#endif
#ifdef HAVE_NETMAP
    ...
    if (run_mode == RUNMODE_PCAP_DEV) {
        ...
        if (strlen(pcap_dev)) {
            if (ConfSetFinal("odp.live-interface", pcap_dev) != 1) {
                ...
                SCLogInfo("set odp.live-interface to %s", pcap_dev);
        } else {
            ...
            else if (strcmp((long_opts[option_index]).name , "af-packet") == 0)
            {
                ...
                if (suri->run_mode == RUNMODE_UNKNOWN) {
                    ...
                    if (optarg) {
                        ...
                        SCLogInfo("Multiple ODP option without interface on each is useless");
                        break;
                    }
                } else {
                    SCLogError(SC_ERR_MULTIPLE_RUN_MODE, "more than one run mode "
...
#if defined HAVE_PFRING || HAVE_NETMAP || HAVE_ODP
            int i = 0;
#ifdef HAVE_PFRING
            i++;
#endif
