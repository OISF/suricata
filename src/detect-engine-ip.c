...
struct ParserScratch {
    ...
    DetectIPRange *pos_v4[33];
    DetectIPRange *neg_v4[33];
    DetectIPRange *pos_v6[129];
    DetectIPRange *neg_v6[129];
    uint32_t pos_v4_cnt;
    uint32_t neg_v4_cnt;
    uint32_t pos_v6_cnt;
    uint32_t neg_v6_cnt;
...
static int BuildTree(struct ParserScratch *ps, DetectIPRange *range, const char *address);
 ...
 * \retval -1 On failure.
 */
static int DetectIPRangeParseSingle(struct ParserScratch *ps, const char *str, const bool negated)
{
    char buf[256] = "";
    ...
    if (strcasecmp(str, "any") == 0) {
        ...
        if (DetectIPRangeParseSingle(ps, "0.0.0.0/0", negated) < 0)
            return -1;
        if (DetectIPRangeParseSingle(ps, "::/0", negated) < 0)
            return -1;
...
static DetectIPRange *Copy(DetectIPRange *r)
{
    DetectIPRange *n = SCCalloc(1, sizeof(*n));
    BUG_ON(n == NULL);
    *n = *r;
    return n;
}
...
static int BuildTree(struct ParserScratch *ps, DetectIPRange *range, const char *address)
{
    if (range->netmask == 0 && range->negated) {
        SCLogError(SC_ERR_ADDRESS_ENGINE_GENERIC, "address %s negates complete IP space", address);
        return -1;
    }

    DetectIPRange *copy = Copy(range);
    if (range->family == AF_INET) {
        BUG_ON(range->netmask > 32);
        if (range->negated) {
            copy->next = ps->neg_v4[range->netmask];
            ps->neg_v4[range->netmask] = copy;
            ps->neg_v4_cnt++;
            /* copy into pos list as well */
            copy = Copy(range);
            copy->next = ps->pos_v4[range->netmask];
            ps->pos_v4[range->netmask] = copy;
        } else {
            copy->next = ps->pos_v4[range->netmask];
            ps->pos_v4[range->netmask] = copy;
            ps->pos_v4_cnt++;
        ...
        BUG_ON(range->family != AF_INET6);
        BUG_ON(range->netmask > 128);

        if (range->negated) {
            copy->next = ps->neg_v6[range->netmask];
            ps->neg_v6[range->netmask] = copy;
            ps->neg_v6_cnt++;
            /* copy into pos list as well */
            copy = Copy(range);
            copy->next = ps->pos_v6[range->netmask];
            ps->pos_v6[range->netmask] = copy;
        } else {
            copy->next = ps->pos_v6[range->netmask];
            ps->pos_v6[range->netmask] = copy;
            ps->pos_v6_cnt++;
...
/**
 * \brief This function parses and return a list of DetectIPRange
 *
    ...
    for (size_t u = 0, x = 0; u < size && x < address_length; u++) {
        ...
        if (!o_set && s[u] == '!') {
            ...
            if (o_set == 1) {
                ...
                if (DetectIPRangeParseSingle(ps, address, neg) < 0) {
                    return -1;
                }
                n_set = 0;
            }
        ...
        const DetectEngineCtx *de_ctx, struct ParserScratch *ps, const char *str)
{
    ...
    */
    for (int x = 0; x <= 32; x++) {
        for (DetectIPRange *i = ps->pos_v4[x]; i != NULL;) {
            SCRadix4Node *node;
            void *user_data = NULL;
    ...
    for (int x = 0; x <= 32; x++) {
        // neg
        for (DetectIPRange *i = ps->neg_v4[x]; i != NULL;) {
            SCRadix4Node *node;
            void *user_data = NULL;
    ...
    for (int x = 0; x <= 128; x++) {
        for (DetectIPRange *i = ps->pos_v6[x]; i != NULL;) {
            SCRadix6Node *node;
            void *user_data = NULL;
    ...
    for (int x = 0; x <= 128; x++) {
        // neg
        for (DetectIPRange *i = ps->neg_v6[x]; i != NULL;) {
            SCRadix6Node *node;
            void *user_data = NULL;
            SCLogDebug("sid %u: negative %s/%u", ps->sid, inet_ntoa(*(struct in_addr *)&i->ip[0]),
                    i->netmask);
...
static int IPTestBug5066v1(void)
{
...
static int IPTestBug5066v2(void)
{
    struct DetectAddresses a = DETECT_ADDRESSES_INITIALIZER;
...
static int IPTestBug5066v3(void)
{
    struct DetectAddresses a = DETECT_ADDRESSES_INITIALIZER;
...
static int IPTestBug5066v4(void)
{
    struct DetectAddresses a = DETECT_ADDRESSES_INITIALIZER;
...
static int IPTestBug5066v5(void)
{
    struct DetectAddresses a = DETECT_ADDRESSES_INITIALIZER;
...
static int IPTestBug5168v1(void)
{
    struct DetectAddresses a = DETECT_ADDRESSES_INITIALIZER;
...
static int IPTestBug5168v2(void)
{
    struct DetectAddresses a = DETECT_ADDRESSES_INITIALIZER;
...
{
    UtRegisterTest("IPTestSig01", IPTestSig01);
    UtRegisterTest("IPTestSig02", IPTestSig02);

    UtRegisterTest("IPTestBug5066v1", IPTestBug5066v1);
    UtRegisterTest("IPTestBug5066v2", IPTestBug5066v2);
    UtRegisterTest("IPTestBug5066v3", IPTestBug5066v3);
    UtRegisterTest("IPTestBug5066v4", IPTestBug5066v4);
    UtRegisterTest("IPTestBug5066v5", IPTestBug5066v5);

    UtRegisterTest("IPTestBug5168v1", IPTestBug5168v1);
    UtRegisterTest("IPTestBug5168v2", IPTestBug5168v2);
