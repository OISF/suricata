 ...
 *
 * TFTP application layer detector and parser for learning and
 * tftp pruposes.
 *
 * This tftp implements a simple application layer for something
 ...
 *     ALPROTO_UNKNOWN.
 */
static AppProto TFTPProbingParser(uint8_t *input, uint32_t input_len,
    uint32_t *offset)
{
...
void RegisterTFTPParsers(void)
{
    ...
    if (AppLayerProtoDetectConfProtoDetectionEnabled("udp", proto_name)) {
        ...
        if (RunmodeIsUnittests()) {
            ...
            AppLayerProtoDetectPPRegister(IPPROTO_UDP, TFTP_DEFAULT_PORT,
                ALPROTO_TFTP, 0, TFTP_MIN_FRAME_LEN, STREAM_TOSERVER,
            ...
            if (!AppLayerProtoDetectPPParseConfPorts("udp", IPPROTO_UDP,
                    proto_name, ALPROTO_TFTP, 0, TFTP_MIN_FRAME_LEN,
                    TFTPProbingParser, NULL)) {
                SCLogDebug("No echo app-layer configuration, enabling echo"
                    " detection UDP detection on port %s.",
                    TFTP_DEFAULT_PORT);
                AppLayerProtoDetectPPRegister(IPPROTO_UDP,
                    TFTP_DEFAULT_PORT, ALPROTO_TFTP, 0,
                    TFTP_MIN_FRAME_LEN, STREAM_TOSERVER,
                    TFTPProbingParser, NULL);
            }
    ...
    if (AppLayerParserConfParserEnabled("udp", proto_name)) {
        ...
        AppLayerParserRegisterGetStateProgressFunc(IPPROTO_UDP,
            ALPROTO_TFTP, TFTPGetStateProgress);
        ...
        AppLayerParserRegisterDetectStateFuncs(IPPROTO_UDP, ALPROTO_TFTP,
            NULL, TFTPGetTxDetectState, TFTPSetTxDetectState);
