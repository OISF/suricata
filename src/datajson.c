...
static json_t *GetSubObjectByKey(json_t *json, char *key)
{
...
static int DatajsonLoadMd5(Dataset *set, char *json_key, char *array_key)
{
    ...
    if (json_key == NULL) {
        ...
        json_array_foreach (json, index, value) {
            ...
            add_ret = DatajsonAdd(set, (const uint8_t *)hash_string, 16, &elt);
            if (add_ret < 0) {
                FatalErrorOnInit("datajson data add failed %s/%s", set->name, set->load);
                continue;
            } else if (add_ret == 0) {
...
static int DatajsonLoadSha256(Dataset *set, char *json_key, char *array_key)
{
    ...
    if (json_key == NULL) {
        ...
        json_array_foreach (json, index, value) {
            ...
            add_ret = DatajsonAdd(set, (const uint8_t *)hash_string, 32, &elt);
            if (add_ret < 0) {
                FatalErrorOnInit("datajson data add failed %s/%s", set->name, set->load);
                continue;
            } else if (add_ret == 0) {
...
static int DatajsonLoadIPv4(Dataset *set, char *json_key, char *array_key)
{
    ...
    if (json_key == NULL) {
        ...
        json_array_foreach (json, index, value) {
            ...
            add_ret = DatajsonAdd(set, (const uint8_t *)&in.s_addr, 4, &elt);
            if (add_ret < 0) {
                FatalErrorOnInit("datajson data add failed %s/%s", set->name, set->load);
                continue;
            } else if (add_ret == 0) {
...
static int DatajsonLoadIPv6(Dataset *set, char *json_key, char *array_key)
{
    ...
    if (json_key == NULL) {
        ...
        json_array_foreach (json, index, value) {
            ...
            add_ret = DatajsonAdd(set, (const uint8_t *)&in6.s6_addr, 16, &elt);
            if (add_ret < 0) {
                FatalErrorOnInit("datajson data add failed %s/%s", set->name, set->load);
                continue;
            } else if (add_ret == 0) {
