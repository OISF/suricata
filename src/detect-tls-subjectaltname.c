...
void DetectTlsSubjectAltNameRegister(void)
{
    ...
    sigmatch_table[DETECT_AL_TLS_SUBJECTALTNAME].desc =
            "sticky buffer to match the TLS certificate buffer";
    ...
    // STODO only to client?
    DetectAppLayerInspectEngineRegister("tls.subjectaltname", ALPROTO_TLS, SIG_FLAG_TOSERVER,
            TLS_STATE_CERT_READY, DetectEngineInspectTlsSubjectAltName, NULL);

    DetectAppLayerMpmRegister("tls.subjectaltname", SIG_FLAG_TOSERVER, 2,
            PrefilterMpmTlsSubjectAltNameRegister, NULL, ALPROTO_TLS, TLS_STATE_CERT_READY);
...
static InspectionBuffer *TlsSubjectAltNameGetData(DetectEngineThreadCtx *det_ctx,
        const DetectEngineTransforms *transforms, Flow *f, uint8_t flags, uint8_t idx, int list_id)
{
    SCEnter();
    ...
    if (flags & STREAM_TOSERVER) { // STODO only toclient?
    ...
    // STODO Remove comment. Verified that the buffers are setup correctly
...
static uint8_t DetectEngineInspectTlsSubjectAltName(DetectEngineCtx *de_ctx,
        ...
        const Signature *s, Flow *f, uint8_t flags, void *alstate, void *txv, uint64_t tx_id)
{
    // STODO control never seems to come here. Check if needed
        ...
        const uint8_t flags)
{
    ...
    for (uint8_t i = 0;; i++) {
        ...
        if (buffer->inspect_len >= mpm_ctx->minlen) {
            // STODO Search doesn't yield anything
