...
static uint32_t ProtoNameHashFunc(HashTable *ht, void *data, uint16_t datalen)
{
    ...
    for (int i = 0; i < len; i++) {
        hash += tolower((unsigned char)p->name[i]);
    }
...
static char ProtoNameHashCompareFunc(void *data1, uint16_t datalen1, void *data2, uint16_t datalen2)
{
    ...
    return (len1 == len2 &&
            (memcmp(p1->name, p2->name, len1) == 0 || memcmp(p1->alias, p2->name, len1) == 0));
}

static void ProtoNameAddEntry(
        const char *proto_name, const char *alias_name, const int proto_number)
{
    ...
    if (alias_name) {
        proto_ent->alias = SCStrdup(alias_name);
    }

    SCLogDebug("new protocol entry: name: \"%s\" [alias: \"%s\"]; protocol number: %d",
            proto_ent->name, proto_ent->alias ? proto_ent->alias : "n/a", proto_ent->number);
    if (0 != HashTableAdd(proto_ht, proto_ent, 0)) {
        SCLogNotice("Unable to add hash table entry for "
                    "name: \"%s\" [alias: \"%s\"]; number: %d",
                proto_ent->name, proto_ent->alias, proto_ent->number);
        SCFree(proto_ent);
    }
}
...
void SCProtoNameInit(void)
{
    ...
    for (uint32_t i = 0; i < ARRAY_SIZE(known_proto); i++) {
        ...
        ProtoNameAddEntry(known_proto[i], proto_aliases[i], i);
    }
}
...
bool SCGetProtoByName(const char *protoname, uint8_t *proto_number)
{
    ...
    ProtoNameHashEntry *proto_ent = HashTableLookup(proto_ht, &proto, 0);
    if (proto_ent) {
