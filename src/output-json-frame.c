...
static void FrameAddPayload(JsonBuilder *js, const TcpStream *stream, const Frame *frame)
{
    ...
    if (frame->rel_offset < 0) {
        offset = (uint64_t)(frame->rel_offset + (int64_t)STREAM_BASE_OFFSET(stream));
        if (StreamingBufferGetData(&stream->sb, &data, &sb_data_len, &data_offset) == 0) {
            SCLogDebug("NO DATA1");
            return;
        }
    } else {
        data_offset = offset = (uint64_t)frame->rel_offset + STREAM_BASE_OFFSET(stream);
        SCLogDebug("offset %" PRIu64, offset);
        if (StreamingBufferGetDataAtOffset(&stream->sb, &data, &sb_data_len, (uint64_t)offset) ==
                0) {
            SCLogDebug("NO DATA1");
            return;
        }
    ...
    SCLogDebug("frame stream_offset %" PRIu64 ", data_offset %" PRIu64
               ", data_len %u frame len %" PRIi64,
            offset, data_offset, sb_data_len, frame->len);
        ...
        const Packet *p, JsonBuilder *jb)
{
    ...
    jb_set_uint(jb, "offset", (uint64_t)abs_offset);
...
static int FrameJson(ThreadVars *tv, JsonFrameLogThread *aft, const Packet *p)
{
    ...
    for (uint32_t idx = 0; idx < frames->cnt; idx++) {
        ...
        if (frame != NULL && frame->rel_offset >= 0) {
            ...
            SCLogDebug("abs_offset %" PRIi64
                       ", frame->rel_offset %d, frames->progress_rel %d win %" PRIi64,
