        ...
        AlertJsonOutputCtx *json_output_ctx, const PacketAlert *pa, JsonBuilder *jb)
{
    ...
    while (kv) {
        const size_t key_size = MIN(kv->key_len, REFERENCE_SYSTEM_NAME_MAX);
        const size_t ref_size = MIN(kv->reference_len, REFERENCE_CONTENT_NAME_MAX);
        const size_t size_needed = key_size + ref_size + 1;
        char kv_store[size_needed];
        // When the reference begins with the key, don't append the key
        // e.g., the reference URI may already contain the scheme son
        // don't append https:// again
        if (strncmp(kv->reference, kv->key, key_size) == 0)
