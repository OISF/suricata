...
MimeDecField * MimeDecFillField(MimeDecEntity *entity, const char *name,
    ...
    if (field == NULL) {
        SCLogError(SC_ERR_MEM_ALLOC, "MimeDecAddField() failed to allocate field");
        return NULL;
    }
 ...
 * \return none
 */
static void DecodeBase64Block(uint8_t ascii[ASCII_BLOCK], uint8_t b64[B64_BLOCK]) {
 ...
 */
static uint32_t DecodeBase64(uint8_t *dest, const char *src, uint32_t len) {
    ...
    if (numDecoded == 0) {
        ...
        PrintChars(SC_LOG_DEBUG, "Base64 failed string", (char*) src, len);
    }
 ...
 */
static char * FindString(const char *src, uint32_t len, const char *find) {
    ...
    for (i = 0; i < len && src[i] != 0; i++) {
        ...
        for (j = 0; i + j < len && find[j] != 0; j++) {

            /* If two chars don't match, then check for other case */
            if (src[i + j] != find[j]) {

                /* Check 'A' against 'a' */
                if (src[i + j] >= UC_START && src[i + j] <= UC_END) {
                    if (src[i + j] + UC_LC_DIFF != find[j]) {
                        break;
                    }

                /* Check 'a' against 'A' */
                } else if (src[i + j] >= LC_START && src[i + j] <= LC_END) {
                    if (src[i + j] - UC_LC_DIFF != find[j]) {
                        break;
                    }
                } else {
                    /* Otherwise a match not found */
                    break;
                }
            }
        }
...
/**
 * \brief Function determines whether a host string is a numeric IP address
 *
 ...
 */
static int IsIpHost(char *host, uint32_t len) {

    int isIp = 1;
    uint32_t i;

    /* Traverse through until end of string or we found non-numeric IP host */
    for (i = 0; i < len && host[i] != 0 && host[i] != '/'; i++) {
...
/**
 * \brief Identified if the IP provided is in fact a valid IP address
 *
 * \param ip The host string
 * \param len The host string length
 *
 * \retval 1 The host is a valid IP address
 * \retval 0 The host is not a valid IP address
 */
static int IsValidIP(char *ip, uint32_t len){
        ...
        if (fptr != NULL) {
            ...
            if (tok == fptr) {
                ...
                if (tempUrlLen > 0) {
                    ...
                    if (IsIpHost(tempUrl, tempUrlLen)) {
                        flags |= URL_IS_IP;

                        PrintChars(SC_LOG_DEBUG, "IP URL", tempUrl, tempUrlLen);

                        /* Check if numeric IP is invalid */
                        if (!IsValidIP(tempUrl, tempUrlLen)) {
                            flags |= URL_IS_INVALID_IP;

                            PrintChars(SC_LOG_DEBUG, "INVALID IP URL ", tempUrl,
                                    tempUrlLen);
                        }
                    }
...
static int MimeIsIpHostTest01(void) {

    if(IsIpHost("192.168.1.1", 11) != 1){
        return 1;
    }

    if(IsIpHost("999.oogle.com", 14) != 0){
        return 1;
    }
...
static int MimeIsValidIPTest01(void) {

    if (IsValidIP("192.168.1.1", 11) != 1) {
        return -1;
    }

    if (IsValidIP("192.168.255.255", 15) != 1) {
        return -1;
    }

    if (IsValidIP("192.168.255.255/testurl.html", 28) != 1) {
        return -1;
    }

    if (IsValidIP("www.google.com", 14) != 0) {
        return -1;
    }
...
void MimeDecRegisterTests(void) {
    ...
    UtRegisterTest("MimeIsIpHostTest01", MimeIsIpHostTest01, 0);
    UtRegisterTest("MimeIsValidIPTest01", MimeIsValidIPTest01, 0);
#endif /* UNITTESTS */
}
