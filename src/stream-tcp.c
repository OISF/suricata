...
static SCMutex ssn_pool_mutex =
        SCMUTEX_INITIALIZER; /**< init only, protect initializing and growing pool */
#ifdef DEBUG
...
void StreamTcpInitConfig(bool quiet)
{
        ...
        if (RunmodeIsUnittests()) {
            ...
            if (ConfGetNode("stream.prealloc-sessions") != NULL) {
                WarnInvalidConfEntry(
                        "stream.prealloc_sessions", "%" PRIu32, stream_config.prealloc_sessions);
            }
        }
    }
    if (!quiet) {
    ...
    if (!quiet) {
        SCLogConfig("stream \"midstream\" session pickups: %s",
                stream_config.midstream ? "enabled" : "disabled");
    }
    ...
    if (!quiet) {
        SCLogConfig("stream \"async-oneside\": %s",
                stream_config.async_oneside ? "enabled" : "disabled");
    }
    ...
    if (!quiet) {
        SCLogConfig("stream \"checksum-validation\": %s",
                stream_config.flags & STREAMTCP_INIT_FLAG_CHECKSUM_VALIDATION ? "enabled"
                                                                              : "disabled");
    }
    ...
    if (!quiet) {
        SCLogConfig("stream.\"inline\": %s",
                stream_config.flags & STREAMTCP_INIT_FLAG_INLINE ? "enabled" : "disabled");
    }
    ...
    if (!quiet) {
        SCLogConfig("stream \"bypass\": %s",
                (stream_config.flags & STREAMTCP_INIT_FLAG_BYPASS) ? "enabled" : "disabled");
    }
    ...
    if (ConfGet("stream.reassembly.memcap", &temp_stream_reassembly_memcap_str) == 1) {
        ...
        if (ParseSizeStringU64(temp_stream_reassembly_memcap_str, &stream_reassembly_memcap_copy) <
                0) {
    ...
    if (ConfGet("stream.reassembly.depth", &temp_stream_reassembly_depth_str) == 1) {
        if (ParseSizeStringU32(temp_stream_reassembly_depth_str, &stream_config.reassembly_depth) <
                0) {
            SCLogError("Error parsing "
    ...
    if (RunmodeIsUnittests()) {
        ...
        if (ssn_pool == NULL) {
            ssn_pool = PoolThreadInit(1, /* thread */
                    ...
                    StreamTcpSessionPoolInit, NULL, StreamTcpSessionPoolCleanup, NULL);
        }
        SCMutexUnlock(&ssn_pool_mutex);
    }
#endif
    ...
    {                                                                                              \
        if ((ssn)->flags & STREAMTCP_FLAG_ASYNC) {                                                 \
            if (SEQ_GT((stream)->next_seq, (stream)->last_ack)) {                                  \
                ...
                SCLogDebug("ssn %p: ASYNC last_ack set to %" PRIu32 ", moved %u forward", (ssn),   \
                        (stream)->next_seq, ack_diff);                                             \
            }                                                                                      \
        }                                                                                          \
    }
...
static inline void StreamTcpCloseSsnWithReset(Packet *p, TcpSession *ssn)
{
    ...
    SCLogDebug("ssn %p: (state: %s) Reset received and state changed to "
               "TCP_CLOSED",
            ssn, StreamTcpStateAsString(ssn->state));
}
...
static int StreamTcpPacketIsRetransmission(TcpStream *stream, Packet *p)
{
    ...
    if (SEQ_LT(TCP_GET_SEQ(p), stream->last_ack) &&
            SEQ_GT((TCP_GET_SEQ(p) + p->payload_len), stream->last_ack)) {
        ...
        if (TCP_HAS_TFO(p)) {
            ...
            if (p->payload_len) {
                ...
                SCLogDebug("ssn: %p (TFO) [len: %d] isn %u base_seq %u next_seq %u payload len %u",
                        ssn, p->tcpvars.tfo.len, ssn->client.isn, ssn->client.base_seq,
...
static TcpStateQueue *StreamTcp3whsFindSynAckBySynAck(TcpSession *ssn, Packet *p)
{
    ...
    while (q != NULL) {
        if (search.flags == q->flags && search.wscale == q->wscale && search.win == q->win &&
                search.seq == q->seq && search.ack == q->ack && search.ts == q->ts) {
            return q;
        }
    ...
    if (p->tcph->th_flags & TH_RST) {
        ...
        if (ssn->queue_len) {
            ...
            if (q != NULL) {
                ...
                SCLogDebug("ssn %p: none found, now checking ACK against original SYN/ACK (state)",
                        ssn);
            }
        }
        ...
        if (PKT_IS_TOCLIENT(p)) {
            ...
            if (ssn->flags & STREAMTCP_FLAG_MIDSTREAM_SYNACK) {
                ...
                if (StreamTcpInlineMode()) {
                    if (p->payload_len > 0 && SEQ_EQ(TCP_GET_ACK(p), ssn->client.last_ack) &&
        ...
        if ((SEQ_EQ(TCP_GET_SEQ(p), ssn->client.next_seq)) &&
            ...
            SCLogDebug("ssn %p: ACK for missing data: ssn->client.next_seq %u", ssn,
                    ssn->client.next_seq);
            ssn->server.window = TCP_GET_WINDOW(p) << ssn->server.wscale;
            ssn->server.next_win = ssn->server.last_ack + ssn->server.window;
        ...
        ThreadVars *tv, TcpSession *ssn, Packet *p, StreamTcpThread *stt)
{
    ...
    if (zerowindowprobe) {
        ...
        SCLogDebug("ack %u last_ack %u next_seq %u", TCP_GET_ACK(p), ssn->server.last_ack,
                ssn->server.next_seq);
        ...
        ThreadVars *tv, TcpSession *ssn, Packet *p, StreamTcpThread *stt)
{
    ...
    if (zerowindowprobe) {
        ...
        SCLogDebug("ssn %p: seq %" PRIu32 " in window, ssn->server.next_win "
                   "%" PRIu32 "",
                ssn, TCP_GET_SEQ(p), ssn->server.next_win);
        ssn->client.window = TCP_GET_WINDOW(p) << ssn->client.wscale;
        ...
        ThreadVars *tv, Packet *p, StreamTcpThread *stt, TcpSession *ssn)
{
    ...
    if (p->tcph->th_flags & TH_RST) {
        ...
        if (PKT_IS_TOSERVER(p)) {
            ...
            if ((p->tcph->th_flags & TH_ACK) && StreamTcpValidateAck(ssn, &ssn->server, p) == 0)
                StreamTcpUpdateLastAck(
                        ssn, &ssn->server, StreamTcpResetGetMaxAck(&ssn->server, TCP_GET_ACK(p)));

            StreamTcpUpdateLastAck(
                    ssn, &ssn->client, StreamTcpResetGetMaxAck(&ssn->client, TCP_GET_SEQ(p)));
            ...
            if ((p->tcph->th_flags & TH_ACK) && StreamTcpValidateAck(ssn, &ssn->client, p) == 0)
                StreamTcpUpdateLastAck(
                        ssn, &ssn->client, StreamTcpResetGetMaxAck(&ssn->client, TCP_GET_ACK(p)));

            StreamTcpUpdateLastAck(
                    ssn, &ssn->server, StreamTcpResetGetMaxAck(&ssn->server, TCP_GET_SEQ(p)));
        ...
        SCLogDebug("ssn (%p: FIN received SEQ"
                   ...
                   " win %" PRIu32 "",
                ssn, ssn->server.next_seq, ssn->client.last_ack, ssn->server.next_win,
                ssn->server.window);
        ...
        SCLogDebug("ssn %p: SYN/ACK packet on state ESTABLISHED... resent. "
                   "Likely due server not receiving final ACK in 3whs",
                ssn);
        return 0;
        ...
        if (PKT_IS_TOSERVER(p)) {
            ...
            SCLogDebug("ssn %p: next SEQ %" PRIu32 ", last ACK %" PRIu32 ","
                       " next win %" PRIu32 ", win %" PRIu32 "",
                    ssn, ssn->client.next_seq, ssn->server.last_ack, ssn->client.next_win,
                    ssn->client.window);
...
static int StreamTcpHandleFin(ThreadVars *tv, StreamTcpThread *stt, TcpSession *ssn, Packet *p)
{
    if (PKT_IS_TOSERVER(p)) {
        ...
        SCLogDebug("ssn %p: =+ next SEQ %" PRIu32 ", last ACK %" PRIu32 "", ssn,
        ...
        if (SEQ_GEQ(TCP_GET_SEQ(p), ssn->server.last_ack) &&
            ...
            SCLogDebug("ssn %p: -> SEQ mismatch, packet SEQ %" PRIu32 " != "
                    ...
                    ssn, TCP_GET_SEQ(p), ssn->server.next_seq, ssn->server.last_ack,
                    ssn->server.window, (ssn->server.last_ack + ssn->server.window));
        ...
        ThreadVars *tv, Packet *p, StreamTcpThread *stt, TcpSession *ssn)
{
    ...
    if (p->tcph->th_flags & TH_RST) {
        ...
        if (PKT_IS_TOSERVER(p)) {
            ...
            if (p->payload_len > 0 &&
                    (SEQ_LEQ((TCP_GET_SEQ(p) + p->payload_len), ssn->client.last_ack))) {
                SCLogDebug("ssn %p: -> retransmission", ssn);
            ...
            if (p->payload_len > 0 &&
                    (SEQ_LEQ((TCP_GET_SEQ(p) + p->payload_len), ssn->server.last_ack))) {
                SCLogDebug("ssn %p: -> retransmission", ssn);
                StreamTcpSetEvent(p, STREAM_CLOSEWAIT_PKT_BEFORE_LAST_ACK);
                SCReturnInt(-1);
...
static int StreamTcpPacketIsKeepAliveACK(TcpSession *ssn, Packet *p)
{
    ...
    if ((ostream->flags & STREAMTCP_STREAM_FLAG_KEEPALIVE) && ack == ostream->last_ack &&
            seq == stream->next_seq) {
    ...
    SCLogDebug("seq %u (%u), ack %u (%u) FLAG_KEEPALIVE: %s", seq, stream->next_seq, ack,
            ostream->last_ack,
...
static int StreamTcpPacketIsFinShutdownAck(TcpSession *ssn, Packet *p)
{
    ...
    if (p->flags & PKT_PSEUDO_STREAM_END)
        return 0;
    if (!(ssn->state == TCP_TIME_WAIT || ssn->state == TCP_CLOSE_WAIT ||
    ...
    SCLogDebug("%" PRIu64 ", seq %u ack %u stream->next_seq %u ostream->next_seq %u", p->pcap_cnt,
            seq, ack, stream->next_seq, ostream->next_seq);
...
static int StreamTcpPacketIsBadWindowUpdate(TcpSession *ssn, Packet *p)
{
    ...
    if (pkt_win < ostream->window) {
        ...
        if (diff > p->payload_len && SEQ_GT(ack, ostream->next_seq) &&
                SEQ_GT(seq, stream->next_seq)) {
            SCLogDebug("%" PRIu64 ", pkt_win %u, stream win %u, diff %u, dsize %u", p->pcap_cnt,
                    pkt_win, ostream->window, diff, p->payload_len);
            ...
            if (((pkt_win > 1024) && (diff > (adiff + 32))) ||
                ...
                SCLogDebug("pkt ACK %u is %u bytes beyond last_ack %u, shrinks window by %u "
                           "(allowing 32 bytes extra): pkt WIN %u",
                        ack, adiff, ostream->last_ack, diff, pkt_win);
...
int StreamTcpPacket(ThreadVars *tv, Packet *p, StreamTcpThread *stt, PacketQueueNoLock *pq)
{
    ...
    if (ssn != NULL) {
        ...
        if (ssn->flags & STREAMTCP_FLAG_ASYNC && ssn->client.tcp_flags != 0 &&
                ssn->server.tcp_flags != 0) {
    ...
    if (ssn != NULL) {
        ...
        } else if (g_detect_disabled && (ssn->client.flags & STREAMTCP_STREAM_FLAG_NOREASSEMBLY) &&
...
static inline int StreamTcpValidateChecksum(Packet *p)
{
    ...
    if (p->level4_comp_csum == -1) {
        if (PKT_IS_IPV4(p)) {
            p->level4_comp_csum = TCPChecksum(p->ip4h->s_ip_addrs, (uint16_t *)p->tcph,
                    (p->payload_len + TCP_GET_HLEN(p)), p->tcph->th_sum);
        } else if (PKT_IS_IPV6(p)) {
            p->level4_comp_csum = TCPV6Checksum(p->ip6h->s_ip6_addrs, (uint16_t *)p->tcph,
                    (p->payload_len + TCP_GET_HLEN(p)), p->tcph->th_sum);
        }
    }
...
static int TcpSessionReuseDoneEnoughSynAck(const Packet *p, const Flow *f, const TcpSession *ssn)
{
    if (FlowGetPacketDirection(f, p) == TOCLIENT) {
        if (ssn == NULL) {
            SCLogDebug(
                    "steam starter packet %" PRIu64 ", ssn %p null. No reuse.", p->pcap_cnt, ssn);
            return 0;
        }
...
TmEcode StreamTcpThreadInit(ThreadVars *tv, void *initdata, void **data)
{
    ...
    *data = (void *)stt;
    ...
    stt->ra_ctx->counter_tcp_reass_overlap_diff_data =
            StatsRegisterCounter("tcp.overlap_diff_data", tv);
    ...
    if (ssn_pool == NULL) {
        ...
        stt->ssn_pool_id = 0;
        SCLogDebug(
                "pool size %d, thread ssn_pool_id %d", PoolThreadSize(ssn_pool), stt->ssn_pool_id);
    } else {
...
static int StreamTcpValidateRst(TcpSession *ssn, Packet *p)
{
    ...
    */
    ...
    if (PKT_IS_TOSERVER(p)) {
        ...
        if (p->tcph->th_flags & TH_ACK && TCP_GET_ACK(p) &&
    ...
    switch (os_policy) {
        ...
        case OS_POLICY_VISTA:
            if (PKT_IS_TOSERVER(p)) {
                ...
                if (SEQ_EQ(TCP_GET_SEQ(p), ssn->server.next_seq)) {
                    SCLogDebug("reset is valid! Packet SEQ: %" PRIu32 " Stream %u", TCP_GET_SEQ(p),
                            ssn->server.next_seq);
                    return 1;
                } else {
                    SCLogDebug("reset is not valid! Packet SEQ: %" PRIu32 " and"
...
static inline int StreamTcpValidateAck(TcpSession *ssn, TcpStream *stream, Packet *p)
{
    ...
    if (SEQ_LT(ack, stream->last_ack)) {
        SCLogDebug(
                "pkt ACK %" PRIu32 " < stream last ACK %" PRIu32, TCP_GET_ACK(p), stream->last_ack);
        ...
        if (stream->window != 0 && SEQ_LT(ack, (stream->last_ack - stream->window))) {
            SCLogDebug("ACK %" PRIu32 " is before last_ack %" PRIu32 " - window "
                       "%" PRIu32 " = %" PRIu32,
                    ack, stream->last_ack, stream->window, stream->last_ack - stream->window);
            goto invalid;
        }
    ...
    if (ssn->state > TCP_SYN_SENT && SEQ_GT(ack, stream->next_win)) {
         ...
         * the syn ack */
    } else if (ssn->state == TCP_SYN_SENT && PKT_IS_TOCLIENT(p) && p->tcph->th_flags & TH_RST &&
    ...
    SCLogDebug("default path leading to invalid: ACK %" PRIu32 ", last_ack %" PRIu32
               " next_win %" PRIu32,
            ack, stream->last_ack, stream->next_win);
invalid:
