...
TmEcode StreamTcpThreadInit(ThreadVars *tv, void *initdata, void **data)
{
    ...
    *data = (void *)stt;
    ...
    if (stream_config.midstream_policy != EXCEPTION_POLICY_NOT_SET) {
        ...
        for (uint8_t i = 0; i < EXCEPTION_POLICY_MAX; i++) {
            bool is_eps_valid = IsMidstreamExceptionPolicyStatsValid(i);
            if (is_eps_valid) {
                if (stream_config.midstream) {
                    snprintf(stream_midstream_enabled_eps_stats.eps_name[i],
                            sizeof(stream_midstream_enabled_eps_stats.eps_name[i]), "%s%s",
                            eps_midstream_str, ExceptionPolicyEnumToString(i, true));
                    stt->counter_tcp_midstream_eps.eps_id[i] = StatsRegisterCounter(
                            stream_midstream_enabled_eps_stats.eps_name[i], tv);
                } else {
                    snprintf(stream_midstream_disabled_eps_stats.eps_name[i],
                            sizeof(stream_midstream_disabled_eps_stats.eps_name[i]), "%s%s",
                            eps_midstream_str, ExceptionPolicyEnumToString(i, true));
                    stt->counter_tcp_midstream_eps.eps_id[i] = StatsRegisterCounter(
                            stream_midstream_disabled_eps_stats.eps_name[i], tv);
                }
            }
        }
    }
