...
static int AFXDPGetDevFlags(int fd, const char *ifname)
{
    struct ifreq ifr;
    ...
    if (ioctl(fd, SIOCGIFFLAGS, &ifr) == -1) {
        SCLogError(SC_ERR_AFXDP_CREATE, "Unable to find type for iface \"%s\": %s", ifname,
                strerror(errno));
        return -1;
    }
...
static TmEcode AFXDPTryReopen(AFXDPThreadVars *ptv)
{
    ...
    /* open (any) socket and check if state = UP or DOWN */
    int sock_fd = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    if (sock_fd == -1) {
        SCLogError(SC_ERR_AFXDP_CREATE, "Couldn't create a AF_PACKET socket, error %s",
                strerror(errno));
        SCReturnInt(TM_ECODE_FAILED);
    }

    int if_flags = AFXDPGetDevFlags(sock_fd, ptv->iface);
    if (if_flags == -1) {
