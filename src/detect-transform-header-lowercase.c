 ...
 *  \retval -1 failure
 */
static int DetectTransformHeaderLowercaseSetup(DetectEngineCtx *de_ctx, Signature *s, const char *optstr)
{
    SCEnter();
...
void DetectTransformHeaderLowercaseRegister(void)
{
    ...
    sigmatch_table[DETECT_TRANSFORM_HEADER_LOWERCASE].desc = "modify buffer via lowercaseing header names";
    sigmatch_table[DETECT_TRANSFORM_HEADER_LOWERCASE].url = "/rules/transforms.html#header_lowercase";
