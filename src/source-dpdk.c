...
static void ReceiveDPDKSetOperationMode(DPDKThreadVars *ptv, DPDKIfaceConfig *iconf);
...
static inline void DPDKDumpCountersRing(DPDKThreadVars *ptv)
{
    ...
    pkts = ptv->prefilter.rx_ring.stats[ptv->queue_id].enq_fail_objs +
           ptv->prefilter.rx_ring.stats[ptv->queue_id].enq_success_objs;
    StatsSetUI64(ptv->tv, ptv->capture_dpdk_imissed,
            pkts - ptv->prefilter.rx_ring.stats[ptv->queue_id].deq_success_objs);
    StatsSetUI64(ptv->tv, ptv->capture_dpdk_tx_errs,
            ptv->prefilter.tx_ring.stats[ptv->queue_id.enq_fail_objs]);
#else
    pkts = ptv->pkts;
#endif
...
static inline void DPDKReleasePacketTxOrFree(Packet *p)
{
    if (p->dpdk_v.tx_ring == NULL) {
        ...
        BUG_ON(PKT_IS_PSEUDOPKT(p));
        rte_ring_enqueue_burst(p->dpdk_v.tx_ring, (void **)&p->dpdk_v.mbuf, 1, NULL);
    }
}
    ...
    while (1) {
        ...
        if (ptv->op_mode == DPDK_ETHDEV_MODE) {
            ...
            nb_rx = rte_ring_dequeue_burst(
                    ptv->prefilter.rx_ring, (void **)ptv->received_mbufs, BURST_SIZE, NULL);
        }
        if (unlikely(nb_rx == 0)) {
            continue;
        }
        ...
        for (uint16_t i = 0; i < nb_rx; i++) {
            ...
            p->dpdk_v.tx_ring = ptv->prefilter.tx_ring;
...
void ReceiveDPDKSetOperationMode(DPDKThreadVars *ptv, DPDKIfaceConfig *iconf)
{
    if (iconf->flags & DPDK_PREFILTER) {
        ptv->op_mode = DPDK_RING_MODE;
    } else {
        ptv->op_mode = DPDK_ETHDEV_MODE;
    }
}
...
{
    ptv->prefilter.rx_ring = iconf->rx_rings[queue_id];
    iconf->rx_rings[queue_id] = NULL;
    ptv->prefilter.tx_ring = iconf->tx_rings[queue_id];
...
static TmEcode ReceiveDPDKThreadInit(ThreadVars *tv, const void *initdata, void **data)
{
    ...
    ReceiveDPDKSetOperationMode(ptv, dpdk_config);
    if (ptv->op_mode == DPDK_ETHDEV_MODE) {
    ...
    } else if (ptv->op_mode == DPDK_RING_MODE) { // DPDK prefilter is active
...
static void ReceiveDPDKThreadExitStatsRing(DPDKThreadVars *ptv)
{
    ...
    SCLogPerf("(%s): Total RX stats of %s: packets %" PRIu64, ptv->tv->name,
            ptv->prefilter.rx_ring->name, pkts);
