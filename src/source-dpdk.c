...
static void DPDKReleasePacket(Packet *p)
{
    ...
    if (!PKT_IS_PSEUDOPKT(p) && (p->dpdk_v.copy_mode == DPDK_COPY_MODE_TAP ||
    ...
    while (1) {
        ...
        for (uint16_t i = 0; i < nb_rx; i++) {
            ...
            if (unlikely(p == NULL)) {
                ...
                // exiting Suricata, we must free all remaining mbufs fetched from the NIC
            ...
            if (PacketSetData(p, rte_pktmbuf_mtod(p->dpdk_v.mbuf, uint8_t *),
                ...
                // freeing currently processed packet handles Suricata (that's why it is skipped)
...
void ReceiveDPDKThreadExitStats(ThreadVars *tv, void *data)
{
    ...
    if (ptv->queue_id == 0) {
        ...
        rte_eth_dev_get_name_by_port(ptv->port_id, port_name);
        rte_eth_stats_get(ptv->port_id, &eth_stats);
