                    ...
                    const Signature *s, const SigMatchCtx *ctx)
{
    ...
    if (ad->flags & ASN1_ABSOLUTE_OFFSET) {
        ...
        // Range check the relative_offset before downcasting to uint16_t
        if (ad->relative_offset > UINT16_MAX
            || ad->relative_offset < -UINT16_MAX) {
                return 0;
        }

        // Check for over/underflows
        if (ad->relative_offset > 0) {
            if (det_ctx->buffer_offset > (UINT16_MAX - (uint16_t)ad->relative_offset)) {
                // overflow: offset exceeds max payload length
                return 0;
            }
        } else {
            if (det_ctx->buffer_offset < (uint16_t)(-ad->relative_offset)) {
                // underflow: offset is invalid in regards to the buffer
                return 0;
            }
        }

        offset = det_ctx->buffer_offset + ad->relative_offset;
    } else {
        offset = 0;
    }
