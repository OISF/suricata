...
static DeStateStore *AllocDeStateStore(void)
{
...
static void DeStateStoreFileNoMatch(DetectEngineState *de_state, uint16_t file_no_match, uint8_t direction)
{
...
int DeStateFlowHasInspectableState(Flow *f, uint16_t alversion, uint8_t direction)
{
    ...
    if (f->de_state == NULL || f->de_state->dir_state[direction & STREAM_TOSERVER ? 0 : 1].cnt == 0) {
        if (f->alproto == ALPROTO_HTTP) {
            if (AppLayerTransactionGetInspectId(f, direction) >= AppLayerGetNoOfTxs(ALPROTO_HTTP, f->alstate))
                r = 2;
            else
                r = 0;
        }
    } else if (f->de_state->dir_state[direction & STREAM_TOSERVER ? 0 : 1].alversion == alversion)
        r = 2;
    else
        r = 1;
                                ...
                                void *alstate, uint16_t alproto, uint16_t alversion)
{
    ...
    if (alproto == ALPROTO_HTTP) {
        ...
        total_txs = AppLayerGetNoOfTxs(alproto, htp_state);
        ...
        for (; tx_id < total_txs; tx_id++) {
            ...
            if (tx_id == (total_txs - 1)) {
                if (AppLayerGetProgress(ALPROTO_HTTP, AppLayerGetTx(alproto, alstate, tx_id), direction) <
                    AppLayerGetProgressDoneValue(ALPROTO_HTTP, direction)) {
                    store_de_state = 1;
    ...
    DeStateStoreFileNoMatch(f->de_state, file_no_match, flags);
                                    ...
                                    uint16_t alproto, uint16_t alversion)
{
    ...
    if (alproto == ALPROTO_HTTP) {
        ...
        total_txs = AppLayerGetNoOfTxs(alproto, alstate);
    }
    ...
    for (; store != NULL; store = store->next) {
             ...
             store_cnt++, state_cnt++)
        {
            ...
            if (alproto == ALPROTO_HTTP) {
                ...
                engine = app_inspection_engine[ALPROTO_HTTP][(flags & STREAM_TOSERVER) ? 0 : 1];
                while (engine != NULL) {
                    if (!(item->flags & engine->inspect_flags) &&
                        s->sm_lists[engine->sm_list] != NULL)
                    {
                        match = engine->Callback(tv, de_ctx, det_ctx, s, f,
                                                 flags, alstate, inspect_tx_id);
                        if (match == 1) {
                            inspect_flags |= engine->inspect_flags;
                            engine = engine->next;
                            continue;
