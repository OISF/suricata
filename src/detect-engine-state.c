...
void DetectEngineStateRestoreByteValues(DetectEngineThreadCtx *det_ctx, DetectEngineState *state)
{
    ...
    /* Swap pointers — avoids memcpy, det_ctx takes ownership of cached values */
...
static inline void ResetTxState(DetectEngineState *s)
{
    if (s) {
        ...
        /* byte_values are cleared since this is a reload and local_ids may change */
        if (s->byte_values != NULL) {
            memset(s->byte_values, 0, s->byte_values_size * sizeof(uint64_t));
        }
    }
}
