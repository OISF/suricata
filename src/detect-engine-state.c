...
void DetectEngineStateRestoreByteValues(DetectEngineThreadCtx *det_ctx, DetectEngineState *state)
{
    ...
    /* Exchange the per-tx cache with the worker scratch buffer. This is a
     * strict permutation: ownership of each allocation moves with the
     * pointer, nothing is aliased, and no copy is made. After the swap,
     * det_ctx holds what state used to hold and vice versa, so each
     * buffer still has exactly one owner and will be freed exactly once
