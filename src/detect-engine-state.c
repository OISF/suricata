...
int DeStateFlowHasInspectableState(Flow *f, uint16_t alversion, uint8_t direction)
{
    ...
    if (f->de_state == NULL || f->de_state->dir_state[direction & STREAM_TOSERVER ? 0 : 1].cnt == 0) {
        if (f->alproto == ALPROTO_HTTP) {
            if (AppLayerTransactionGetInspectId(f, direction) >= AppLayerGetTxCnt(ALPROTO_HTTP, f->alstate))
                r = 2;
            else
                r = 0;
        }
    ...
    if (alproto == ALPROTO_HTTP) {
        ...
        for (; tx_id < total_txs; tx_id++) {
            ...
            if (tx_id == (total_txs - 1)) {
                if (AppLayerGetAlstateProgress(ALPROTO_HTTP, AppLayerGetTx(alproto, alstate, tx_id), direction) <
                    AppLayerGetAlstateProgressCompletionStatus(ALPROTO_HTTP, direction)) {
                    store_de_state = 1;
