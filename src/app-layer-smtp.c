...
static int SMTPProcessCommandDATA(
        SMTPState *state, SMTPTransaction *tx, Flow *f, AppLayerParserState *pstate)
{
    SCEnter();
    DEBUG_VALIDATE_BUG_ON(tx == NULL);
    ...
    if (state->current_line_len == 1 && state->current_line[0] == '.') {
        ...
        // ProcessDataChunk
        FileAppendData(&tx->files_ts, state->current_line,
    ...
    if (state->current_command == SMTP_COMMAND_DATA &&
        ...
        if (smtp_config.decode_mime && tx->mime_state != NULL) {
            int ret = MimeDecParseLine((const uint8_t *)state->current_line,
                    state->current_line_len, state->current_line_delimiter_len, tx->mime_state);
            if (ret != MIME_DEC_OK) {
                              ...
                              AppLayerParserState *pstate)
{
    ...
    switch (state->current_command) {
        ...
        case SMTP_COMMAND_DATA:
            return SMTPProcessCommandDATA(state, tx, f, pstate);
