...
typedef struct DetectFtpModeData_ {
    bool active;
} DetectFtpModeData;
        ...
        void *txv, const Signature *s, const SigMatchCtx *m)
{
    ...
    return 1 ? ftpmoded->active == tx->active : 0;
}
...
{
    DetectFtpModeData *ftpmoded = SCCalloc(1, sizeof(DetectFtpModeData));
    if (unlikely(ftpmoded == NULL))
        return NULL;

    char *token = SCParseToken(optstr, strlen(optstr));
    if (!token) {
        SCLogError("unable to extract token value from \"%s\"", optstr);
        goto error;
    } else if (!strcmp(token, "active")) {
        ftpmoded->active = true;
    } else if (!strcmp(token, "passive")) {
        ftpmoded->active = false;
    } else {
