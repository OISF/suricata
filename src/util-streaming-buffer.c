        ...
        StreamingBuffer *sb, const StreamingBufferConfig *cfg, const uint64_t slide_offset)
{
    ...
    if (to_shift) {
        ...
        if (s > 0) {
            ...
            if (to_shift->next && slide_offset + new_mem_size >= to_shift->next->stream_offset) {
                ...
                if (start->buf_size < next->buf_size) {
                    ...
                    SCMemmove(next->buf + next_data_offset, next->buf_size - next_data_offset,
                    ...
                    SCMemmove(start->buf, start->buf_size, start->buf + s, new_data_size);
        ...
        just_main:
            SCLogDebug("s %u new_data_size %u", s, new_data_size);
            SCMemmove(to_shift->buf, to_shift->buf_size, to_shift->buf + s, new_data_size);
        ...
        if (sb->head != NULL) {
            ...
            if (slide < sb->region.buf_size) {
                ...
                SCLogDebug("sliding %u forward, size of original buffer left after slide %u", slide,
                        size);
                SCMemmove(sb->region.buf, sb->region.buf_size, sb->region.buf + slide, size);
        ...
        uint32_t dst_buf_size)
{
    ...
    if (dst_copy_offset != 0)
        SCMemmove(dst->buf + dst_copy_offset, dst->buf_size - dst_copy_offset, dst->buf, old_size);
