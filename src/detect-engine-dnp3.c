    ...
    void *alstate, void *txv, uint64_t tx_id)
{
    ...
    if (detect->type == DNP3_DETECT_FUNCTION_CODE) {
        return tx->app_function_code == detect->function_code;
    }
    else if (detect->type == DNP3_DETECT_INTERNAL_INDICATOR) {
        return ((tx->iin1 & (detect->ind_flags >> 8)) ||
            (tx->iin2 & (detect->ind_flags & 0xf)));
    }
