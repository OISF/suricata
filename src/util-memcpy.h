...
/** \brief AVX-512 optimized 64 byte version of MemcpyToLower
 *  Reuses logic from SCMemcmpLowercaseAVX512_512.
 *  For each vector, see if elements are within the UPPER range,
 *  and if it is, add the 0x20 to turn them into lowercase.
 */
...
#if defined(__AVX2__)
static inline void MemcpyToLowerAVX2(uint8_t *dst, const uint8_t *src, size_t n);
#endif
...
#if defined(__AVX2__)
    if (n >= 32) {
        return MemcpyToLowerAVX2(d, s, n);
    }
#endif
#if defined(__SSE4_2__)
    if (n >= 16) {
...
/** \brief AVX2 optimized 32 byte version of MemcpyToLower
 *  Reuses logic from SCMemcmpLowercaseAVX2.
 *  For each vector, see if elements are within the UPPER range,
 *  and if it is, add the 0x20 to turn them into lowercase.
 */
static inline void MemcpyToLowerAVX2(uint8_t *dst, const uint8_t *src, size_t n)
{
    ...
    do {
        ...
        /* unaligned loading of the bytes to copy */
...
/** \brief SSE4.2 optimized 16 byte version of MemcpyToLower
 *  Reuses logic from SCMemcmpLowercaseSSE42.
 *  For each vector, see if elements are within the UPPER range,
 *  and if it is, add the 0x20 to turn them into lowercase.
 */
static inline void MemcpyToLowerSSE42(uint8_t *dst, const uint8_t *src, size_t n)
