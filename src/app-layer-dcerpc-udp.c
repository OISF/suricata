...
{
    if (flags & 0x04) {
        return rs_dcerpc_udp_parse_request(f, dcerpc_state, pstate, input, input_len,
                                           local_data, flags);
    }
    return rs_dcerpc_udp_parse_response(f, dcerpc_state, pstate, input, input_len,
                                        local_data, flags);
}
...
static uint16_t DCERPCUDPProbe(Flow *f, uint8_t direction,
...
{
    if (AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_UDP, ALPROTO_DCERPC,
            "|04 00|", 2, 0, STREAM_TOSERVER, DCERPCUDPProbe, 0, 0) < 0)
    {
        return -1;
    }
