...
static inline int packetGetIcmpv6MTU(Packet *p, uint32_t *picmpv6mtu) {
 ...
 * \param m pointer to the sigmatch that we will cast into DetectU32Data
 *
 * \retval 0 no match
 * \retval 1 match
 */
...
{
    uint32_t picmpv6mtu;
    if (packetGetIcmpv6MTU(p, &picmpv6mtu) == 0) {
        return 0;
    }
...
{
    //DetectU32Data *icmpv6mtud = (DetectU32Data *)ptr;
    SCFree(ptr);
}
