...
typedef struct DetectMQTTTypeData_ {
    uint32_t query_type;
} DetectMQTTTypeData;
...
void DetectMQTTTypeRegister (void)
{
    ...
    sigmatch_table[DETECT_AL_MQTT_TYPE].url = DOC_URL DOC_VERSION "/rules/mqtt-keywords.html#mqtt-type";
...
static int DetectMQTTTypeMatch(DetectEngineThreadCtx *det_ctx,
                               ...
                               const SigMatchCtx *ctx)
{
    const DetectMQTTTypeData *de = (const DetectMQTTTypeData *)ctx;
    uint32_t type;
 ...
 */
static DetectMQTTTypeData *DetectMQTTTypeParse(const char *rawstr)
{
    int i;
    DetectMQTTTypeData *de = NULL;
#define MAX_SUBSTRINGS 30
    ...
    de = SCMalloc(sizeof(DetectMQTTTypeData));
    if (unlikely(de == NULL))
        goto error;

    de->query_type = types[i].code;
 ...
 */
void DetectMQTTTypeFree(void *de_ptr)
{
    DetectMQTTTypeData *de = (DetectMQTTTypeData *)de_ptr;
    if(de) SCFree(de);
}
