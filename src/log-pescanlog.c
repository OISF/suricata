...
#define PE_SECT_FLAG_UDATA    0x00000080
#define PE_SECT_FLAG_RES1     0x00010000
#define PE_SECT_FLAG_RES2     0x00020000
#define PE_SECT_FLAG_RES3     0x00040000
#define PE_SECT_FLAG_RES4     0x00080000
#define PE_SECT_FLAG_ALIGN    0x00f00000
#define PE_SECT_FLAG_DISCARD  0x02000000
#define PE_SECT_FLAG_NOCACHE  0x04000000
#define PE_SECT_FLAG_NOPAGE   0x08000000
#define PE_SECT_FLAG_SHARE    0x10000000
#define PE_SECT_FLAG_EXEC     0x20000000
...
typedef struct LogPescanLogThread_ {
    LogPescanFileCtx *pescanlog_ctx; /**< LogFileCtx has the pointer to the file
                                      * and a mutex to allow multithreading */
    uint32_t pe_cnt; /**< Count of PEs found */
    MemBuffer *buffer; /**< Output buffer for writing log entries */
    uint8_t *scanbuf; /**< Output buffer for scanning files for PE */
} LogPescanLogThread;

/**
 * \brief General safe function to add a variable argument string to an existing
 ...
 */
static int AddToBuffer(char *buf, uint32_t len, uint32_t pos,
        const char * format, ...) {
    ...
    /* First make sure there is at least two bytes available
     * (one char + null-terminating char) */
    if (pos > len - 2) {
        return 0;
    }
    ...
    if (tlen > MAX_FIELD_SIZE) {
        SCLogDebug("temp size exceeded");
        return 0;
    }

    /* Not enough room to add anything so quit */
    if (tlen + pos > len - 2) {
        SCLogDebug("total buf size exceeded");
        return 0;
    }
...
/**
 * \brief Prints the PE Score and anomaly flag information into a pre-existing
 * string buffer
 *
 * The last parameter is a pointer to the libpescan function
 * "double pescore(peattrib_t *)" which should be passed in if the score is to be
 * re-generated, otherwise NULL is acceptable when the score was already generated
 * elsewhere.
 *
 ...
 * \retval n number of bytes added to the buffer (excluding null-terminator)
 */
static uint32_t PrintPEScore( peattrib_t * wp, char * buf, uint32_t len) {
    uint32_t pos = 0, idx;

    /* Init buffer */
    buf[0] = 0;

    /* Always provide score, flags, and flag count */
    pos += AddToBuffer(buf, len, pos, "pe_score=%g ", wp->pescore);
    pos += AddToBuffer(buf, len, pos, "pe_flags=0x%x ", wp->fvflags);
    pos += AddToBuffer(buf, len, pos, "pe_flagcnt=%d ", wp->fvflagcnt);
    ...
    for (idx = 0; idx < PE_ANOMALY_COUNT; idx++) {
        ...
        if (wp->fv[idx] > 0) {
            pos += AddToBuffer(buf, len, pos, "%s=%ld ", pe_anomalies[idx],
                    wp->fv[idx]);
        }
    }
...
/**
 * \brief Prints the PE Header information (excluding score / anomaly flags)
 * into a pre-existing string buffer
 *
 * This function only prints information that is meaningful for any PE and is
 * not used as a classifier for good / bad PEs.
 *
 * \param wp a pointer to the PE header attributes struct
 * \param buf the buffer in which to add the string
 * \param len the size allocated to the buffer
...
static uint32_t PrintPEHeader( peattrib_t *wp, char *buf, uint32_t len ) {
    ...
    pos += AddToBuffer(buf, len, pos, "pe_hdr_type=%s ",
            (wp->pesize==PE_32)?"PE32":((wp->pesize==PE_64)?"PE64":"??") );
    pos += AddToBuffer(buf, len, pos, "pe_os_version=%d.%d ",
            wp->os_major,wp->os_minor);
    ...
    pos += AddToBuffer(buf, len, pos, "pe_link_ver=%d.%d ", wp->link_major,
            wp->link_minor);
    ...
    for(i = 0; i < wp->nsections; i++) {
        ...
        pos += AddToBuffer(buf, len, pos, "' ");
        pos += AddToBuffer(buf, len, pos,"pe_section_raw_size=0x%x ",
                wp->sx[i].raw_size);
        pos += AddToBuffer(buf, len, pos, "pe_section_raw_offset=0x%x ",
                wp->sx[i].raw_offset);
        pos += AddToBuffer(buf, len, pos, "pe_section_virt_size=0x%x ",
                wp->sx[i].virt_size);
        pos += AddToBuffer(buf, len, pos, "pe_section_virt_addr=0x%x ",
                wp->sx[i].virt_rva);
 ...
 * \param ipproto IP Protocol (AF_INET for IPv4 or AF_INET6 for IPv6)
 *
 * \retval TM_ECODE_OK on Success
 */
static TmEcode LogPescanLogIPWrapper(ThreadVars *tv, Packet *p, void *data,
        PacketQueue *pq, PacketQueue *postpq, int ipproto) {
    SCEnter();
    ...
    for (ff = ffc->head; ff != NULL; ff = ff->next) {
        ...
        if (ff->flags & FILE_NOPESCAN) {
            ...
            /* Any files that were not scanned by the detector should be scanned
             * here */
            PEScanConfig *pecfg = PEScanGetConfig();
            if (ff->chunks_head != NULL && ((ff->state == FILE_STATE_CLOSED) ||
                    (pecfg != NULL && ff->size >= pecfg->wait_scan_bytes))) {

                /* Scan the file */
                PEScanFile(ff, aft->scanbuf);
            } else {
                continue;
            }
        }

        /* If not a PE, then skip to the next file */
        if (ff->peattrib == NULL) {
            continue;
        }

        /* If we reached this point, then we have a PE
         * that has been scanned but needs to be logged
         */
        ...
        PrintRawUriBuf((char *) aft->buffer->buffer, &aft->buffer->offset,
                aft->buffer->size, ff->name, ff->name_len);
        MemBufferWriteString(aft->buffer, "\" ");

        MemBufferWriteString(aft->buffer, "PE-SCORE ");
        pebuflen = PrintPEScore(ff->peattrib, pebuf, PE_BUFFER_SIZE);
        if (pebuflen >= 0) {
            MemBufferWriteString(aft->buffer, "[");
            PrintRawUriBuf((char *) aft->buffer->buffer, &aft->buffer->offset,
                    aft->buffer->size, (uint8_t *) pebuf, pebuflen);
            MemBufferWriteString(aft->buffer, "] ");
        }
 ...
 * \retval TM_ECODE_OK on Success
 */
TmEcode LogPescanLogIPv4(ThreadVars *tv, Packet *p, void *data, PacketQueue *pq,
        PacketQueue *postpq) {
 ...
 * \retval TM_ECODE_OK on Success
 */
TmEcode LogPescanLogIPv6(ThreadVars *tv, Packet *p, void *data, PacketQueue *pq,
        PacketQueue *postpq) {
 ...
 *
 * The function callback does quick checks such as making sure the packet is
 * contained within a valid flow and is using the TCP protocol.
 *
 * \param tv pointer to thread-specific variables (unused)
 * \param p pointer to the current packet
 * \param data pointer to the current threading context
 ...
 * \retval TM_ECODE_OK on Success
 */
TmEcode LogPescanLog (ThreadVars *tv, Packet *p, void *data, PacketQueue *pq,
        PacketQueue *postpq) {
    SCEnter();
...
TmEcode LogPescanLogThreadInit(ThreadVars *t, void *initdata, void **data) {
    ...
    if (unlikely(initdata == NULL)) {
        SCLogDebug("Error getting context for PEScanLog.  \"initdata\" argument NULL");
        SCFree(aft);
        return TM_ECODE_FAILED;
    }

    aft->buffer = MemBufferCreateNew(OUTPUT_BUFFER_SIZE);
    if (unlikely(aft->buffer == NULL)) {
        SCFree(aft);
        return TM_ECODE_FAILED;
    }
    ...
    aft->scanbuf = SCMalloc(maxSize);
    if (unlikely(aft->scanbuf == NULL)) {
        SCFree(aft);
        return TM_ECODE_FAILED;
    }
...
OutputCtx *LogPescanLogInitCtx(ConfNode *conf) {
    ...
    if (unlikely(pescanlog_ctx == NULL)) {
        LogFileFreeCtx(file_ctx);
        return NULL;
    }
