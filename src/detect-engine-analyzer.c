...
analyzer_t AnalyzerItems[] = {
    ...
    { 0, 0,  false, true, "file_data", "http server body"},
    ...
    { 0, 0,  false, false, "http_protocol", "http response line" },
    ...
    { 0, 0,  false, false, "http.server", "http response line" },
    { 0, 0,  false, false, "http.location", "http response line" },
};

/* Map items to AnalyzerItems array; the key is the item id */
int32_t AnalyzerItemMap[300];

void EngineAnalysisItemsInit(void);
void EngineAnalysisItemsInit(void)
{
    memset(AnalyzerItemMap, -1, sizeof(AnalyzerItemMap));

    for(size_t i = 0; i < sizeof(AnalyzerItems)/sizeof(analyzer_t); i++) {
        analyzer_t *analyzer_item = &AnalyzerItems[i];
        analyzer_item->item_id = DetectBufferTypeGetByName(analyzer_item->item_name);
        AnalyzerItemMap[analyzer_item->item_id] = i;
    }
}
        ...
        const Signature *s, const char *line)
{
    ...
    for (list_id = 0; list_id < (int)s->init_data->smlists_array_size; list_id++) {
        ...
        for (sm = s->init_data->smlists[list_id]; sm != NULL; sm = sm->next) {
            if (sm->type == DETECT_PCRE) {
                int32_t item_slot = AnalyzerItemMap[list_id];
                if (item_slot == -1) {
                    rule_pcre++;
                    continue;
                }
                ...
                rule_pcre_http++;
                AnalyzerItems[item_slot].have_bufcount++;
                ...
                if (AnalyzerItems[item_slot].check_encoding_match) {
    ...
    // XXX fix this
    uint32_t http_method_buf;
    uint32_t http_server_body_buf;
    for (size_t i = 0; i < sizeof(AnalyzerItems)/sizeof(analyzer_t); i++) {
        if (0 == strcmp(AnalyzerItems[i].item_name, "http_method")) http_method_buf = AnalyzerItems[i].have_bufcount;
        if (0 == strcmp(AnalyzerItems[i].item_name, "file_data")) http_server_body_buf = AnalyzerItems[i].have_bufcount;
    }
    if (http_method_buf) {
