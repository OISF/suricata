...
static bool LogUnixSocketNewThreadedCtx(
        LogFileCtx *parent_ctx, const char *log_path, int sock_type, ThreadLogFileHashEntry *entry);
        ...
        SCConfNode *conf, LogFileCtx *log_ctx, const char *default_filename, int rotate)
{
    ...
    if (log_ctx->threaded) {
        if (strcasecmp(filetype, "unix_stream") == 0) {
            FatalError("Socket stream file type do not support threaded output, use client side "
                       "multiple connections with this type of socket");
        }
    }
#endif
    ...
    if (!new) {
        if (parent_ctx->is_sock) {
            ...
            if (LogUnixSocketNewThreadedCtx(
                ...
                SCLogError("Unable to open slot %d for file %s", entry->slot_number,
                        parent_ctx->filename);
        ...
        LogFileCtx *parent_ctx, const char *log_path, int sock_type, ThreadLogFileHashEntry *entry)
{
    ...
    *thread = *parent_ctx;
    ...
    /* Don't bail if socket is not available yet. May be able to connect later.
     * SCLogFileWriteSocket() handles fp == NULL by attempting reconnection. */
    thread->fp = SCLogOpenUnixSocketFp(fname, sock_type, 1);
    thread->sock_type = sock_type;
    thread->filename = SCStrdup(fname);
    if (!thread->filename) {
        SCLogError("Unable to duplicate filename for context entry %p", entry);
        goto error;
    }
    thread->is_regular = false;
