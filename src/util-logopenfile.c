...
void LogFileDeInitCtx(LogFileCtx *lf_ctx, int thread_id)
{
    if (lf_ctx->type == LOGFILE_TYPE_PLUGIN && lf_ctx->threaded) {
        if (lf_ctx->threads->lf_slots[thread_id]) {
            LogFileCtx *thread_file_ctx = lf_ctx->threads->lf_slots[thread_id];
            lf_ctx->plugin.plugin->ThreadDeinit(
                    thread_file_ctx->plugin.init_data, thread_file_ctx->plugin.thread_data);
            lf_ctx->threads->lf_slots[thread_id] = NULL;
            SCFree(thread_file_ctx);
        }
    }
}
    ...
    if (lf_ctx->threaded) {
        ...
        for(int i = 0; i < lf_ctx->threads->slot_count; i++) {
            if (lf_ctx->threads->lf_slots[i]) {
                if (lf_ctx->type == LOGFILE_TYPE_FILE) {
        ...
        if (lf_ctx->type == LOGFILE_TYPE_PLUGIN) {
            lf_ctx->plugin.plugin->ThreadDeinit(
                    lf_ctx->plugin.init_data, lf_ctx->plugin.thread_data);
        ...
        if (lf_ctx->type == LOGFILE_TYPE_PLUGIN) {
            ...
            if (lf_ctx->type != LOGFILE_TYPE_PLUGIN && lf_ctx->parent) {
