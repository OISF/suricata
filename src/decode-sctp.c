    ...
    while (offset < len) {
        ...
        switch (chunk->type) {
            ...
            case SCTP_CHUNK_TYPE_INIT_ACK:
                ...
                if (chunk->type == SCTP_CHUNK_TYPE_INIT && vtag != 0) {
        ...
        const uint8_t *pkt, uint16_t len)
{
    ...
    if (p->l4.vars.sctp.has_init) {
        ...
        if (p->l4.vars.sctp.first_chunk == SCTP_CHUNK_TYPE_INIT_ACK) {
    ...
    if (p->l4.vars.sctp.first_chunk == SCTP_CHUNK_TYPE_SHUTDOWN) {
...
static int SCTPDecodeValidInitTest01(void)
{
    ...
    FAIL_IF(p->sp != 1234);
    FAIL_IF(p->dp != 80);
    FAIL_IF(p->l4.vars.sctp.first_chunk != SCTP_CHUNK_TYPE_INIT);
...
static int SCTPDecodeMultiChunkTest06(void)
{
    ...
    FAIL_IF(p->l4.vars.sctp.first_chunk != SCTP_CHUNK_TYPE_DATA);
    FAIL_IF(!p->l4.vars.sctp.has_data);
    FAIL_IF(p->l4.vars.sctp.has_init);
...
static int SCTPDecodeInitNotAloneTest07(void)
{
    uint8_t raw_sctp[] = {
        ...
        0x01,
        0x00,
        0x01, /* num_outbound=1, num_inbound=1 */
        0x00,
        0x00,
        0x00,
        0x01, /* initial_tsn=1 */
