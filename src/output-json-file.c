...
static void ndpiJsonBuilderTLSQUIC(Flow *f, JsonBuilder *js, ThreadVars *tv)
{
    ...
    if (!unknown_tls_version) {
        ...
        jb_set_string(js, "cipher", ndpi_cipher2str(f->ndpi_flow->protos.tls_quic.server_cipher, unknown_cipher));
        ...
        if (f->ndpi_flow->protos.tls_quic.tls_supported_versions)
            jb_set_string(js, "tls_supported_versions", f->ndpi_flow->protos.tls_quic.tls_supported_versions);
...
void ndpiJsonBuilder (Flow *f, JsonBuilder *js, ThreadVars *tv)
{
    ...
    if (f->ndpi_flow->risk) {
        ...
        for (risk_id = 0; risk_id < NDPI_MAX_RISK; risk_id++)
            ...
            char str[256];
            snprintf(str, sizeof(str), "%s (%s)", ndpi_risk2code(risk_id), ndpi_risk2str(risk_id));
    ...
    switch (l7_protocol.proto.master_protocol ? l7_protocol.proto.master_protocol : l7_protocol.proto.app_protocol) {
       ...
       case NDPI_PROTOCOL_UBNTAC2:
        ...
        case NDPI_PROTOCOL_NATPMP:
            ...
            inet_ntop(AF_INET, &f->ndpi_flow->protos.natpmp.external_address.ipv4, buf, sizeof(buf));
        ...
        case NDPI_PROTOCOL_HTTP_PROXY:
            ...
            if (f->ndpi_flow->http.request_content_type != NULL) {
                jb_set_string(js, "request_content_type",
                              f->ndpi_flow->http.request_content_type);
            }
            if (f->ndpi_flow->http.detected_os != NULL) {
                jb_set_string(js, "detected_os",
                              f->ndpi_flow->http.detected_os);
            }
            if (f->ndpi_flow->http.nat_ip != NULL) {
                jb_set_string(js, "nat_ip",
                              f->ndpi_flow->http.nat_ip);
            }
            jb_close(js);
            break;
        case NDPI_PROTOCOL_QUIC:
            ...
            ndpi_quic_version2str(quic_version, sizeof(quic_version),
                                  f->ndpi_flow->protos.tls_quic.quic_version);
        ...
        case NDPI_PROTOCOL_MAIL_IMAP:
            ...
            jb_set_uint(js, "auth_failed",
                        f->ndpi_flow->l4.tcp.ftp_imap_pop_smtp.auth_failed);
            jb_close(js);
            break;
        ...
        case NDPI_PROTOCOL_STUN:
            ...
            if (f->ndpi_flow->stun.mapped_address.port) {
                jb_set_string(js,  "mapped_address", print_ndpi_address_port(&f->ndpi_flow->stun.mapped_address, buf, sizeof(buf)));
            }
            if (f->ndpi_flow->stun.peer_address.port) {
                jb_set_string(js,  "peer_address", print_ndpi_address_port(&f->ndpi_flow->stun.peer_address, buf, sizeof(buf)));
            }
            if (f->ndpi_flow->stun.relayed_address.port) {
                jb_set_string(js,  "relayed_address", print_ndpi_address_port(&f->ndpi_flow->stun.relayed_address, buf, sizeof(buf)));
            }
            if (f->ndpi_flow->stun.response_origin.port) {
                jb_set_string(js,  "response_origin", print_ndpi_address_port(&f->ndpi_flow->stun.response_origin, buf, sizeof(buf)));
            }
            if (f->ndpi_flow->stun.other_address.port) {
                jb_set_string(js,  "other_address", print_ndpi_address_port(&f->ndpi_flow->stun.other_address, buf, sizeof(buf)));
            }
            jb_close(js);
            break;
