        ...
        const uint8_t flags, const AppProto alproto)
{
    ...
    if (PKT_IS_TOSERVER(p)) {
        ...
        SCLogDebug("toserver: frames %p", frames);
    } else {
        frames = &frames_container->toclient;
        SCLogDebug("toclient: frames %p", frames);
    }
                ...
                if (frame->type == e->type) {
                    ...
                    if (e->v1.Callback(det_ctx, e, s, p, frames, frame, idx) == true) {
                        ...
                        *alert_flags |= PACKET_ALERT_FLAG_FRAME;
                        ...
                        SCLogDebug("sig %u match! frame id %" PRIi64, s->id, det_ctx->frame_id);
                        return true;
                    }
        ...
        const Frame *frame, const int list_id, const uint32_t idx, const bool first)
{
    ...
    if (data_right_edge > acked)
        data_len = acked - offset;

    const int64_t frame_start_abs_offset = frame->rel_offset + (int64_t)STREAM_BASE_OFFSET(stream);
    const uint64_t usable_right_edge = MIN(data_right_edge, acked);

    bool have_end = false;

    if (frame->len > 0) {
        const int64_t frame_avail_data_abs = (int64_t)MIN(acked, data_right_edge);
    ...
    SCLogDebug("frame %p rel_offset %d type %u len %" PRIi64 " ci_flags %02x (start:%s, end:%s)",
