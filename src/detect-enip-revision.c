...
{
    uint16_t value;
    if (!rs_enip_get_revision(txv, &value))
        SCReturnInt(0);
...
void DetectEnipRevisionRegister(void)
{
    ...
    DetectAppLayerInspectEngineRegister2("enip.revision", ALPROTO_ENIP, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.revision", ALPROTO_ENIP, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectGenericList, NULL);
