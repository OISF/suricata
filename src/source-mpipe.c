...
/* Release Packet and always send a copy. */
...
Packet *MpipeProcessPacket(MpipeThreadVars *ptv, gxio_mpipe_idesc_t *idesc)
{
    ...
    if (unlikely((p->mpipe_v.copy_mode = channel_to_equeue[idesc->channel].copy_mode) !=
        ...
        if (p->mpipe_v.copy_mode == MPIPE_COPY_MODE_IPS)
          p->ReleasePacket = MpipeReleasePacketCopyIPS;
        else
          p->ReleasePacket = MpipeReleasePacketCopyTap;
    } else {
...
TmEcode ReceiveMpipeThreadInit(ThreadVars *tv, void *initdata, void **data)
{
    ...
    unsigned int num_workers = tile_num_pipelines;
