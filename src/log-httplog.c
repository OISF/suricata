                            ...
                            PacketQueue *postpq, int ipproto)
{
    ...
    total_txs = AppLayerGetNoOfTxs(ALPROTO_HTTP, htp_state);
    tx_id = AppLayerTransactionGetLogId(p->flow);
    tx_progress_done_value_ts = AppLayerGetProgressDoneValue(ALPROTO_HTTP, 0);
    tx_progress_done_value_tc = AppLayerGetProgressDoneValue(ALPROTO_HTTP, 1);
    ...
    for (; tx_id < total_txs; tx_id++)
    {
        ...
        if (!(((AppLayerParserStateStore *)p->flow->alparser)->id_flags & APP_LAYER_TRANSACTION_EOF)) {
            tx_progress = AppLayerGetProgress(ALPROTO_HTTP, tx, 0);
            if (tx_progress < tx_progress_done_value_ts)
                break;

            tx_progress = AppLayerGetProgress(ALPROTO_HTTP, tx, 1);
