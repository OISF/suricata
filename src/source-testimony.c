...
#ifndef HAVE_TESTIMONY

TmEcode NoTestimonySupportExit(ThreadVars *tv, const void *initdata, void **data);

void TmModuleReceiveTestimonyRegister (void)
{
    tmm_modules[TMM_RECEIVEAFP].name = "ReceiveTestimony";
    tmm_modules[TMM_RECEIVEAFP].ThreadInit = NoTestimonySupportExit;
...
{
    tmm_modules[TMM_DECODEAFP].name = "DecodeTestimony";
    tmm_modules[TMM_DECODEAFP].ThreadInit = NoTestimonySupportExit;
...
TmEcode NoTestimonySupportExit(ThreadVars *tv, const void *initdata, void **data)
{
    SCLogError(SC_ERR_NO_TESTIMONY,"Error creating thread %s: you do not have "
               "support for Testimony enabled, on Linux host please recompile "
               "with --enable-testimony", tv->name);
    exit(EXIT_FAILURE);
}

#else /* We have Testimony support */
#include <testimony.h>
