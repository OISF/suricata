...
static void *SCACCudaDispatcher(void *arg)
{
    ...
    SCLogInfo("AC Cuda Mpm Dispatcher using a timeout of \"%"PRIu32"\" micro-seconds", sleep_interval_ms);

    MTSBA_Data *mtsba_data = CudaHL_Module_GetData(MPM_AC_CUDA_MODULE_NAME, MPM_AC_CUDA_MODULE_MTSBA_NAME);
    ...
    if (CudaHL_GetCudaModule(&cuda_module, "util-mpm-ac-cuda-kernel") < 0) {
    ...
    r = SCCudaMemAlloc(&cuda_packets_buffer_d, 50 * 1024 * 1024);
    if (r < 0) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "SCCudaMemAlloc failure.");
        exit(EXIT_FAILURE);
    }
    r = SCCudaMemAlloc(&cuda_offset_buffer_d, 200 * 1024 * 1024);
    if (r < 0) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "SCCudaMemAlloc failure.");
        exit(EXIT_FAILURE);
    }
    r = SCCudaMemAlloc(&cuda_results_buffer_d, 400 * 1024 * 1024);
    if (r < 0) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "SCCudaMemAlloc failure.");
        exit(EXIT_FAILURE);
    }
    r = SCCudaMemAllocHost((void **)&cuda_results_buffer_h, 400 * 1024 * 1024);
    if (r < 0) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "SCCudaMemAlloc failure.");
        exit(EXIT_FAILURE);
    }

    MTSBA_CulledInfo mtsba_culled_info;
    memset(&mtsba_culled_info, 0, sizeof(mtsba_culled_info));
    ...
    while (1) {
        ...
        /**************** 1 SEND ****************/
        MTSBA_CullCompletedSlices(mtsba_data, &mtsba_culled_info);
        if (mtsba_culled_info.no_of_items == 0)
            continue;
#if 0
        SCLogInfo("1 - mtsba_culled_info.no_of_items-%"PRIu32" "
                  "mtsba_culled_info.buffer_len - %"PRIu32" "
                  "mtsba_culled_info.average size - %f "
                  "mtsba_culled_info.d_buffer_start_offset - %"PRIu32" "
                  "mtsba_culled_info.op_buffer_start_offset - %"PRIu32" "
                  "mtsba_data.no_of_items - %"PRIu32"  "
                  "mtsba_data.d_buffer_read - %"PRIu32" "
                  "mtsba_data.d_buffer_write - %"PRIu32" "
                  "mtsba_data.op_buffer_read - %"PRIu32" "
                  "mtsba_data.op_buffer_write - %"PRIu32"\n",
                  mtsba_culled_info.no_of_items,
                  mtsba_culled_info.d_buffer_len,
                  mtsba_culled_info.d_buffer_len / (float)mtsba_culled_info.no_of_items,
                  mtsba_culled_info.d_buffer_start_offset,
                  mtsba_culled_info.op_buffer_start_offset,
                  mtsba_data->no_of_items,
                  mtsba_data->d_buffer_read,
                  mtsba_data->d_buffer_write,
                  mtsba_data->op_buffer_read,
                  mtsba_data->op_buffer_write);
#endif
        r = SCCudaMemcpyHtoD(cuda_packets_buffer_d, (mtsba_data->d_buffer + mtsba_culled_info.d_buffer_start_offset), mtsba_culled_info.d_buffer_len);
        if (r < 0) {
            SCLogError(SC_ERR_AC_CUDA_ERROR, "SCCudaMemcpyHtoD failure.");
            exit(EXIT_FAILURE);
        }
        r = SCCudaMemcpyHtoD(cuda_offset_buffer_d, (mtsba_data->o_buffer + mtsba_culled_info.op_buffer_start_offset), sizeof(uint32_t) * mtsba_culled_info.no_of_items);
        if (r < 0) {
            SCLogError(SC_ERR_AC_CUDA_ERROR, "SCCudaMemcpyHtoD failure.");
            exit(EXIT_FAILURE);
        }
        ...
        r = SCCudaLaunchKernel(kernel,
                               (mtsba_culled_info.no_of_items / 32) + 1, 1, 1,
                               32, 1, 1,
                               0, 0,
                               args, NULL);
        if (r < 0) {
        ...
        r = SCCudaMemcpyDtoH(cuda_results_buffer_h, cuda_results_buffer_d, sizeof(uint32_t) * (mtsba_culled_info.d_buffer_len * 2));
        if (r < 0) {
        ...
        /************* 1 Parse Results ************/
        uint32_t i_op_start_offset = mtsba_culled_info.op_buffer_start_offset;
        uint32_t no_of_items = mtsba_culled_info.no_of_items;
        uint32_t *o_buffer = mtsba_data->o_buffer;
        uint32_t d_buffer_start_offset = mtsba_culled_info.d_buffer_start_offset;
        for (uint32_t i = 0; i < no_of_items; i++, i_op_start_offset++) {
            Packet *p = (Packet *)mtsba_data->p_buffer[i_op_start_offset];

            p->cuda_gpu_matches = cuda_results_buffer_h[((o_buffer[i_op_start_offset] - d_buffer_start_offset) * 2)];
            if (p->cuda_gpu_matches != 0) {
                memcpy(p->cuda_results,
                       cuda_results_buffer_h + ((o_buffer[i_op_start_offset] - d_buffer_start_offset) * 2),
                       (cuda_results_buffer_h[((o_buffer[i_op_start_offset] - d_buffer_start_offset) * 2)] * sizeof(uint32_t) * 2) + 4);
            }
        ...
        if (no_of_items != 0)
            MTSBA_ReportCulledConsumption(mtsba_data, &mtsba_culled_info);
    } /* while (1) */
                                          ...
                                          PatternMatcherQueue *pmq)
{
    ...
    for (uint32_t i = 0; i < cuda_matches; i += 2) {
        uint32_t offset = results[i];
        uint32_t state = results[i + 1];
...
int MpmCudaBufferSetup(void)
{
    ...
    r = SCCudaMemAllocHost((void *)&d_buffer, conf->mtsba_buffer_size);
    if (r < 0) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "Cuda alloc host failure.");
        return -1;
    }
    SCLogInfo("Allocated a cuda d_buffer - %"PRIu32" bytes", conf->mtsba_buffer_size);
    r = SCCudaMemAllocHost((void *)&o_buffer, sizeof(uint32_t) * UTIL_MPM_CUDA_MTSBA_OPBUFFER_ITEMS_DEFAULT);
    if (r < 0) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "Cuda alloc host failue.");
        return -1;
    }
    r = SCCudaMemAllocHost((void *)&p_buffer, sizeof(void *) * UTIL_MPM_CUDA_MTSBA_OPBUFFER_ITEMS_DEFAULT);
    if (r < 0) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "Cuda alloc host failure.");
        return -1;
    }
    ...
    MTSBA_Data *mtsba = MTSBA_RegisterNew(d_buffer, conf->mtsba_buffer_size, o_buffer, p_buffer, UTIL_MPM_CUDA_MTSBA_OPBUFFER_ITEMS_DEFAULT);
    if (mtsba == NULL) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "Error registering new mtsba instance.");
        return -1;
    }
    CudaHL_Module_StoreData(MPM_AC_CUDA_MODULE_NAME, MPM_AC_CUDA_MODULE_MTSBA_NAME, mtsba);
...
int MpmCudaBufferDeSetup(void)
{
    ...
    r = SCCudaMemFreeHost(mtsba_data->d_buffer);
    if (r < 0) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "Error freeing cuda host memory.");
        return -1;
    }
    r = SCCudaMemFreeHost(mtsba_data->o_buffer);
    if (r < 0) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "Error freeing cuda host memory.");
        return -1;
    }
    r = SCCudaMemFreeHost(mtsba_data->p_buffer);
    if (r < 0) {
        SCLogError(SC_ERR_AC_CUDA_ERROR, "Error freeing cuda host memory.");
        return -1;
    }
