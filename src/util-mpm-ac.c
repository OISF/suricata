...
//SCLogInfo("oldsize %d size %d cnt %u ctx->state_count %u", oldsize, size, cnt, ctx->state_count);
...
//SCLogInfo("memsetting at offset %d, %d bytes", oldsize, (size - oldsize));
    memset(((uint8_t *)ctx->output_table + oldsize), 0, (size - oldsize));
//SCLogInfo("ctx->output_table %p, %p, %lu", ctx->output_table, ctx->output_table + oldsize, (ctx->output_table + oldsize) - ctx->output_table);
