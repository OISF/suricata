        ...
        uint16_t addrs_cnt, const Address *a)
{
    ...
    for (uint16_t idx = 0; idx < addrs_cnt; idx++) {
        ...
        if (match_addr[0] == addrs[idx].ip[0] && match_addr[1] == addrs[idx].ip[1] &&
                match_addr[2] == addrs[idx].ip[2] && match_addr[3] == addrs[idx].ip[3]) {
            SCReturnInt(1);
        }
        if (match_addr[0] == addrs[idx].ip2[0] && match_addr[1] == addrs[idx].ip2[1] &&
                match_addr[2] == addrs[idx].ip2[2] && match_addr[3] == addrs[idx].ip2[3]) {
            SCReturnInt(1);
        }
