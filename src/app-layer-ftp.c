                            ...
                            void *local_data, const uint8_t flags)
{
    ...
    if (state->command == FTP_COMMAND_EPRT) {
        // Length includes terminating null character explicitly added in port space alloc
        uint16_t dyn_port = rs_ftp_active_eprt(state->port_line, state->port_line_size);
        if (dyn_port == 0) {
            retcode = 0;
            goto tx_complete;
        }
    ...
    if (state->command == FTP_COMMAND_PORT) {
        if ((flags & STREAM_TOCLIENT)) {
            // Length includes terminating null character explicitly added in port space alloc
            uint16_t dyn_port = rs_ftp_active_port(state->port_line, state->port_line_size);
            if (dyn_port == 0) {
                retcode = 0;
                goto tx_complete;
            }
