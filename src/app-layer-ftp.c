...
static int FTPSetMemcap(uint64_t size);
static uint64_t FTPGetMemcap(void);
...
static void FTPParseMemcap(void)
{
    ...
    MemcapListRegisterMemcap("applayer-proto-ftp", "app-layer.protocols.ftp.memcap", FTPSetMemcap,
            FTPGetMemcap, FTPMemuseGlobalCounter);
