...
int DecodeNSH(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p, const uint8_t *pkt, uint32_t len)
{
    ...
    uint16_t version = ntohs(hdr->ver_flags_len) >> 14;
    if (version != 0) {
    ...
    uint16_t length = (ntohs(hdr->ver_flags_len) & 0x003f) * 4;
    if (length >= len) {
...
#ifdef DEBUG
    uint32_t spi_si = ntohl(hdr->spi_si);
    uint32_t spi = ((spi_si & 0xFFFFFF00) >> 8);
    uint8_t si = (uint8_t) (spi_si & 0xFF);
    SCLogDebug("NSH: version %u length %u spi %u si %u next_protocol %u",
               version, length, spi, si, next_protocol);
#endif
...
static uint8_t valid_nsh_packet[] = {
  0x00, 0x04, 0x02, 0x01, 0x00, 0x00, 0x02, 0x02,
  0x45, 0x10, 0x00, 0x3c, 0x78, 0x8f, 0x40, 0x00,
  0x3f, 0x06, 0x79, 0x05, 0x0b, 0x06, 0x06, 0x06,
  0x33, 0x06, 0x06, 0x06, 0xbd, 0x2e, 0x00, 0x16,
  0xc9, 0xee, 0x07, 0x62, 0x00, 0x00, 0x00, 0x00,
  0xa0, 0x02, 0x16, 0xd0, 0x2f, 0x36, 0x00, 0x00,
  0x02, 0x04, 0x05, 0xb4, 0x04, 0x02, 0x08, 0x0a,
  0xa9, 0x5f, 0x7f, 0xed, 0x00, 0x00, 0x00, 0x00,
  0x01, 0x03, 0x03, 0x07
};
...
static int DecodeNSHTestHeaderTooSmall(void)
{
    ...
    if (!ENGINE_ISSET_EVENT(p, NSH_HEADER_TOO_SMALL)) {
        ret = 0;
    }
...
static int DecodeNSHTestUnsupportedVersion(void)
{
    ...
    if (!ENGINE_ISSET_EVENT(p, NSH_UNSUPPORTED_VERSION)) {
        ret = 0;
    }
...
static int DecodeNSHTestPacketTooSmall(void)
{
    ...
    if (!ENGINE_ISSET_EVENT(p, NSH_BAD_HEADER_LENGTH)) {
        ret = 0;
    }
...
static int DecodeNSHTestReservedType(void)
{
    ...
    if (!ENGINE_ISSET_EVENT(p, NSH_RESERVED_TYPE)) {
        ret = 0;
    }
...
static int DecodeNSHTestUnsupportedType(void)
{
    ...
    DecodeNSH(&tv, &dtv, p, valid_nsh_packet, sizeof(valid_nsh_packet));
    valid_nsh_packet[2] = 0x02;
    if (!ENGINE_ISSET_EVENT(p, NSH_UNSUPPORTED_TYPE)) {
        ret = 0;
    }
...
static int DecodeNSHTestUnknownPayload(void)
{
    ...
    if (!ENGINE_ISSET_EVENT(p, NSH_UNKNOWN_PAYLOAD)) {
        ret = 0;
    }
...
{
#ifdef UNITTESTS
    UtRegisterTest("DecodeNSHTestHeaderTooSmall",
                   DecodeNSHTestHeaderTooSmall);
    UtRegisterTest("DecodeNSHTestUnsupportedVersion",
                   DecodeNSHTestUnsupportedVersion);
    UtRegisterTest("DecodeNSHTestPacketTooSmall",
                   DecodeNSHTestPacketTooSmall);
    UtRegisterTest("DecodeNSHTestReservedType",
                   DecodeNSHTestReservedType);
    UtRegisterTest("DecodeNSHTestInvalidType",
                   DecodeNSHTestInvalidType);
    UtRegisterTest("DecodeNSHTestUnsupportedType",
                   DecodeNSHTestUnsupportedType);
    UtRegisterTest("DecodeNSHTestUnknownPayload",
                   DecodeNSHTestUnknownPayload);
#endif /* UNITTESTS */
}
