        ...
        Flow *f, struct MQTTSubscribeTopicGetDataArgs *cbdata, int list_id, bool first)
{
    ...
    const uint8_t *data;
    uint32_t data_len;
    if (rs_mqtt_tx_get_topic(cbdata->txv, (uint16_t)cbdata->local_id,
                &data, &data_len) == 0) {
        return NULL;
    }
...
void DetectMQTTSubscribeTopicRegister (void)
{
    ...
    sigmatch_table[DETECT_AL_MQTT_SUBSCRIBE_TOPIC].desc = "sticky buffer to match MQTT (UN)SUBSCRIBE topic";
