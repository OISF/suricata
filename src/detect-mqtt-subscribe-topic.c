...
void DetectMQTTSubscribeTopicFree(DetectEngineCtx *de_ctx, void *);
                               ...
                               const SigMatchCtx *ctx)
{
    ...
    rs_mqtt_tx_has_topic(txv, &matched, topic);
    if (matched) {
        return 1;
    }
 ...
 */
static char *DetectMQTTSubscribeTopicParse(const char *rawstr)
{
    char *de = NULL;
 ...
 */
void DetectMQTTSubscribeTopicFree(DetectEngineCtx *de_ctx, void *de_ptr)
{
    char *de = (char *)de_ptr;
    SCFree(de);
}
...
static int MQTTSubscribeTopicTestParse01 (void)
{
    ...
    FAIL_IF_NULL(de);
    DetectMQTTSubscribeTopicFree(NULL, de);
