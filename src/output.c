        ...
        void *state, void *tx, uint64_t tx_id)
{
    ...
    SimpleJsonAppLayerLogger *al = GetAppProtoSimpleJsonLogger(f->alproto);
    if (al == NULL) {
        return TM_ECODE_FAILED;
    }
...
static SimpleJsonAppLayerLogger simple_json_applayer_loggers[ALPROTO_MAX] = {
    ...
    { ALPROTO_TFTP, (SimpleJsonTxLogFunc)rs_tftp_log_json_request },
    { ALPROTO_IKE, NULL }, // special: uses state
    { ALPROTO_KRB5, (SimpleJsonTxLogFunc)rs_krb5_log_json_response },
...
void RegisterAppProtoAppLayerLogger(AppProto alproto, SimpleJsonTxLogFunc log)
{
    if (alproto < ALPROTO_MAX) {
