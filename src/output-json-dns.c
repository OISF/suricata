...
static void LogQuery(json_t *js, DNSTransaction *tx,
...
static void OutputAnswer(json_t *js, DNSTransaction *tx, DNSAnswerEntry *entry)
{
...
static void OutputFailure(json_t *js, DNSTransaction *tx, DNSQueryEntry *entry)
{
...
static void LogAnswers(json_t *js, DNSTransaction *tx, uint64_t tx_id)
{
    ...
    if (tx->rcode) {
        ...
        TAILQ_FOREACH(query, &tx->query_list, next) {
            OutputFailure( js, tx, query);
        }
    }

    DNSAnswerEntry *entry = NULL;
    TAILQ_FOREACH(entry, &tx->answer_list, next) {
        OutputAnswer(js, tx, entry);
    }
...
static void LogQueryToThread(LogDnsLogThread *aft, json_t *djs, DNSTransaction *tx,
        uint64_t tx_id, DNSQueryEntry *entry)
{
...
static void OutputAnswerToThread(LogDnsLogThread *aft, json_t *djs, DNSTransaction *tx, DNSAnswerEntry *entry)
{
...
static void OutputFailureToThread(LogDnsLogThread *aft, json_t *djs, DNSTransaction *tx, DNSQueryEntry *entry)
{
...
static void LogAnswersToThread(LogDnsLogThread *aft, json_t *js, DNSTransaction *tx, uint64_t tx_id)
{
    ...
    if (tx->rcode) {
        ...
        DNSQueryEntry *query = NULL;
        TAILQ_FOREACH(query, &tx->query_list, next) {
            OutputFailureToThread(aft, js, tx, query);
        }
    }

    DNSAnswerEntry *entry = NULL;
    TAILQ_FOREACH(entry, &tx->answer_list, next) {
        OutputAnswerToThread(aft, js, tx, entry);
    }
    ...
    const Packet *p, Flow *f, void *alstate, void *txptr, uint64_t tx_id)
{
    ...
    TAILQ_FOREACH(query, &tx->query_list, next) {
        ...
        LogQueryToThread(td, js, tx, tx_id, query);
    ...
    const Packet *p, Flow *f, void *alstate, void *txptr, uint64_t tx_id)
{
    ...
    LogAnswersToThread(td, js, tx, tx_id);
...
void JsonDNSLogJSON(json_t *js, DNSState *dns_state) 
{
    ...
    TAILQ_FOREACH(tx, &dns_state->tx_list, next) {
        ...
        if (tx->replied) {
            ...
            if (unlikely(sjs == NULL))
                return;

             LogAnswers(js, tx, tx->tx_id);

        } else {

            json_t *qajs = json_array();
            if (unlikely(qajs == NULL))
                return;
            ...
            TAILQ_FOREACH(query, &tx->query_list, next) {
                ...
                LogQuery(qjs, tx, tx->tx_id, query);
                json_array_append_new(qajs, qjs);
           }

           json_object_set_new(js, "queries", qajs);
        }
    }
}
