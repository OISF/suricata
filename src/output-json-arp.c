...
static int JsonArpLogger(ThreadVars *tv, void *thread_data, const Packet *p)
{
    ...
    JsonBuilder *jb = CreateEveHeader((Packet *)p, LOG_DIR_PACKET, "arp", NULL, thread->ctx);
    if (unlikely(jb == NULL)) {
        return TM_ECODE_OK;
    }
    ...
    jb_set_uint(jb, "hw_size", p->arph->hw_size);
    jb_set_uint(jb, "proto_size", p->arph->proto_size);
