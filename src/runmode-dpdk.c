    ...
    if (dev_info->rx_offload_capa & RTE_ETH_RX_OFFLOAD_RSS_HASH) {
        if (iconf->nb_rx_queues > 1) {
            ...
            if (strcmp(dev_info->driver_name, "net_bonding") == 0) {
                dev_driver = net_bonding_device_driver_get(iconf->port_id);
            }
        ...
        const DPDKIfaceConfig *iconf, const struct rte_eth_dev_info *dev_info)
{
    ...
    ret = rte_eth_dev_info_get(iconf->port_id, &post_conf_dev_info);
    if (ret < 0) {
    ...
    if (strcmp(dev_info->driver_name, "net_bonding") == 0) {
        ret = net_bonding_devices_use_same_driver(iconf->port_id);
        if (ret < 0) {
