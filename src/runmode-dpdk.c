...
static void InitEal()
{
    ...
    if (retval < 0) { // retval binded to the result of rte_eal_init
        FatalError(SC_ERR_DPDK_EAL_INIT, "DPDK EAL initialization error");
    }
}
...
static int ConfigSetCopyMode(DPDKIfaceConfig *iconf, const char *entry_str)
{
    SCEnter();
    if (entry_str == NULL) {
        SCLogError(SC_ERR_INVALID_VALUE, "Interface %s has no copy mode specified, changing to %s ",
...
static void DumpRSSFlags(const uint64_t requested, const uint64_t actual)
{
    ...
    SCLogNotice("ETH_RSS_TUNNEL %sset", ((requested & ETH_RSS_TUNNEL) == ETH_RSS_TUNNEL) ? "" : "NOT ");
    ...
    SCLogNotice("ETH_RSS_NONFRAG_IPV4_TCP %sset", (requested & ETH_RSS_NONFRAG_IPV4_TCP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV4_UDP %sset", (requested & ETH_RSS_NONFRAG_IPV4_UDP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV4_SCTP %sset", (requested & ETH_RSS_NONFRAG_IPV4_SCTP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV4_OTHER %sset", (requested & ETH_RSS_NONFRAG_IPV4_OTHER) ? "" : "NOT ");
    ...
    SCLogNotice("ETH_RSS_NONFRAG_IPV6_TCP %sset", (requested & ETH_RSS_NONFRAG_IPV6_TCP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV6_UDP %sset", (requested & ETH_RSS_NONFRAG_IPV6_UDP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV6_SCTP %sset", (requested & ETH_RSS_NONFRAG_IPV6_SCTP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV6_OTHER %sset", (requested & ETH_RSS_NONFRAG_IPV6_OTHER) ? "" : "NOT ");
    ...
    SCLogNotice("ETH_RSS_TUNNEL %sset", ((actual & ETH_RSS_TUNNEL) == ETH_RSS_TUNNEL) ? "" : "NOT ");
    ...
    SCLogNotice("ETH_RSS_NONFRAG_IPV4_TCP %sset", (actual & ETH_RSS_NONFRAG_IPV4_TCP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV4_UDP %sset", (actual & ETH_RSS_NONFRAG_IPV4_UDP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV4_SCTP %sset", (actual & ETH_RSS_NONFRAG_IPV4_SCTP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV4_OTHER %sset", (actual & ETH_RSS_NONFRAG_IPV4_OTHER) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_IPV6 %sset", (actual & ETH_RSS_IPV6) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_FRAG_IPV6 %sset", (actual & ETH_RSS_FRAG_IPV6) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV6_TCP %sset", (actual & ETH_RSS_NONFRAG_IPV6_TCP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV6_UDP %sset", (actual & ETH_RSS_NONFRAG_IPV6_UDP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV6_SCTP %sset", (actual & ETH_RSS_NONFRAG_IPV6_SCTP) ? "" : "NOT ");
    SCLogNotice("ETH_RSS_NONFRAG_IPV6_OTHER %sset", (actual & ETH_RSS_NONFRAG_IPV6_OTHER) ? "" : "NOT ");
    ...
    *port_conf = (struct rte_eth_conf){
    ...
    if (dev_info->rx_offload_capa & DEV_RX_OFFLOAD_RSS_HASH) {
        if (iconf->nb_rx_queues > 1) {
            ...
            // RSS is configured in a specific way for a driver i40e and DPDK version
        ...
        const struct rte_eth_conf *port_conf)
{
    ...
    if (unlikely(iconf->nb_rx_queues >= RTE_ETHDEV_QUEUE_STAT_CNTRS)) {
        SCLogWarning(SC_LOG_WARNING,
                "Thread statistics can be incomplete. Set number of threads to %d "
                "or increase RTE_ETHDEV_QUEUE_STAT_CNTRS when compiling DPDK",
