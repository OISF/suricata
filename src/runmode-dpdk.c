...
static int ConfigSetCopyIface(DPDKIfaceConfig *iconf, const char *entry_str)
{
    ...
    if (retval < 0) {
        SCLogError(SC_ERR_DPDK_CONF,
                "Name of the copy interface (%s) for the interface %s is not valid", entry_str,
                iconf->iface);
        SCReturnInt(retval);
    }
...
static int ConfigSetCopyMode(DPDKIfaceConfig *iconf, const char *entry_str)
{
    SCEnter();
    if (entry_str == NULL) {
        SCLogError(SC_ERR_INVALID_VALUE, "Interface %s has no copy mode specified ", iconf->iface);
        SCReturnInt(-EINVAL);
    }
    ...
    if (strcmp(entry_str, "none") == 0) {
        ...
        SCLogError(SC_ERR_INVALID_VALUE,
                "Copy mode \"%s\" is invalid for interface %s. Choose from (none|tap|ips)",
                entry_str, iconf->iface);
        SCReturnInt(-EINVAL);
    }
