...
static void DetectTransformLuaxformSerialize(uint8_t **data, uint32_t *length, const void *context)
{
    if (context) {
        DetectLuaxformData *lua = (DetectLuaxformData *)context;
        *data = (uint8_t *)lua->serialized_data;
        *length = strlen(lua->serialized_data);
        SCLogNotice("returning %d bytes: \"%s\"", *length, *data);
    }
}
...
static void DetectTransformLuaxformFree(DetectEngineCtx *de_ctx, void *ptr)
{
    if (ptr != NULL) {
        ...
        if (lua->serialized_data)
            SCFree((void *)lua->serialized_data);
...
static DetectLuaxformData *DetectLuaxformParse(DetectEngineCtx *de_ctx, const char *optsstr)
{
    ...
    lua->serialized_data = strdup(optsstr);
    if (unlikely(lua->copystr == NULL || lua->serialized_data == NULL)) {
        FatalError("unable to allocate memory for Lua transform: %s", optsstr);
    }
        ...
        DetectEngineThreadCtx *det_ctx, InspectionBuffer *buffer, void *options)
{
    ...
    if (LUA_OK != lua_pcall(tlua->luastate, 2, LUA_MULTRET, 0)) {
        ...
        return_value_count = lua_gettop(tlua->luastate);
...
void DetectTransformLuaxformRegister(void)
{
    ...
    sigmatch_table[DETECT_TRANSFORM_LUAXFORM].TransformSerialize = DetectTransformLuaxformSerialize;
}
