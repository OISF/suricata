...
static void TransformLuaxform(InspectionBuffer *buffer, void *options)
{
    ...
    DetectLuaThreadData *tlua = (DetectLuaThreadData *)DetectThreadCtxGetKeywordThreadCtx(
            buffer->det_ctx, lua->thread_ctx_id);
    if (tlua == NULL)
        return;
    ...
    lua_pop(tlua->luastate, 2); // Pop the result string
}
