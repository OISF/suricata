 ...
 */
static inline uint16_t SetUniquePortPoints(
        const DetectPort *p, uint8_t *unique_list, uint16_t size_list)
{
...
static inline void SetFinalUniquePortPoints(
        const uint8_t *unique_list, const uint16_t size_unique_arr, UniquePortPoint *final_arr)
{
    for (int i = 0, j = 0; i < MAX_PORT_VAL; i++) {
...
static inline int CreatePortList(DetectEngineCtx *de_ctx, const uint8_t *unique_list,
        const uint16_t size_list, SCIntervalTree *it, DetectPort **list)
{
...
static DetectPort *RulesGroupByPorts(DetectEngineCtx *de_ctx, uint8_t ipproto, uint32_t direction)
{
    ...
    uint8_t *unique_port_points = (uint8_t *)SCCalloc(MAX_PORT_VAL, sizeof(uint8_t));
