...
static int SRepCatSplitLine(char *line, uint8_t *cat, char *shortname, size_t shortname_len)
{
    ...
    int c;
    if (ByteExtractStringInt32(&c, 10, 0, (const char *)ptrs[0]) < 0 || c >= SREP_MAX_CATS)
        return -1;
...
static int SRepSplitLine(SRepCIDRTree *cidr_ctx, char *line, Address *ip, uint8_t *cat, uint8_t *value)
{
    ...
    int c;
    if (ByteExtractStringInt32(&c, 10, 0, (const char *)ptrs[1]) < 0 || c >= SREP_MAX_CATS)
        return -1;

    int v;
    if (ByteExtractStringInt32(&v, 10, 0, (const char *)ptrs[2]) < 0 || v > SREP_MAX_VAL)
        return -1;
