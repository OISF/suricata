...
static void SRepCIDRAddNetblock(SRepCIDRTree *cidr_ctx, char *ip, int cat, int value)
{
    ...
    if (strchr(ip, ':') != NULL) {
        ...
        if (SCRadixAddKeyIPV6String(ip, cidr_ctx->srepIPV6_tree, (void *)user_data) == NULL) {
        ...
        if (SCRadixAddKeyIPV4String(ip, cidr_ctx->srepIPV4_tree, (void *)user_data) == NULL) {
            SCLogWarning(SC_ERR_INVALID_VALUE,
...
{
    void *user_data = NULL;
    (void)SCRadixFindKeyIPV4BestMatch(ipv4_addr, cidr_ctx->srepIPV4_tree, &user_data);
    if (user_data == NULL)
        return 0;
...
{
    void *user_data = NULL;
    (void)SCRadixFindKeyIPV6BestMatch(ipv6_addr, cidr_ctx->srepIPV6_tree, &user_data);
    if (user_data == NULL)
        return 0;
...
int SRepInit(DetectEngineCtx *de_ctx)
{
    ...
    cidr_ctx->srepIPV4_tree = SCRadixCreateRadixTree(SRepCIDRFreeUserData, NULL);
    if (cidr_ctx->srepIPV4_tree == NULL) {
        SCLogDebug("Error initializing Reputation IPV4 with CIDR module");
        exit(EXIT_FAILURE);
    }
    SCLogDebug("Reputation IPV4 with CIDR module initialized");

    cidr_ctx->srepIPV6_tree = SCRadixCreateRadixTree(SRepCIDRFreeUserData, NULL);
    if (cidr_ctx->srepIPV6_tree == NULL) {
