...
int SRepInit(DetectEngineCtx *de_ctx)
{
    ...
    for (i = 0; i < SREP_MAX_CATS; i++) {
        cidr_ctx->srepIPV4_tree[i] = SCRadixCreateRadixTree(SRepCIDRFreeUserData, NULL);
        if (cidr_ctx->srepIPV4_tree[i] == NULL) {
    ...
    for (i = 0; i < SREP_MAX_CATS; i++) {
        cidr_ctx->srepIPV6_tree[i] = SCRadixCreateRadixTree(SRepCIDRFreeUserData, NULL);
        if (cidr_ctx->srepIPV6_tree[i] == NULL) {
