        ...
        AppProto alproto, uint8_t type)
{
    ...
    DetectBufferTypeSupportsMpm(name);
    DetectBufferTypeSupportsRecords(name);
    DetectBufferTypeSupportsTransformations(name);
    int sm_list = DetectBufferTypeGetByName(name);
    if (sm_list == -1) {
    ...
    am->direction = direction;
    am->priority = priority;
    am->type = DETECT_BUFFER_MPM_TYPE_RECORD;

    am->PrefilterRegisterWithListId = PrefilterRegister;
    am->record_v1.alproto = alproto;
    am->record_v1.type = type;
    SCLogNotice("type %u", type);
    SCLogNotice("am type %u", am->record_v1.type);

    if (g_mpm_list[DETECT_BUFFER_MPM_TYPE_RECORD] == NULL) {
        g_mpm_list[DETECT_BUFFER_MPM_TYPE_RECORD] = am;
    } else {
        DetectBufferMpmRegistery *t = g_mpm_list[DETECT_BUFFER_MPM_TYPE_RECORD];
        while (t->next != NULL) {
            t = t->next;
        }
        t->next = am;
    ...
    g_mpm_list_cnt[DETECT_BUFFER_MPM_TYPE_RECORD]++;
        ...
        DetectEngineTransforms *transforms)
{
    ...
    DetectBufferMpmRegistery *t = de_ctx->rec_mpms_list;
    while (t) {
        if (t->sm_list == parent_id) {
            DetectBufferMpmRegistery *am = SCCalloc(1, sizeof(*am));
            BUG_ON(am == NULL);
            ...
            am->id = de_ctx->rec_mpms_list_cnt++;
        ...
        AppProto alproto, uint8_t type)
{
    ...
    DetectEngineBufferTypeSupportsRecords(de_ctx, name);
    ...
    /* see if we use a unique or shared mpm ctx for this type */
    int confshared = 0;
    if (ConfGetBool("detect.mpm.rec.shared", &confshared) == 1)
        shared = confshared;
    ...
    if (de_ctx->rec_mpms_list == NULL) {
        de_ctx->rec_mpms_list = am;
    } else {
        DetectBufferMpmRegistery *t = de_ctx->rec_mpms_list;
        while (t->next != NULL) {
            t = t->next;
...
{
    const DetectBufferMpmRegistery *list = g_mpm_list[DETECT_BUFFER_MPM_TYPE_RECORD];
    while (list != NULL) {
        DetectBufferMpmRegistery *n = SCCalloc(1, sizeof(*n));
        BUG_ON(n == NULL);
    ...
    de_ctx->rec_mpms_list_cnt = g_mpm_list_cnt[DETECT_BUFFER_MPM_TYPE_RECORD];
    SCLogDebug("mpm: de_ctx rec_mpms_list %p %u", de_ctx->rec_mpms_list, de_ctx->rec_mpms_list_cnt);
}
...
{
    SCLogDebug("preparing rec mpm");
    int r = 0;
    const DetectBufferMpmRegistery *am = de_ctx->rec_mpms_list;
    while (am != NULL) {
...
void MpmStoreReportStats(const DetectEngineCtx *de_ctx)
{
    ...
    int rec_mpms_cnt = de_ctx->buffer_type_id;
    uint32_t recstats[rec_mpms_cnt + 1]; // +1 to silence scan-build
    memset(&recstats, 0x00, sizeof(recstats));
            ...
            if (am != NULL) {
                switch (am->type) {
                    ...
                    case DETECT_BUFFER_MPM_TYPE_APP:
                        ...
                        appstats[am->sm_list]++;
                        break;
                    case DETECT_BUFFER_MPM_TYPE_RECORD:
    ...
    if (!(de_ctx->flags & DE_QUIET)) {
        ...
        const DetectBufferMpmRegistery *um = de_ctx->rec_mpms_list;
        while (um != NULL) {
            if (recstats[um->sm_list] > 0) {
                const char *name = um->name;
                SCLogPerf("Record MPM \"%s\": %u", name, recstats[um->sm_list]);
            }
            um = um->next;
        }
    }
...
static MpmStore *MpmStorePrepareBufferRecord(
...
static void PrepareRecordMpms(DetectEngineCtx *de_ctx, SigGroupHead *sh)
{
    if (de_ctx->rec_mpms_list_cnt == 0)
        return;

    sh->init->rec_mpms = SCCalloc(de_ctx->rec_mpms_list_cnt, sizeof(MpmCtx *));
    BUG_ON(sh->init->rec_mpms == NULL);

    DetectBufferMpmRegistery *a = de_ctx->rec_mpms_list;
    while (a != NULL) {
        SCLogDebug("a %s direction %d PrefilterRegisterWithListId %p", a->name, a->direction,
                a->PrefilterRegisterWithListId);
        MpmStore *mpm_store = MpmStorePrepareBufferRecord(de_ctx, sh, a);
        if (mpm_store != NULL) {
            sh->init->rec_mpms[a->id] = mpm_store->mpm_ctx;
