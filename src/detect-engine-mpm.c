...
void RetrieveFPForSig(const DetectEngineCtx *de_ctx, Signature *s)
{
    ...
    int count_bidir_toclient_sm_list = 0;
    int priority;
            ...
            if (s->flags & SIG_FLAG_BOTHDIR) {
                ...
                if (DetectBufferToClient(de_ctx, tmp->list_id, s->alproto)) {
                    if (count_final_sm_list == 0) {
                        ...
                        final_sm_list[count_bidir_toclient_sm_list++] = tmp->list_id;
                    }
                    continue;
                }
            }
            // we may erase bidir toclient buffers here as intended if we have a better choice
    ...
    if ((s->flags & SIG_FLAG_BOTHDIR) && count_final_sm_list == 0) {
        // forced to pick a fast_pattern to client for bidir signature
        count_final_sm_list = count_bidir_toclient_sm_list;
    }
