...
void RetrieveFPForSig(const DetectEngineCtx *de_ctx, Signature *s)
{
    ...
    for (int list_id = DETECT_SM_LIST_PMATCH; list_id < nlists; list_id++) {
        ...
        if (list_id == DETECT_SM_LIST_POSTMATCH ||
            list_id == DETECT_SM_LIST_TMATCH ||
            list_id == DETECT_SM_LIST_SUPPRESS ||
            list_id == DETECT_SM_LIST_THRESHOLD)
            continue;
...
static MpmStore *MpmStorePrepareBufferAppLayer(DetectEngineCtx *de_ctx,
        SigGroupHead *sgh, const DetectBufferMpmRegistery *am, const struct SidsArray *sa)
{
    ...
    MpmStore lookup = { sa->sids_array, sa->sids_array_size, am->direction,
    ...
    MpmStore *result = MpmStoreLookup(de_ctx, &lookup);
    if (result == NULL) {
        SCLogDebug("new unique mpm for %s %s",
                am->name,
                am->direction == SIG_FLAG_TOSERVER ? "toserver" : "toclient");
        ...
        SigGroupHead *sgh, const DetectBufferMpmRegistery *am, const struct SidsArray *sa)
{
    ...
    MpmStore *result = MpmStoreLookup(de_ctx, &lookup);
    if (result == NULL) {
        SCLogDebug("new unique mpm for %s",
                am->name);
...
static MpmStore *MpmStorePrepareBufferFrame(
        DetectEngineCtx *de_ctx, SigGroupHead *sgh, const DetectBufferMpmRegistery *am, const struct SidsArray *sa)
{
    ...
    MpmStore lookup = { sa->sids_array, sa->sids_array_size, am->direction, MPMB_MAX, am->sm_list, 0, NULL };
    SCLogDebug("am->sm_list %d", am->sm_list);
