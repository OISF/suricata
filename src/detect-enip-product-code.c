...
static int DetectEnipProductCodeSetup(DetectEngineCtx *de_ctx, Signature *s, const char *rulestr)
{
    ...
    if (SigMatchAppendSMToList(
                de_ctx, s, DETECT_ENIP_PRODUCTCODE, (SigMatchCtx *)du16, g_enip_product_code_id) == NULL) {
...
/**
 * \brief This function is used to match enip product_code type rule option on a transaction with those
 * passed via enip_product_code:
 *
 * \retval 0 no match
 * \retval 1 match
 */
...
{
    uint16_t value;
    if (!rs_enip_get_product_code(txv, &value))
        SCReturnInt(0);
...
void DetectEnipProductCodeRegister(void)
{
    ...
    DetectAppLayerInspectEngineRegister2("enip.product_code", ALPROTO_ENIP, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.product_code", ALPROTO_ENIP, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectGenericList, NULL);
