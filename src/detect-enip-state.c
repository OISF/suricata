...
static int DetectEnipStateSetup(DetectEngineCtx *de_ctx, Signature *s, const char *rulestr)
{
    ...
    if (SigMatchAppendSMToList(
                de_ctx, s, DETECT_ENIP_STATE, (SigMatchCtx *)du8, g_enip_state_id) == NULL) {
 ...
 * \retval 1 match
 */
static int DetectEnipStateMatch(DetectEngineThreadCtx *det_ctx, Flow *f, uint8_t flags,
        void *state, void *txv, const Signature *s, const SigMatchCtx *ctx)

{
    uint8_t value;
    if (!rs_enip_get_state(txv, &value))
        SCReturnInt(0);
...
void DetectEnipStateRegister(void)
{
    ...
    DetectAppLayerInspectEngineRegister2("enip.state", ALPROTO_ENIP, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.state", ALPROTO_ENIP, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectGenericList, NULL);
