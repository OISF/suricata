...
/**
 * \test Test that SMB dce_opnum doesn't match for a dcerpc request with another opnum.
 */
static int NoDetectAnotherSmbDceOpnumInRequestTest(void)
{
    int result = 0;
    Signature *s = NULL;
    ThreadVars th_v;
    ...
    SigMatchSignatures(&th_v, de_ctx, det_ctx, p);
    if (PacketAlertCheck(p, 1)) {
        SCLogDebug("sig 1 matched in req1 and it shouldn't");
        goto end;
    }
...
static void DetectDceOpnumRegisterTests(void)
{
    ...
    UtRegisterTest("NoDetectAnotherSmbDceOpnumInRequestTest",
                   NoDetectAnotherSmbDceOpnumInRequestTest);
}
#endif /* UNITTESTS */
