...
#define IF_HTP_PERSONALITY_NUM(p) \
    ...
    if ((strcasecmp("TOMCAT_6_0", str) == 0) ||
        ...
        SCLogError(SC_ERR_OPTION_OBSOLETE, "Personality %s no "
 ...
 * \brief A direct flick off libhtp-0.5.x
 */
static int64_t htp_parse_content_length(bstr *b)
{
 ...
 *        them in the future.
 */
static inline bstr *HTPGenerateNormalizedUri(htp_tx_t *tx, htp_uri_t *uri)
{
    if (uri == NULL)
        return NULL;
...
static int HTPCallbackRequestLine(htp_tx_t *tx)
{
    ...
    request_uri_normalized = HTPGenerateNormalizedUri(tx, tx->parsed_uri);
