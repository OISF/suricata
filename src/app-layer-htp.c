...
static void AppLayerHtpComputeChunkLength(uint64_t content_len_so_far, uint32_t body_limit,
                                          uint32_t stream_depth, uint8_t flags, uint32_t *len)
{
    if (!(flags & HTP_STREAM_DEPTH_SET) && body_limit > 0 &&
        (content_len_so_far < body_limit) &&
        (content_len_so_far + *len) > body_limit)
    {
        *len = body_limit - content_len_so_far;
    } else if ((flags & HTP_STREAM_DEPTH_SET) && stream_depth > 0 &&
               (content_len_so_far < stream_depth) &&
               (content_len_so_far + *len) > stream_depth)
    {
        *len = stream_depth - content_len_so_far;
    }
...
static int HTPCallbackRequestBodyData(htp_tx_data_t *d)
{
        ...
        AppLayerHtpCheckStreamDepth(tx_ud))
    {
        ...
        AppLayerHtpComputeChunkLength(tx_ud->request_body.content_len_so_far,
...
static int HTPCallbackResponseBodyData(htp_tx_data_t *d)
{
        ...
        AppLayerHtpCheckStreamDepth(tx_ud))
    {
        ...
        AppLayerHtpComputeChunkLength(tx_ud->response_body.content_len_so_far,
...
static int HTPParserTest27(void)
{
    ...
    if (AppLayerHtpCheckStreamDepth(tx_ud)) {
        AppLayerHtpComputeChunkLength(tx_ud->request_body.content_len_so_far,
                                      0,
                                      FileReassemblyDepth(),
                                      tx_ud->tsflags,
