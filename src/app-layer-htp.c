...
SCEnumCharMap http_decoder_event_table[] = {
    ...
    { "REQUEST_LINE_NO_PROTOCOL", HTP_LOG_CODE_REQUEST_LINE_NO_PROTOCOL },
...
static int HtpRequestBodySetupMultipart(const htp_tx_t *tx, HtpTxUserData *htud)
{
    ...
    if (h != NULL && htp_header_value_len(h) > 0) {
        ...
        int r = HTTPParseContentTypeHeader((uint8_t *)"boundary=", 9, htp_header_value_ptr(h),
                htp_header_value_len(h), &boundary, &boundary_len);
        if (r == 1) {
#ifdef PRINT
        ...
        if (filename != NULL) {
            ...
            if (h_content_range != NULL) {
                result = HTPFileOpenWithRange(hstate, htud, filename, (uint16_t)filename_len, data,
                        data_len, HtpGetActiveResponseTxID(hstate),
                        htp_header_value(h_content_range), htud);
            } else {
                result = HTPFileOpen(hstate, htud, filename, (uint16_t)filename_len, data, data_len,
