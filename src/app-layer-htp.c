        ...
        const uint8_t *chunks_buffer, uint32_t chunks_buffer_len, bool eof)
{
    ...
    DEBUG_VALIDATE_BUG_ON(AppLayerParserGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP1, tx,
                                  STREAM_TOSERVER) > HTP_REQUEST_BODY);
    ...
    if (eof) {
        ...
        if (htud->tsflags & HTP_FILENAME_SET && !(htud->tsflags & HTP_DONTSTORE)) {
            ...
            HTPFileClose(hstate, htud, cur_buf, cur_buf_len, FILE_TRUNCATED, STREAM_TOSERVER);
        }
    ...
    while (cur_buf_len > 0) {
        MimeParserResult r = rs_mime_parse(htud->mime_state, cur_buf, cur_buf_len, &consumed);
        ...
        switch (r) {
            ...
            case MimeFileOpen:
                ...
                if (filename_len > 0) {
                    ...
                    result = HTPFileOpen(hstate, htud, filename, filename_len, NULL, 0,
            ...
            case MimeFileChunk:
                if (htud->tsflags & HTP_FILENAME_SET && !(htud->tsflags & HTP_DONTSTORE)) {
                    result = HTPFileStoreChunk(hstate, cur_buf, consumed, STREAM_TOSERVER);
                    if (result == -1) {
                        goto end;
                    } else if (result == -2) {
            ...
            case MimeFileClose:
                if (htud->tsflags & HTP_FILENAME_SET && !(htud->tsflags & HTP_DONTSTORE)) {
                    ...
                    HTPFileClose(hstate, htud, cur_buf, lastsize, 0, STREAM_TOSERVER);
                }
                htud->tsflags &= ~HTP_FILENAME_SET;
                break;
