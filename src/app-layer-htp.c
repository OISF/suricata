...
static int HTPStateGetProgress(void *tx, uint8_t direction);
static uint64_t HTPStateGetNoOfTxs(void *alstate);
static int HTPStateGetProgressDoneValue(uint8_t direction);
...
void HTPStateFree(void *state)
{
    ...
    if (s->connp != NULL) {
        ...
        uint64_t tx_id;
        uint64_t total_txs = HTPStateGetNoOfTxs(state);
...
static int HTPCallbackRequest(htp_connp_t *connp) {
    ...
    SCLogDebug("transaction_cnt %"PRIu16", list_size %"PRIuMAX,
               hstate->transaction_cnt, HTPStateGetNoOfTxs(hstate));
...
static uint64_t HTPStateGetNoOfTxs(void *alstate)
{
    return (int)list_size(((htp_tx_t *)alstate)->connp->conn->transactions);
}
...
void RegisterHTPParsers(void)
{
    ...
    AppLayerRegisterGetProgressFunc(ALPROTO_HTTP, HTPStateGetProgress);
    AppLayerRegisterGetNoOfTxs(ALPROTO_HTTP, HTPStateGetNoOfTxs);
    AppLayerRegisterGetTx(ALPROTO_HTTP, HTPStateGetTx);
    AppLayerRegisterGetProgressDoneValue(ALPROTO_HTTP, HTPStateGetProgressDoneValue);
