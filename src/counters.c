...
/** \internal
 *  \brief Find an existing counter by name or allocate a new one and append it.
 *
 *  Walks the counter list in \a pctx looking for \a name.  If found, sets
 *  \a *existing_id to the counter's id and returns NULL.  Otherwise allocates
 *  a new StatsCounter, assigns it an id, sets the type and short_name,
 *  appends it to the list, and returns the pointer so the caller can fill
 *  in type-specific fields (Func, did1/did2, FuncWithCtx, etc.).
 *
 *  \param name         Counter name (must outlive the counter).
 *  \param pctx         Public thread context that owns the counter list.
 *  \param type_q       Counter type (used to pick the id namespace).
 *  \param[out] existing_id  Set to the id of the existing counter when one is
 *                           found (caller should return this id).
 *
 *  \retval ptr  Newly allocated StatsCounter ready for caller to populate.
 *  \retval NULL Counter already existed (\a existing_id is valid) or alloc failed
 *               (\a existing_id is 0).
 */
static StatsCounter *StatsFindOrAllocCounter(const char *name, StatsPublicThreadContext *pctx,
        enum StatsType type_q, uint16_t *existing_id)
{
    *existing_id = 0;
...
/**
 * \brief Registers a context-aware counter.
 *
 * Like StatsRegisterQualifiedCounter() but stores a context-aware getter
 * (FuncWithCtx/FuncCtx) instead of a zero-argument Func.
 */
static uint16_t StatsRegisterQualifiedCounterWithCtx(const char *name,
        StatsPublicThreadContext *pctx, int type_q, uint64_t (*Func)(void *), void *ctx)
{
    uint16_t existing_id;
    StatsCounter *pc = StatsFindOrAllocCounter(name, pctx, type_q, &existing_id);
    if (pc == NULL)
        return existing_id;
        ...
        const char *name, uint64_t (*Func)(void *), void *ctx)
{
    ...
    BUG_ON(stats_ctx == NULL);
#endif
    uint16_t id = StatsRegisterQualifiedCounterWithCtx(
            name, &(stats_ctx->global_counter_ctx), STATS_TYPE_FUNC, Func, ctx);
    s.id = id;
    return s;
}
