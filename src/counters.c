...
static void *SCPerfMgmtThread(void *arg)
{
    ...
    tm->ThreadInit(tv_local, NULL, &stats_thread_data);
    BUG_ON(stats_thread_data == NULL);
    SCLogInfo("stats_thread_data %p", &stats_thread_data);
...
static int SCPerfOutputCounterFileIface(ThreadVars *tv)
{
    ...
    if (stats_table.nstats == 0) {
        ...
        BUG_ON(stats_table.stats == NULL); // TODO
