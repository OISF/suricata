 ...
 */
static inline void OutputTxLogFiles(ThreadVars *tv,
        OutputFileLoggerThreadData *file_td,
        ...
        const bool eof)
{
    ...
    if (p->flowflags & FLOW_PKT_TOSERVER) {
        opposing_finished = ffc_opposing == NULL || (ffc_opposing->head == NULL && tc_ready);
        SCLogDebug("opposing_finished %d ffc_opposing %p ffc_opposing->head %p tc_ready %d",
                opposing_finished, ffc_opposing, ffc_opposing ? ffc_opposing->head : NULL,
                tc_ready);
    } else {
        opposing_finished = ffc_opposing == NULL || (ffc_opposing->head == NULL && ts_ready);
        SCLogDebug("opposing_finished %d ffc_opposing %p ffc_opposing->head %p ts_ready %d",
                opposing_finished, ffc_opposing, ffc_opposing ? ffc_opposing->head : NULL,
                ts_ready);
    }
    SCLogDebug("opposing_finished %d", opposing_finished);

    if (ffc || ffc_opposing)
        SCLogDebug("pcap_cnt %" PRIu64 " flow %p tx %p tx_id %" PRIu64
                   " ffc %p ffc_opposing %p tx_progress_ts %d tx_progress_tc %d tx_complete %d",
                p->pcap_cnt, f, tx, tx_id, ffc, ffc_opposing, tx_progress_ts, tx_progress_tc,
                tx_complete);
    ...
    if (ffc) {
        ...
        if (filedata_td)
            OutputFiledataLogFfc(tv, filedata_td, p, ffc, tx, tx_id, txd, packet_dir,
                    file_close, file_trunc, packet_dir);
        if (file_td)
            OutputFileLogFfc(tv, file_td, p, ffc, tx, tx_id, txd, file_close, file_trunc,
                    packet_dir);
    }
    ...
    if (!is_file_tx || tx_done) {
        ...
        if (file_td) {
            txd->logged.flags |= LOGGER_FILE;
            SCLogDebug("setting LOGGER_FILE => %08x", txd->logged.flags);
        }
        if (filedata_td) {
            txd->logged.flags |= LOGGER_FILEDATA;
            SCLogDebug("setting LOGGER_FILEDATA => %08x", txd->logged.flags);
        }
    } else {
        SCLogDebug("pcap_cnt %" PRIu64 " flow %p tx %p tx_id %" PRIu64
                   " NOT SETTING FILE FLAGS ffc %p ffc_opposing %p tx_progress_ts %d "
                   "tx_progress_tc %d tx_complete %d",
                p->pcap_cnt, f, tx, tx_id, ffc, ffc_opposing, tx_progress_ts, tx_progress_tc,
                tx_complete);
    }
}
...
static void OutputTxLogList0(ThreadVars *tv,
        OutputTxLoggerThreadData *op_thread_data,
...
static TmEcode OutputTxLogThreadInit(ThreadVars *tv, const void *_initdata, void **data)
{
    ...
    *data = (void *)td;
    ...
    SCLogDebug(
            "file_thread_data %p filedata_thread_data %p", td->file, td->filedata);
