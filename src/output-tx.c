...
int OutputRegisterTxLogger(char *name, uint16_t alproto, TxLogger LogFunc, OutputCtx *output_ctx) {
    OutputTxLogger *o = SCMalloc(sizeof(*o));
    if (o == NULL)
        return -1;
...
static TmEcode OutputTxLog(ThreadVars *tv, Packet *p, void *thread_data, PacketQueue *pq, PacketQueue *postpq) {
    ...
    uint16_t alproto = f->alproto;//AppLayerGetProtoFromPacket(p);

    /** \todo proper func, but don't want to hinder anoop too much */
    if (al_proto_table[alproto].StateGetTx == NULL)
        goto end;
    ...
    uint64_t tx_id = AppLayerTransactionGetLogId(f);
    ...
    for (; tx_id < total_txs; tx_id++)
    {
        ...
        if (!(((AppLayerParserStateStore *)f->alparser)->id_flags
                    & APP_LAYER_TRANSACTION_EOF)) {
            int tx_progress = AppLayerGetAlstateProgress(alproto, tx, 0);
            if (tx_progress < tx_progress_done_value_ts) {
                SCLogDebug("progress not far enough, not logging");
                break;
            }

            tx_progress = AppLayerGetAlstateProgress(alproto, tx, 1);
...
static TmEcode OutputTxLogThreadInit(ThreadVars *tv, void *initdata, void **data) {
    ...
    *data = (void *)td;
    ...
    SCLogInfo("OutputTxLogThreadInit happy (*data %p)", *data);
