...
int DecodeARP(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p, const uint8_t *pkt, uint32_t len)
{
    ...
    if (ntohs(arph->hw_type) != ETHERNET_HW_TYPE) {
        ENGINE_SET_INVALID_EVENT(p, ARP_INVALID_HARDWARE);
        return TM_ECODE_FAILED;
    }

    if (ntohs(arph->proto_type) != ETHERNET_TYPE_IP) {
        ENGINE_SET_INVALID_EVENT(p, ARP_INVALID_PROTOCOL);
        return TM_ECODE_FAILED;
    }
