...
File *HttpRangeClose(HttpRangeContainerBlock *c, uint16_t flags)
{
    ...
    if (c->current) {
        ...
        if (c->container->files->tail &&
            ...
            HttpRangeContainerBuffer *res = HTTP_RANGES_RB_INSERT(&c->container->fragment_tree, c->current);
            if (res) {
...
static void HttpRangeBlockDerefContainer(HttpRangeContainerBlock *b)
{
    if (b && b->container) {
        ...
        DEBUG_VALIDATE_BUG_ON(SC_ATOMIC_GET(b->container->hdata->use_cnt) > (uint32_t)INT_MAX);
        b->container = NULL;
    }
}
