...
static int FileAppendFileDataFilePtr(File *ff, FileData *ffd)
{
    ...
    if (ff->sha256_ctx)
        HASH_Update(ff->sha256_ctx, ffd->data, ffd->len);
#endif
    SCReturnInt(0);
}
...
#ifdef HAVE_NSS
        /* no storage but forced md5, sha1 or/and sha256 */
        if (ffc->tail->md5_ctx) {
            if (ffc->tail->md5_ctx)
        ...
        uint32_t data_len, uint16_t flags)
{
    ...
    if (data != NULL) {
        ...
        if (ff->flags & FILE_NOSTORE) {
#ifdef HAVE_NSS
            /* no storage but md5, sha1 or/and sha256 */
            if (ff->md5_ctx)
...
void FileDisableStoringForFile(File *ff)
{
    ...
    if (ff->state == FILE_STATE_OPENED && ff->size >= (uint64_t)FileMagicSize()) {
        if (g_file_force_md5 == 0 && g_file_force_sha1 == 0 &&
                g_file_force_sha256 == 0 && g_file_force_tracking == 0) {
