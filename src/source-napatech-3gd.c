...
{
	SCEnter();
	uint8_t stream_id = (uint32_t )initdata;
	*data = NULL;
	int status;
	char errbuf[100];

	SCLogInfo("Napatech 3GD Thread Stream ID:%d", stream_id);
	...
	/*  Use max_pending_packets as our maximum number of packets read
		from the NAPATECH 3GD buffer.
	 */
	SCLogInfo("Opening NAPATECH 3GD Stream: %d for processing", ntv3->stream_id);
	...
	if ((status = NT_NetRxOpen(&(ntv3->rx_stream), "SuricataStream", NT_NET_INTERFACE_PACKET, ntv3->stream_id, -1)) != NT_SUCCESS) {
		...
		SCLogError(SC_ERR_NAPATECH_3GD_OPEN_FAILED, "Failed to open NAPATECH 3GD Stream: %d - %s", ntv3->stream_id, errbuf);
		SCFree(ntv3);
		SCReturnInt(TM_ECODE_FAILED);
	}

	SCLogInfo("Started processing packets from NAPATECH 3GD Stream: %d", ntv3->stream_id);
...
TmEcode Napatech3GDStreamLoop(ThreadVars *tv, void *data, void *slot)
{
	...
	uint64_t pkt_ts;
	uint32_t pkt_ts_remainder;
	NtNetBuf_t packet_buffer;
	Napatech3GDThreadVars *ntv3 = (Napatech3GDThreadVars *)data;
	SCLogInfo("Napatech 3GD Packet Stream Loop Started for Stream ID: %u", ntv3->stream_id);
	...
	while (!(suricata_ctl_flags & (SURICATA_STOP | SURICATA_KILL))) {
		...
		if (status == NT_STATUS_TIMEOUT || status == NT_STATUS_TRYAGAIN) {
			...
			SCLogError(SC_ERR_NAPATECH_3GD_STREAM_NEXT_FAILED,
					   "Failed to read from Napatech 3GD Stream: %d",
					   ntv3->stream_id);
			SCReturnInt(TM_ECODE_FAILED);
		}
		...
		switch(NT_NET_GET_PKT_TIMESTAMP_TYPE(packet_buffer)) {
			case NT_TIMESTAMP_TYPE_NATIVE_NDIS:
				...
				// pkt_ts_remainder = pkt_ts % 100000000;
			...
			default:
				SCLogError(SC_ERR_NAPATECH_3GD_TIMESTAMP_TYPE_NOT_SUPPORTED,
						   "Packet from Napatech 3GD Stream: %d does not have a supported timestamp format", 
						   ntv3->stream_id);
				NT_NetRxRelease(ntv3->rx_stream, packet_buffer);	
				SCReturnInt(TM_ECODE_FAILED);
		}
		...
		if (TmThreadsSlotProcessPkt(ntv3->tv, ntv3->slot, p) != TM_ECODE_OK) {
			TmqhOutputPacketpool(ntv3->tv, p);
			NT_NetRxRelease(ntv3->rx_stream, packet_buffer);
			SCReturnInt(TM_ECODE_FAILED);
		}
...
void Napatech3GDStreamThreadExitStats(ThreadVars *tv, void *data)
{
	...
	SCLogInfo("Stream: %d; Packets: %"PRIu64"; Drops: %"PRIu64" (%5.2f%%); Bytes: %"PRIu64, ntv3->stream_id, ntv3->pkts, ntv3->drops, percent, ntv3->bytes);
}
