        ...
        Packet *p, uint64_t tx_id, uint8_t flags)
{
    ...
    if (p->alerts.cnt == 0 || (p->alerts.cnt > 0 &&
        ...
        p->alerts.alerts[p->alerts.cnt].record_id = (flags & PACKET_ALERT_FLAG_RECORD) ? det_ctx->record_id : 0;
    } else {
        ...
        p->alerts.alerts[i].record_id = (flags & PACKET_ALERT_FLAG_RECORD) ? det_ctx->record_id : 0;
    }
