...
static void DetectConfigFree(void *);
        ...
        const uint64_t tx_id, const DetectConfigData *config)
{
    ...
    if (tx) {
        ...
        if (txd) {
            SCLogNotice("tx %p txd %p: log_flags %x", tx, txd, txd->log_flags);
            txd->log_flags |= BIT_U8(config->type);
        } else {
        ...
        Packet *p, const DetectConfigData *config)
{
    ...
    if (this_tx) {
        SCLogNotice("tx logic here: tx_id %"PRIu64, det_ctx->tx_id);
        ConfigApplyTx(p->flow, det_ctx->tx_id, config);
    } else if (this_flow) {
        SCLogNotice("flow logic here");
    } else {
        abort();
    }
...
#define MAX_SUBSTRINGS 30
    ...
    SCLogNotice("scopeval %s", scopeval);
    ...
    s->flags |= SIG_FLAG_NOALERT; //TODO use config action instead
    return 0;
...
static void DetectConfigFree(void *ptr)
{
    if (ptr != NULL) {
        SCFree(ptr);
    }
...
static int DetectConfigTest01(void)
{
    ...
    Signature *s = DetectEngineAppendSig(de_ctx,
            "alert dns any any -> any any ("    // TODO action -> config
