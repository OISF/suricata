...
static const char *HSCacheDefaultDir(void)
{
    static char hash_file_path[PATH_MAX];
    int r = PathMerge(
            hash_file_path, sizeof(hash_file_path), ConfigGetLogDirectory(), "hyperscan-cache");
    if (r)
        return NULL;
...
static const char *HSCacheConstructFPath(uint64_t hs_db_hash)
{
    ...
    r = PathMerge(hash_file_path, sizeof(hash_file_path), HSCacheDefaultDir(), filename);
    if (r)
        return NULL;
...
{
    if (SCCreateDirectoryTree(HSCacheDefaultDir(), true) != 0)
        return -1;
...
static int HSSaveCache(hs_database_t *hs_db, uint64_t hs_db_hash)
{
    ...
    if (SCPathExists(hash_file_static)) {
        ...
        // hash collision. If this happens will older not used caches it is fine
