...
static uint32_t SCHSPatternHash(const SCHSPattern *p, uint32_t hash)
{
    ...
    hash = hashlittle(&p->len, sizeof(p->len), hash);
    hash = hashlittle(&p->flags, sizeof(p->flags), hash);
    hash = hashlittle(p->original_pat, p->len, hash);
    hash = hashlittle(&p->id, sizeof(p->id), hash);
    hash = hashlittle(&p->offset, sizeof(p->offset), hash);
    hash = hashlittle(&p->depth, sizeof(p->depth), hash);
    hash = hashlittle(&p->sids_size, sizeof(p->sids_size), hash);
    hash = hashlittle(p->sids, p->sids_size * sizeof(SigIntId), hash);
    return hash;
}
...
int SCHSPreparePatterns(MpmCtx *mpm_ctx)
{
    ...
    if (pd_cached != NULL) {
        SCLogDebug("Reusing cached database %p with %u patterns (ref_cnt=%u)",
    ...
    SCLogDebug("Built %u patterns into a database of size %zu bytes",
               mpm_ctx->pattern_cnt, ctx->hs_db_size);
                       ...
                       uint32_t matchsize)
{
    ...
    if (g_scratch_proto == NULL) {
        SCLogError(SC_ERR_FATAL, "No scratch space prototype");
        exit(EXIT_FAILURE);
    }
    ...
    ctx->scratch = NULL;
    err = hs_clone_scratch(g_scratch_proto, (hs_scratch_t **)&ctx->scratch);
    if (err != HS_SUCCESS) {
                          ...
                          void *ctx)
{
    ...
    SCLogDebug("Hyperscan Match %u: id=%u @ %llu (pat id=%u)",
               cctx->match_count, id, to, pat->id);
