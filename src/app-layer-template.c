...
static void TemplateSetTxLogged(void *state, uint64_t tx_id, uint32_t logger)
{
    TemplateTransaction *tx = TemplateGetTx(state, tx_id);
    if (tx)
        tx->logged |= logger;
}

static int TemplateGetTxLogged(void *state, uint64_t tx_id, uint32_t logger)
{
    TemplateTransaction *tx = TemplateGetTx(state, tx_id);
