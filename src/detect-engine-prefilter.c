        ...
        DetectTransaction *tx)
{
    ...
    do {
        ...
        if (engine->ctx.app.tx_min_progress != -1) {
            ...
            } else if (tx->detect_progress > engine->ctx.app.tx_min_progress &&
                       tx->tx_progress < engine->ctx.app.tx_max_progress) {
                // we already ran prefilter at tx_min_progress, and are not yet at tx_max_progress
                goto next;
            }
#ifdef DEBUG
            ...
            if (tx->tx_progress > engine->ctx.app.tx_min_progress && engine->is_last_for_progress &&
                ...
                if (tx->tx_progress > engine->ctx.app.tx_max_progress) {
                    tx->detect_progress = engine->ctx.app.tx_max_progress + 1;
                } else if (tx->detect_progress < engine->ctx.app.tx_min_progress + 1) {
