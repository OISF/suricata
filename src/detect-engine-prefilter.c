...
int PrefilterAppendRecordEngine(DetectEngineCtx *de_ctx, SigGroupHead *sgh,
        PrefilterRecordFn PrefilterRecordFunc, AppProto alproto, uint8_t rec_type, void *pectx,
        void (*FreeFunc)(void *pectx), const char *name)
{
    if (sgh == NULL || PrefilterRecordFunc == NULL || pectx == NULL)
        return -1;
    ...
    e->rec_type = rec_type;
    e->alproto = alproto;
    e->PrefilterRecord = PrefilterRecordFunc;
    e->pectx = pectx;
    e->Free = FreeFunc;

    if (sgh->init->rec_engines == NULL) {
        sgh->init->rec_engines = e;
    } else {
        PrefilterEngineList *t = sgh->init->rec_engines;
        while (t->next != NULL) {
            t = t->next;
        }
...
void PrefilterCleanupRuleGroup(const DetectEngineCtx *de_ctx, SigGroupHead *sgh)
{
    ...
    if (sgh->rec_engines) {
        PrefilterFreeEngines(de_ctx, sgh->rec_engines);
        sgh->rec_engines = NULL;
    }
}
...
void PrefilterSetupRuleGroup(DetectEngineCtx *de_ctx, SigGroupHead *sgh)
{
    ...
    if (sgh->init->rec_engines != NULL) {
        uint32_t cnt = 0;
        for (el = sgh->init->rec_engines; el != NULL; el = el->next) {
            cnt++;
            de_ctx->prefilter_maxid = MAX(de_ctx->prefilter_maxid, el->gid);
        }
        sgh->rec_engines = SCMallocAligned(cnt * sizeof(PrefilterEngine), CLS);
        if (sgh->rec_engines == NULL) {
            return;
        }
        memset(sgh->rec_engines, 0x00, (cnt * sizeof(PrefilterEngine)));

        PrefilterEngine *e = sgh->rec_engines;
        for (el = sgh->init->rec_engines; el != NULL; el = el->next) {
            e->local_id = el->id;
            e->ctx.rec_type = el->rec_type;
            e->cb.PrefilterRecord = el->PrefilterRecord;
            e->alproto = el->alproto;
            e->pectx = el->pectx;
            el->pectx = NULL; // e now owns the ctx
            e->gid = el->gid;
