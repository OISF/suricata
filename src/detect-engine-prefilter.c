        ...
        DetectTransaction *tx)
{
    ...
    SCLogDebug("packet %" PRIu64 " tx %p progress %d tx->prefilter_progress %02x", p->pcap_cnt,
            tx->tx_ptr, tx->tx_progress, tx->prefilter_progress);
    ...
    do {
        ...
        if (engine->ctx.tx_min_progress != -1) {
            ...
            if (tx->tx_progress > engine->ctx.tx_min_progress) {
                 ...
                 * least once already. */
                if (tx->prefilter_progress > engine->ctx.tx_min_progress) {
                    SCLogDebug("tx already marked progress as beyond engine: %u > %u",
                            tx->prefilter_progress, engine->ctx.tx_min_progress);
                    goto next;
                }
            }
            ...
            if (tx->tx_progress > engine->ctx.tx_min_progress && engine->is_last_for_progress) {
                ...
                tx->prefilter_progress = engine->ctx.tx_min_progress + 1;
                SCLogDebug(
                        "tx->tx_progress %d engine->ctx.tx_min_progress %d "
                        "engine->is_last_for_progress %d => tx->prefilter_progress updated to %02x",
