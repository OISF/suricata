...
static void SCRedisAsyncCommandCallback(redisAsyncContext *ac, void *r, void *privdata)
{
    ...
    if (reply == NULL) {
        if (ctx->connected == 1)
            SCLogInfo("Missing reply from redis. Maybe disconnected!");
        ctx->connected = 0;
    } else {
        ctx->connected = 1;
...
static void SCRedisAsyncEchoCommandCallback(redisAsyncContext *ac, void *r, void *privdata)
{
    ...
    if (reply) {
        if (ctx->connected == 0) {
            SCLogInfo("Received ECHO reply from redis. It's connected!");
            ctx->connected = 1;
            ctx->tried = 0;
            ctx->batch_count = 0;
        }
        ...
        if (ctx->connected == 1) {
            SCLogInfo("Missing ECHO reply from redis. Maybe disconnected!");
        }
        else {
...
int SCConfLogOpenRedis(ConfNode *redis_node, void *lf_ctx)
{
    ...
    if (is_async) {
        ...
        if (SCConfLogReopenAsyncRedis(log_ctx) == -1) {
            return -1;
        }
        SCLogRedisContext * ctx = log_ctx->redis;
        if (ctx) {
 ...
 */
void SCLogRedisContextFree(SCLogRedisContext *ctx, int is_async)
{
    if (ctx == NULL) {
        return;
    }
