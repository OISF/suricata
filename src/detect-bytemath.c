                          ...
                          uint16_t payload_len, uint64_t *value, uint8_t endian)
{
    ...
    if (extbytes > len) {
        SCLogDebug("data byte value %d extends past data area", extbytes);
        return 0;
    }
    ptr += extbytes;
 ...
 *            On failure, NULL.
 */
static DetectByteMathData *DetectByteMathParse(const char *arg, char **rvalue)
{
...
static int MyUTHPacketMatchSig(Packet *p, const char *sig)
{
    ...
    FAIL_IF(PacketAlertCheck(p, de_ctx->sig_list->id) == 1);
