...
/*
 * From Section 3.5.34, Snort Manual
 ...
 * These resulting variables can be referenced later in the rule, instead of
 * using hard-coded values.
 *
 * Format
 *   byte_math:bytes <bytes_to_extract>, offset <offset_value>, oper <operator>,
 *   rvalue <r_value>, result <result_variable>
 *   [, relative]
 *   [, endian <endian>]
 *   [, string <number type>][, dce]
...
static void DetectByteMathFree(void *);
...
int DetectByteMathDoMatch(DetectEngineThreadCtx *det_ctx, const SigMatchData *smd,
                          ...
                          uint16_t payload_len, uint64_t *value, uint8_t endian)
{
    DetectByteMathData *data = (DetectByteMathData *)smd->ctx;
    const uint8_t *ptr;
    int32_t len;
    uint64_t val;
    int extbytes;
 ...
 *            On failure, NULL.
 */
static inline DetectByteMathData *DetectByteMathParse(const char *arg, char **rvalue)
{
    DetectByteMathData *bmd = NULL;
    int ret, res;
#define MAX_SUBSTRINGS 60
    ...
    switch (bmd->oper) {
    	case DETECT_BYTEMATH_OPERATOR_LSHIFT:
    	case DETECT_BYTEMATH_OPERATOR_RSHIFT:
            /* nbytes has already been validated to be in [1, 10] */
            if (bmd->nbytes > 4) {
...
static int DetectByteMathSetup(DetectEngineCtx *de_ctx, Signature *s, const char *arg)
{
    ...
    data = DetectByteMathParse(arg, &rvalue);
    if (data == NULL)
        goto error;
 ...
 */
static void DetectByteMathFree(void *ptr)
{
    if (ptr != NULL) {
...
static int DetectByteMathParseTest01(void)
{
    ...
    DetectByteMathData *bmd = DetectByteMathParse("bytes 4, offset 2, oper +,"
...
static int DetectByteMathParseTest02(void)
{
    ...
    /* bytes value invalid */
    DetectByteMathData *bmd = DetectByteMathParse("bytes 257, offset 2, oper +, "
...
static int DetectByteMathParseTest03(void)
{
    ...
    /* bytes value invalid */
    DetectByteMathData *bmd = DetectByteMathParse("bytes 11, offset 2, oper +, "
                                                  "rvalue 39, result bar", NULL);
    result = bmd == NULL;
...
static int DetectByteMathParseTest04(void)
{
    ...
    DetectByteMathData *bmd = DetectByteMathParse("bytes 4, offset 70000, oper +,"
                                                  " rvalue 39, result bar", NULL);
    result = bmd == NULL;
...
static int DetectByteMathParseTest05(void)
{
    ...
    /* oper value invalid */
    DetectByteMathData *bmd = DetectByteMathParse("bytes 11, offset 16, oper &,"
                                                  "rvalue 39, result bar", NULL);
    result = bmd == NULL;
...
static int DetectByteMathParseTest06(void)
{
    ...
    DetectByteMathData *bmd = DetectByteMathParse("bytes 4, offset 0, oper +,"
...
static int DetectByteMathParseTest13(void)
{
    ...
    result = bmd &&
             ...
             bmd->rvalue == 39 &&
             strcmp(bmd->result, "bar") == 0 &&
             bmd->bitmask_val == 0x8f40 &&
             bmd->bitmask_shift_count == 6 &&
             bmd->flags == flags &&
             bmd->endian == DETECT_BYTEMATH_ENDIAN_BIG &&
             bmd->base == DETECT_BYTEMATH_BASE_DEC;
...
static int MyUTHPacketMatchSig(Packet *p, const char *sig);
static int MyUTHPacketMatchSig(Packet *p, const char *sig)
{
    int result = 1;
