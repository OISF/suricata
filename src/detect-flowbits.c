...
static int DetectFlowbitMatchToggle (Packet *p, const DetectFlowbitsData *fd)
{
    ...
    bool set = FlowBitToggle(p->flow, fd->idx);
    return set == true;
}
...
static int AddSids(const DetectEngineCtx *de_ctx, const struct FBAnalyzer *fba,
        ...
        struct PrefilterEngineFlowbits *ctx, const DetectFlowbitsData *fb, const Signature *s)
{
    ...
    if (pfb == NULL) {
        ...
        int r = AddSids(de_ctx, fba, fb, pfb);
        if (r < 0) {
            return -1;
        } else if (r == 0) {
            SCLogDebug("no sids added");
