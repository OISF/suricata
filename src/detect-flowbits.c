...
static inline int DetectFlowbitValidateCallbackDo(Signature *s, uint8_t cmd, uint16_t idx)
{
    ...
    for (SigMatch *sm = list; sm != NULL; sm = sm->next) {
        ...
        if ((fd->idx == idx) && (fd->cmd == cmd)) {
            SCLogError("invalid flowbit command combination in the same signature");
            return -1;
        }
    }
    return 0;
