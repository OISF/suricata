...
static int DetectFlowbitValidate(Signature *s, DetectFlowbitsData *fd)
{
    ...
    for (uint8_t i = 0; i < ARRAY_SIZE(icmds_map); i++) {
        if (fd->cmd == icmds_map[i].cmd1) {
            ...
            if (DetectFlowbitValidateDo(s, cmd2, fd->idx) == -1) {
                SCLogWarning("invalid flowbit command combination in the same signature: %s and %s",
            ...
            if (DetectFlowbitValidateDo(s, cmd1, fd->idx) == -1) {
                SCLogWarning("invalid flowbit command combination in the same signature: %s and %s",
