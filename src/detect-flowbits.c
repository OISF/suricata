...
static int FlowbitOrAddData(char *, DetectFlowbitsData *, DetectEngineCtx *);
...
static int FlowbitOrAddData(char *arrptr, DetectFlowbitsData *cd, DetectEngineCtx *de_ctx)
{
    ...
    cd->or_list = SCMalloc(i * sizeof(uint32_t));
...
int DetectFlowbitSetup (DetectEngineCtx *de_ctx, Signature *s, const char *rawstr)
{
    ...
    if (strchr(fb_name, '|') != NULL) {
        int retval = FlowbitOrAddData(fb_name, cd, de_ctx);
        if (retval == -1) {
            goto error;
        }
        cd->cmd = fb_cmd;
