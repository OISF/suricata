...
static int DetectFlowbitValidate(Signature *s, DetectFlowbitsData *fd)
{
    ...
    struct DetectFlowbitInvalidCmdMap_ icmds_map[] = {
        ...
        { DETECT_FLOWBITS_CMD_ISSET, DETECT_FLOWBITS_CMD_ISNOTSET }, /* unsatisfiable at runtime */
    };

    uint8_t len_cmds_map = ARRAY_SIZE(icmds_map);
    int ret = 0;

    for (uint8_t i = 0; i < len_cmds_map; i++) {
        if (fd->cmd == icmds_map[i].cmd1) {
            ret = DetectFlowbitValidateDo(
                    s, icmds_map[i].cmd2, icmds_map[i].cmd1, fd->idx, (i == len_cmds_map - 1));
            if (ret != 1) {
                return ret;
            }
        } else if (fd->cmd == icmds_map[i].cmd2) {
            ret = DetectFlowbitValidateDo(
                    s, icmds_map[i].cmd1, icmds_map[i].cmd2, fd->idx, (i == len_cmds_map - 1));
            if (ret != 1) {
                return ret;
            }
        }
