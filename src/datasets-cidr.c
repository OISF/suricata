 ...
 */
static int ParseCIDRAny(
        const char *cidr_str, struct in_addr *v4, int *mask4, struct in6_addr *v6, int *mask6)
{
        ...
        CIDRIPv4Type *ipv4, const char *set_name, const uint8_t *addr, uint8_t prefix)
{
    ...
    if (existing != NULL) {
        ...
        rc = -2;
    } else if (SCRadix4AddKeyIPV4Netblock(&ipv4->tree, &radix4_cfg, masked, prefix, NULL) == NULL) {
        ...
        CIDRIPv6Type *ipv6, const char *set_name, const uint8_t *addr, uint8_t prefix)
{
    ...
    if (existing != NULL) {
        ...
        rc = -2;
    } else if (SCRadix6AddKeyIPV6Netblock(&ipv6->tree, &radix6_cfg, masked, prefix, NULL) == NULL) {
...
int DatasetRemoveCIDRString(Dataset *set, const char *cidr_str)
{
    ...
    switch (ParseCIDRAny(cidr_str, &v4, &mask4, &v6, &mask6)) {
        case 1:
            return CIDRRemoveIPv4Netblock(&cidr->ipv4, (const uint8_t *)&v4.s_addr, (uint8_t)mask4)
                           ? 1
                           : 0;
        case 2:
            return CIDRRemoveIPv6Netblock(&cidr->ipv6, (const uint8_t *)&v6.s6_addr, (uint8_t)mask6)
                           ? 1
                           : 0;
    }
    SCLogDebug("invalid CIDR address format: %s", cidr_str);
