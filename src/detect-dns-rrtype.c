...
static int DetectDnsRrtypeSetup(DetectEngineCtx *de_ctx, Signature *s, const char *str)
{
    ...
    if (SigMatchAppendSMToList(
                de_ctx, s, DETECT_AL_DNS_RRTYPE, (SigMatchCtx *)detect, dns_rrtype_list_id) == NULL) {
...
static int DetectDnsRrtypeMatch(DetectEngineThreadCtx *det_ctx, Flow *f, uint16_t flags, void *state,
...
void DetectDnsRrtypeRegister(void)
{
    ...
    sigmatch_table[DETECT_AL_DNS_RRTYPE].desc = "Match the DNS header rrtype flag.";
