 ...
 * \param local_id multi-instance buffer index and index into SCTPVars data_offsets/data_lens
 *
...
{
    DEBUG_VALIDATE_BUG_ON(local_id >= SCTP_MAX_DATA_CHUNKS);
    ...
    const uint16_t offset = p->l4.vars.sctp.data_offsets[(uint8_t)local_id];
    const uint16_t len = p->l4.vars.sctp.data_lens[(uint8_t)local_id];
    if (len == 0) {
        ...
        uint8_t *_alert_flags)
{
    ...
    for (uint8_t i = 0; i < cnt; i++) {
        InspectionBuffer *buffer = GetBuffer(det_ctx, transforms, p, list_id, (uint32_t)i);
        ...
        DetectEngineThreadCtx *det_ctx, Packet *p, const void *pectx)
{
    ...
    for (uint8_t i = 0; i < cnt; i++) {
        InspectionBuffer *buffer = GetBuffer(det_ctx, ctx->transforms, p, list_id, (uint32_t)i);
        if (buffer == NULL || buffer->inspect == NULL)
            continue;
