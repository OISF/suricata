 ...
 */
#define SC_ATOMIC_DECLARE(type, name) _Atomic(type) name##_sc_atomic__
 ...
 *
 */
#define SC_ATOMIC_EXTERN(type, name) extern _Atomic(type)(name##_sc_atomic__)

/**
 *  \brief wrapper for declaring an atomic variable and initializing it.
 **/
#define SC_ATOMIC_DECL_AND_INIT(type, name) _Atomic(type)(name##_sc_atomic__) = 0
 ...
 **/
#define SC_ATOMIC_RESET(name) SC_ATOMIC_INIT(name)
 ...
 */
#define SC_ATOMIC_ADD(name, val) atomic_fetch_add(&(name##_sc_atomic__), (val))
 ...
 */
#define SC_ATOMIC_SUB(name, val) atomic_fetch_sub(&(name##_sc_atomic__), (val))
 ...
 */
#define SC_ATOMIC_OR(name, val) atomic_fetch_or(&(name##_sc_atomic__), (val))
 ...
 */
#define SC_ATOMIC_AND(name, val) atomic_fetch_and(&(name##_sc_atomic__), (val))
 ...
 *  \retval var value
 */
#define SC_ATOMIC_GET(name) atomic_load(&(name##_sc_atomic__))

#define SC_ATOMIC_LOAD_EXPLICIT(name, order) atomic_load_explicit(&(name##_sc_atomic__), (order))
 ...
 *  \retval var value
 */
#define SC_ATOMIC_SET(name, val) atomic_store(&(name##_sc_atomic__), (val))
