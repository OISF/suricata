...
typedef struct SpmTableElmt_ {
    ...
    uint8_t *(*Scan)(const SpmCtx *ctx, SpmThreadCtx *thread_ctx, const uint8_t *haystack,
...
SpmCtx *SpmInitCtx(
        const uint8_t *needle, uint16_t needle_len, int nocase, SpmGlobalThreadCtx *g_thread_ctx);

void SpmDestroyCtx(SpmCtx *ctx);

uint8_t *SpmScan(const SpmCtx *ctx, SpmThreadCtx *thread_ctx, const uint8_t *haystack,
        uint32_t haystack_len);
