...
typedef struct PcapFileDirectoryVars_
{
    ...
    TAILQ_HEAD(, PendingFile_) directory_content;
    ...
    if (temp_dir == NULL) //if null, our filename may just be a normal file
    {
        switch (errno)
        {
            ...
            case EMFILE:
                SCLogError(
                    SC_ERR_FOPEN,
                    "%s: The per-process limit on the number of open file descriptors has been reached",
                    filename
                );
                break;
            ...
            case ENFILE:
                SCLogError(
                    SC_ERR_FOPEN,
                    "%s: The system-wide limit on the number of open file descriptors has been reached",
                    filename
                );
                break;
...
TmEcode PcapDirectoryInsertFile(
    ...
    if(TAILQ_EMPTY(&pv->directory_content)) {
        ...
        while(file_to_compare != NULL) {
            ...
            if(PcapDirectoryGetModifiedTime(file_to_compare->filename, &modified_time) == TM_ECODE_FAILED) {
                SCReturnInt(TM_ECODE_FAILED);
            }
...
TmEcode PcapDirectoryPopulateBuffer(
    ...
    while ((dir = readdir(pv->directory)) != NULL)
    {
        ...
        if (written > 0 && written < PATH_MAX)
        {
            ...
            file_to_add->filename = SCMalloc(sizeof(char *) * (written + 1));
            if (unlikely(file_to_add->filename == NULL))
            {
                SCLogError(SC_ERR_MEM_ALLOC, "Failed to allocate file name");

                SCReturnInt(TM_ECODE_FAILED);
            }
            memcpy(file_to_add->filename, pathbuff, written + 1);
        ...
        *older_than = time(NULL) - pv->delay;
        ...
        while (status == TM_ECODE_OK && !TAILQ_EMPTY(&pv->directory_content)) {
            ...
            if(current_file != NULL) {
                ...
                if (PcapDirectoryGetModifiedTime(current_file->filename, &temp_time) != 0) {
                    temp_time.tv_sec = *newer_than;
                }
                SCLogDebug("Processed file %s, processed up to %ld", current_file->filename, temp_time.tv_sec);
...
TmEcode ReceivePcapFileDirectoryLoop(PcapFileThreadVars *tv, PcapFileDirectoryVars *ptv)
{
    ...
    while (status == TM_ECODE_OK) {
        if (ptv->directory != NULL) {
            ...
            //check directory
            if(PcapDetermineDirectoryOrFile(ptv->filename, &(ptv->directory)) == TM_ECODE_FAILED) {
