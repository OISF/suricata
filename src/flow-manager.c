...
again:
    ...
    *pos = (end == hash_max) ? hash_min : end;
    ...
    // SCLogNotice("hash %u:%u slice %u-%u rows %u rows_left %u *pos %u", hash_min, hash_max, start,
    // end, rows, rows_left, *pos);
...
*/
    ...
    while (1)
    {
        ...
        if (ts_ms >= next_run_ms) {
            ...
            GetWorkUnitSizing(pass_in_sec, rows, mp, emerg, &sleep_per_wu, &rows_per_wu);
            if (pmp != mp) {
                SCLogNotice("rows_per_wu %u, sleep_per_wu %" PRIu64 ", mp %u%%", rows_per_wu,
                        sleep_per_wu, mp);
            }
