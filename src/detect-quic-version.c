...
void QuicVersionRegisterTests(void);
void DetectQuicVersionFree(DetectEngineCtx *de_ctx, void *);

static int DetectEngineInspectQuicVersionGeneric(ThreadVars *tv, DetectEngineCtx *de_ctx,
...
#ifdef UNITTESTS
    sigmatch_table[DETECT_AL_QUIC_VERSION].RegisterTests = QuicVersionRegisterTests;
#endif

    DetectAppLayerInspectEngineRegister("quic.version", ALPROTO_QUIC, SIG_FLAG_TOSERVER, 1,
            DetectEngineInspectQuicVersionGeneric);
        ...
        void *state, void *txv, const Signature *s, const SigMatchCtx *ctx)
{
    ...
    version = rs_quic_tx_get_version(state);
    SCLogNotice("QUIC: %d", version);
