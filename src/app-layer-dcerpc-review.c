...
static enum DCERPCStatus DCERPCStatePDUStart(struct DCERPCState *state);
static enum DCERPCStatus DCERPCStateParseBindContexts(struct DCERPCState *state);
...
{
    int i;
    struct ListInterface *item = l->head;
    struct ListInterface *next_item;
    i = 0;
    while (i < l->size) {
        next_item = item->next;
        Free(item);
...
void ListAppend(void *item, struct List *l)
{
    if (l->tail == NULL) {
        ...
        ((struct ListInterface *)l->tail)->next = item;
        ((struct ListInterface *)item)->prev = l->tail;
    }
    l->tail = item;
    l->size++;
...
void *ListGetAtIndex(int index, struct List *l)
{
    ...
    struct ListInterface *head = l->head;
...
void ListRemoveItem(void *item, struct List *l)
{
    ...
    tmp = item;
    if (tmp->prev != NULL)
        ((struct ListInterface *)tmp->prev)->next = tmp->next;
    if (tmp->next != NULL)
        ((struct ListInterface *)tmp->next)->prev = tmp->prev;
    l->size--;
...
/* we don't need pop at this point of time */
#if 0
...
static void DCERPCAppendUUID(uint8_t *uuid, uint32_t iv, uint32_t context_id, struct List *l)
{
    struct UUID *new = SCMalloc(sizeof(*new));
    if (new == NULL)
        return;
...
static enum DCERPCStatus DCERPCStatePDUEnd(struct DCERPCState *state)
{
    struct DCERPCConnp *connp = &state->connp[state->curr_direction];
...
static enum DCERPCStatus DCERPCStateIgnorePDU(struct DCERPCState *state)
{
    int retval;
    uint16_t jumped;
    struct DCERPCConnp *connp = &state->connp[state->curr_direction];
...
static enum DCERPCStatus DCERPCStateParseBind(struct DCERPCState *state)
{
...
static enum DCERPCStatus DCERPCStateParseBindAckResults(struct DCERPCState *state)
{
    ...
    for (; connp->tmp16[0] < connp->tmp8[0]; connp->tmp16[0]++) {
        switch (connp->in_state_progress) {
            case INTERNAL_STATE_RESULT:
                ...
                connp->in_state_progress++;
                connp->pdu_bytes_processed += 2;
                struct UUID *uuid = ListGetAtIndex(connp->tmp16[0], &state->uuids);
                if (uuid == NULL)
                    return DCERPC_ERROR;
...
static enum DCERPCStatus DCERPCStateParseBindAckResultList(struct DCERPCState *state)
{
...
static enum DCERPCStatus DCERPCStateParseBindAck(struct DCERPCState *state)
{
...
static enum DCERPCStatus DCERPCStateParseBindAndAckCommon(struct DCERPCState *state)
{
...
static enum DCERPCStatus DCERPCStateParseStub(struct DCERPCState *state)
{
    struct DCERPCConnp *connp = &state->connp[state->curr_direction];
    struct DCERPCTx *tx = connp->curr_tx;
    uint16_t copied;
    int retval;
...
static enum DCERPCStatus DCERPCStateDetermineStub(struct DCERPCState *state)
{
    struct DCERPCConnp *connp = &state->connp[state->curr_direction];
    struct DCERPCTx *tx = connp->curr_tx;
...
static enum DCERPCStatus DCERPCStateParseRequest(struct DCERPCState *state)
{
...
#define INTERNAL_STATE_OBJECT 3
    ...
    switch (connp->in_state_progress) {
        ...
        case INTERNAL_STATE_P_CONT_ID:
            ...
            struct UUID *uuid = state->uuids.head;
            while (uuid != NULL) {
...
static enum DCERPCStatus DCERPCStateParseResponse(struct DCERPCState *state)
{
#define INTERNAL_STATE_ALLOC_HINT 0
#define INTERNAL_STATE_P_CONT_ID 1
...
static enum DCERPCStatus DCERPCStateParsePType(struct DCERPCState *state)
{
    struct DCERPCConnp *connp = &state->connp[state->curr_direction];
    ...
    if (ptype == 11 || ptype == 12 || ptype == 14 || ptype == 15) {
        ...
        if (connp->rpc_vers_minor == 0 || connp->pfc_flags & 0x01) {
            if (ptype == 11 || ptype == 14)
                ListReset(&state->uuids, DCERPCFreeUUID);
        ...
        if (connp->pfc_flags & 0x01) {
            ...
            ListAppend(tx, &state->transactions);
...
static enum DCERPCStatus DCERPCStateParseHeader(struct DCERPCState *state)
{
...
{
    struct DCERPCState *state = (struct DCERPCState *)alstate;
...
static int DCERPCStateSetup(struct DCERPCState *state)
{
...
{
    struct DCERPCState *state = SCMalloc(sizeof(struct DCERPCState));
    if (unlikely(state == NULL))
        return NULL;
    memset(state, 0, sizeof(struct DCERPCState));
...
{
    struct DCERPCState *state = (struct DCERPCState *)s;
    ...
    ListReset(&state->uuids, DCERPCFreeUUID);
    ListReset(&state->transactions, DCERPCFreeDCERPCTx);
...
{
    return ((struct DCERPCTx *)tx)->progress[direction];
}

static uint64_t DCERPCStateGetTxCnt(void *alstate)
{
    return (uint64_t)ListSize(&((struct DCERPCState *)alstate)->transactions);
}

static void *DCERPCStateGetTx(void *alstate, uint64_t tx_id)
{
    return ListGetAtIndex(tx_id, &((struct DCERPCState *)alstate)->transactions);
}
...
void DCERPCStateTransactionFree(void *alstate, uint16_t id)
{
    ...
    for (tx_id = ListIndex(&state->transactions); tx_id < id; tx_id++) {
        tx = ListGetAtIndex(tx_id, &state->transactions);
        BUG_ON(tx == NULL);
        ListRemoveItem(tx, &state->transactions);
        SCFree(tx->stub[0]);
        SCFree(tx->stub[1]);
        SCFree(tx);
    }
...
int DCERPCParserTest01(void)
{
    ...
    if (state->connp[0].rpc_vers_minor != 0 ||
        ...
        ListSize(&state->uuids) != 1 ||
        ListSize(&state->transactions) != 0) {
        printf("failure1\n");
        goto end;
    }
    uuid = ListGetAtIndex(0, &state->uuids);
    ...
    tx = ListGetAtIndex(0, &state->transactions);
    if (tx == NULL ||
        tx->call_id != 1 ||
        tx->opnum != 5 ||
        tx->stub_len[0] != 0 ||
...
int DCERPCParserTest05(void)
{
    ...
    tx = ListGetAtIndex(1, &state->transactions);
    if (tx == NULL ||
        tx->call_id != 1 ||
        tx->opnum != 21 ||
        tx->stub_len[0] != 52 ||
...
int DCERPCParserTest07(void)
{
    ...
    tx = ListGetAtIndex(2, &state->transactions);
    if (tx == NULL ||
        tx->call_id != 3 ||
        tx->opnum != 26 ||
        tx->stub_len[0] != 40 ||
...
int DCERPCParserTest08(void)
{
    ...
    uuid = ListGetAtIndex(1, &state->uuids);
    ...
    uuid = ListGetAtIndex(2, &state->uuids);
    ...
    uuid = ListGetAtIndex(3, &state->uuids);
    ...
    uuid = ListGetAtIndex(4, &state->uuids);
    ...
    uuid = ListGetAtIndex(5, &state->uuids);
    ...
    uuid = ListGetAtIndex(6, &state->uuids);
    ...
    uuid = ListGetAtIndex(7, &state->uuids);
    ...
    uuid = ListGetAtIndex(8, &state->uuids);
    ...
    uuid = ListGetAtIndex(9, &state->uuids);
    ...
    uuid = ListGetAtIndex(10, &state->uuids);
    ...
    uuid = ListGetAtIndex(11, &state->uuids);
    ...
    uuid = ListGetAtIndex(12, &state->uuids);
    ...
    uuid = ListGetAtIndex(13, &state->uuids);
    ...
    uuid = ListGetAtIndex(14, &state->uuids);
    ...
    uuid = ListGetAtIndex(15, &state->uuids);
