...
{
    uint16_t value;
    if (!rs_enip_get_protocol_version(txv, flags, &value))
        SCReturnInt(0);
...
void DetectEnipProtocolVersionRegister(void)
{
    ...
    DetectAppLayerInspectEngineRegister2("enip.protocol_version", ALPROTO_ENIP, SIG_FLAG_TOSERVER,
            0, DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.protocol_version", ALPROTO_ENIP, SIG_FLAG_TOCLIENT,
            0, DetectEngineInspectGenericList, NULL);
