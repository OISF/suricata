 ...
 * \retval -1 on Failure
 */
static int DetectEnipProtocolVersionSetup(DetectEngineCtx *de_ctx, Signature *s, const char *rulestr)
{
    ...
    if (SigMatchAppendSMToList(
                de_ctx, s, DETECT_ENIP_PROTOCOLVERSION, (SigMatchCtx *)du16, g_enip_protocol_version_id) == NULL) {
...
/**
 * \brief This function is used to match enip protocol_version type rule option on a transaction with those
 * passed via enip_protocol_version:
 *
 * \retval 0 no match
 * \retval 1 match
 */
...
{
    uint16_t value;
    if (!rs_enip_get_protocol_version(txv, &value))
        SCReturnInt(0);
...
void DetectEnipProtocolVersionRegister(void)
{
    ...
    sigmatch_table[DETECT_ENIP_PROTOCOLVERSION].desc = "rules for detecting EtherNet/IP protocol_version";
    sigmatch_table[DETECT_ENIP_PROTOCOLVERSION].url = "/rules/enip-keyword.html#enip-protocol-version";
    ...
    DetectAppLayerInspectEngineRegister2("enip.protocol_version", ALPROTO_ENIP, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.protocol_version", ALPROTO_ENIP, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectGenericList, NULL);
