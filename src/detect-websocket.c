...
static int DetectWebSocketFinMatch(DetectEngineThreadCtx *det_ctx, Flow *f, uint8_t flags,
        void *state, void *txv, const Signature *s, const SigMatchCtx *ctx)
{
    bool mask = SCWebSocketGetFin(txv);
...
static int DetectWebSocketFinSetup(DetectEngineCtx *de_ctx, Signature *s, const char *rawstr)
{
    if (DetectSignatureSetAppProto(s, ALPROTO_WEBSOCKET) < 0)
        return -1;
    ...
    if (SigMatchAppendSMToList(de_ctx, s, DETECT_WEBSOCKET_FIN, (SigMatchCtx *)dummyptr,
                websocket_tx_id) == NULL) {
        return -1;
    }
...
void DetectWebsocketRegister(void)
{
    ...
    sigmatch_table[DETECT_WEBSOCKET_FIN].name = "websocket.fin";
    sigmatch_table[DETECT_WEBSOCKET_FIN].desc = "match WebSocket fin flag";
    sigmatch_table[DETECT_WEBSOCKET_FIN].url = "/rules/websocket-keywords.html#websocket-fin";
    sigmatch_table[DETECT_WEBSOCKET_FIN].AppLayerTxMatch = DetectWebSocketFinMatch;
    sigmatch_table[DETECT_WEBSOCKET_FIN].Setup = DetectWebSocketFinSetup;
