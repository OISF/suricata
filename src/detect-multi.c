...
int DetectMultiSetup(DetectEngineCtx *de_ctx, Signature *s, const char *arg)
{
    ...
    switch (index_type) {
        ...
        case DetectMultiIndexNb:
            ...
            sigmatch_table[DETECT_MULTI_NB].Free(de_ctx, sm_ctx);
            return -1;
        case DetectMultiIndexPrecise:
            prec = (DetectUintIndexPrecise *)sm_ctx;
            if (prec->pos < 0) {
                SCDetectMultiIndexFree(sm_ctx);
...
void DetectMultiRegister(void)
{
    ...
    sigmatch_table[DETECT_MULTI_NB].name = "__multi_nb";
    ...
    sigmatch_table[DETECT_MULTI_INDEX].name = "__multi_index";
    ...
    // just a name for --engine-analysis
    sigmatch_table[DETECT_MULTI_ALL].name = "__multi_all";
    sigmatch_table[DETECT_MULTI_ALL_OR_ABSENT].name = "__multi_all_or_absent";
}
