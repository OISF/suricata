...
static int DetectEnipVendorIdSetup(DetectEngineCtx *de_ctx, Signature *s, const char *rulestr)
{
    ...
    if (SigMatchAppendSMToList(
                de_ctx, s, DETECT_ENIP_VENDORID, (SigMatchCtx *)du16, g_enip_vendor_id_id) == NULL) {
...
{
    uint16_t value;
    if (!rs_enip_get_vendor_id(txv, &value))
        SCReturnInt(0);
...
void DetectEnipVendorIdRegister(void)
{
    ...
    DetectAppLayerInspectEngineRegister2("enip.vendor_id", ALPROTO_ENIP, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.vendor_id", ALPROTO_ENIP, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectGenericList, NULL);
