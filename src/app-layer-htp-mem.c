...
void HTPParseMemcap()
{
    ...
    MemcapListRegisterMemcap("applayer-proto-http", "app-layer.protocols.http.memcap", HTPSetMemcap,
            HTPGetMemcap, HTPMemuseGlobalCounter);
    ...
    if ((ConfGet("app-layer.protocols.http.memcap", &conf_val)) == 1)
    {
        if (ParseSizeStringU64(conf_val, &memcap) < 0) {
            ...
            GlobalMemcapReached(memcap, "http.memcap", true);
