...
int PEScanFile(File *file) {
    ...
    for (ffd = file->chunks_head; ffd != NULL; ffd = ffd->next) {
        ...
        bytes = ((ffd->len + dlen <= MAX_SCAN_BYTES) ?
                ffd->len : MAX_SCAN_BYTES - dlen);
        ...
        if (ret != PE_NOT_PE && ret != PE_INDETERMINATE) {
            ...
            flen = snprintf(filename, (file->name_len < MAX_FILE_NAME ?
                    file->name_len + 1 : MAX_FILE_NAME), "%s", file->name);
            if (flen > 0) {
            ...
            if (ret == PE_TRUNCATED) {
                ...
                if (flen > 0) {
                    SCLogDebug("Truncated PE found: \"%s\" Size: %lu",
                            filename, file->size);
