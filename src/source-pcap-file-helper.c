...
/* Thread-local current pfv, valid only while a pcap-file RX thread is active.
 * Worker threads that inject pseudo packets (flow timeout, shutdown flush,
 * detect/log flush) run on different threads without this TLS set, so we also
 * maintain a global fallback pointer while a file is active. */
static thread_local PcapFileFileVars *pcap_current_pfv = NULL;
static PcapFileFileVars *pcap_current_pfv_global = NULL;
...
void CleanupPcapFileFileVars(PcapFileFileVars *pfv)
{
    ...
    pfv->shared = NULL;
    if (pcap_current_pfv_global == pfv) {
        pcap_current_pfv_global = NULL;
    }
    SCFree(pfv);
}
...
void PcapFileCallbackLoop(char *user, struct pcap_pkthdr *h, u_char *pkt)
{
    ...
    /* Inform capture-neutral hook about packet creation so capture
     * implementations can attach context without generic code changes. */
...
PcapFileFileVars *PcapFileGetCurrentPfvGlobal(void)
{
    return pcap_current_pfv_global;
}
...
static void PcapCaptureOnPacketWithAlerts(const Packet *p)
{
    ...
    if (pfv == NULL) {
        ...
        if (pfv == NULL) {
            pfv = PcapFileGetCurrentPfvGlobal();
        }
    }
    if (pfv != NULL) {
...
__attribute__((constructor)) static void PcapFileInstallCaptureHooks(void)
{
