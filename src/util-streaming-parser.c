...
int StreamingParserGetU8(void *ctx, uint8_t *ret_input)
{
    ...
    if (tctx->temp_buffer_len > 0) {
        *ret_input = tctx->temp_buffer.u8[0];
        tctx->temp_buffer.u64 = tctx->temp_buffer.u64 >> 8;
...
static int StreamingParserGetValue(void *ctx, void *ret_input, uint8_t psize)
{
    ...
    if (psize <= tctx->temp_buffer_len) {
        if (psize == 2) {
            *((uint16_t *)ret_input) = tctx->temp_buffer.u16[0];
        } else if (psize == 4) {
            *((uint32_t *)ret_input) = tctx->temp_buffer.u32[0];
        } else {
            *((uint64_t *)ret_input) = tctx->temp_buffer.u64;
        }
        /* Since a type can't be shifted by a value >= width of the type, we
         * need to split it */
        tctx->temp_buffer.u64 = tctx->temp_buffer.u64 >> (psize * 8 - 1);
        tctx->temp_buffer.u64 = tctx->temp_buffer.u64 >> 1;
    ...
    if (tctx->data_len < (psize - tctx->temp_buffer_len) ) {
        for (u = 0; u < tctx->data_len; u++)
            tctx->temp_buffer.u8[tctx->temp_buffer_len++] = tctx->data[u];
        tctx->data_len = 0;
...
{
    int16_t value;
    int retval = StreamingParserGetValue(ctx, (void *)&value, sizeof(int16_t));
    if (retval == STREAMING_PARSER_RDATA)
        return STREAMING_PARSER_RDATA;
...
{
    int32_t value;
    int retval = StreamingParserGetValue(ctx, (void *)&value, sizeof(int32_t));
    if (retval == STREAMING_PARSER_RDATA)
        return STREAMING_PARSER_RDATA;
...
{
    int64_t value;
    int retval = StreamingParserGetValue(ctx, (void *)&value, sizeof(int64_t));
    if (retval == STREAMING_PARSER_RDATA)
        return STREAMING_PARSER_RDATA;
...
int StreamingParserTestU8(void *ctx, uint8_t *ret_input)
{
    ...
    *ret_input = *(tctx->data++);
    tctx->data_len--;
    tctx->temp_buffer.u8[0] = *ret_input;
    tctx->temp_buffer_len++;
...
static int StreamingParserTestValue(void *ctx, void *ret_input, uint8_t psize)
{
    ...
    if (tctx->data_len < (psize - tctx->temp_buffer_len) ) {
        ...
        if (tctx->temp_buffer_len == 0) {
            if (psize == 2) {
                *((uint16_t *)ret_input) = *((uint16_t *)tctx->data);
                tctx->temp_buffer.u64 = *((uint16_t *)ret_input);
            } else if (psize == 4) {
                *((uint32_t *)ret_input) = *((uint32_t *)tctx->data);
                tctx->temp_buffer.u64 = *((uint32_t *)ret_input);
            } else {
                *((uint64_t *)ret_input) = *((uint64_t *)tctx->data);
                tctx->temp_buffer.u64 = *((uint64_t *)ret_input);
            }
            tctx->temp_buffer_len = psize;
            tctx->data += psize;
            tctx->data_len -= psize;
...
{
    int16_t value;
    int retval = StreamingParserTestValue(ctx, (void *)&value, sizeof(int16_t));
    if (retval == STREAMING_PARSER_RDATA)
        return STREAMING_PARSER_RDATA;
...
{
    int32_t value;
    int retval = StreamingParserTestValue(ctx, (void *)&value, sizeof(int32_t));
    if (retval == STREAMING_PARSER_RDATA)
        return STREAMING_PARSER_RDATA;
...
{
    int64_t value;
    int retval = StreamingParserTestValue(ctx, (void *)&value, sizeof(int64_t));
    if (retval == STREAMING_PARSER_RDATA)
        return STREAMING_PARSER_RDATA;
