...
typedef struct THashTableContext_ {
     ...
     *  the next poll triggers a single combined scan that refills all
     *  four with a consistent snapshot. Only accessed from the stats
     *  management thread, so no atomics. */
    uint32_t bucket_stats_entries;
    uint32_t bucket_stats_max;
    uint32_t bucket_stats_avg;
...
uint64_t THashGetterNonEmptyBuckets(void *ctx);
uint64_t THashGetterCounter(void *ctx);
