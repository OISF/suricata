...
{
    DetectTransformFromBase64Data *b64d = NULL;
    int r = -1;

    SCEnter();
    b64d = DetectTransformFromBase64DecodeParse(opts_str);
    if (b64d == NULL)
        SCReturnInt(r);

    if (b64d->flags & DETECT_TRANSFORM_BASE64_FLAG_NBYTES_VAR) {
        DetectByteIndexType index;
        if (!DetectByteRetrieveSMVar(b64d->nbytes_str, s, &index)) {
            SCLogError("unknown byte_ keyword var seen in from_base64 - %s", b64d->nbytes_str);
            goto error;
        }
        b64d->nbytes = index;
    }
    ...
    if (b64d->flags & DETECT_TRANSFORM_BASE64_FLAG_OFFSET_VAR) {
        DetectByteIndexType index;
        if (!DetectByteRetrieveSMVar(b64d->offset_str, s, &index)) {
            SCLogError("unknown byte_ keyword var seen in from_base64 - %s", b64d->offset_str);
            goto error;
        }
        b64d->offset = index;
    }
...
static void TransformFromBase64Decode(InspectionBuffer *buffer, void *options)
{
    ...
    DetectBase64Mode mode = DETECT_TRANSFORM_FROM_BASE64_MODE_DEFAULT;

    uint32_t decode_length = input_len;
    if (b64d) {
        mode = b64d->mode;
        if (b64d->offset) {
            if (b64d->offset > input_len) {
                SCLogDebug("offset %d exceeds length %d; returning", b64d->offset, input_len);
                return;
            }
            input += b64d->offset;
            decode_length -= b64d->offset;
        }

        if (b64d->nbytes) {
            if (b64d->nbytes > decode_length) {
                SCLogDebug("byte count %d plus offset %d exceeds length %d; returning",
                        b64d->nbytes, b64d->offset, input_len);
                return;
            }
            decode_length = b64d->nbytes;
        }
    }
...
static int DetectTransformFromBase64DecodeTest01(void)
{
    ...
    TransformFromBase64Decode(&buffer, NULL);
