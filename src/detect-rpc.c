...
#define MAX_SUBSTRINGS 30
    ...
    for (i = 0; i < (ret - 1); i++) {
        if (args[i]) {
            switch (i) {
                case 0:
                    if (ByteParseStringUint32(&rd->program, 10, strlen(args[i]), args[i]) <= 0) {
                ...
                case 1:
                    if (args[i][0] != '*') {
                        if (ByteParseStringUint32(&rd->program_version, 10, strlen(args[i]), args[i]) <= 0) {
                ...
                case 2:
                    if (args[i][0] != '*') {
                        if (ByteParseStringUint32(&rd->procedure, 10, strlen(args[i]), args[i]) <= 0) {
