...
void DefragInitConfig(char quiet)
{
    ...
    MemcapListRegisterMemcap("defrag", "defrag.memcap", DefragTrackerSetMemcap,
            DefragTrackerGetMemcap, DefragTrackerGetMemuse);
    ...
    if ((ConfGet("defrag.memcap", &conf_val)) == 1)
    {
        if (ParseSizeStringU64(conf_val, &defrag_memcap) < 0) {
            ...
            GlobalMemcapReached(defrag_memcap, "defrag.memcap", false);
