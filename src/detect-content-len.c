...
#define PARSE_REGEX "^\\s*(?:(<|>|=|!=)\\s*,\\s*([0-9]{1,5})|([0-9]{1,5})\\s*(<>)\\s*([0-9]{1,5}))\\s*$"
...
#define MAX_SUBSTRINGS 30
    ...
    const char *op = NULL;
    const char *op_arg1 = NULL;
    const char *op_arg2 = NULL;
    ...
    if (ret == 3) {
        ...
        ret = pcre_get_substring((char *)len_str, ov, MAX_SUBSTRINGS, 2, &op_arg1);
        if (ret < 0) {
            SCLogError(SC_ERR_PCRE_GET_SUBSTRING, "pcre_get_substring failed");
            goto error;
        }
        SCLogDebug("op_arg1 - \"%s\"", op_arg);
    } else {
        ret = pcre_get_substring(len_str, ov, MAX_SUBSTRINGS, 3, &op_arg1);
        if (ret < 0) {
            SCLogError(SC_ERR_PCRE_GET_SUBSTRING, "pcre_get_substring failed");
            goto error;
        }
        SCLogDebug("op_arg1 - \"%s\"", op);

        ret = pcre_get_substring((char *)len_str, ov, MAX_SUBSTRINGS, 4, &op);
        if (ret < 0) {
            SCLogError(SC_ERR_PCRE_GET_SUBSTRING, "pcre_get_substring failed");
            goto error;
        }
        ...
        ret = pcre_get_substring(len_str, ov, MAX_SUBSTRINGS, 5, &op_arg2);
        if (ret < 0) {
            SCLogError(SC_ERR_PCRE_GET_SUBSTRING, "pcre_get_substring failed");
            goto error;
        }
        SCLogDebug("op_arg2 - \"%s\"", op_arg2);
    }
    ...
    if (ParseSizeStringU32(op_arg1, &cld->len1) < 0)
        goto error;
    if (cld->len1 > 65536) {
        SCLogError(SC_ERR_INVALID_SIGNATURE, "content_len has invalid "
                   "length - %s.  Please enter a value in the range "
                   "0 - 65536", op_arg1);
        goto error;
    }
    ...
    if (op_arg2 == NULL) {
        ...
        cld->op = DETECT_CONTENT_LEN_RA;
        if (ParseSizeStringU32(op_arg2, &cld->len2) < 0)
            goto error;
        if (cld->len2 > 65536) {
            SCLogError(SC_ERR_INVALID_SIGNATURE, "content_len has invalid "
                       "range specified - %s<>%s.  Please enter values "
                       "in the range 0 - 65536", op_arg1, op_arg2);
            goto error;
        }
        if (cld->len1 > cld->len2) {
            SCLogError(SC_ERR_INVALID_SIGNATURE, "content_len range invalid. "
                       "%s<>%s - arg1 should be <= arg2.", op_arg1, op_arg2);
            goto error;
        }
    }

    pcre_free_substring(op);
    pcre_free_substring(op_arg1);
    pcre_free_substring(op_arg2);
...
static int DetectContentLenTest01(void)
{
    ...
    if (cld->op != DETECT_CONTENT_LEN_LT || cld->len1 != 10) {
        printf("if (cld->op != DETECT_CONTENT_LEN_LT || cld->len1 != 10)\n");
        goto end;
    }
...
static int DetectContentLenTest02(void)
{
    ...
    if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 10) {
        printf("if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 10)\n");
        goto end;
    }
...
static int DetectContentLenTest03(void)
{
    ...
    DetectContentLenData *cld = DetectContentLenParse("8<>10");
    if (cld == NULL) {
        printf("if (cld == NULL)\n");
        goto end;
    }

    if (cld->op != DETECT_CONTENT_LEN_RA || cld->len1 != 8 || cld->len2 != 10) {
        printf("if (cld->op != DETECT_CONTENT_LEN_LE || cld->len1 != 8 || cld->len2 != 10)\n");
        goto end;
    }
...
static int DetectContentLenTest04(void)
{
    ...
    if (cld->op != DETECT_CONTENT_LEN_EQ || cld->len1 != 10) {
        printf("if (cld->op != DETECT_CONTENT_LEN_EQ || cld->len1 != 10)\n");
        goto end;
    }
...
static int DetectContentLenTest05(void)
{
    ...
    if (cld->op != DETECT_CONTENT_LEN_LT || cld->len1 != 65536) {
        printf("if (cld->op != DETECT_CONTENT_LEN_LT || cld->len1 != 65536)\n");
        goto end;
    }
...
static int DetectContentLenTest06(void)
{
    ...
    if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 65536) {
        printf("if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 65536)\n");
        goto end;
    }
...
static int DetectContentLenTest07(void)
{
    ...
    DetectContentLenData *cld = DetectContentLenParse("0<>65536");
    if (cld == NULL) {
        printf("if (cld == NULL)\n");
        goto end;
    }

    if (cld->op != DETECT_CONTENT_LEN_RA || cld->len1 != 0 || cld->len2 != 65536) {
        printf("if (cld->op != DETECT_CONTENT_LEN_RA || cld->len1 != 0(%"PRIu32") || cld->len2 != 65536(%"PRIu32")\n",
               cld->len1, cld->len2);
        goto end;
    }
...
static int DetectContentLenTest08(void)
{
    ...
    if (cld->op != DETECT_CONTENT_LEN_EQ || cld->len1 != 65536) {
        printf("if (cld->op != DETECT_CONTENT_LEN_EQ || cld->len1 != 65536)\n");
        goto end;
    }
...
static int DetectContentLenTest14(void)
{
    ...
    DetectContentLenData *cld = DetectContentLenParse("0<>65537");
    if (cld != NULL) {
        printf("if (cld != NULL)\n");
        goto end;
    }
...
int DetectContentLenTest15(void)
{
    int result = 0;
...
int DetectContentLenTest16(void)
{
    int result = 0;
...
int DetectContentLenTest17(void)
{
    int result = 0;
...
int DetectContentLenTest18(void)
{
    int result = 0;
...
int DetectContentLenTest19(void)
{
    int result = 0;
...
int DetectContentLenTest20(void)
{
    int result = 0;
...
int DetectContentLenTest21(void)
{
    int result = 0;
...
int DetectContentLenTest22(void)
{
    int result = 0;
...
int DetectContentLenTest27(void)
{
    ...
    if (cld->op != DETECT_CONTENT_LEN_LT || cld->len1 != 15) {
        printf("if (cld->op != DETECT_CONTENT_LEN_LT || cld->len1 != 15)\n");
        goto end;
    }
...
int DetectContentLenTest51(void)
{
    ...
    cld = de_ctx->sig_list->sm_lists[DETECT_SM_LIST_HCDMATCH]->ctx;
    if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 20) {
        printf("if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 20)\n");
        goto end;
    }
    ...
    if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 30) {
        printf("if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 30)\n");
        goto end;
    }
    ...
    if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 15) {
        printf("if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 15)\n");
        goto end;
    }
    cld = de_ctx->sig_list->sm_lists[DETECT_SM_LIST_UMATCH]->next->next->next->ctx;
    if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 25) {
        printf("if (cld->op != DETECT_CONTENT_LEN_GT || cld->len1 != 25)\n");
        goto end;
    }
