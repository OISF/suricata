...
static void AppLayerProtoDetectPEGetIpprotos(AppProto alproto, uint8_t *ipprotos);
        ...
        uint32_t buflen, uint16_t searchlen, bool *rflow)
{
    ...
    if (s->cd->offset > searchlen) {
        SCLogDebug(
                "s->co->offset (%" PRIu16 ") > searchlen (%" PRIu16 ")", s->cd->offset, searchlen);
        SCReturnUInt(ALPROTO_UNKNOWN);
    }
    if (s->cd->depth > searchlen) {
        SCLogDebug("s->co->depth (%" PRIu16 ") > searchlen (%" PRIu16 ")", s->cd->depth, searchlen);
        SCReturnUInt(ALPROTO_UNKNOWN);
    }
    ...
    SCLogDebug(
            "s->co->offset (%" PRIu16 ") s->cd->depth (%" PRIu16 ")", s->cd->offset, s->cd->depth);

    uint8_t *found = SpmScan(s->cd->spm_ctx, tctx->spm_thread_ctx, sbuf, ssearchlen);
    if (found == NULL) {
        SCReturnUInt(ALPROTO_UNKNOWN);
    }
    ...
    if (s->PPFunc == NULL) {
        ...
        if (s->pp_min_depth > buflen) {
            SCLogDebug(
                    "PP can't be run yet as pp_min_depth %u > buflen %u", s->pp_min_depth, buflen);
        ...
        if (r == s->alproto) {
            SCLogDebug("found %s/%u, rdir %02x reverse_flow? %s", AppProtoToString(r), r, rdir,
        ...
        uint32_t buflen, uint8_t flags, AppProto *pm_results, bool *rflow)
{
    ...
    uint32_t search_cnt = mpm_table[pm_ctx->mpm_ctx.mpm_type].Search(
            &pm_ctx->mpm_ctx, mpm_tctx, &tctx->pmq, buf, searchlen);
    if (search_cnt == 0) {
    ...
    for (uint32_t cnt = 0; cnt < tctx->pmq.rule_id_array_cnt; cnt++) {
        ...
        while (s != NULL) {
            ...
            if (AppProtoIsValid(proto) && !(pm_results_bf[proto / 8] & (1 << (proto % 8)))) {
...
static AppLayerProtoDetectProbingParserPort *AppLayerProtoDetectGetProbingParsers(
        AppLayerProtoDetectProbingParser *pp, uint8_t ipproto, uint16_t port)
{
    AppLayerProtoDetectProbingParserPort *pp_port = NULL;
...
{
    while (pe != NULL) {
        if ((buflen < pe->min_depth) || (alproto_masks[0] & pe->alproto_mask)) {
            pe = pe->next;
            continue;
        }
...
again_midstream:
    ...
    SCLogDebug("%u->%u %s", sp, dp, (dir == STREAM_TOSERVER) ? "toserver" : "toclient");
    ...
    if (dir == idir) {
        ...
        if (alproto_masks[0] == mask) {
            ...
            SCLogDebug("%s, mask is now %08x, needed %08x, so done",
                    (dir == STREAM_TOSERVER) ? "toserver" : "toclient", alproto_masks[0], mask);
        } else {
...
noparsers:
    ...
    SCLogDebug("%s, mask is now %08x", (idir == STREAM_TOSERVER) ? "toserver" : "toclient",
            alproto_masks[0]);
    SCReturnUInt(alproto);
}
...
static void AppLayerProtoDetectPPGetIpprotos(AppProto alproto, uint8_t *ipprotos)
{
    SCEnter();
...
static AppLayerProtoDetectProbingParserElement *AppLayerProtoDetectProbingParserElementCreate(
        AppProto alproto, uint16_t port, uint16_t min_depth, uint16_t max_depth)
{
...
static AppLayerProtoDetectProbingParserElement *AppLayerProtoDetectProbingParserElementDuplicate(
        AppLayerProtoDetectProbingParserElement *pe)
{
    SCEnter();
        ...
        for (; pp_port != NULL; pp_port = pp_port->next) {
            if (pp_port->dp != NULL) {
                ...
                printf("        Destination port: (max-depth: %" PRIu16 ", "
                       "mask - %" PRIu32 ")\n",
                        pp_port->dp_max_depth, pp_port->alproto_mask);
                pp_pe = pp_port->dp;
            ...
            printf("        Source port: (max-depth: %" PRIu16 ", "
                   "mask - %" PRIu32 ")\n",
                    pp_port->sp_max_depth, pp_port->alproto_mask);
            pp_pe = pp_port->sp;
            for (; pp_pe != NULL; pp_pe = pp_pe->next) {
...
static void AppLayerProtoDetectProbingParserElementAppend(
...
static void AppLayerProtoDetectProbingParserAppend(
        AppLayerProtoDetectProbingParser **head_pp, AppLayerProtoDetectProbingParser *new_pp)
{
    SCEnter();
...
static void AppLayerProtoDetectProbingParserPortAppend(
        AppLayerProtoDetectProbingParserPort **head_port,
...
static void AppLayerProtoDetectInsertNewProbingParser(AppLayerProtoDetectProbingParser **pp,
        uint8_t ipproto, uint16_t port, AppProto alproto, uint16_t min_depth, uint16_t max_depth,
        uint8_t direction, ProbingParserFPtr ProbingParser1, ProbingParserFPtr ProbingParser2)
{
    SCEnter();
        ...
        if (zero_port != NULL) {
            ...
            for (; zero_pe != NULL; zero_pe = zero_pe->next) {
                ...
                if (zero_pe->max_depth == 0)
                    curr_port->dp_max_depth = zero_pe->max_depth;
                if (curr_port->dp_max_depth != 0 && curr_port->dp_max_depth < zero_pe->max_depth) {
                    curr_port->dp_max_depth = zero_pe->max_depth;
                }
            ...
            for (; zero_pe != NULL; zero_pe = zero_pe->next) {
                ...
                if (zero_pe->max_depth == 0)
                    curr_port->sp_max_depth = zero_pe->max_depth;
                if (curr_port->sp_max_depth != 0 && curr_port->sp_max_depth < zero_pe->max_depth) {
                    curr_port->sp_max_depth = zero_pe->max_depth;
                }
    ...
    AppLayerProtoDetectProbingParserElement *new_pe = AppLayerProtoDetectProbingParserElementCreate(
            alproto, curr_port->port, min_depth, max_depth);
    if (new_pe == NULL)
        goto error;
    curr_pe = new_pe;
    AppLayerProtoDetectProbingParserElement **head_pe;
        ...
        while (temp_port != NULL && temp_port->port != 0) {
            if (direction & STREAM_TOSERVER) {
                ...
                if (curr_pe->max_depth == 0)
                    temp_port->dp_max_depth = curr_pe->max_depth;
                if (temp_port->dp_max_depth != 0 && temp_port->dp_max_depth < curr_pe->max_depth) {
                    temp_port->dp_max_depth = curr_pe->max_depth;
                }
                ...
                if (curr_pe->max_depth == 0)
                    temp_port->sp_max_depth = curr_pe->max_depth;
                if (temp_port->sp_max_depth != 0 && temp_port->sp_max_depth < curr_pe->max_depth) {
                    temp_port->sp_max_depth = curr_pe->max_depth;
                }
                AppLayerProtoDetectProbingParserElementAppend(
...
static void AppLayerProtoDetectPMGetIpprotos(AppProto alproto, uint8_t *ipprotos)
{
    SCEnter();
...
static int AppLayerProtoDetectPMMapSignatures(AppLayerProtoDetectPMCtx *ctx)
{
    ...
    for (s = ctx->head; s != NULL;) {
        next_s = s->next;
        s->id = id++;
        SCLogDebug("s->id %u offset %u depth %u", s->id, s->cd->offset, s->cd->depth);

        if (s->cd->flags & DETECT_CONTENT_NOCASE) {
            mpm_ret = MpmAddPatternCI(&ctx->mpm_ctx, s->cd->content, s->cd->content_len,
                    s->cd->offset, s->cd->depth, s->cd->id, s->id, 0);
            if (mpm_ret < 0)
                goto error;
        } else {
            mpm_ret = MpmAddPatternCS(&ctx->mpm_ctx, s->cd->content, s->cd->content_len,
                    s->cd->offset, s->cd->depth, s->cd->id, s->id, 0);
            if (mpm_ret < 0)
                goto error;
        }
...
static int AppLayerProtoDetectPMAddSignature(AppLayerProtoDetectPMCtx *ctx, DetectContentData *cd,
        AppProto alproto, uint8_t direction, ProbingParserFPtr PPFunc, uint16_t pp_min_depth,
        uint16_t pp_max_depth)
{
    SCEnter();
...
static int AppLayerProtoDetectPMRegisterPattern(uint8_t ipproto, AppProto alproto,
        const char *pattern, uint16_t depth, uint16_t offset, uint8_t direction, uint8_t is_cs,
        ProbingParserFPtr PPFunc, uint16_t pp_min_depth, uint16_t pp_max_depth)
{
    ...
    DetectContentData *cd =
            DetectContentParseEncloseQuotes(alpd_ctx.spm_global_thread_ctx, pattern);
    if (cd == NULL)
        goto error;
    cd->depth = depth;
    cd->offset = offset;
    if (!is_cs) {
        ...
        cd->spm_ctx = SpmInitCtx(cd->content, cd->content_len, 1, alpd_ctx.spm_global_thread_ctx);
        if (cd->spm_ctx == NULL) {
            goto error;
        }
        ...
        ProbingParserFPtr ProbingParser2)
{
    ...
    while (temp_dp != NULL) {
        ...
        if (port == 0 && temp_dp->port2 != 0)
            port++;
        for (;;) {
            AppLayerProtoDetectInsertNewProbingParser(&alpd_ctx.ctx_pp, ipproto, port, alproto,
        ...
        ProbingParserFPtr ProbingParserTs, ProbingParserFPtr ProbingParserTc)
{
    ...
    r = snprintf(param, sizeof(param), "%s%s%s", "app-layer.protocols.", alproto_name,
            ".detection-ports");
    if (r < 0) {
    ...
    if (port_node != NULL && port_node->val != NULL) {
        AppLayerProtoDetectPPRegister(ipproto, port_node->val, alproto, min_depth, max_depth,
        ...
        uint16_t depth, uint16_t offset, uint8_t direction)
{
    ...
    int r = AppLayerProtoDetectPMRegisterPattern(ipproto, alproto, pattern, depth, offset,
            direction, 1 /* case-sensitive */, NULL, 0, 0);
    SCReturnInt(r);
}
        ...
        ProbingParserFPtr PPFunc, uint16_t pp_min_depth, uint16_t pp_max_depth)
{
    SCEnter();
    int r = AppLayerProtoDetectPMRegisterPattern(ipproto, alproto, pattern, depth, offset,
            direction, 1 /* case-sensitive */, PPFunc, pp_min_depth, pp_max_depth);
    SCReturnInt(r);
}
        ...
        uint16_t depth, uint16_t offset, uint8_t direction)
{
    SCEnter();
    int r = AppLayerProtoDetectPMRegisterPattern(ipproto, alproto, pattern, depth, offset,
            direction, 0 /* !case-sensitive */, NULL, 0, 0);
    SCReturnInt(r);
}
...
static int AppLayerProtoDetectTest03(void)
{
     ...
     * since it sets internal structures which depends on the above function. */
    ...
    bool rflow = false;
    uint32_t cnt = AppLayerProtoDetectPMGetProto(
            alpd_tctx, &f, l7data, sizeof(l7data), STREAM_TOCLIENT, pm_results, &rflow);
    FAIL_IF(cnt != 1);
...
static int AppLayerProtoDetectTest04(void)
{
    ...
    bool rdir = false;
    uint32_t cnt = AppLayerProtoDetectPMGetProto(
            alpd_tctx, &f, l7data, sizeof(l7data), STREAM_TOCLIENT, pm_results, &rdir);
    FAIL_IF(cnt != 1);
    FAIL_IF(pm_results[0] != ALPROTO_HTTP1);
...
static int AppLayerProtoDetectTest11(void)
{
    ...
    bool rdir = false;
    uint32_t cnt = AppLayerProtoDetectPMGetProto(
            alpd_tctx, &f, l7data, sizeof(l7data), STREAM_TOSERVER, pm_results, &rdir);
    FAIL_IF(cnt != 1);
    FAIL_IF(pm_results[0] != ALPROTO_HTTP1);

    memset(pm_results, 0, sizeof(pm_results));
    cnt = AppLayerProtoDetectPMGetProto(
            alpd_tctx, &f, l7data_resp, sizeof(l7data_resp), STREAM_TOCLIENT, pm_results, &rdir);
    FAIL_IF(cnt != 1);
    FAIL_IF(pm_results[0] != ALPROTO_HTTP1);
...
static int AppLayerProtoDetectPPTestData(AppLayerProtoDetectProbingParser *pp,
        AppLayerProtoDetectPPTestDataIPProto *ip_proto, int no_of_ip_proto)
{
    int result = 0;
    int i = -1, j = -1, k = -1;
#ifdef DEBUG
    ...
    for (i = 0; i < no_of_ip_proto; i++, pp = pp->next) {
        ...
        for (k = 0; k < ip_proto[i].no_of_port; k++, pp_port = pp_port->next) {
            ...
            for (j = 0; j < ip_proto[i].port[k].ts_no_of_element;
                ...
                if (pp_element->alproto_mask !=
                        ip_proto[i].port[k].toserver_element[j].alproto_mask) {
                    goto end;
                }
            ...
            dir = 1;
#endif
            for (j = 0; j < ip_proto[i].port[k].tc_no_of_element;
                    j++, pp_element = pp_element->next) {
                ...
                if (pp_element->alproto_mask !=
                        ip_proto[i].port[k].toclient_element[j].alproto_mask) {
                    goto end;
                }
...
static int AppLayerProtoDetectTest15(void)
{
    ...
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "80", ALPROTO_SMB, 5, 6, STREAM_TOSERVER,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "80", ALPROTO_FTP, 7, 10, STREAM_TOSERVER,
            ProbingParserDummyForTesting, NULL);

    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "81", ALPROTO_DCERPC, 9, 10, STREAM_TOSERVER,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "81", ALPROTO_FTP, 7, 15, STREAM_TOSERVER,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "0", ALPROTO_SMTP, 12, 0, STREAM_TOSERVER,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "0", ALPROTO_TLS, 12, 18, STREAM_TOSERVER,
            ProbingParserDummyForTesting, NULL);

    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "85", ALPROTO_DCERPC, 9, 10, STREAM_TOSERVER,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "85", ALPROTO_FTP, 7, 15, STREAM_TOSERVER,
            ProbingParserDummyForTesting, NULL);
    result = 1;

    AppLayerProtoDetectPPRegister(IPPROTO_UDP, "85", ALPROTO_IMAP, 12, 23, STREAM_TOSERVER,
            ProbingParserDummyForTesting, NULL);

    /* toclient */
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "0", ALPROTO_JABBER, 12, 23, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "0", ALPROTO_IRC, 12, 14, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);

    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "85", ALPROTO_DCERPC, 9, 10, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "81", ALPROTO_FTP, 7, 15, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "0", ALPROTO_TLS, 12, 18, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "80", ALPROTO_HTTP1, 5, 8, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "81", ALPROTO_DCERPC, 9, 10, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "90", ALPROTO_FTP, 7, 15, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "80", ALPROTO_SMB, 5, 6, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_UDP, "85", ALPROTO_IMAP, 12, 23, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "0", ALPROTO_SMTP, 12, 17, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    AppLayerProtoDetectPPRegister(IPPROTO_TCP, "80", ALPROTO_FTP, 7, 10, STREAM_TOCLIENT,
            ProbingParserDummyForTesting, NULL);
    ...
    AppLayerProtoDetectPPTestDataElement element_tc_81[] = { { "ftp", ALPROTO_FTP, 81,
                                                                     1 << ALPROTO_FTP, 7, 15 },
        { "dcerpc", ALPROTO_DCERPC, 81, 1 << ALPROTO_DCERPC, 9, 10 },
        { "jabber", ALPROTO_JABBER, 0, 1 << ALPROTO_JABBER, 12, 23 },
        { "irc", ALPROTO_IRC, 0, 1 << ALPROTO_IRC, 12, 14 },
    ...
    AppLayerProtoDetectPPTestDataElement element_tc_85[] = { { "dcerpc", ALPROTO_DCERPC, 85,
                                                                     1 << ALPROTO_DCERPC, 9, 10 },
        { "jabber", ALPROTO_JABBER, 0, 1 << ALPROTO_JABBER, 12, 23 },
        { "irc", ALPROTO_IRC, 0, 1 << ALPROTO_IRC, 12, 14 },
        { "tls", ALPROTO_TLS, 0, 1 << ALPROTO_TLS, 12, 18 },
        { "smtp", ALPROTO_SMTP, 0, 1 << ALPROTO_SMTP, 12, 17 } };
    ...
    AppLayerProtoDetectPPTestDataElement element_tc_90[] = { { "ftp", ALPROTO_FTP, 90,
                                                                     1 << ALPROTO_FTP, 7, 15 },
        { "jabber", ALPROTO_JABBER, 0, 1 << ALPROTO_JABBER, 12, 23 },
        { "irc", ALPROTO_IRC, 0, 1 << ALPROTO_IRC, 12, 14 },
        { "tls", ALPROTO_TLS, 0, 1 << ALPROTO_TLS, 12, 18 },
    ...
    AppLayerProtoDetectPPTestDataElement element_tc_0[] = { { "jabber", ALPROTO_JABBER, 0,
                                                                    1 << ALPROTO_JABBER, 12, 23 },
        { "irc", ALPROTO_IRC, 0, 1 << ALPROTO_IRC, 12, 14 },
        { "tls", ALPROTO_TLS, 0, 1 << ALPROTO_TLS, 12, 18 },
        { "smtp", ALPROTO_SMTP, 0, 1 << ALPROTO_SMTP, 12, 17 } };
...
static int AppLayerProtoDetectTest19(void)
{
    ...
    int r = AppLayerParserParse(
            NULL, alp_tctx, f, ALPROTO_FTP, STREAM_TOSERVER, http_buf1, http_buf1_len);
    if (r != 0) {
        printf("toserver chunk 1 returned %" PRId32 ", expected 0: ", r);
        goto end;
    }
