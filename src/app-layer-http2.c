...
void HTTP2MimicHttp1Request(void *alstate_orig, void *h2s)
{
    ...
    rs_http2_tx_set_method(
            h2s, bstr_ptr(htp_tx_request_method(h1tx)), bstr_len(htp_tx_request_method(h1tx)));
    if (htp_tx_request_uri(h1tx) != NULL) {
        ...
        rs_http2_tx_set_uri(
                h2s, bstr_ptr(htp_tx_request_uri(h1tx)), bstr_len(htp_tx_request_uri(h1tx)));
    }
    ...
    for (size_t i = 0; i < nbheaders; i++) {
        ...
        if (h != NULL) {
            rs_http2_tx_add_header(h2s, htp_header_name_ptr(h), htp_header_name_len(h),
