...
void AppLayerSetupCounters(uint8_t ipproto);
...
static void AppLayerIncFlowCounter(ThreadVars *tv, uint8_t ipproto, AppProto alproto)
{
    if (likely(tv)) {
...
void AppLayerIncTxCounter(ThreadVars *tv, uint8_t ipproto, AppProto alproto,
                          uint64_t step)
{
    if (likely(tv)) {
        uint8_t ipproto_map = FlowGetProtoMapping(ipproto);
        ...
        *alproto = AppLayerProtoDetectGetProto(app_tctx->alpd_tctx,
        ...
        if (*alproto != ALPROTO_UNKNOWN) {
            ...
            if (*alproto_otherdir != ALPROTO_UNKNOWN) {
                AppLayerIncFlowCounter(tv, IPPROTO_TCP, *alproto);
            }
...
int AppLayerSetup(void)
{
    ...
    AppLayerSetupCounters(IPPROTO_UDP);
    AppLayerSetupCounters(IPPROTO_TCP);
