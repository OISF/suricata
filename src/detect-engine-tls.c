        ...
        SigGroupHead *sgh, MpmCtx *mpm_ctx)
{
    ...
    return PrefilterAppendTxEngine(de_ctx, sgh, PrefilterTxTlsJa3Hash,
...
/** \brief TLS JA3 str Mpm prefilter callback
 *
 *  \param det_ctx detection engine thread ctx
 *  \param p packet to inspect
 *  \param f flow to inspect
 ...
 *  \param pectx inspection context
 */
static void PrefilterTxTlsJa3Str(DetectEngineThreadCtx *det_ctx,
        const void *pectx, Packet *p, Flow *f, void *txv,
        const uint64_t idx, const uint8_t flags)
{
    SCEnter();
    ...
    const uint8_t *buffer = (uint8_t *)ssl_state->ja3_str->data;
    const uint32_t buffer_len = ssl_state->ja3_str->used;
        ...
        SigGroupHead *sgh, MpmCtx *mpm_ctx)
{
    ...
    return PrefilterAppendTxEngine(de_ctx, sgh, PrefilterTxTlsJa3Str,
                                   ALPROTO_TLS, 0, mpm_ctx, NULL, "ja3_str");
}
 ...
 *  \retval 1 Match
 */
int DetectEngineInspectTlsJa3Str(ThreadVars *tv, DetectEngineCtx *de_ctx,
        DetectEngineThreadCtx *det_ctx, const Signature *s,
        const SigMatchData *smd, Flow *f, uint8_t flags, void *alstate,
        void *txv, uint64_t tx_id)
{
    ...
    buffer = (uint8_t *)ssl_state->ja3_str->data;
    buffer_len = ssl_state->ja3_str->used;
