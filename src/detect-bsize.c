...
value_error:
    if (bsz->mode == DETECT_BSIZE_RA) {
        SCLogError(SC_ERR_INVALID_SIGNATURE,
                "signature can't match as required content length %ld exceeds bsize range: %" PRIu64 "-%" PRIu64,
                needed, bsz->lo, bsz->hi);
    } else {
        SCLogError(SC_ERR_INVALID_SIGNATURE,
                "signature can't match as required content length %ld exceeds bsize value: %" PRIu64,
                needed, bsz->lo);
    }
