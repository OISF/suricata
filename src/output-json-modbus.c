    ...
    const Packet *p, Flow *f, void *state, void *tx, uint64_t tx_id)
{
    ...
    JsonBuilder *js = CreateEveHeader(p, LOG_DIR_PACKET, "modbus", NULL);
    if (unlikely(js == NULL)) {
        return TM_ECODE_OK;
    }
    EveAddCommonOptions(&thread->modbuslog_ctx->cfg, p, f, js);
...
static OutputInitResult OutputModbusLogInitSub(ConfNode *conf,
...
static TmEcode JsonModbusLogThreadInit(ThreadVars *t, const void *initdata, void **data)
{
    ...
    thread->file_ctx = LogFileEnsureExists(thread->modbuslog_ctx->file_ctx, t->id);
    if (!thread->file_ctx) {
        goto error_exit;
    }
...
void JsonModbusLogRegister(void)
{
    ...
    OutputRegisterTxSubModule(
        ...
        "JsonModbusLog",
        "eve-log.modbus",
        OutputModbusLogInitSub,
        ALPROTO_MODBUS,
        JsonModbusLogger,
        JsonModbusLogThreadInit,
        JsonModbusLogThreadDeinit,
        NULL
    );
