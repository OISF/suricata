        ...
        uint8_t flags, File *file, const Signature *s, const SigMatchCtx *m)
{
    ...
    result = yr_rules_scan_mem(yara->rules, data, data_len, yara_fast_scan,
    ...
    if (result != ERROR_SUCCESS &&
        ...
        if (yara_rule_file == NULL) {
            SCLogWarning(SC_ERR_FOPEN,
                         "error opening yara rule file: %s",
                         yara->filename);
            goto error;
        }
        ...
        if (result != ERROR_SUCCESS) {
            ...
            yr_compiler_destroy(compiler);
            SCLogWarning(SC_WARN_YARA_ERROR,
                         "error accessing compiled yara rules errors: %d",
                         result);
            goto error;
        }
        ...
        SCLogDebug("compiled yara rule file %s", yara->filename);
    }
