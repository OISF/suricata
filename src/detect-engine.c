...
int DetectEngineBufferTypeRegisterWithRecordEngines(DetectEngineCtx *de_ctx, const char *name,
        const int direction, const AppProto alproto, const uint8_t rec_type)
{
    ...
    if (direction & SIG_FLAG_TOSERVER) {
        DetectEngineRecordMpmRegister(de_ctx, map->name, SIG_FLAG_TOSERVER, 2,
                PrefilterGenericMpmRecordRegister, alproto, rec_type);
        DetectEngineRecordInspectEngineRegister(de_ctx, map->name, SIG_FLAG_TOSERVER,
                DetectEngineInspectRecordBufferGeneric, alproto, rec_type);
    }
    if (direction & SIG_FLAG_TOCLIENT) {
        DetectEngineRecordMpmRegister(de_ctx, map->name, SIG_FLAG_TOCLIENT, 2,
                PrefilterGenericMpmRecordRegister, alproto, rec_type);
        DetectEngineRecordInspectEngineRegister(de_ctx, map->name, SIG_FLAG_TOCLIENT,
                DetectEngineInspectRecordBufferGeneric, alproto, rec_type);
    }
