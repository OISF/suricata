...
DetectEngineCtx *DetectEngineGetCurrent(void)
{
    ...
    master->list->ref_cnt++;
    SCLogInfo("master->list %p ref_cnt %u", master->list, master->list->ref_cnt);
    SCMutexUnlock(&master->lock);
    return master->list;
}
...
int DetectEngineAddToMaster(DetectEngineCtx *de_ctx)
{
    ...
    SCLogInfo("adding de_ctx %p to master", de_ctx);
...
int DetectEngineMoveToFreeList(DetectEngineCtx *de_ctx)
{
    ...
    SCLogInfo("detect engine %p moved to free list (%u refs)", de_ctx, de_ctx->ref_cnt);
...
void DetectEnginePruneFreeList(void)
{
    ...
    while (instance) {
        ...
        SCLogInfo("detect engine %p has %u ref(s)", instance, instance->ref_cnt);
        ...
        if (instance->ref_cnt == 0) {
            ...
            SCLogInfo("freeing detect engine %p", instance);
...
int DetectEngineReload(void)
{
    ...
    if (old_de_ctx == NULL)
        return -1;
    SCLogInfo("get ref to old_de_ctx %p", old_de_ctx);
    ...
    SCLogInfo("set up new_de_ctx %p", new_de_ctx);
    ...
    SCLogInfo("going to reload the threads to use new_de_ctx %p", new_de_ctx);
    /* update the threads */
    DetectEngineReloadThreads(new_de_ctx);
    SCLogInfo("threads now run new_de_ctx %p", new_de_ctx);
