    ...
    switch (smd->type) {
        ...
        case DETECT_MULTI_INDEX:
            ...
            if (prec->pos < 0) {
                ...
                // TODO unreachable so far
                // need to get count if eof, and have local_id = count + prec->pos;
            } else {
                local_id = prec->pos;
            }
            // get the buffer
            ...
            // smd + 1 is not NULL thanks to DetectMultiValidateContentCallback
            const bool match = DetectEngineContentInspectionBuffer(de_ctx, det_ctx, s, smd + 1,
            ...
            buffer = DetectGetMultiData(det_ctx, transforms, f, flags, txv, engine->sm_list,
                    local_id + 1, engine->v2.GetMultiData);
            if (buffer && buffer->inspect) {
                // we have next buffer in the multi-buffer
                return DETECT_ENGINE_INSPECT_SIG_CANT_MATCH;
            }
            return DETECT_ENGINE_INSPECT_SIG_NO_MATCH;
    }
