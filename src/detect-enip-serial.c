...
{
    uint32_t value;
    if (!rs_enip_get_serial(txv, &value))
        SCReturnInt(0);
...
void DetectEnipSerialRegister(void)
{
    ...
    DetectAppLayerInspectEngineRegister2("enip.serial", ALPROTO_ENIP, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.serial", ALPROTO_ENIP, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectGenericList, NULL);
