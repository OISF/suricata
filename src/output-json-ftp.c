...
bool EveFTPLogCommand(void *vtx, JsonBuilder *jb)
{
    ...
    if (!TAILQ_EMPTY(&tx->response_list)) {
        ...
        TAILQ_FOREACH (wrapper, &tx->response_list, next) {
            ...
            FTPResponse *response = wrapper->response;

            for (uint32_t i = 0; i < response->count; i++) {
                if (!reply_truncated && response->truncated) {
                    reply_truncated = true;
                }
                if (response->lines[i].code != -1) {
                    if (!is_cc_array_open) {
                        jb_open_array(jb, "completion_code");
                        is_cc_array_open = true;
                    }
                    jb_append_uint(jb, response->lines[i].code);
                }
                if (response->lines[0].length) {
                    jb_append_string_from_bytes(js_resplist,
                            (const uint8_t *)response->lines[i].response,
                            strlen(response->lines[i].response));
                    resp_cnt++;
                }
            }
        }
