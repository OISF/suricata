...
static void JsonFTPLogJSON(json_t *tjs, Flow *f, FTPTransaction *tx)
{
    ...
    } else if (tx->command != FTP_COMMAND_UNKNOWN) {
        cjs = json_object();
        if (cjs != NULL) {
            json_object_set_new(cjs, "command", json_string((char*)(FtpCommands[tx->command])));
            /* skip past 4-character plus space command name */
            /* XXX all commands currently handled are 4 chars long */
            if (tx->request_length > 5) {
                json_object_set_new(cjs, "request",
                    json_stringn((char *)tx->request + 5, tx->request_length - 5));
            }
            if (tx->response_length > 4) {
                /* skip past 3-digit error code and space */
                json_object_set_new(cjs, "response",
                    json_stringn((char *)tx->response + 4, tx->response_length - 4));
                json_object_set_new(cjs, "response_code",
                    json_stringn((char *)tx->response, 3));
            }
            if (tx->dyn_port) {
                json_object_set_new(cjs, "dynamic_port", json_integer(tx->dyn_port));
            }
            if (tx->command == FTP_COMMAND_PORT || tx->command == FTP_COMMAND_EPRT) {
    ...
    if (cjs != NULL) {
        json_object_set_new(tjs, "current", cjs);
    }
}
    ...
    const Packet *p, Flow *f, void *state, void *vtx, uint64_t tx_id)
{
    ...
    if (tx) {
        SCLogDebug("tx %p; command %d", tx, tx->command);
    }
