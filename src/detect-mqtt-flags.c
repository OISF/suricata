...
void DetectMQTTFlagsRegister (void)
{
    ...
    sigmatch_table[DETECT_AL_MQTT_FLAGS].url = DOC_URL DOC_VERSION "/rules/mqtt-keywords.html#mqtt-flags";
                               ...
                               const SigMatchCtx *ctx)
{
    ...
    rs_mqtt_tx_get_flags(txv, &retain, &dup);
    switch (de->retain) {
        case MUST_BE_SET:
            if (!retain) return 0;
            break;
 ...
 */
void DetectMQTTFlagsFree(void *de_ptr)
{
