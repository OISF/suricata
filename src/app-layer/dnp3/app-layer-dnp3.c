 ...
 */
static void DNP3HandleUserDataRequest(DNP3State *dnp3, const uint8_t *input, uint32_t input_len)
{
    DNP3LinkHeader *lh;
...
static void DNP3HandleUserDataResponse(DNP3State *dnp3, const uint8_t *input, uint32_t input_len)
{
    DNP3LinkHeader *lh;
    DNP3TransportHeader th;
    DNP3ApplicationHeader *ah;
 ...
 *     like DNP3.
 */
static int DNP3HandleRequestLinkLayer(DNP3State *dnp3, const uint8_t *input, uint32_t input_len)
{
    SCEnter();
        ...
        StreamSlice stream_slice, void *local_data)
{
    ...
    if (buffer->len) {
        ...
        processed = DNP3HandleRequestLinkLayer(
                dnp3, buffer->buffer + buffer->offset, buffer->len - buffer->offset);
        if (processed < 0) {
            goto error;
        }
 ...
 *     like look DNP3.
 */
static int DNP3HandleResponseLinkLayer(DNP3State *dnp3, const uint8_t *input, uint32_t input_len)
{
    SCEnter();
    uint32_t processed = 0;
...
void RegisterDNP3Parsers(void)
{
    ...
    if (AppLayerProtoDetectConfProtoDetectionEnabledDefault("tcp", proto_name, false)) {
        ...
        if (RunmodeIsUnittests()) {
            AppLayerProtoDetectPPRegister(IPPROTO_TCP, DNP3_DEFAULT_PORT, ALPROTO_DNP3, 0,
                    sizeof(DNP3LinkHeader), STREAM_TOSERVER, DNP3ProbingParser, DNP3ProbingParser);
        } else {
            if (!AppLayerProtoDetectPPParseConfPorts("tcp", IPPROTO_TCP, proto_name, ALPROTO_DNP3,
