                                     ...
                                     void *tx, uint64_t idx)
{
    ...
    if (htp_state->cfg->decomp_swf_enabled) {
        ...
        if (file_swf_type == FILE_IS_NOT_FLASH ||
            file_swf_type == FILE_FLASH_NO_COMPRESSION)
        {
            goto skip;
        }

        int r = FileDecompressFlashFile(&buffer, &buffer_len,
        ...
        if (r == FILE_FLASH_DECOMP_NOK) {
            SCLogError(SC_ERR_FLASH_DECOMPRESSION,
		       "Flash decompression is not working!");
        }
    }
skip:
                                      ...
                                      void *tx, uint64_t tx_id)
{
    ...
    if (htp_state->cfg->decomp_swf_enabled) {
        ...
        r = FileDecompressFlashFile(&buffer, &buffer_len,
                                    htp_state->cfg->decomp_swf_type,
                                    htp_state->cfg->decomp_swf_decompress_depth,
                                    htp_state->cfg->decomp_swf_compress_depth);
        if (r == FILE_FLASH_DECOMP_NOK) {
...
skip:
    ...
    r = DetectEngineContentInspection(de_ctx, det_ctx, s, s->sm_lists[DETECT_SM_LIST_FILEDATA],
                                      f,
                                      buffer,
                                      buffer_len,
                                      stream_start_offset,
