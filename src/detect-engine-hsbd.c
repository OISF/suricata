        ...
        const uint64_t idx, const uint8_t flags)
{
    ...
    uint32_t decompressed_buffer_len = 0;
    ...
    if (htp_state->cfg->swf_decompression_enabled) {
            ...
            swf_file_type == FILE_SWF_LZMA_COMPRESSION)
        {
            ...
            if (ret == 1) {
                ...
                buffer_len = decompressed_buffer_len;
                /* the decompressed buffer is stored into the detect engine
                 * in order to inspect it later without decompressing it twice
                 */
                det_ctx->decompressed_hsbd_buffer = decompressed_buffer;
                det_ctx->decompressed_hsbd_buffer_len = decompressed_buffer_len;
            }
        }
    }
        ...
        Flow *f, uint8_t flags, void *alstate, void *tx, uint64_t tx_id)
{
    ...
    if (det_ctx->decompressed_hsbd_buffer) {
        buffer = det_ctx->decompressed_hsbd_buffer;
        buffer_len = det_ctx->decompressed_hsbd_buffer_len;
    } else {
        buffer = DetectEngineHSBDGetBufferForTX(tx, tx_id,
                                                de_ctx, det_ctx,
                                                f, htp_state,
                                                flags,
                                                &buffer_len,
...
void DetectEngineCleanHSBDBuffers(DetectEngineThreadCtx *det_ctx)
{
    ...
    if (det_ctx->decompressed_hsbd_buffer) {
        SCFree(det_ctx->decompressed_hsbd_buffer);
        det_ctx->decompressed_hsbd_buffer = NULL;
        det_ctx->decompressed_hsbd_buffer_len = 0;
    }
