...
static int RunTest (struct TestSteps *steps, const char *sig[], const char *yaml)
{
    ...
    for (i = 0; sig[i] != NULL; i++) {
        SCLogDebug("sig %s", sig[i]);
        DetectEngineAppendSig(de_ctx, (char *)sig[i]);
    }
...
";
    ...
    struct TestSteps steps[] = {
        {   (const uint8_t *)"GET /index.html HTTP/1.0\r\n"
            ...
            "User-Agent: Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.1.7) Gecko/20091221 Firefox/3.5.7\r\n"
            "\r\n",
            0, STREAM_TOSERVER, 1, 0 },
        {   (const uint8_t *)"HTTP/1.0 200 ok\r\n"
            "Content-Type: text/html\r\n"
            "Content-Length: 6\r\n"
            "\r\n"
            "ab",
            0, STREAM_TOCLIENT, 1, 1 },
        {   (const uint8_t *)"cd",
            0, STREAM_TOCLIENT, 2, 1 },
        {   (const uint8_t *)"ef",
            0, STREAM_TOCLIENT, 3, 1 },
        {   NULL, 0, 0, 0, 0},
    };
