...
void DetectMQTTConnackSessionPresentFree(void *);
...
typedef struct DetectMQTTConnackSessionPresentData_ {
...
void DetectMQTTConnackSessionPresentRegister (void)
{
    ...
    sigmatch_table[DETECT_AL_MQTT_CONNACK_SESSION_PRESENT].url = DOC_URL DOC_VERSION "/rules/mqtt-keywords.html#mqtt-connack-session-present";
...
static int DetectMQTTConnackSessionPresentMatch(DetectEngineThreadCtx *det_ctx,
                               ...
                               const SigMatchCtx *ctx)
{
    const DetectMQTTConnackSessionPresentData *de = (const DetectMQTTConnackSessionPresentData *)ctx;
    bool value = false;
 ...
 */
static DetectMQTTConnackSessionPresentData *DetectMQTTConnackSessionPresentParse(const char *rawstr)
{
    DetectMQTTConnackSessionPresentData *de = NULL;
#define MAX_SUBSTRINGS 30
    ...
    if (ret < 1) {
        SCLogError(SC_ERR_PCRE_MATCH, "invalid session_present flag definition: %s", rawstr);
        return NULL;
    }

    de = SCMalloc(sizeof(DetectMQTTConnackSessionPresentData));
    if (unlikely(de == NULL))
        return NULL;
    de->session_present = false;

    if (strcmp(rawstr, "yes") == 0) {
        de->session_present = true;
 ...
 */
void DetectMQTTConnackSessionPresentFree(void *de_ptr)
{
    DetectMQTTConnackSessionPresentData *de = (DetectMQTTConnackSessionPresentData *)de_ptr;
    if(de) SCFree(de);
}
...
static int MQTTConnackSessionPresentTestParse01 (void)
{
    ...
    DetectMQTTConnackSessionPresentFree(de);
