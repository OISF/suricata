...
static int DetectEnipDeviceTypeSetup(DetectEngineCtx *de_ctx, Signature *s, const char *rulestr)
{
    ...
    if (SigMatchAppendSMToList(
                de_ctx, s, DETECT_ENIP_DEVICETYPE, (SigMatchCtx *)du16, g_enip_device_type_id) == NULL) {
...
/**
 * \brief This function is used to match enip device_type type rule option on a transaction with those
 * passed via enip_device_type:
 *
 * \retval 0 no match
 * \retval 1 match
 */
...
{
    uint16_t value;
    if (!rs_enip_get_device_type(txv, &value))
        SCReturnInt(0);
...
void DetectEnipDeviceTypeRegister(void)
{
    ...
    DetectAppLayerInspectEngineRegister2("enip.device_type", ALPROTO_ENIP, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.device_type", ALPROTO_ENIP, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectGenericList, NULL);
