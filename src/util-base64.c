...
static inline bool AmpleDestinationSize(
        uint32_t dest_size, uint32_t total_decoded, uint32_t cur_decoded)
{
    if (dest_size < total_decoded + cur_decoded) {
        return false;
    }
    return true;
}
        ...
        uint32_t len, uint32_t *consumed_bytes, uint32_t *decoded_bytes)
{
    ...
    for (uint32_t i = 0; i < len; i++) {
        ...
        if (bbidx == B64_BLOCK) {
            ...
            if (!AmpleDestinationSize(dest_size, *decoded_bytes, numDecoded_blk)) {
