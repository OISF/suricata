...
{
    if (DetectBufferSetActiveList(de_ctx, s, g_ftp_ccode_buffer_id) < 0)
        return -1;
...
static bool DetectFTPCompletionCodeGetData(void *txv, uint8_t _flow_flags, uint32_t index,
...
{
    return DetectHelperGetMultiData(det_ctx, transforms, _f, _flow_flags, txv, list_id, index,
            DetectFTPCompletionCodeGetData);
}
