...
void DetectRegisterAppLayerHookLists(void)
{
    for (AppProto a = ALPROTO_FAILED + 1; a < g_alproto_max; a++) {
        ...
        if (strcmp(alproto_name, "http") == 0)
            alproto_name = "http1";
        SCLogNotice("alproto %u/%s", a, alproto_name);
        ...
        SCLogNotice("- hook %s:%s list %s (%u)", alproto_name, "request_name", ts_tx_complete,
        ...
        DetectAppLayerInspectEngineRegister(tc_tx_complete, a, SIG_FLAG_TOCLIENT, max_progress_tc,
                DetectEngineInspectGenericList, NULL);
        SCLogNotice("- hook %s:%s list %s (%u)", alproto_name, "response_name", tc_tx_complete,
        ...
        for (int p = 0; p <= max_progress_ts; p++) {
            ...
            if (name != NULL) {
                ...
                SCLogNotice("- hook %s:%s list %s (%u)", alproto_name, name, list_name,
