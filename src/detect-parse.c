 ...
 */
static inline int SigParseToken(const char **input, char *output,
    const size_t output_size)
{
    ...
    while (len && isblank(*token)) {
        token++;
        len--;
    }

    char *endptr = strpbrk(token, " \t\n\r");
    if (endptr != NULL) {
        token_len = endptr - token;
    } else {
        token_len = len;
    }
    strlcpy(output, token, token_len + 1);
    *input += token_len + 1;
 ...
 * \param output_size length of output buffer.
 */
static inline int SigParseList(const char **input, char *output,
    const size_t output_size)
{
    int in_list = 0;
        ...
        Signature *s, const char *sigstr, SignatureParser *parser, uint8_t addrs_direction)
{
    ...
    size_t optlen = strlen(index);
    for (size_t i = optlen; i > 0; i--) {
    ...
    strlcpy(parser->opts, index, optlen + 1);
