        ...
        FirewallPolicyChainAdd(FirewallPolicyChain *chain, const char *fmt, ...)
{
    ...
    if (strlcpy(chain->path[chain->len++], path, FW_POLICY_PATH_MAX) >= FW_POLICY_PATH_MAX) {
        FatalError("%s: firewall policy config path too long", path);
    }
}
...
static int ResolveFirewallPolicy(
        struct DetectFirewallPolicy *out, const FirewallPolicyChain *chain)
{
    for (uint8_t i = 0; i < chain->len; i++) {
        ...
        struct DetectFirewallPolicies *fw_policies)
{
    ...
    /* A state can lack both a parser state name and a generic alias, e.g. pgsql's
     * intermediate state. Such a hook cannot be addressed directly in config, but
     * it still inherits the default-policy tiers below, and it still needs an
     * entry so the policy table stays fully populated for every hook. */
    ...
    int r = ResolveFirewallPolicy(&app_pol->policy, &chain);
    if (r < 0) {
        SCFree(app_pol);
        return -1;
    }
