        ...
        Signature *s, const char *addrstr, char flag)
{
    ...
    struct DetectAddresses a = DetectParseAddress(de_ctx, addrstr);
    if (a.ipv4.head == NULL && a.ipv6.head == NULL) // TODO helper?
        return -1;
...
static bool SigHasSameSourceAndDestination(const Signature *s)
{
    ...
    if (!(s->flags & SIG_FLAG_SRC_ANY) || !(s->flags & SIG_FLAG_DST_ANY)) {
        if (!(DetectAddressesCompare(&s->ip_src, &s->ip_dst)))
            return false;
    }
...
static int SigParseBidirWithSameSrcAndDest02(void)
{
    ...
    // overlaps, these are the same because 1.2.3.4/1.2.3.5 are no-ops
    s = DetectEngineAppendSig(
            de_ctx, "alert tcp [1.2.3.0/24, 1.2.3.5] any <> [1.2.3.0/24, 1.2.3.4] any (sid:6;)");
    FAIL_IF_NULL(s);
    FAIL_IF_NOT(CountSigsWithSid(de_ctx, 6) == 1);
    FAIL_IF(s->init_data->init_flags & SIG_FLAG_INIT_BIDIREC);
