...
static void TransformDomain(InspectionBuffer *buffer)
{
    ...
    /* dot not found or less than 2 characters follow dot */
    unsigned int remain = input + input_len - dot - 1;
    if (!dot || remain < 2) {
        return;
    }
    ...
    uint32_t output_size = end - begin;
    if (output_size) {
        memcpy(os, begin, output_size);
        SCLogDebug("2nd level Domain is %.*s", output_size, os);
...
{
    const uint8_t *input = (const uint8_t *)"suricon.na";
    uint32_t input_len = strlen((char *)input);

    const char *result = "suricon";
    uint32_t result_len = strlen((char *)result);
