...
static int g_l2tp_ports[L2TP_MAX_PORTS] = { L2TP_DEFAULT_PORT, L2TP_UNSET_PORT,
    L2TP_UNSET_PORT, L2TP_UNSET_PORT };
...
int DecodeL2TP(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p, const uint8_t *pkt, uint32_t len)
{
    ...
    if (p->proto == IPPROTO_UDP) {
        ...
        if (l2tp_hdr->version == 1 || l2tp_hdr->version == 2) {
            ...
            SCLogDebug("L2TP Invalid. Type: %u Version: %u Reserved: %u", l2tp_hdr->type, l2tp_hdr->version, l2tp_hdr->reserved);
            ENGINE_SET_INVALID_EVENT(p, L2TP_INVALID_VER);
            return TM_ECODE_FAILED;
        }
        len -= sizeof(L2TPoverUDPDataHdr);
        pkt += sizeof(L2TPoverUDPDataHdr);
    } else if (p->proto != IPPROTO_L2TP) {
    ...
    */
    ...
    Packet *tp = PacketTunnelPktSetup(
                tv, dtv, p, pkt, len, DECODE_TUNNEL_ETHERNET);
    if (tp != NULL) {
...
static int DecodeL2TPTest01(void)
{
    ...
    0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x08, 0x00, /* Ethernet Header */
    0x45, 0x00, 0x00, 0x38, 0x00, 0x01, 0x00, 0x00, 0x40, 0x11, /* Inner IPv4 Header */
    0xf9, 0x60, 0xc0, 0xa8, 0x00, 0x01, 0xc0, 0xa8, 0x00, 0x02,
    0x04, 0xd2, 0x00, 0x35, 0x00, 0x24, 0x4b, 0xa3, /* Inner UDP Header */
...
static int DecodeL2TPTest05(void)
{
    ...
    0x00, 0x01, 0x00, 0x00, 0x40, 0x73, 0xf8, 0xcc,
    0xc0, 0xa8, 0x00, 0x01, 0xc0, 0xa8, 0x00, 0x02,
    0x11, 0x22, 0x33, 0x44, /* Session ID - 4 Bytes */
    0xca, 0xfe, 0xba, 0xbe, 0xfa, 0xce, 0xbe, 0xef, /* L2TPv3 Cookie - 8 Bytes */
    0x00, 0x00, 0x00, 0x00, /* L2TPv3 L2 Generic Sublayer - 4 Bytes */
    0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x08, 0x00, /* Ethernet Header */
    0x45, 0x00, 0x00, 0x38, 0x00, 0x01, 0x00, 0x00, 0x40, 0x11, /* Inner IPv4 Header */
