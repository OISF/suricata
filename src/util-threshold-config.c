...
#define MAX_SUBSTRINGS 30
    ...
    switch(rule_type) {
        ...
        case THRESHOLD_TYPE_RATE:
            if (strlen(rule_extend) > 0) {
                ...
                if (ByteParseStringUint32(&parsed_timeout, 10, strlen(th_timeout), th_timeout) <= 0) {
                    goto error;
                }
    ...
    switch (rule_type) {
        ...
        case THRESHOLD_TYPE_RATE:
            ...
            if (ByteParseStringUint32(&parsed_count, 10, strlen(th_count), th_count) <= 0) {
                goto error;
            }
            if (parsed_count == 0) {
            ...
            if (ByteParseStringUint32(&parsed_seconds, 10, strlen(th_seconds), th_seconds) <= 0) {
                goto error;
            }
    ...
    if (ByteParseStringUint32(&id, 10, strlen(th_sid), th_sid) <= 0) {
        goto error;
    }

    if (ByteParseStringUint32(&gid, 10, strlen(th_gid), th_gid) <= 0) {
        goto error;
    }
