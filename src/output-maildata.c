...
typedef struct OutputMaildataLogger_ {
    MaildataLogger LogFunc;
    OutputCtx *output_ctx;
    struct OutputMaildataLogger_ *next;
    const char *name;
    TmmId module_id;
} OutputMaildataLogger;

static OutputMaildataLogger *list = NULL;
...
int OutputRegisterMaildataLogger(const char *name, MaildataLogger LogFunc, OutputCtx *output_ctx)
{
    int module_id = TmModuleGetIdByName(name);
    if (module_id < 0)
        return -1;

    OutputMaildataLogger *op = SCMalloc(sizeof(*op));
    if (op == NULL)
        return -1;
    ...
    if (list == NULL)
        ...
        OutputMaildataLogger *t = list;
        while (t->next)
            t = t->next;
        t->next = op;
    }

    SCLogDebug("OutputRegisterMaildataLogger happy");
    return 0;
}

SC_ATOMIC_DECLARE(unsigned int, mail_id);
...
{
    OutputMaildataLogger *logger = list;
...
static TmEcode OutputMaildataLog(ThreadVars *tv, Packet *p, void *thread_data, PacketQueue *pq, PacketQueue *postpq)
{
    ...
    if (ffc != NULL) {
        ...
        for (ff = ffc->head; ff != NULL; ff = ff->next) {
            ...
            if (FileSize(ff) == ff->content_stored &&
                ...
                CallLoggers(tv, store, p, ff, NULL, 0, OUTPUT_MAILDATA_FLAG_CLOSE);
            ...
            if (ff->file_id == 0) {
                /* new file */
                ff->file_id = SC_ATOMIC_ADD(mail_id, 1);
                flags |= OUTPUT_MAILDATA_FLAG_OPEN;
            } else {
                /* existing file */
            }
            ...
            if (ff->state >= FILE_STATE_CLOSED)
                flags |= OUTPUT_MAILDATA_FLAG_CLOSE;
 ...
 */
static void LogMaildataLogLoadWaldo(const char *path)
{
    char line[16] = "";
    unsigned int id = 0;
    ...
    if (fgets(line, (int)sizeof(line), fp) != NULL) {
        if (sscanf(line, "%10u", &id) == 1) {
            SCLogInfo("id %u", id);
            (void) SC_ATOMIC_CAS(&mail_id, 0, id);
        }
    }
    fclose(fp);
}
 ...
 *  \param path full path for the waldo file
 */
static void LogMaildataLogStoreWaldo(const char *path)
{
    char line[16] = "";

    if (SC_ATOMIC_GET(mail_id) == 0) {
        SCReturn;
    }
 ...
 *  loggers */
static TmEcode OutputMaildataLogThreadInit(ThreadVars *tv, void *initdata, void **data)
{
    ...
    *data = (void *)td;
    ...
    SCLogDebug("OutputMaildataLogThreadInit happy (*data %p)", *data);
...
/* todo */      BUG_ON(ts == NULL);
    ...
    if (g_waldo_init == 0) {
        ConfNode *node = ConfGetNode("mail-store-waldo");
        if (node == NULL) {
            ...
            if (outputs) {
                ...
                TAILQ_FOREACH(output, &outputs->head, next) {
                    ...
                    if (!(strcmp(output->val, "mail") == 0 || strcmp(output->val, "mail-store") == 0))
                        continue;
                    ...
                    if (file == NULL) {
                        ...
                        file = ConfNodeLookupChild(outputs, "mail");
                        if (file == NULL)
        ...
        if (node != NULL) {
            ...
            if (waldo != NULL && strlen(waldo) > 0) {
                ...
                LogMaildataLogLoadWaldo(g_waldo);
            }
        }
        g_waldo_init = 1;
    }
...
static TmEcode OutputMaildataLogThreadDeinit(ThreadVars *tv, void *thread_data)
{
    OutputLoggerThreadData *op_thread_data = (OutputLoggerThreadData *)thread_data;
    OutputLoggerThreadStore *store = op_thread_data->store;
    OutputMaildataLogger *logger = list;
    ...
    if (g_waldo_deinit == 0) {
        if (strlen(g_waldo) > 0) {
            ...
            LogMaildataLogStoreWaldo(g_waldo);
        }
        g_waldo_deinit = 1;
    }
    SCMutexUnlock(&g_waldo_mutex);
...
static void OutputMaildataLogExitPrintStats(ThreadVars *tv, void *thread_data)
{
    OutputLoggerThreadData *op_thread_data = (OutputLoggerThreadData *)thread_data;
    OutputLoggerThreadStore *store = op_thread_data->store;
    OutputMaildataLogger *logger = list;
...
{
    tmm_modules[TMM_MAILDATALOGGER].name = "__maildata_logger__";
    tmm_modules[TMM_MAILDATALOGGER].ThreadInit = OutputMaildataLogThreadInit;
    tmm_modules[TMM_MAILDATALOGGER].Func = OutputMaildataLog;
    tmm_modules[TMM_MAILDATALOGGER].ThreadExitPrintStats = OutputMaildataLogExitPrintStats;
    tmm_modules[TMM_MAILDATALOGGER].ThreadDeinit = OutputMaildataLogThreadDeinit;
    tmm_modules[TMM_MAILDATALOGGER].cap_flags = 0;

    SC_ATOMIC_INIT(mail_id);
}
...
void OutputMaildataShutdown(void)
{
    OutputMaildataLogger *logger = list;
    while (logger) {
        OutputMaildataLogger *next_logger = logger->next;
        SCFree(logger);
        logger = next_logger;
    }
