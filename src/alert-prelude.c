...
static int PacketToData(const Packet *p, const PacketAlert *pa, idmef_alert_t *alert, AlertPreludeCtx *ctx)
{
    ...
    if (p->flow != NULL) {
        ...
        if (proto == ALPROTO_HTTP) {
            ...
            if (js) {
                json_object_foreach(s_js,key_js, value_js) {
                    sprintf(local_key,"http_%s", key_js);
            ...
            if (ssh_state) {
                ...
                if (js != NULL) {
                    ...
                    json_object_foreach(js,key_js, value_js) {
                        sprintf(local_key,"tls_%s", key_js);
                        JsonToAdditionnalData(local_key, value_js, alert);
                    }
                    json_decref(js);
                }
        ...
        if (proto == ALPROTO_SSH) {
            ...
            if (ssh_state) {
                ...
                if (js != NULL) {
                    ...
                    if (s_js != NULL ) {
                        json_object_foreach(s_js,key_js, value_js) {
                            sprintf(local_key,"ssh_server_%s", key_js);
                            JsonToAdditionnalData(local_key, value_js, alert);
                        }
                    }
                    ...
                    if (s_js != NULL ) {
                        json_object_foreach(s_js,key_js, value_js) {
                            sprintf(local_key,"ssh_client_%s", key_js);
        ...
        if (proto == ALPROTO_SMTP) {
            ...
            if (js) {
                json_object_foreach(js,key_js, value_js) {
                    sprintf(local_key,"smtp_%s", key_js);
                    JsonToAdditionnalData(local_key, value_js, alert);
                }
            }
            json_decref(js);
            ...
            if (js) {
                json_object_foreach(js,key_js, value_js) {
                    sprintf(local_key,"email_%s", key_js);
                    JsonToAdditionnalData(local_key, value_js, alert);
                }
            }
            json_decref(js);
