        ...
        const enum DetectContentInspectionType inspection_mode)
{
    ...
    // Allow ABSENT with a NULL buffer so it can react to the transform-error
    // flag (or a truly-absent buffer). All other keywords dereference the
    // buffer, so short-circuit them here to avoid a NULL deref.
    ...
    if (smd->type == DETECT_CONTENT) {
        ...
        switch (dad->mode) {
            ...
            case DETECT_ABSENT_MUST_SUCCEED:
                if (flags & DETECT_CI_FLAGS_ERROR) {
                    SCLogDebug("absent: must_succeed: error flag, no match");
                    goto no_match;
                }
                goto match;
            default:
