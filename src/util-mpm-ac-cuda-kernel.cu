                                 ...
                                 unsigned char *tolower)
{
    ...
    unsigned int (*state_table_u32)[256] = (unsigned int (*)[256])*((unsigned long *)(d_buffer + (o_buffer[tid] - d_buffer_start_offset) + 8));
    ...
    for (unsigned int i = 0; i < buflen; i++) {
        state = state_table_u32[state & 0x00FFFFFF][tolower[buf[i]]];
        if (state & 0xFF000000) {
            results[matches++] = i;
                                 ...
                                 unsigned char *tolower)
{
    ...
    unsigned int (*state_table_u32)[256] = (unsigned int (*)[256])*((unsigned int *)(d_buffer + (o_buffer[tid] - d_buffer_start_offset) + 4));
