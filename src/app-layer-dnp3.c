...
/**
 * \brief Initialize the group/vartion object size table.
 */
static void DNP3InitGroupVarSizeMap(void)
{
    ...
    /* G1V1. BSTRn */
    GROUP_VAR_SIZE_MAP[1][1] = 1;

    /* G1V2. BSTR8 */
    GROUP_VAR_SIZE_MAP[1][2] = 8;

    /* G2V1. BSTR8. */
    GROUP_VAR_SIZE_MAP[2][1] = 8;

    /* G2V2. Flag + DNP3TIME. */
    GROUP_VAR_SIZE_MAP[2][2] = 8 + (DNP3_OBJ_TIME_SIZE * 8);

    /* G2V3. BSTR8 + UINT16. */
    GROUP_VAR_SIZE_MAP[2][3] = 8 + 16;

    /* G3V1. UINT2. */
    GROUP_VAR_SIZE_MAP[3][1] = 2;

    /* G3V2. BSTR6 + UINT2. */
    GROUP_VAR_SIZE_MAP[3][2] = 8;

    /* G10V1. BSTRn. */
    GROUP_VAR_SIZE_MAP[10][1] = 1;

    /* G10V2. BSTR8. */
    GROUP_VAR_SIZE_MAP[10][2] = 8;

    /* G12V1. UINT4+BSTR1+BSTR1+UINT2+UINT8+UINT32+UINT32+UINT7+BSTR1. */
    GROUP_VAR_SIZE_MAP[12][1] = 4 + 1 + 1 + 2 + 8 + 32 + 32 + 7 + 1;

    /* G20V1. BSTR8 + UINT32. */
    GROUP_VAR_SIZE_MAP[20][1] = 8 + 32;

    /* G21V1. BSTR8 + UINT32. */
    GROUP_VAR_SIZE_MAP[21][1] = 8 + 32;

    /* G30V1. BSTR8 + INT32. */
    GROUP_VAR_SIZE_MAP[30][1] = 8 + 32;

    /* G30V2. BSTR8 + INT16. */
    GROUP_VAR_SIZE_MAP[30][2] = 8 + 16;

    /* G30V3. INT32. */
    GROUP_VAR_SIZE_MAP[30][3] = 32;

    /* G30V4. INT16. */
    GROUP_VAR_SIZE_MAP[30][4] = -16;

    /* G30V5. BSTR8 + FLT32. */
    GROUP_VAR_SIZE_MAP[30][5] = 8 + 32;

    /* G30V6. BSTR8 + FLT64. */
    GROUP_VAR_SIZE_MAP[30][6] = 8 + 64;

    /* BSTR8 + INT32. */
    GROUP_VAR_SIZE_MAP[40][1] = 8 + 32;

    /* DNP3TIME */
    GROUP_VAR_SIZE_MAP[50][1] = DNP3_OBJ_TIME_SIZE * 8;

    /* DNP3TIME + UINT32. */
    GROUP_VAR_SIZE_MAP[50][2] = (DNP3_OBJ_TIME_SIZE * 8) + 32;

    /* DNP3TIME. */
    GROUP_VAR_SIZE_MAP[50][3] = DNP3_OBJ_TIME_SIZE * 8;

    /* DNP3TIME + UINT32 + UINT8. */
    GROUP_VAR_SIZE_MAP[50][4] = (DNP3_OBJ_TIME_SIZE * 8) + 32 + 8;

    /* All known group 60 objects have size 0. */
 ...
 */
static void DNP3DecodeApplicationObjects(uint8_t *buf, uint32_t len,
    DNP3ObjectList *objects)
{
    ...
    while (len) {
        ...
        switch (range_code) {
            ...
            default:
                SCLogInfo("Unhandled range code: 0x%02x", range_code);
                abort();
                break;
        }
    ...
    unknown:
        SCLogDebug("Unknown variation 0x%02x (%d) for group 0x%02x (%d)",
    ...
    uint32_t input_len)
{
    ...
    tx->request_buffer = DNP3ReassembleApplicationLayer(input + th_offset,
        input_len - th_offset, &tx->request_buffer,
        &tx->request_buffer_len);
    tx->request_done = 1;

    DNP3DecodeApplicationObjects(
    ...
    uint32_t input_len)
{
    ...
    if (tx->response_done) {
        ...
        DNP3DecodeApplicationObjects(tx->response_buffer + offset,
