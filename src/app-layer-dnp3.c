    ...
    uint32_t len, DNP3ObjectList *objects)
{
    ...
    while (len) {
        ...
        int prefix_len = DNP3_OBJ_PREFIX_CODE_MAP[object->prefix_code];
        if (prefix_len < 0) {
            SCLogNotice("Unhandled prefix code: %d", object->prefix_code);
            goto done;
        }
        ...
        switch (object->range_code) {
            ...
            case 0x02: {
                ...
                object->start = DNP3_SWAP32((uint32_t)(*buf));
                buf = buf + sizeof(uint32_t);
                object->stop = DNP3_SWAP32((uint32_t)(*buf));
                object->count = object->stop - object->start + 1;
                break;
            }
            case 0x06:
            ...
            case 0x08: {
                ...
                object->count = *(uint16_t *)(buf + offset);
                object->stop = object->count - 1;
                offset += sizeof(uint16_t);
                break;
            }
            case 0x03:
            ...
            case 0x09:
            case 0x0b:
                SCLogDebug("Range code 0x%02x not yet implemented.",
                    object->range_code);
                goto done;
            default:
