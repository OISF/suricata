    0x7926, 0xfabc, 0xcce2, 0x9600, 0xa05e, 0x6e26, 0x5878, 0x029a, 0x34c4, 0xb75e, 0x8100, 0xdbe2,
...
static int DNP3CheckCRC(const uint8_t *block, uint32_t len)
{
    ...
    if (((crc & 0xff) == block[crc_offset]) && ((crc >> 8) == block[crc_offset + 1])) {
        return 1;
    }
...
static int DNP3ReassembleApplicationLayer(
        const uint8_t *input, uint32_t input_len, uint8_t **output, uint32_t *output_len)
{
    ...
    while ((uint32_t)offset < input_len) {
        ...
        memcpy(*output + *output_len, input + offset, block_size - DNP3_CRC_LEN);
...
static int DNP3HasUserData(const DNP3LinkHeader *header, uint8_t direction)
{
    if (direction == STREAM_TOSERVER) {
        return header->len >=
               DNP3_LINK_HDR_LEN + sizeof(DNP3TransportHeader) + sizeof(DNP3ApplicationHeader);
    } else {
...
static void DNP3BufferTrim(DNP3Buffer *buffer)
{
    if (buffer->offset == buffer->len) {
        ...
        memmove(buffer->buffer, buffer->buffer + buffer->offset, buffer->len - buffer->offset);
...
static void DNP3ObjectFree(DNP3Object *object)
{
    if (object->points != NULL) {
        DNP3FreeObjectPointList(object->group, object->variation, object->points);
    }
    SCFree(object);
}
...
static int DNP3DecodeApplicationObjects(
        DNP3Transaction *tx, const uint8_t *buf, uint32_t len, DNP3ObjectList *objects)
{
    int retval = 0;
    ...
    while (len) {
        ...
        switch (object->range_code) {
            ...
            default:
                SCLogDebug("Range code 0x%02x is reserved.", object->range_code);
                goto done;
        }
        ...
        int event = DNP3DecodeObject(header->group, header->variation, &buf, &len,
                object->prefix_code, object->start, object->count, object->points);
        if (event) {
        ...
        Flow *f, DNP3State *dnp3, const uint8_t *input, uint32_t input_len)
{
    ...
    if (!DNP3CheckUserDataCRCs(
                input + sizeof(DNP3LinkHeader), input_len - sizeof(DNP3LinkHeader))) {
        return;
    }
        ...
        Flow *f, DNP3State *dnp3, const uint8_t *input, uint32_t input_len)
{
    ...
    while (input_len) {
        ...
        if (!DNP3CheckUserDataCRCs(
                    input + sizeof(DNP3LinkHeader), frame_len - sizeof(DNP3LinkHeader))) {
...
static int DNP3GetAlstateProgress(void *tx, uint8_t direction)
{
    ...
    if (dnp3->flooded && (dnp3->transaction_max - dnp3tx->tx_num >= DNP3_DEFAULT_REQ_FLOOD_COUNT)) {
...
void RegisterDNP3Parsers(void)
{
    ...
    if (SCAppLayerParserConfParserEnabled("tcp", proto_name)) {
        ...
        AppLayerParserRegisterParser(IPPROTO_TCP, ALPROTO_DNP3, STREAM_TOSERVER, DNP3ParseRequest);
        AppLayerParserRegisterParser(IPPROTO_TCP, ALPROTO_DNP3, STREAM_TOCLIENT, DNP3ParseResponse);

        AppLayerParserRegisterStateFuncs(IPPROTO_TCP, ALPROTO_DNP3, DNP3StateAlloc, DNP3StateFree);
        ...
        AppLayerParserRegisterTxFreeFunc(IPPROTO_TCP, ALPROTO_DNP3, DNP3StateTxFree);

        AppLayerParserRegisterGetStateProgressFunc(
                IPPROTO_TCP, ALPROTO_DNP3, DNP3GetAlstateProgress);
        AppLayerParserRegisterStateProgressCompletionStatus(ALPROTO_DNP3, 1, 1);

        AppLayerParserRegisterGetEventInfo(IPPROTO_TCP, ALPROTO_DNP3, DNP3StateGetEventInfo);
        AppLayerParserRegisterGetEventInfoById(
                IPPROTO_TCP, ALPROTO_DNP3, DNP3StateGetEventInfoById);
        ...
        SCLogConfig("Parser disabled for protocol %s. "
                    "Protocol detection still on.",
                proto_name);
    }

#ifdef UNITTESTS
    AppLayerParserRegisterProtocolUnittests(IPPROTO_TCP, ALPROTO_DNP3, DNP3ParserRegisterTests);
#endif
...
{
    uint8_t request[] = { /* DNP3 start. */
        0x05, 0x64, 0x1a, 0xc4, 0x02, 0x00, 0x01, 0x00, 0xa5, 0xe9,

        /* Transport header. */
        0xff,

        /* Application layer - segment 1. */
        0xc9, 0x05, 0x0c, 0x01, 0x28, 0x01, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00,
        0x72, 0xef,
    ...
    FAIL_IF(!DNP3CheckCRC(request + sizeof(DNP3LinkHeader), DNP3_BLOCK_SIZE + DNP3_CRC_LEN));
...
static int DNP3CheckUserDataCRCsTest(void)
{
    ...
    uint8_t data_valid[] = {
        0xff, 0xc9, 0x05, 0x0c, 0x01, 0x28, 0x01, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00,
        0x00, 0x72, 0xef, /* CRC. */
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, /* CRC. */
    };
    ...
    uint8_t data_invalid[] = {
        ...
        0x00, 0x00, 0x00, 0x00, 0x01, /* Invalid byte. */
        0xff, 0xff,                   /* CRC. */
    };
    ...
    *(uint16_t *)(three_bytes_good_crc + 1) = DNP3ComputeCRC(three_bytes_good_crc, 1);
    FAIL_IF(!DNP3CheckUserDataCRCs(three_bytes_good_crc, sizeof(three_bytes_good_crc)));
...
static int DNP3ParserCheckLinkHeaderCRC(void)
{
    ...
    uint8_t request[] = { /* DNP3 start. */
        ...
        /* Application layer. */
        0xc9, 0x05, 0x0c, 0x01, 0x28, 0x01, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00,
        0x72, 0xef, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff
    };
...
static int DNP3ReassembleApplicationLayerTest01(void)
{
    ...
    /* Valid frame. */
    FAIL_IF(!DNP3ReassembleApplicationLayer(payload, sizeof(payload), &output, &reassembled_len));
    FAIL_IF(output == NULL);
    ...
    reassembled_len = 0;
    output = NULL;
    FAIL_IF(DNP3ReassembleApplicationLayer(payload, 1, &output, &reassembled_len));
    FAIL_IF(output != NULL);
    ...
    reassembled_len = 0;
    output = NULL;
    FAIL_IF(DNP3ReassembleApplicationLayer(payload, 2, &output, &reassembled_len));
    FAIL_IF(output != NULL);
    FAIL_IF(reassembled_len != 0);
    ...
    reassembled_len = 0;
    output = NULL;
    FAIL_IF(DNP3ReassembleApplicationLayer(payload, 3, &output, &reassembled_len));
    FAIL_IF(output != NULL);
    FAIL_IF(reassembled_len != 0);
    ...
    reassembled_len = 0;
    output = NULL;
    FAIL_IF(!DNP3ReassembleApplicationLayer(payload, 4, &output, &reassembled_len));
    FAIL_IF(output == NULL);
    ...
    reassembled_len = 0;
    FAIL_IF(DNP3ReassembleApplicationLayer(
            short_payload1, sizeof(short_payload1), &output, &reassembled_len));
    ...
    reassembled_len = 0;
    FAIL_IF(DNP3ReassembleApplicationLayer(
            short_payload2, sizeof(short_payload2), &output, &reassembled_len));
    SCFree(output);
    output = NULL;
...
{
    uint8_t pkt[] = { 0x05, 0x64, 0x05, 0xc9, 0x03, 0x00, 0x04, 0x00, 0xbd, 0x71 };
    uint8_t rdir = 0;
    ...
    FAIL_IF(DNP3ProbingParser(NULL, STREAM_TOSERVER, pkt, sizeof(DNP3LinkHeader) - 1, &rdir) !=
            ALPROTO_UNKNOWN);
...
static int DNP3ParserTestRequestResponse(void)
{
    ...
    uint8_t response[] = { /* DNP3 start. */
        0x05, 0x64, 0x1c, 0x44, 0x01, 0x00, 0x02, 0x00, 0xe2, 0x59,

        /* Transport header. */
        0xc3,

        /* Application layer. */
        0xc9, 0x81, 0x00, 0x00, 0x0c, 0x01, 0x28, 0x01, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00,
        0x7a, 0x65, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff
    };
    ...
    SCMutexLock(&flow.m);
    FAIL_IF(AppLayerParserParse(
            NULL, alp_tctx, &flow, ALPROTO_DNP3, STREAM_TOSERVER, request, sizeof(request)));
    ...
    SCMutexLock(&flow.m);
    FAIL_IF(AppLayerParserParse(
            NULL, alp_tctx, &flow, ALPROTO_DNP3, STREAM_TOCLIENT, response, sizeof(response)));
    SCMutexUnlock(&flow.m);
...
static int DNP3ParserTestUnsolicitedResponseConfirm(void)
{
    ...
    uint8_t response[] = { 0x05, 0x64, 0x16, 0x44, 0x01, 0x00, 0x02, 0x00, 0x89, 0xe5, 0xc4, 0xfa,
        0x82, 0x00, 0x00, 0x02, 0x02, 0x17, 0x01, 0x01, 0x81, 0xa7, 0x75, 0xd8, 0x32, 0x4c, 0x81,
        0x3e, 0x01, 0xa1, 0xc9 };

    /* Confirm. */
    uint8_t confirm[] = { 0x05, 0x64, 0x08, 0xc4, 0x02, 0x00, 0x01, 0x00, 0xd3, 0xb7, 0xc0, 0xda,
        0x00, 0x6a, 0x3d };
    ...
    SCMutexLock(&flow.m);
    FAIL_IF(AppLayerParserParse(
            NULL, alp_tctx, &flow, ALPROTO_DNP3, STREAM_TOSERVER, confirm, sizeof(confirm)));
    SCMutexUnlock(&flow.m);
...
static int DNP3ParserTestPartialFrame(void)
{
    ...
    uint8_t request_partial2[] = { /* Remainder of application layer. */
        0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x72, 0xef, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff,
        0xff
    };
    ...
    uint8_t response_partial2[] = { 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x7a, 0x65, 0x00,
    ...
    SCMutexLock(&flow.m);
    r = AppLayerParserParse(NULL, alp_tctx, &flow, ALPROTO_DNP3, STREAM_TOSERVER, request_partial1,
    ...
    FAIL_IF(memcmp(state->request_buffer.buffer, request_partial1, sizeof(request_partial1)));
    ...
    SCMutexLock(&flow.m);
    r = AppLayerParserParse(NULL, alp_tctx, &flow, ALPROTO_DNP3, STREAM_TOSERVER, request_partial2,
    ...
    SCMutexLock(&flow.m);
    r = AppLayerParserParse(NULL, alp_tctx, &flow, ALPROTO_DNP3, STREAM_TOCLIENT, response_partial1,
    ...
    SCMutexLock(&flow.m);
    r = AppLayerParserParse(NULL, alp_tctx, &flow, ALPROTO_DNP3, STREAM_TOCLIENT, response_partial2,
...
static int DNP3ParserTestMultiFrame(void)
{
    ...
    memcpy(combined + sizeof(unsol_response1), unsol_response2, sizeof(unsol_response2));
    ...
    r = AppLayerParserParse(
            NULL, alp_tctx, &flow, ALPROTO_DNP3, STREAM_TOCLIENT, combined, sizeof(combined));
    SCMutexUnlock(&flow.m);
    FAIL_IF(r != 0);
...
static int DNP3ParserTestParsePDU01(void)
{
    ...
    const uint8_t pkt[] = { 0x05, 0x64, 0x0b, 0xc4, 0x17, 0x00, 0xef, 0xff, 0xc4, 0x8f, 0xe1, 0xc8,
        0x01, 0x01, 0x00, 0x06, 0x77, 0x6e };
...
{
    const uint8_t pkt[] = { 0x05, 0x64, 0x63, 0xc4, 0x04, 0x00, 0x03, 0x00, 0xc7, 0xee, 0xc7, 0xc9,
        0x1b, 0x46, 0x03, 0x5b, 0x01, 0x55, 0x00, 0x1a, 0x00, 0x3b, 0x00, 0x00, 0x00, 0x00, 0x9e,
        0xc7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0xff, 0xff, 0x00, 0x1e, 0x00, 0x43, 0x3a, 0x2f, 0x74, 0x65, 0x6d, 0x70, 0x2f,
        0x44, 0x4e, 0x50, 0x44, 0x65, 0x67, 0x7d, 0x76, 0x69, 0x63, 0x65, 0x43, 0x6f, 0x6e, 0x66,
        0x69, 0x67, 0x75, 0x72, 0x61, 0x74, 0x69, 0x6f, 0x93, 0x0c, 0x6e, 0x20, 0x77, 0x72, 0x69,
        0x74, 0x74, 0x65, 0x6e, 0x20, 0x74, 0x6f, 0x20, 0x52, 0x65, 0x6d, 0x35, 0x20, 0x6f, 0x74,
        0x65, 0x20, 0x44, 0x65, 0x76, 0x69, 0x63, 0x65, 0x2e, 0x78, 0x6d, 0x6c, 0xc4, 0x8b };
    ...
    FAIL_IF_NOT(strcmp(data->filename,
...
static int DNP3ParserUnknownEventAlertTest(void)
{
    ...
    uint8_t pkt[] = { 0x05, 0x64, 0x63, 0xc4, 0x04, 0x00, 0x03, 0x00, 0xc7, 0xee,
        ...
        0x5b, 0x01, 0x55, 0x00, 0x1a, 0x00, 0x3b, 0x00, 0x00, 0x00, 0x00, 0x9e, 0xc7, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff,
        0xff, 0x00, 0x1e, 0x00, 0x43, 0x3a, 0x2f, 0x74, 0x65, 0x6d, 0x70, 0x2f, 0x44, 0x4e, 0x50,
        0x44, 0x65, 0x67, 0x7d, 0x76, 0x69, 0x63, 0x65, 0x43, 0x6f, 0x6e, 0x66, 0x69, 0x67, 0x75,
        0x72, 0x61, 0x74, 0x69, 0x6f, 0x93, 0x0c, 0x6e, 0x20, 0x77, 0x72, 0x69, 0x74, 0x74, 0x65,
        0x6e, 0x20, 0x74, 0x6f, 0x20, 0x52, 0x65, 0x6d, 0x35, 0x20, 0x6f, 0x74, 0x65, 0x20, 0x44,
        0x65, 0x76, 0x69, 0x63, 0x65, 0x2e, 0x78, 0x6d, 0x6c, 0xc4, 0x8b };
...
{
    uint8_t packet_bytes[] = { 0x05, 0x64, 0x08, 0xc4, 0x03, 0x00, 0x04, 0x00, 0xbf, 0xe9, 0xc1,
        0xc1, 0x82, 0xc5, 0xee };
    ...
    int r = AppLayerParserParse(NULL, alp_tctx, &flow, ALPROTO_DNP3, STREAM_TOCLIENT, packet_bytes,
            sizeof(packet_bytes));
...
void DNP3ParserRegisterTests(void)
{
    ...
    UtRegisterTest("DNP3ReassembleApplicationLayerTest01", DNP3ReassembleApplicationLayerTest01);
    UtRegisterTest("DNP3ProbingParserTest", DNP3ProbingParserTest);
    UtRegisterTest("DNP3ParserTestRequestResponse", DNP3ParserTestRequestResponse);
    UtRegisterTest(
            "DNP3ParserTestUnsolicitedResponseConfirm", DNP3ParserTestUnsolicitedResponseConfirm);
    ...
    UtRegisterTest("DNP3ParserUnknownEventAlertTest", DNP3ParserUnknownEventAlertTest);
