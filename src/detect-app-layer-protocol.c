...
static DetectAppLayerProtocolData *DetectAppLayerProtocolParse(const char *arg, bool negate)
{
    ...
    if (sep) {
        ...
        } else if (strcmp(sep + 1, "toserver") == 0) {
            mode = DETECT_ALPROTO_TOSERVER;
        } else if (strcmp(sep + 1, "toclient") == 0) {
...
PrefilterPacketAppProtoMatch(DetectEngineThreadCtx *det_ctx, Packet *p, const void *pectx)
{
    ...
    switch (ctx->v1.u8[3]) {
        ...
        case DETECT_ALPROTO_TOCLIENT:
            ...
            break;
        case DETECT_ALPROTO_EITHER:
            if (AppProtoEquals(ctx->v1.u16[0], f->alproto_tc) ^ ctx->v1.u8[2]) {
                PrefilterAddSids(&det_ctx->pmq, ctx->sigs_array, ctx->sigs_cnt);
            }
            if (AppProtoEquals(ctx->v1.u16[0], f->alproto_ts) ^ ctx->v1.u8[2]) {
                PrefilterAddSids(&det_ctx->pmq, ctx->sigs_array, ctx->sigs_cnt);
            }
            break;
    }
    ...
    if (alproto != ALPROTO_UNKNOWN) {
        if (AppProtoEquals(ctx->v1.u16[0], alproto) ^ ctx->v1.u8[2]) {
            PrefilterAddSids(&det_ctx->pmq, ctx->sigs_array, ctx->sigs_cnt);
        }
    }
}
