                                  ...
                                  const Packet *p)
{
    ...
    for (int i = 0; i < p->events.cnt; i++) {
        ...
        if (is_decode && !log_decode) continue;
        if (!is_decode && !log_stream) continue;
                        ...
                        bool is_pktlayer, const char *layer, uint64_t tx_id)
{
    ...
    for (int i = decoder_events->event_last_logged; i < decoder_events->cnt; i++) {
        ...
        if (r == 0) {
            json_object_set_new(ajs, "type", json_string("application-layer"));
        ...
        ConfNode *conf)
{
    ...
    if (conf != NULL) {
        ...
        if (typeconf != NULL) {
            SetFlag(typeconf, "application-layer", LOG_JSON_APPLAYER_TYPE, &flags);
