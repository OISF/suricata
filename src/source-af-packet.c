...
static inline void AFPFlushBlock(struct block_desc *pbd)
{
    pbd->h1.block_status = TP_STATUS_KERNEL;
}

static inline int AFPParsePacketV3(AFPThreadVars *ptv, struct block_desc *pbd, struct tpacket3_hdr *ppd)
{
...
static inline int AFPWalkBlock(AFPThreadVars *ptv, struct block_desc *pbd, const int block_num)
{
...
int AFPReadFromRingV3(AFPThreadVars *ptv)
{
    ...
    while (1) {
        ...
        if (unlikely(AFPWalkBlock(ptv, pbd, ptv->frame_offset) != AFP_READ_OK))
            break;
...
static int AFPComputeRingParamsV3(AFPThreadVars *ptv)
{
    ...
    ptv->req3.tp_frame_nr = ptv->ring_size;
