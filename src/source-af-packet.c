 ...
 *
 * Given a sockaddr_ll of a frame, use the pkttype_filter to decide if the
...
{
    uint32_t pkttype_bit = sll->sll_pkttype > 31 ? 0UL : BIT_U32(sll->sll_pkttype);
    return (ptv->pkttype_filter_mask & pkttype_bit) != 0;
}
