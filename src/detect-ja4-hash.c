...
int Ja4IsDisabled(const char *type)
{
    bool is_enabled = SSLJA4IsEnabled();
    if (is_enabled == 0) {
        if (strcmp(type, "rule") != 0) {
            SCLogWarning("JA4 is disabled, skipping %s", type);
        }
        return 1;
    }
...
static int DetectJa4HashSetup(DetectEngineCtx *de_ctx, Signature *s, const char *str)
{
    ...
    /* Check if JA4 is disabled */
    if (!RunmodeIsUnittests() && Ja4IsDisabled("rule")) {
        if (!SigMatchSilentErrorEnabled(de_ctx, DETECT_AL_JA4_HASH)) {
            SCLogError("JA4 support is not enabled");
        }
        return -2;
    }
    s->init_data->init_flags |= SIG_FLAG_INIT_JA4;
        ...
        const int list_id)
{
    ...
    if (buffer->inspect == NULL) {
        ...
        uint8_t data[SC_JA4_HEX_LEN];
        SCJA4GetHash(ssl_state->client_connp.ja4, (uint8_t(*)[SC_JA4_HEX_LEN])data);

        InspectionBufferSetup(det_ctx, list_id, buffer, data, 0);
        InspectionBufferCopy(buffer, data, SC_JA4_HEX_LEN);
        ...
        const int list_id)
{
    ...
    if (buffer->inspect == NULL) {
        ...
        InspectionBufferCopy(buffer, (uint8_t *)b, SC_JA4_HEX_LEN);
        InspectionBufferApplyTransforms(buffer, transforms);
    }
    return buffer;
}
