...
#define KEYWORD_NAME_INITIATOR "ike.spi_initiator"
#define KEYWORD_DOC_INITIATOR  "ike-keywords.html#ike-spi_initiator";
#define BUFFER_NAME_INITIATOR  "ike.spi_initiator"
#define BUFFER_DESC_INITIATOR  "ike spi initiator"

#define KEYWORD_NAME_RESPONDER "ike.spi_responder"
#define KEYWORD_DOC_RESPONDER  "ike-keywords.html#ike-spi_responder";
#define BUFFER_NAME_RESPONDER  "ike.spi_responder"
#define BUFFER_DESC_RESPONDER  "ike spi responder"
...
void DetectIkeSpiRegister(void)
{
    ...
    sigmatch_table[DETECT_AL_IKE_SPI_INITIATOR].url = "/rules/" KEYWORD_DOC_INITIATOR
    sigmatch_table[DETECT_AL_IKE_SPI_INITIATOR].desc = "sticky buffer to match on the IKE spi initiator";
    sigmatch_table[DETECT_AL_IKE_SPI_INITIATOR].Setup = DetectSpiInitiatorSetup;
    sigmatch_table[DETECT_AL_IKE_SPI_INITIATOR].flags |= SIGMATCH_NOOPT|SIGMATCH_INFO_STICKY_BUFFER;

    DetectAppLayerInspectEngineRegister2(BUFFER_NAME_INITIATOR, ALPROTO_IKE,
            SIG_FLAG_TOSERVER, 1,
            DetectEngineInspectBufferGeneric, GetInitiatorData);

    DetectAppLayerMpmRegister2(BUFFER_NAME_INITIATOR, SIG_FLAG_TOSERVER, 1,
            PrefilterGenericMpmRegister, GetInitiatorData, ALPROTO_IKE,
            1);
    ...
    sigmatch_table[DETECT_AL_IKE_SPI_RESPONDER].url = "/rules/" KEYWORD_DOC_RESPONDER
    sigmatch_table[DETECT_AL_IKE_SPI_RESPONDER].desc = "sticky buffer to match on the IKE spi responder";
    sigmatch_table[DETECT_AL_IKE_SPI_RESPONDER].Setup = DetectSpiResponderSetup;
    sigmatch_table[DETECT_AL_IKE_SPI_RESPONDER].flags |= SIGMATCH_NOOPT|SIGMATCH_INFO_STICKY_BUFFER;

    DetectAppLayerInspectEngineRegister2(BUFFER_NAME_RESPONDER, ALPROTO_IKE,
            SIG_FLAG_TOCLIENT, 1,
            DetectEngineInspectBufferGeneric, GetResponderData);

    DetectAppLayerMpmRegister2(BUFFER_NAME_RESPONDER, SIG_FLAG_TOCLIENT, 1,
            PrefilterGenericMpmRegister, GetResponderData, ALPROTO_IKE,
            1);
