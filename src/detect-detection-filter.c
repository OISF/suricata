...
/* test seam from detect-engine-threshold.c */
...
static int DetectDetectionFilterDistinctDstPortNoDoubleCount(void)
{
    ...
    Signature *s = DetectEngineAppendSig(de_ctx,
            ...
            "detection_filter: track by_dst, count 1, seconds 60, unique_on dst_port; sid:22;)");
    FAIL_IF_NULL(s);
...
static int DetectDetectionFilterDistinctSrcPortNoDoubleCount(void)
{
    ...
    Signature *s = DetectEngineAppendSig(de_ctx,
            ...
            "detection_filter: track by_src, count 1, seconds 60, unique_on src_port; sid:23;)");
    FAIL_IF_NULL(s);
