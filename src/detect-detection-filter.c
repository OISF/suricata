...
static int DetectDetectionFilterTestParse02 (void)
{
    ...
    FAIL_IF(df && (df->track == TRACK_DST || df->track == TRACK_SRC) && (df->count == 10) &&
            (df->seconds == 60));
    DetectDetectionFilterFree(NULL, df);
...
static int DetectDetectionFilterTestParse04 (void)
{
    ...
    FAIL_IF(df && (df->track == TRACK_DST) && (df->count == 10) && (df->seconds == 60));
...
static int DetectDetectionFilterTestParse06 (void)
{
    ...
    FAIL_IF_NOT_NULL(df);
    FAIL_IF(df->track == TRACK_DST);
    FAIL_IF(df->count == 10);
    FAIL_IF(df->seconds == 0);
    DetectDetectionFilterFree(NULL, df);
