...
static bool DetectByteListMismatch(
        int sm_list, int found_list, bool strict, const char *arg, const DetectEngineCtx *de_ctx)
{
    if (sm_list == -1 || sm_list == found_list)
        return false;
        ...
        DetectByteIndexType *index, const DetectEngineCtx *de_ctx)
{
    ...
    if (bed_sm != NULL) {
        if (DetectByteListMismatch(sm_list, found_list, strict, arg, de_ctx)) {
            return false;
        }
