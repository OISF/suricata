...
json_t *JsonMQTTAddMetadata(const Flow *f, uint64_t tx_id)
{
    ...
    if (state) {
        ...
        if (tx) {
            return rs_mqtt_logger_log(state, tx, MQTT_DEFAULTS);
        }
    }
    ...
    const Packet *p, Flow *f, void *state, void *tx, uint64_t tx_id)
{
    ...
    json_t *js = CreateJSONHeader(p, dir, "mqtt");
    if (unlikely(js == NULL)) {
        return TM_ECODE_FAILED;
    }

    json_t *mqtt_js = rs_mqtt_logger_log(state, tx, thread->mqttlog_ctx->flags);
    if (unlikely(mqtt_js == NULL)) {
        goto error;
    }
    json_object_set_new(js, "mqtt", mqtt_js);
