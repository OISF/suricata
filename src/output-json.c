        ...
        if (gv->type == DETECT_FLOWVAR || gv->type == DETECT_FLOWINT) {
            ...
            if (fv->datatype == FLOWVAR_TYPE_STR && fv->key == NULL) {
                ...
                if (varname) {
                     ...
                     */
                    char *tmp_var = SCStrdup(varname);
                    if (tmp_var == NULL) {
                        continue;
                    }
                    char *saveptr;
                    char *tok = strtok_r(tmp_var, ";", &saveptr);
                    if (tok == NULL) {
                        SCFree(tmp_var);
                        continue;
                    }
                    ...
                    while (tok) {
                        ...
                        if (colon) {
                            *colon = '\0';
                            ...
                            if (is_num) {
                                uint32_t value;
                                if (StringParseU32RangeCheck(&value, 10, 0, val, 0, UINT32_MAX) >
                                        0) {
                                    SCJbSetUint(js_entropyvals, key, value);
                                } else {
                                    SCLogDebug("Invalid value from entropy varname for key %s; "
                                               "value \"%s\"",
                                            key, val);
                                    break;
                                }
