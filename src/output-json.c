...
void EveFileInfo(JsonBuilder *jb, const File *ff, const bool stored)
{
    ...
    switch (ff->state) {
        case FILE_STATE_CLOSED:
            ...
            if (ff->flags & FILE_MD5) {
                char str[256];
                PrintHexString(str, sizeof(str), (uint8_t *)ff->md5, sizeof(ff->md5));
                jb_set_string(jb, "md5", str);
            }
            if (ff->flags & FILE_SHA1) {
                char str[256];
                PrintHexString(str, sizeof(str), (uint8_t *)ff->sha1, sizeof(ff->sha1));
    ...
    if (ff->flags & FILE_SHA256) {
        char str[256];
        PrintHexString(str, sizeof(str), (uint8_t *)ff->sha256, sizeof(ff->sha256));
