...
static inline int JSONFormatAndAdd(json_t *parent, const char *key,
...
static int JSONFlowAppendMACAddrs(uint8_t *val, int direction, void *data) {
    ...
    if (direction == TOSERVER) {
        JSONFormatAndAdd(info->dst, NULL, val, true);
    } else {
        JSONFormatAndAdd(info->src, NULL, val, true);
    }
    return 0;
}
...
int CreateJSONEther(json_t *parent, const Packet *p, const Flow *f) {
    ...
    if (p == NULL) {
        ...
        JSONFormatAndAdd(ejs, "src_mac", src, false);
        JSONFormatAndAdd(ejs, "dst_mac", dst, false);
    }
