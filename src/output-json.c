        ...
        OutputJsonCommonSettings *cfg)
{
    ...
    if (PacketIsIPv4(p)) {
        ...
        PrintInetIPv6((const void *)src_ip, addr->src_ip, sizeof(addr->src_ip), cfg->shorten_ipv6);
        PrintInetIPv6((const void *)dst_ip, addr->dst_ip, sizeof(addr->dst_ip), cfg->shorten_ipv6);
    }
...
OutputInitResult OutputJsonInitCtx(SCConfNode *conf)
{
    ...
    if (conf) {
        ...
        const SCConfNode *shorten_ipv6 = SCConfNodeLookupChild(conf, "ipv6-addr-shorten");
        if (shorten_ipv6 && shorten_ipv6->val && SCConfValIsTrue(shorten_ipv6->val)) {
            SCLogConfig("Will shorten IPv6 addresses in EVE output per RFC 5952");
            json_ctx->cfg.shorten_ipv6 = true;
        } else {
            json_ctx->cfg.shorten_ipv6 = false;
        }
