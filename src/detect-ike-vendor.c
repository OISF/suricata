...
static InspectionBuffer *IkeVendorGetData(DetectEngineThreadCtx *det_ctx,
                                          const DetectEngineTransforms *transforms,
                                          Flow *f, struct IkeVendorGetDataArgs *cbdata, int list_id, bool first)
{
    SCEnter();
    ...
    const uint8_t *data;
    uint32_t data_len;
    if (rs_ike_tx_get_vendor(cbdata->txv, (uint16_t)cbdata->local_id,
                             &data, &data_len) == 0) {
        return NULL;
    }
 ...
 */
static void PrefilterTxIkeVendor(DetectEngineThreadCtx *det_ctx,
      const void *pectx,
      ...
      const uint64_t idx, const uint8_t flags)
{
    ...
    while(1) {
        ...
        InspectionBuffer *buffer = IkeVendorGetData(det_ctx, ctx->transforms,
...
static int PrefilterMpmIkeVendorRegister(DetectEngineCtx *de_ctx,
        ...
        const DetectBufferMpmRegistery *mpm_reg, int list_id)
{
    ...
    return PrefilterAppendTxEngine(de_ctx, sgh, PrefilterTxIkeVendor,
                                   mpm_reg->app_v2.alproto, mpm_reg->app_v2.tx_min_progress,
                                   pectx, PrefilterMpmIkeVendorFree, mpm_reg->pname);
}

static int DetectEngineInspectIkeVendor(
        ...
        Flow *f, uint8_t flags, void *alstate, void *txv, uint64_t tx_id)
{
    ...
    while(1) {
        struct IkeVendorGetDataArgs cbdata = { local_id, txv, };
        InspectionBuffer *buffer = IkeVendorGetData(det_ctx, transforms, f, &cbdata, engine->sm_list, false);
...
{
    sigmatch_table[DETECT_AL_IKE_VENDOR].name = "ikev1.vendor";
    ...
    DetectAppLayerMpmRegister2("ikev1.vendor", SIG_FLAG_TOSERVER, 2,
            PrefilterMpmIkeVendorRegister, NULL,
            ALPROTO_IKE, 1
            );

    DetectAppLayerInspectEngineRegister2("ikev1.vendor",
            ALPROTO_IKE, SIG_FLAG_TOSERVER, 2,
            DetectEngineInspectIkeVendor, NULL
            );

    g_ike_vendor_buffer_id = DetectBufferTypeGetByName("ikev1.vendor");
}
