 ...
 * \param str configuration string to look for
...
static TmEcode BypassedFlowManagerThreadInit(ThreadVars *t, const void *initdata, void **data)
{
    ...
    *data = ftd;
    ...
    int retval = BypassedFlowManagerGetConf(
            root_node, "delay", FLOW_BYPASS_DELAY, &ftd->flow_bypass_delay);
    if (retval != 0 || ftd->flow_bypass_delay <= 0) {
        if (ftd->flow_bypass_delay <= 0)
            SCLogError("flow.bypass.delay must be greater than 0");
        SCFree(ftd);
        *data = NULL;
        return TM_ECODE_FAILED;
    }
