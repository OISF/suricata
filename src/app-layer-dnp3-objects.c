...
/**
 * \brief Read an uint64_t from a buffer.
 *
 * Reads an uint64_t from a buffer advancing the pointer and
 * decrementing the length.
 *
 * \param buf A pointer to the buffer to read from.
 * \param len A pointer to the buffer length.
 ...
 *    otherwise 0 is returned.
 */
static int DNP3ReadUint64(const uint8_t **buf, uint32_t *len, uint64_t *out)
{
    if (*len < (int)sizeof(*out)) {
        return 0;
    }
    *out = DNP3_SWAP64(*(uint64_t *)(*buf));
    *buf += sizeof(*out);
    *len -= sizeof(*out);
    return 1;
}
 ...
 */
static int DNP3PrefixIsSize(uint8_t prefix_code)
{
    switch (prefix_code) {
        case 0x04:
        case 0x05:
    ...
    DNP3PointList *points)
{
    ...
    while (count--) {
        ...
        if (!DNP3ReadUint64(buf, len, (uint64_t *)&object->value)) {
            goto error;
        }
    ...
    DNP3PointList *points)
{
    ...
    while (count--) {
        ...
        if (!DNP3ReadUint32(buf, len, (uint32_t *)&object->deadband_value)) {
            goto error;
        }
    ...
    DNP3PointList *points)
{
    ...
    while (count--) {
        ...
        if (!DNP3ReadUint64(buf, len, (uint64_t *)&object->commanded_value)) {
            goto error;
        }
