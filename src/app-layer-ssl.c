        ...
        const uint8_t *const initial_input, const uint32_t input_len)
{
    ...
    if (rc > 0) {
        ...
        SSLParserHSReset(connp);
        /* the certificate phase data is incomplete (an undecodable
         * certificate): suppress the phase advance, consume the bytes */
        connp->phase_suppressed = true;
    }
    return input_len;
}
                                   ...
                                   uint32_t input_len, uint8_t direction)
{
    ...
    /* a fresh handshake message: the suppression set by a failed decode
     * of the previous message has been consumed by now */
 ...
 */
static void SSLv3HandshakePhaseAdvance(SSLState *ssl_state, const uint8_t hs_type)
{
    /* some handshake messages (certificates) clear the connp handshake
     * type before returning, so the message type is passed in */

    /* a handshake message that failed to decode suppresses its own phase
     * advance: the phase data (SNI, certificates) does not exist */
    ...
    if (ssl_state->curr_connp == &ssl_state->client_connp) {
        ...
        if (hs_type == SSLV3_HS_SERVER_HELLO) {
    ...
    while (input_len) {
        ...
        if (ssl_state->curr_connp->hs_buffer != NULL) {
            ...
            if (ssl_state->curr_connp->hs_buffer_message_size <=
                ...
                SSLv3HandshakePhaseAdvance(
                        ssl_state, ssl_state->curr_connp->hs_buffer_message_type);
        ...
        if (record_len > input_len) {
            ...
            SSLv3HandshakePhaseAdvance(ssl_state, full_record_hs_type);
            input += retval;
            input_len -= retval;
        ...
        AppLayerParserState *pstate, StreamSlice stream_slice)
{
    ...
    if ((ssl_state->flags & SSL_AL_FLAG_NEED_CLIENT_CERT) &&
        ...
        UpdateServerState(ssl_state, TLS_STATE_SERVER_DATA);
               ...
               !ssl_state->server_connp.cert_data_incomplete &&
 ...
 * Certificate through the handshake parser: a failed decode must suppress
 * the phase advance of its own message only - the bytes are consumed,
 * the next message parses and advances normally, and a cert failure
 * keeps the track out of the cert phase. */
static int TLSPhaseSuppressionTest(void)
{
    ...
    /* truncated hello: suppressed, client stays in client_started */
    ssl_state->curr_connp = &ssl_state->client_connp;
    ...
    FAIL_IF(ssl_state->client_state != TLS_STATE_CLIENT_STARTED);

    /* valid hello: the flag is reset per message, server advances */
    ssl_state->curr_connp = &ssl_state->server_connp;
    ...
    FAIL_IF(ssl_state->server_state != TLS_STATE_SERVER_HELLO);

    /* garbage certificate: suppressed, server stays in server_hello */
    ssl_state->curr_connp = &ssl_state->server_connp;
 ...
 * extraction from the fixture), and the cert phase is suppressed (the
 * phase data is incomplete). The incomplete-data condition is sticky - a
 * following ServerHelloDone resets the per-message suppression but the
 * tracks stay out of the cert and data phases. The valid certificate body
...
static int TLSPhaseCertChainSkipTest(void)
{
    ...
    /* the bytes are consumed (rc > 0) but the cert phase is suppressed */
    int rc = SSLv3ParseHandshakeProtocol(ssl_state, msg, msg_len, 1);
    FAIL_IF(rc < 0);
    FAIL_IF(!ssl_state->server_connp.phase_suppressed);
    FAIL_IF(ssl_state->server_state != TLS_STATE_SERVER_STARTED);
     ...
     * message does), so the tracks stay out of the cert and data phases */
