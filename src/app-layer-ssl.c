...
static void SSLParseCipherSuitesFile(FILE *stream) {
    ...
    size_t chunck_sz = 512;
    char *line = NULL;
    size_t n = 0;
    ssize_t bytes_read = 0;

    g_suites = SCCalloc(chunck_sz, sizeof(SSLCipherSuite));
    if (! g_suites) {
    ...
    while( (bytes_read = getline(&line, &n, stream)) > 0 ) {
        ...
        if (g_suites_num % chunck_sz == 0) {

            SSLCipherSuite* orig_suites = g_suites;
            g_suites = SCRealloc(orig_suites,
                    (g_suites_num + chunck_sz) * sizeof(SSLCipherSuite));
 ...
 */
static void SSLLoadCipherSuitesFile(char * cipher_suites_filename)
{
    FILE *stream = NULL;
    ...
    if (g_suites == g_suites_default) {
        ...
        SCLogInfo("loaded %zd cipher suites from memory",
                (uintmax_t) g_suites_num);
    } else {
        SCLogInfo("loaded %zd cipher suites from %s",
                (uintmax_t) g_suites_num, cipher_suites_filename);
    }
