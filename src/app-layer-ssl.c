        ...
        const uint8_t *input, const uint32_t input_len, const StreamSlice stream_slice)
{
    ...
    if (ssl_state->curr_connp->bytes_processed < SSLV3_RECORD_HDR_LEN) {
        ...
        AppLayerFrameNew(ssl_state->f, &stream_slice, input,
        ...
        AppLayerParserState *pstate, StreamSlice stream_slice)
{
    ...
    uint32_t counter = 0;
    int32_t input_len = (int32_t)stream_slice.input_len;
    ssl_state->f = f;
    const uint8_t *input = stream_slice.input;
    ...
    } else if (stream_slice.input == NULL || stream_slice.input_len == 0) {
