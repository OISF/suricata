...
SCEnumCharMap tls_record_table [ ] = {
    { "pdu", TLS_RECORD_PDU, },
    { "hdr", TLS_RECORD_HDR, },
    { "data", TLS_RECORD_DATA, },
    { NULL, -1 },
};
...
static int SSLStateGetRecordIdByName(const char *rec_name) {
    int id = SCMapEnumNameToValue(rec_name, tls_record_table);
    if (id < 0) {
        SCLogError(SC_ERR_INVALID_ENUM_MAP, "unknown record type \"%s\"",  rec_name);
        return -1;
    }
    return id;
}

static const char *SSLStateGetRecordNameById(const uint8_t rec_id) {
    const char *name = SCMapEnumValueToName(rec_id, tls_record_table);
    return name;
}
...
void RegisterSSLParsers(void)
{
    ...
    if (AppLayerParserConfParserEnabled("tcp", proto_name)) {
        ...
        AppLayerParserRegisterGetRecordFuncs(IPPROTO_TCP, ALPROTO_TLS, SSLStateGetRecordIdByName, SSLStateGetRecordNameById);
