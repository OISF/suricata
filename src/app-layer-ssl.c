                                             ...
                                             const uint8_t *initial_input)
{
    ...
    *input += 2;
    ...
    while (processed_len < extensions_len)
    {
        ...
        *input += 2;
        ...
        switch (ext_type) {
            ...
            {
                rc = TLSDecodeHSHelloExtensionSni(ssl_state, input, input_len,
                                                  initial_input);
                if (rc != 0)
                    goto end;
            ...
            {
                rc = TLSDecodeHSHelloExtensionEllipticCurves(ssl_state, input,
                                                             input_len,
                                                             initial_input,
                                                             ja3_elliptic_curves);
                if (rc != 0)
            ...
            {
                rc = TLSDecodeHSHelloExtensionEllipticCurvePF(ssl_state, input,
                                                              input_len,
                                                              initial_input,
        ...
        if ((ssl_state->current_flags & SSL_AL_FLAG_STATE_CLIENT_HELLO) &&
            ...
            if (TLSDecodeValueIsGREASE(ext_type) != 1) {
                int rc = Ja3BufferAddValue(ja3_extensions, ext_type);
                if (rc != 0)
                    goto error;
            }
        }
    ...
    if ((ssl_state->current_flags & SSL_AL_FLAG_STATE_CLIENT_HELLO) &&
            ssl_config.enable_ja3) {
        int rc = Ja3BufferAppendBuffer(ssl_state->ja3_str, ja3_extensions);
        if (rc != 0)
            goto error;
