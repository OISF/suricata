...
SCMD5Context *
MD5_NewContext(HASH_HashType type)
{
    if (type != HASH_AlgMD5) {
        return NULL;
    }
	...
	SCMD5Context *cx = (SCMD5Context *)SCCalloc(1, sizeof(SCMD5Context));
	if (cx == NULL) {
...
void 
MD5_DestroyContext(SCMD5Context *cx)
{
	memset(cx, 0, sizeof *cx);
    SCFree(cx);
}
...
static void
md5_prep_state_le(SCMD5Context *cx)
{
	PRUint32 tmp;
...
static void
md5_prep_buffer_le(SCMD5Context *cx, const PRUint8 *beBuf)
{
...
static void
md5_compress(SCMD5Context *cx, const PRUint32 *wBuf)
{
	PRUint32 a, b, c, d;
	PRUint32 tmp;
	a = cx->cv[0];
...
void 
MD5_Update(SCMD5Context *cx, const unsigned char *input, unsigned int inputLen)
{
...
void 
MD5_End(SCMD5Context *cx, unsigned char *digest,
...
void
MD5_EndRaw(SCMD5Context *cx, unsigned char *digest,
           unsigned int *digestLen, unsigned int maxDigestLen)
{
#ifndef IS_LITTLE_ENDIAN
	PRUint32 tmp;
...
#if 0
unsigned int 
MD5_FlattenSize(SCMD5Context *cx)
{
	return sizeof(*cx);
}

SECStatus 
MD5_Flatten(SCMD5Context *cx, unsigned char *space)
{
...
{
	SCMD5Context *cx = MD5_NewContext();
	if (cx)
		memcpy(cx, space, sizeof(*cx));
	return cx;
}

void MD5_Clone(SCMD5Context *dest, SCMD5Context *src) 
{
	memcpy(dest, src, sizeof *dest);
}

void 
MD5_TraceState(SCMD5Context *cx)
{
