...
static int NapatechRegisterDeviceStreams(void)
{
    ...
    if (use_all_streams && auto_config) {
        SCLogError(SC_ERR_RUNMODE, "napatech.auto-config cannot be used in configuration file at the same time as napatech.with use-all-streams.");
    }
    ...
    if (use_hw_bypass && auto_config == 0) {
        SCLogError(SC_ERR_RUNMODE, "napatech auto-config must be enabled when using napatech.use-all-streams.");
        exit(EXIT_FAILURE);
    }
