...
static int DetectEnipIdentityStatusSetup(DetectEngineCtx *de_ctx, Signature *s, const char *rulestr)
{
    ...
    if (SigMatchAppendSMToList(
                de_ctx, s, DETECT_ENIP_IDENTITYSTATUS, (SigMatchCtx *)du16, g_enip_identity_status_id) == NULL) {
...
/**
 * \brief This function is used to match enip identity_status type rule option on a transaction with those
 * passed via enip_identity_status:
 *
 * \retval 0 no match
 * \retval 1 match
 */
...
{
    uint16_t value;
    if (!rs_enip_get_identity_status(txv, &value))
        SCReturnInt(0);
...
void DetectEnipIdentityStatusRegister(void)
{
    ...
    sigmatch_table[DETECT_ENIP_IDENTITYSTATUS].desc = "rules for detecting EtherNet/IP identity_status";
    sigmatch_table[DETECT_ENIP_IDENTITYSTATUS].url = "/rules/enip-keyword.html#enip-identity-status";
    ...
    DetectAppLayerInspectEngineRegister2("enip.identity_status", ALPROTO_ENIP, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.identity_status", ALPROTO_ENIP, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectGenericList, NULL);
