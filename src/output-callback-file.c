        ...
        const Packet *p, const File *ff, const uint64_t tx_id, uint32_t dir, ThreadVars *tv)
{
    ...
    switch (ff->state) {
        case FILE_STATE_CLOSED:
            ...
            if (ff->flags & FILE_MD5) {
                PrintRawLineHexBuf(md5, sizeof(md5), ff->md5, sizeof(ff->md5));
                event.fileinfo.md5 = md5;
            }
            if (ff->flags & FILE_SHA1) {
                PrintRawLineHexBuf(sha1, sizeof(sha1), ff->sha1, sizeof(ff->sha1));
    ...
    if (ff->flags & FILE_SHA256) {
        PrintRawLineHexBuf(sha256, sizeof(sha256), ff->sha256, sizeof(ff->sha256));
