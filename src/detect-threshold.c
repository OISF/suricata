...
#define PARSE_REGEX                                                                                \
    "^\\s*(track|type|count|seconds)\\s+(limit|both|threshold|by_dst|by_src|by_both|by_rule|\\d+)" \
    "\\s*,\\s*(track|type|count|seconds)\\s+(limit|both|threshold|by_dst|by_src|by_both|by_rule|"  \
    "\\d+)\\s*,\\s*(track|type|count|seconds)\\s+(limit|both|threshold|by_dst|by_src|by_both|by_"  \
    "rule|\\d+)\\s*,\\s*(track|type|count|seconds)\\s+(limit|both|threshold|by_dst|by_src|by_"     \
    "both|by_rule|\\d+)\\s*"
...
static int DetectThresholdTestSig3(void)
{
    ...
    s = de_ctx->sig_list =
            SigInit(de_ctx, "alert tcp any any -> any 80 (msg:\"Threshold limit\"; threshold: type "
                            "limit, track by_dst, count 5, seconds 60; sid:10;)");
    if (s == NULL) {
        goto end;
    }
