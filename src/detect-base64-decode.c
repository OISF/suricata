...
void DetectBase64DecodeRegister(void)
{
    ...
    decode_re = pcre_compile(decode_pattern, 0, &pcre_errptr, &pcre_erroffset,
        NULL);
    if (decode_re == NULL) {
...
static int ParseUnsignedLong(const char *str, unsigned long *value)
{
    unsigned long val;
    char *ep;
...
static int DetectBase64DecodeParse(const char *str, unsigned long *bytes,
    unsigned long *offset, uint8_t *relative)
{
    ...
    *relative = 0;
    ...
    pcre_rc = pcre_exec(decode_re, NULL, str, strlen(str), 0, 0, ov, max);
    if (pcre_rc < 3) {
        goto error;
    }
    ...
    if (pcre_rc >= 3) {
        if (pcre_get_substring((char *)str, ov, max, 2, &bytes_str) > 0) {
            if (!ParseUnsignedLong(bytes_str, bytes)) {
    ...
    if (pcre_rc >= 5) {
        if (pcre_get_substring((char *)str, ov, max, 4, &offset_str)) {
            if (!ParseUnsignedLong(offset_str, offset)) {
                SCLogError(SC_ERR_INVALID_RULE_ARGUMENT,
