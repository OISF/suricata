...
static bool EBPFCreateFlowForKey(ThreadVars *th_v, FlowLookupStruct *fls,
                                 struct flows_stats *flowstats, LiveDevice *dev, void *key,
...
typedef bool (*OpFlowForKey)(ThreadVars *th_v, FlowLookupStruct *fls,
                            struct flows_stats * flowstats, LiveDevice*dev, void *key,
                            size_t skey, FlowKey *flow_key, struct timespec *ctime,
                            uint64_t pkts_cnt, uint64_t bytes_cnt,
                            int mapfd, int cpus_count);
 ...
 */
static int EBPFForEachFlowV4Table(ThreadVars *th_v, FlowLookupStruct *fls,
                                  ...
                                  )
{
    ...
    while (bpf_map_get_next_key(mapfd, &key, &next_key) == 0) {
        ...
        dead_flow = EBPFOpFlowForKey(th_v, fls, &flowstats, dev, &next_key, sizeof(next_key), &flow_key,
 ...
 * running a callback function on each flow.
 */
static int EBPFForEachFlowV6Table(ThreadVars *th_v, FlowLookupStruct *fls,
                                  LiveDevice *dev, const char *name,
                                  struct timespec *ctime,
                                  struct ebpf_timeout_config *tcfg,
                                  OpFlowForKey EBPFOpFlowForKey
                                  )
{
    ...
    while (bpf_map_get_next_key(mapfd, &key, &next_key) == 0) {
        ...
        flow_key.recursion_level = 0;
        pkts_cnt = EBPFOpFlowForKey(th_v, fls, &flowstats, dev, &next_key, sizeof(next_key), &flow_key,
                                    ctime, pkts_cnt, bytes_cnt,
                                    mapfd, tcfg->cpus_count);
        if (pkts_cnt > 0) {
            found = 1;
...
int EBPFCheckBypassedFlowCreate(ThreadVars *th_v, FlowLookupStruct *fls,
                                struct timespec *curtime, void *data)
{
    ...
    while(LiveDeviceForEach(&ldev, &ndev)) {
        EBPFForEachFlowV4Table(th_v, fls, ldev, "flow_table_v4",
                curtime,
                cfg, EBPFCreateFlowForKey);
        EBPFForEachFlowV6Table(th_v, fls, ldev, "flow_table_v6",
                curtime,
                cfg, EBPFCreateFlowForKey);
    }
