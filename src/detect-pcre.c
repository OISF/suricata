 ...
 */
static int DetectPcreExec(DetectParseRegex *regex, const char *str, const size_t strlen,
                            int start_offset, int options,  int *ovector, int ovector_size)
{
#ifdef PCRE_HAVE_JIT_EXEC
    /* Ensure jit stack allocation succeeded when jit is available */
    if (regex->jit_stack)
        ...
        size_t capture_names_size, bool negate, AppProto *alproto)
{
    ...
    if (jit) {
        ...
        if (pd->parse_regex.jit_stack == NULL) {
            SCLogWarning(SC_WARN_PCRE_JITSTACK, "pcre jit stack failed");
        } else {
