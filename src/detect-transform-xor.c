...
static int DetectTransformXorSetup(DetectEngineCtx *de_ctx, Signature *s, const char *optstr)
{
    ...
    if (keylen % 2 == 1) {
        SCLogError(SC_ERR_INVALID_SIGNATURE, "Odd length for hexadecimal xor transform key");
        DetectTransformXorFree(de_ctx, pxd);
        SCReturnInt(-1);
    }
    if (keylen / 2 > UINT8_MAX) {
        SCLogError(SC_ERR_INVALID_SIGNATURE, "Too big key length for xor transform");
        DetectTransformXorFree(de_ctx, pxd);
        SCReturnInt(-1);
    }
    pxd->length = keylen / 2;
    ...
    for (size_t i = 0; i < keylen / 2; i++) {
        if ((isxdigit(optstr[2 * i])) && (isxdigit(optstr[2 * i + 1]))) {
            ...
            SCLogError(
                    SC_ERR_INVALID_SIGNATURE, "Invalid hexadecimal character in xor transform key");
            DetectTransformXorFree(de_ctx, pxd);
            SCReturnInt(-1);
        }
    }
...
static void DetectTransformXor(InspectionBuffer *buffer, void *options)
{
    ...
    printf("lola %d %d\n", input_len, buffer->inspect_offset);
