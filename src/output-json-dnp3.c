...
static void json_set_base64(json_t *js, char *name, uint8_t *buf, int len)
{
    unsigned long encoded_len = MAX(5, len * 2);
    uint8_t encoded[encoded_len];

    if (len > 0) {
        Base64Encode((unsigned char *)buf, len, encoded, &encoded_len);
        json_object_set_new(js, name, json_string((char *)encoded));
    }
    else {
        json_object_set_new(js, name, json_string(""));
    }
}
...
static json_t *JsonDNP3LogObjects(DNP3ObjectList *objects, uint8_t include_data)
{
    ...
    TAILQ_FOREACH(object, objects, next) {
        ...
        json_object_set_new(objs, "prefix", json_integer(object->prefix));
        json_object_set_new(objs, "range", json_integer(object->range));

        if (include_data) {
            json_set_base64(objs, "data", object->data, object->len);
        }
    ...
    const Packet *p, DNP3Transaction *dnp3tx)
{
    ...
    json_object_set_new(al, "complete",
        json_boolean(dnp3tx->request_decode_complete));
    ...
    const Packet *p, DNP3Transaction *dnp3tx)
{
    ...
    json_object_set_new(al, "complete",
        json_boolean(dnp3tx->response_decode_complete));
...
static OutputCtx *OutputDNP3LogInitSub(ConfNode *conf, OutputCtx *parent_ctx)
{
    ...
    const char *object_data = ConfNodeLookupChildValue(conf, "object-data");
    if (object_data != NULL && ConfValIsTrue(object_data)) {
        dnp3log_ctx->include_object_data = 1;
    }
