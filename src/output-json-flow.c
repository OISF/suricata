...
static SCJsonBuilder *CreateEveHeaderFromFlow(const Flow *f, OutputJsonCommonSettings *cfg)
{
    ...
    if ((f->flags & FLOW_DIR_REVERSED) == 0) {
        if (FLOW_IS_IPV4(f)) {
            ...
            PrintInetIPv6((const void *)&(f->src.address), srcip, sizeof(srcip), cfg->shorten_ipv6);
            PrintInetIPv6((const void *)&(f->dst.address), dstip, sizeof(dstip), cfg->shorten_ipv6);
        }
        sp = f->sp;
        dp = f->dp;
    } else {
        if (FLOW_IS_IPV4(f)) {
            ...
            PrintInetIPv6((const void *)&(f->dst.address), srcip, sizeof(srcip), cfg->shorten_ipv6);
            PrintInetIPv6((const void *)&(f->src.address), dstip, sizeof(dstip), cfg->shorten_ipv6);
        }
        sp = f->dp;
        dp = f->sp;
    }
