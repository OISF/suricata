...
static void EveFlowLogJSON(OutputJsonThreadCtx *aft, JsonBuilder *jb, Flow *f)
{
    ...
    if (f->flags & FLOW_TRIGGERED_EXCEPTION_POLICY) {
        ...
        for (uint i; i < EXCEPTION_POLICY_TARGETS_MAX; i++) {
            if (f->applied_exception_policy.eps_targets[i]) {
                JsonBuilder *jb_policy = jb_new_object();
                if (jb_policy != NULL) {
                    jb_set_string(jb_policy, "target", ExceptionPolicyTargetFlagToString(i));
                    jb_set_string(jb_policy, "policy",
                            ExceptionPolicyEnumToString(ExceptionPolicyTargetPolicy(i), true));
                }
                jb_close(jb_policy);
