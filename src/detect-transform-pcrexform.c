...
static int DetectTransformPcrexformSetup (DetectEngineCtx *de_ctx, Signature *s, const char *regexstr)
{
    ...
    if (pxd->regex == NULL) {
        ...
        SCLogError(SC_ERR_PCRE_COMPILE, "pcre2 compile of \"%s\" failed at "
                   "offset %d: %s", regexstr, (int) eo, buffer);
        SCFree(pxd);
        SCReturnInt(-1);
    }
    ...
    if (pcre2_pattern_info(pxd->regex, PCRE2_INFO_CAPTURECOUNT, &nb) < 0) {
        SCLogError(SC_ERR_INVALID_SIGNATURE, "pcrexform failed getting info about namecount");
        DetectTransformPcrexformFree(de_ctx, pxd);
        SCReturnInt(-1);
    }
    if (nb != 1) {
        SCLogError(SC_ERR_INVALID_SIGNATURE, "pcrexform needs exactly one substring capture, found %" PRIu32, nb);
        DetectTransformPcrexformFree(de_ctx, pxd);
        SCReturnInt(-1);
    }
...
static void DetectTransformPcrexform(InspectionBuffer *buffer, void *options)
{
    ...
    match_data = pcre2_match_data_create_from_pattern(pxd->regex, NULL);
    int ret = pcre2_match(pxd->regex, (PCRE2_SPTR8) input, input_len, 0, 0, match_data, NULL);

    if (ret > 0) {
        const char *str;
        PCRE2_SIZE caplen;
        ret = pcre2_substring_get_bynumber(match_data, 0, (PCRE2_UCHAR8 **) &str, &caplen);
