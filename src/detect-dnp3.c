...
{
    if (ByteParseStringUint8(fc, 10, strlen(str), str) >= 0) {
        return 1;
    }
...
static int DetectDNP3IndParse(const char *str, uint16_t *flags)
{
    *flags = 0;
    ...
    if (ByteParseStringUint16(flags, 0, strlen(str), str) > 0) {
        return 1;
    }
...
static int DetectDNP3ObjParse(const char *str, uint8_t *group, uint8_t *var)
{
    ...
    *sep = '\0';
    ...
    if (ByteParseStringUint8(group, 0, strlen(groupstr), groupstr) < 0) {
        return 0;
    }

    if (ByteParseStringUint8(var, 0, strlen(varstr), varstr) < 0) {
        return 0;
    }
