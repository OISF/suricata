        ...
        const char *rulestr)
{
    ...
    void *cipserviced = NULL;
    SigMatch *sm = NULL;

    if (DetectSignatureSetAppProto(s, ALPROTO_ENIP) != 0)
        return -1;

    cipserviced = rs_enip_parse_cip_service(rulestr);
    if (cipserviced == NULL)
        return -1;
 ...
 * \retval 1 match
 */
static int DetectCipServiceMatch(DetectEngineThreadCtx *det_ctx,
...
void DetectCipServiceRegister(void)
{
    ...
    sigmatch_table[DETECT_CIPSERVICE].desc = "match on CIP Service, and optionnally class and attribute";
