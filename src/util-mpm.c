...
static void *MpmCudaConfParse(ConfNode *node)
{
    ...
    if (node != NULL)
        value = ConfNodeLookupChildValue(node, "mtsba-buffer-size");
    else
        value = NULL;
    if (value == NULL) {
        /* default */
        conf->mtsba_buffer_size = UTIL_MPM_CUDA_MTSBA_DBUFFER_SIZE_DEFAULT;
    } else if (ParseSizeStringU32(value, &conf->mtsba_buffer_size) < 0) {
        SCLogError(SC_ERR_INVALID_YAML_CONF_ENTRY, "Invalid entry for %s."
                   "mtsba-buffer-size - \"%s\"", node->name, value);
        exit(EXIT_FAILURE);
    }
...
void MpmTableSetup(void) {
    ...
    MpmACRegister();
#ifdef __SC_CUDA_SUPPORT__
    CudaHL_AddCudaProfileFromConf("mpm", MpmCudaConfParse, MpmCudaConfFree);
    MpmACCudaRegister();
