 ...
 */
static inline int InsertSegmentDataCustom(
        TcpStream *stream, TcpSegment *seg, uint8_t *data, uint16_t data_len)
{
    ...
    SCLogDebug("stream %p buffer %p, stream_offset %" PRIu64 ", "
            ...
            stream, &stream->sb, stream_offset, data_offset, seg->seq, stream->base_seq, data_len);
...
#ifdef DEBUG
    {
        ...
        SCLogDebug("stream %p seg %p data in buffer %p of len %u and offset %" PRIu64, stream, seg,
                &stream->sb, mydata_len, mydata_offset);
...
static inline bool CheckOverlap(struct TCPSEG *tree, TcpSegment *seg)
{
    ...
    SCLogDebug("start. SEQ %u payload_len %u. Right edge: %u. Seg %p", seg->seq, seg->payload_len,
            re, seg);
...
static int DoInsertSegment(TcpStream *stream, TcpSegment *seg, TcpSegment **dup_seg, Packet *p)
{
    ...
    if (res) {
        SCLogDebug("seg has a duplicate in the tree seq %u/%u", res->seq, res->payload_len);
 ...
 */
static int DoHandleDataOverlap(
        TcpStream *stream, const TcpSegment *list, const TcpSegment *seg, uint8_t *buf, Packet *p)
{
    SCLogDebug("handle overlap for segment %p seq %u len %u re %u, "
               "list segment %p seq %u len %u re %u",
            seg, seg->seq, p->payload_len, SEG_SEQ_RIGHT_EDGE(seg), list, list->seq,
            TCP_SEG_LEN(list), SEG_SEQ_RIGHT_EDGE(list));
    ...
    SCLogDebug("data_is_different %s, use_new_data %s", data_is_different ? "yes" : "no",
            use_new_data ? "yes" : "no");
    ...
    if (data_is_different && !use_new_data) {
        ...
        SCLogDebug("here goes nothing: list %u %u, seg %u %u", list_offset, list_len, seg_offset,
                seg_len);
...
static int DoHandleDataCheckBackwards(TcpStream *stream, TcpSegment *seg, uint8_t *buf, Packet *p)
{
    ...
    SCLogDebug("check tree backwards: insert data for segment %p seq %u len %u re %u", seg,
            seg->seq, TCP_SEG_LEN(seg), SEG_SEQ_RIGHT_EDGE(seg));
    ...
    RB_FOREACH_REVERSE_FROM (tree_seg, TCPSEG, s) {
        ...
        SCLogDebug("(back) tree seg %u len %u re %u overlap? %s", tree_seg->seq,
                TCP_SEG_LEN(tree_seg), SEG_SEQ_RIGHT_EDGE(tree_seg), overlap ? "yes" : "no");
...
static int DoHandleDataCheckForward(TcpStream *stream, TcpSegment *seg, uint8_t *buf, Packet *p)
{
    ...
    SCLogDebug("check list forward: insert data for segment %p seq %u len %u re %u", seg, seg->seq,
            TCP_SEG_LEN(seg), seg_re);
    ...
    RB_FOREACH_FROM (tree_seg, TCPSEG, s) {
        ...
        if (SEQ_GT(seg_re, tree_seg->seq))
            ...
            SCLogDebug("tree segment %u too far ahead, "
                       "no more overlaps can happen",
                    tree_seg->seq);
            break;
        }

        SCLogDebug("(fwd) in-tree seg %u len %u re %u overlap? %s", tree_seg->seq,
                TCP_SEG_LEN(tree_seg), SEG_SEQ_RIGHT_EDGE(tree_seg), overlap ? "yes" : "no");
 ...
 */
static int DoHandleData(ThreadVars *tv, TcpReassemblyThreadCtx *ra_ctx, TcpStream *stream,
        TcpSegment *seg, TcpSegment *tree_seg, Packet *p)
{
    int result = 0;
    TcpSegment *handle = seg;

    SCLogDebug("insert data for segment %p seq %u len %u re %u", seg, seg->seq, TCP_SEG_LEN(seg),
            SEG_SEQ_RIGHT_EDGE(seg));
...
int StreamTcpReassembleInsertSegment(ThreadVars *tv, TcpReassemblyThreadCtx *ra_ctx,
        TcpStream *stream, TcpSegment *seg, Packet *p, uint32_t pkt_seq, uint8_t *pkt_data,
        uint16_t pkt_datalen)
{
    ...
    if (likely(r == 0)) {
        ...
        if (r == 2) {
            SCLogDebug("duplicate segment %u/%u, discard it", seg->seq, seg->payload_len);
            ...
            if (SCLogDebugEnabled()) {
                ...
                RB_FOREACH_SAFE (s, TCPSEG, &stream->seg_tree, safe) {
                    SCLogDebug("tree: seg %p, SEQ %" PRIu32 ", LEN %" PRIu16 ", SUM %" PRIu32
                               "%s%s%s",
                            s, s->seq, TCP_SEG_LEN(s), (uint32_t)(s->seq + TCP_SEG_LEN(s)),
...
static inline uint64_t GetLeftEdge(Flow *f, TcpSession *ssn, TcpStream *stream)
{
    ...
    if (use_raw) {
        ...
        if (use_app) {
            ...
            SCLogDebug("left_edge %" PRIu64 ", using only raw:%" PRIu64, left_edge, raw_progress);
        }
    } else if (use_app) {
        const uint64_t app_le = GetLeftEdgeForApp(f, ssn, stream);
        left_edge = app_le;
    ...
    if (left_edge > 0) {
        ...
        RB_FOREACH(seg, TCPSEG, &stream->seg_tree)
        {
            ...
            if (SegmentInUse(stream, seg)) {
                ...
                SCLogDebug(
                        "in-use seg before left_edge, adjust to %" PRIu64 " and bail", left_edge);
                break;
            }
        }
    }
...
void StreamTcpPruneSession(Flow *f, uint8_t flags)
{
    ...
    if (left_edge && left_edge > STREAM_BASE_OFFSET(stream)) {
        ...
        SCLogDebug("stream base_seq %u at stream offset %" PRIu64, stream->base_seq,
    ...
    RB_FOREACH_SAFE (seg, TCPSEG, &stream->seg_tree, safe) {
        SCLogDebug("seg %p, SEQ %" PRIu32 ", LEN %" PRIu16 ", SUM %" PRIu32, seg, seg->seq,
                TCP_SEG_LEN(seg), (uint32_t)(seg->seq + TCP_SEG_LEN(seg)));
