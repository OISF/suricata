...
int StreamTcpReassemblyConfig(void) {
    struct Sizes {
        uint16_t pktsize;
        uint32_t prealloc;
    };
    /* sort small to big */
    int SortByPktsize(const void *a, const void *b)
    {
        const struct Sizes *s0 = a;
        const struct Sizes *s1 = b;
        return s0->pktsize - s1->pktsize;
    }

    struct Sizes sizes[256];
    ...
    if (segs != NULL) {
        ...
        TAILQ_FOREACH(seg, &segs->head, next) {
            ...
            SCLogInfo("segsize->val %s", segsize->val);
            SCLogInfo("segpre->val %s", segpre->val);
    ...
    Pool **my_segment_pool = SCMalloc(npools * sizeof(Pool *));
    SCMutex *my_segment_lock = SCMalloc(npools * sizeof(SCMutex));
    uint16_t *my_segment_pktsizes = SCMalloc(npools * sizeof(uint16_t));
    if (my_segment_pool == NULL || my_segment_lock == NULL || my_segment_pktsizes == NULL) {
