...
static void ThresholdDistinctInit(ThresholdEntry *te, const DetectThresholdData *td)
{
    ...
    /* IP-based distinct counting uses hash table, pre-sized to count */
        ...
        te->distinct_ip_hash =
                HashTableInit(nbuckets, HashTableGenericHash, HashTableDefaultCompare, NULL);
...
static inline void ThresholdDistinctAddIP(ThresholdEntry *te, const Address *addr)
{
    ...
    if (found == NULL) {
        /* New IP, add to hash and increment count */
        if (HashTableAdd(te->distinct_ip_hash, (void *)addr->addr_data32, key_size) == 0) {
            te->current_count++;
        }
    }
}
