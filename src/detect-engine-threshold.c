...
/**
 * \brief Add a distinct value (port or IP) based on unique_on type.
 *
 * If the distinct tracking structure (bitmap/buffer) is allocated, the value
 * is recorded there (which also bumps current_count for new distinct values).
 * Otherwise (allocation failed or test mode), we fall back to classic counting
 * by simply incrementing current_count.
 */
static inline void ThresholdDistinctAdd(
        ThresholdEntry *te, const DetectThresholdData *td, const Packet *p)
{
    ...
    /* tracking not active, fall back to classic counting */
    te->current_count++;
}
        ...
        const uint32_t sid, const uint32_t gid, const uint32_t rev)
{
    ...
    switch (td->type) {
        ...
        default:
             ...
             * ThresholdDistinctInit resets current_count to 0, so ThresholdDistinctAdd
             * records the value (or falls back to counting) bringing current_count to 1. */
            if (td->type == TYPE_DETECTION && td->unique_on != DF_UNIQUE_NONE) {
                ThresholdDistinctAdd(te, td, p);
            }
            break;
    }
