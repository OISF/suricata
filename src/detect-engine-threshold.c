...
typedef struct ThresholdEntry_ {
    ...
    union {
        ...
        struct {
            ...
            /* Hash table for unique IPs, pre-sized to td->count buckets. */
            HashTable *distinct_ip_hash;
        };
    };
...
static void ThresholdDistinctIPFree(void *ptr)
{
    SCFree(ptr);
}
...
static void ThresholdDistinctInit(ThresholdEntry *te, const DetectThresholdData *td)
{
    ...
    if (g_threshold_force_alloc_fail) {
        ...
        te->distinct_ip_hash = NULL;
        return;
    }
#endif

    /* Allocate tracking structure for distinct counting (bitmap for ports, hash table for IPs).
    ...
    /* IP-based distinct counting uses a hash table pre-sized to `count` buckets.
     *
     * Memory allocation happens in two phases:
     *   1. Init (here): the bucket pointer array (nbuckets * sizeof(void *))
     *      is allocated by HashTableInit and checked against the memcap.
     *   2. Runtime (ThresholdDistinctAddIP): each new unique IP allocates a
     *      HashTableBucket + a copy of the IP bytes via SCMalloc. These
     *      per-entry allocations are bounded by `count` (at most count
     *      entries before the window expires and the table is reset).
     *      The SCFree callback ensures all copies are freed on reset/free.
     *
     * Only the phase-1 array is tracked in memuse/memcap counters. Phase-2
     * per-entry overhead is small and bounded, so it is not tracked. */
    else if (td->unique_on == DF_UNIQUE_SRC_IP || td->unique_on == DF_UNIQUE_DST_IP) {
        const uint32_t nbuckets = td->count;
        const uint32_t init_size = nbuckets * sizeof(void *);
        if (ctx.thash != NULL && !THASH_CHECK_MEMCAP(ctx.thash, init_size)) {
            SC_ATOMIC_ADD(threshold_bitmap_alloc_fail, 1);
            te->distinct_ip_hash = NULL;
            return;
        }

        te->distinct_ip_hash = HashTableInit(
                nbuckets, HashTableGenericHash, HashTableDefaultCompare, ThresholdDistinctIPFree);
        if (te->distinct_ip_hash == NULL) {
            SC_ATOMIC_ADD(threshold_bitmap_alloc_fail, 1);
        } else {
            if (ctx.thash != NULL) {
                (void)SC_ATOMIC_ADD(ctx.thash->memuse, init_size);
            }
            SC_ATOMIC_ADD(threshold_bitmap_memuse, init_size);
        }
    }
}
...
static void ThresholdDistinctReset(ThresholdEntry *te)
{
    ...
    /* Reset IP hash table: free the old table (which frees all entry copies
     * via the SCFree callback) and allocate a fresh one with the same size.
     * If re-init fails, decrement memory counters since the array is gone. */
    if (te->distinct_ip_hash) {
        const uint32_t nbuckets = te->distinct_ip_hash->array_size;
        HashTableFree(te->distinct_ip_hash);
        te->distinct_ip_hash = HashTableInit(
                nbuckets, HashTableGenericHash, HashTableDefaultCompare, ThresholdDistinctIPFree);
        if (te->distinct_ip_hash == NULL) {
            const uint32_t hash_size = nbuckets * sizeof(void *);
            if (ctx.thash != NULL) {
                (void)SC_ATOMIC_SUB(ctx.thash->memuse, hash_size);
            }
            SC_ATOMIC_SUB(threshold_bitmap_memuse, hash_size);
        }
    }
    te->current_count = 0;
}
...
static inline void ThresholdDistinctAddIP(ThresholdEntry *te, const Address *addr)
{
    if (te->distinct_ip_hash == NULL) {
        return;
    }

    /* Create a key from the address bytes */
    uint16_t key_size;
    ...
    /* Check if this IP is already in the hash table */
    void *found = HashTableLookup(te->distinct_ip_hash, (void *)addr->addr_data32, key_size);
    if (found == NULL) {
        /* HashTableAdd stores the pointer without copying, so we must
         * provide an owned copy that outlives the packet. The hash table
         * was created with SCFree as its Free callback, so these copies
         * are freed automatically on HashTableFree/Remove. */
        void *copy = SCMalloc(key_size);
        if (copy == NULL) {
            return;
        }
        memcpy(copy, addr->addr_data32, key_size);
        if (HashTableAdd(te->distinct_ip_hash, copy, key_size) == 0) {
            te->current_count++;
        } else {
            SCFree(copy);
        }
        ...
        ThresholdEntry *te, const DetectThresholdData *td, const Packet *p)
{
    if (td->unique_on == DF_UNIQUE_SRC_PORT || td->unique_on == DF_UNIQUE_DST_PORT) {
        ...
        if (te->distinct_ip_hash) {
            ...
            ThresholdDistinctAddIP(te, addr);
            return true;
        }
    }
    return false;
...
static void ThresholdEntryFree(void *ptr)
{
    ...
    if (e->distinct_ip_hash) {
        const uint32_t hash_size = e->distinct_ip_hash->array_size * sizeof(void *);
        if (ctx.thash != NULL) {
            (void)SC_ATOMIC_SUB(ctx.thash->memuse, hash_size);
        }
        SC_ATOMIC_SUB(threshold_bitmap_memuse, hash_size);
        HashTableFree(e->distinct_ip_hash);
        e->distinct_ip_hash = NULL;
    }
}
