 ...
 */
static int PICompareAndUpdate(const SCPortIntervalNode *a, SCPortIntervalNode *b)
{
    if (a->port2 > b->max) {
        b->max = a->port2;
    }
...
IRB_GENERATE(PI, SCPortIntervalNode, irb, PICompareAndUpdate);
 ...
 */
int PIInsertPort(DetectEngineCtx *de_ctx, SCPortIntervalTree *it, const DetectPort *p)
{
 ...
 */
static bool IsOverlap(const uint16_t port, const uint16_t port2, const SCPortIntervalNode *ptr)
{
...
static void FindOverlaps(DetectEngineCtx *de_ctx, const uint16_t port, const uint16_t port2,
        SCPortIntervalNode *root, DetectPort **list)
{
