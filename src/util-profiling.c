...
void ProfliteEnable(const char *setting)
{
    if (strcmp(setting, "global") == 0) {
        SC_ATOMIC_OR(proflite_flags, PROFLITE_ENABLED_BIT);
    } else if (strcmp(setting, "app_http") == 0) {
        SC_ATOMIC_OR(proflite_flags, PROFLITE_ENABLED_BIT|PROFLITE_ALPROTO_HTTP_BIT);
    } else if (strcmp(setting, "only_app_http") == 0) {
        SC_ATOMIC_SET(proflite_flags, PROFLITE_ENABLED_BIT|PROFLITE_ALPROTO_HTTP_BIT);
    } else {
        abort();
    }
    SCLogNotice("enabled \"%s\". Flags now %"PRIx64, setting, SC_ATOMIC_GET(proflite_flags));
}
...
void ProfliteRegisterCounters(ThreadVars *tv)
{
    for (enum ProfileLiteTracker t = 0; t < PLT_SIZE; t++) {
        ...
        c->avg = StatsRegisterCounter(n->avg, tv);
    ...
    if (p->ip4h || p->ip6h) {
        switch (p->proto) {
            ...
            case IPPROTO_UDP:
                ...
                if (p->proflite_alproto == ALPROTO_DNS) {
                    if (flags & PROFLITE_ALPROTO_DNS_BIT) {
                        Update(tv, PLT_ALPROTO_DNS, usecs);
                    }
                } else if (p->proflite_alproto == ALPROTO_UNKNOWN ||
