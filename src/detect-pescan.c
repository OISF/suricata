...
/* Prototypes */
static int DetectPescanMatch (ThreadVars *, DetectEngineThreadCtx *, Flow *, uint8_t, File *, Signature *, SigMatch *);
...
void DetectPescanRegister(void) {
    ...
    sigmatch_table[DETECT_PESCAN].alproto = ALPROTO_HTTP; /* TODO: This should allow any protocol */
...
/**
 * \brief Compare two floating point double-precision numbers using EPSILON value for handling margin of error
 *
...
static int CheckScoreMatch (DetectPescanData *pedata, peattrib_t *peattrib) {
    ...
    case DETECT_PESCAN_RA:
        ...
        match = (DblCmp(peattrib->pescore, pedata->score1) == 1 && DblCmp(peattrib->pescore, pedata->score2) == -1);
        break;

    case DETECT_PESCAN_RAEQ:
        SCLogDebug("Match on RAEQ");
        match = (DblCmp(peattrib->pescore, pedata->score1) >= 0 && DblCmp(peattrib->pescore, pedata->score2) <= 0);
        break;
...
/**
 * \brief match the specified pescan search (match occurs when PE found and if score comparison is a match)
 *
        ...
        uint8_t flags, File *file, Signature *s, SigMatch *m)
{
    ...
    if (pefound) {
        ...
        if (match == 0) {
            SCLogInfo("PE does not match rule signature");
        }
        else {
 ...
 *
 * If no arguments are specified, then all PE scores will be considered a match for the rule
...
#define MAX_SUBSTRINGS 30
    ...
    /* Process any options (pescan:[score1][operator][score2]) - default is to match on any PE regardless of score */
    if (str != NULL && str[0] != 0) {
...
int DetectPescanTest01(void){
    ...
    char data[] = "";
...
int DetectPescanParseTest03(void){
    ...
    if(pescore != NULL && pescore->score1 == 0 && pescore->mode == DETECT_PESCAN_ANY)
        result = 1;
    SCFree(pescore);
...
int DetectPescanParseTest05(void){
    ...
    if(pescore != NULL && pescore->score1 == 5 && pescore->mode == DETECT_PESCAN_LT)
        result = 1;
    SCFree(pescore);
...
int DetectPescanParseTest06(void){
    ...
    if(pescore != NULL && pescore->score1 == 0 && pescore->mode == DETECT_PESCAN_RA)
        result = 1;
    SCFree(pescore);
...
int DetectPescanParseTest07(void){
    ...
    pescore = DetectPescanParse(">=5.");
    if(pescore != NULL && pescore->score1 == 5.0 && pescore->mode == DETECT_PESCAN_GTEQ)
        result = 1;
    SCFree(pescore);
...
int DetectPescanParseTest10(void){
    ...
    pescore = DetectPescanParse(".-=.1");
    if(pescore != NULL && pescore->score1 == 0 && pescore->mode == DETECT_PESCAN_RAEQ)
        result = 1;
    SCFree(pescore);
...
int DetectPescanInitTest01(void){
    ...
    s = de_ctx->sig_list = SigInit(de_ctx,"alert http any any -> any any (msg:\"with pescan\"; pescan; sid:12345;)");
    if(s == NULL)
        goto end;


    s = s->next = de_ctx->sig_list = SigInit(de_ctx,"alert http any any -> any any (msg:\"with pescan and a min score for 1\"; pescan:1 ; sid:12345;)");
    if(s == NULL)
        goto end;
