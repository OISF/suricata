...
static void * DetectPescanThreadInit(void *data);
static void DetectPescanThreadFree(void *ptr);
        ...
        uint8_t flags, File *file, Signature *s, SigMatch *m)
{
    ...
    /* Get rule options and scan buffer from context */
    pedata = (DetectPescanData *) m->ctx;
    if (pedata == NULL) {
        goto error;
    }
    uint8_t *scanbuf = (uint8_t *) DetectThreadCtxGetKeywordThreadCtx(det_ctx,
            pedata->thread_ctx_id);
    if (scanbuf == NULL)
        goto error;
    SCLogDebug("Scan buffer: %p", scanbuf);
    ...
    if (pefound) {
        ...
        if (match == 0) {
            ...
            SCLogDebug("PE matches rule signature");
        }
    }
...
error:
    SCLogDebug("Error condition occurred");
    if (file->peattrib != NULL) {
...
static int DetectPescanSetup(DetectEngineCtx *de_ctx, Signature *s, char *str)
{
    ...
    pedata->thread_ctx_id = DetectRegisterThreadCtxFuncs(de_ctx, PESCAN_KEYWORD,
            DetectPescanThreadInit, (void *) pedata, DetectPescanThreadFree, 0);
    if (pedata->thread_ctx_id == -1)
        goto error;
...
/**
 * \brief Init thread local data
 *
 * In this case, it is the scan buffer that is alloc'd once per thread.
 *
 * \param data the DetectPescanData pointer (unused)
 */
static void * DetectPescanThreadInit(void *data) {
    /* Create prealloc'd buffer for scanning files */
    uint32_t maxSize = PEScanGetMaxBytes(NULL);
    SCLogDebug("Allocating max size for thread local pescan buffer: %d", maxSize);
    uint8_t *scanbuf = SCMalloc(maxSize);
    if (unlikely(scanbuf == NULL)) {
        SCLogError(SC_ERR_MEM_ALLOC, "Could not allocate memory for scan buffer");
        return NULL;
    }
...
/**
 * \brief Free thread local data (scan buffer)
...
int DetectPescanMatchTest01(void){
    ...
    DetectPescanThreadFree(scanbuf);
