    ...
    if (ConfGetChildValueWithDefault(if_root, if_default, "copy-mode", &copymodestr) == 1) {
        if (aconf->out_iface == NULL) {
            ...
            if (aconf->flags & AFP_TPACKET_V3) {
                SCLogError(SC_ERR_RUNMODE, "Using tpacket_v3 in IPS mode will result in high latency");
            }
            ...
            if (aconf->flags & AFP_TPACKET_V3) {
                SCLogError(SC_ERR_RUNMODE, "Using tpacket_v3 in TAP mode will result in high latency");
            }
        } else {
