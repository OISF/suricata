                ...
                TAILQ_FOREACH(lnode, &node->head, next)
                {
                    ...
                    if (strncmp(lnode->val, "all", 4) == 0) {
                        ...
                        if (cpu_spec != CONFIG_SPECIFIER_UNDEFINED) {
                            SCLogError(SC_ERR_NAPATECH_PARSE_CONFIG, "Only one Napatech port specifier type allowed.");
                            exit(EXIT_FAILURE);
                        }
                        ...
                        if (cpu_spec == CONFIG_SPECIFIER_RANGE) {
                            SCLogError(SC_ERR_NAPATECH_PARSE_CONFIG, "Napatech port range specifiers cannot be combined with individual stream specifiers.");
                            exit(EXIT_FAILURE);
                        }
        ...
        if (NapatechIsAutoConfigEnabled() && (set_cpu_affinity == 1)) {
            ...
            if ((ntstreams = ConfGetNode("napatech.streams")) == NULL) {
                ...
                if (NapatechIsAutoConfigEnabled() && (set_cpu_affinity == 0)) {
                    SCLogError(SC_ERR_RUNMODE, "if set-cpu-affinity: no in conf then napatech.streams must be defined");
                }
                exit(EXIT_FAILURE);
            }
            ...
            TAILQ_FOREACH(stream, &ntstreams->head, next)
            {
                ...
                if (strchr(stream->val, '-')) {
                    if (stream_spec != CONFIG_SPECIFIER_UNDEFINED) {
                        SCLogError(SC_ERR_NAPATECH_PARSE_CONFIG, "Only one Napatech stream range specifier allowed.");
                        return -1;
                    }
                    ...
                    if (stream_spec == CONFIG_SPECIFIER_RANGE) {
                        SCLogError(SC_ERR_NAPATECH_PARSE_CONFIG, "Napatech stream range specifiers cannot be combined with individual stream specifiers.");
                        exit(EXIT_FAILURE);
                    }
    ...
    *filter_id = 0;
    ...
    if ((status = NT_NTPL(hconfig, ntpl_cmd, &ntpl_info, NT_NTPL_PARSER_VALIDATE_NORMAL)) == NT_SUCCESS) {
        *filter_id = ntpl_info.ntplId;
        SCLogInfo("hashmode: %s ID: %d", hash_mode, status);
    } else {
        NAPATECH_NTPL_ERROR(ntpl_cmd, ntpl_info, status);
        status = 0;
...
static uint32_t GetStreamNUMAs(uint32_t stream_id, int stream_numas[])
{
    ...
    for (int i = 0; i < MAX_HOSTBUFFERS; ++i) stream_numas[i] = -1;
...
uint32_t NapatechSetupTraffic(uint32_t first_stream, uint32_t last_stream, uint32_t *filter_id, uint32_t *hash_id)
{
    ...
    TAILQ_FOREACH(port, &ntports->head, next)
    {
        ...
        if (port == NULL) {
            SCLogError(SC_ERR_NAPATECH_STREAMS_REGISTER_FAILED, "Couldn't Parse Port Configuration");
            exit(EXIT_FAILURE);
        }

        uint8_t start, end;
        if (strncmp(port->val, "all", 4) == 0) {
            /* check that the sting in the config file is correctly specified */
            if (stream_spec != CONFIG_SPECIFIER_UNDEFINED) {
                SCLogError(SC_ERR_NAPATECH_PARSE_CONFIG, "Only one Napatech port specifier type allowed.");
                exit(EXIT_FAILURE);
            ...
            char copystr[16];
            strlcpy(copystr, port->val, 16);

            start = atoi(copystr);
            end = atoi(strchr(copystr, '-') + 1);
            snprintf(ports_spec, 32, "port == (%d..%d)", start, end);

        } else {
            char temp_ports_spec[32];
            ...
            if (first_iteration) {
                snprintf(ports_spec, 32, "port == %s", port->val);
                first_iteration = false;
            } else {
                snprintf(temp_ports_spec, 32, "%s", ports_spec);
                snprintf(ports_spec, 32, "%s, %s", temp_ports_spec, port->val);
            }
        }
    }

    /* Build the NTPL command */
    snprintf(ntpl_cmd, 64, "assign[streamid=(%d..%d)] = %s", first_stream, last_stream, ports_spec);
    ...
    if ((status = NT_NTPL(hconfig, ntpl_cmd, &ntpl_info, NT_NTPL_PARSER_VALIDATE_NORMAL)) == NT_SUCCESS) {
        *filter_id = ntpl_info.ntplId;
        ...
        SCLogInfo("NTPL filter assignment \"%s\" returned filter id %4d", ntpl_cmd, *filter_id);
    } else {
        NAPATECH_NTPL_ERROR(ntpl_cmd, ntpl_info, status);
        status = ntpl_info.u.errorData.errCode;
        return false;
