...
uint16_t getNumaNode(uint16_t stream_id) {
    int status;
    ...
    numaNode = info.u.streamID.data.numaNode;
...
CurrentStats current_stats[MAX_STREAMS];

CurrentStats getNapatechCurrentStats(uint16_t id) {
...
static bool RegisteredStream(uint16_t stream_id, uint16_t num_registered, NapatechStreamConfig registered_streams[]) {
...
//        stream_config[i].configured = false;
    }
...
//            stream_config[instance_cnt].configured = true;
...
//                stream_config[instance_cnt].configured = true;
    ...
    if ((status = NT_InfoClose(info_stream)) != NT_SUCCESS) {
        NT_ExplainError(status, error_buffer, sizeof (error_buffer) - 1);
        SCLogError(SC_ERR_NAPATECH_STREAMS_REGISTER_FAILED, "NT_InfoClose failed: %s", error_buffer);
        exit(EXIT_FAILURE);
    }
