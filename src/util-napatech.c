 ...
 * \stream_config array of stream configuration structures
 * \total_counters - cumulative count of all packets received.
 * \dispatch_host, - Count of packets that were delivered to the host buffer
 * \dispatch_drop - count of packets that were dropped as a result of a rule
 * \dispatch_fwd - count of packets forwarded out the egress port as the result of a rule
 * \is_inline - are we running in inline mode?
 * \enable_stream_stats - are per thread/stream statistics enabled.
 * \streamCounters - counters for each thread/stream configured.
 *
 */
        ...
        )
{
    ...
    for (stream_cnt = 0; stream_cnt < num_streams; ++stream_cnt) {
        ...
        if (enable_stream_stats) {
			...
			StatsSetUI64(tv, stream_counters[inst_id].drop, current_stats[stream_id].current_drops);
        }
    ...
    StatsSetUI64(tv, total_counters.drop, total_stats.current_drops);
    ...
    StatsSetUI64(tv, dispatch_host.pkts, hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[0].pkts);
    StatsSetUI64(tv, dispatch_host.byte, hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[0].octets);

    if (is_inline) {
        StatsSetUI64(tv, dispatch_drop.pkts, hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[1].pkts
                                            + hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[3].pkts);
        StatsSetUI64(tv, dispatch_drop.byte, hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[1].octets
                                            + hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[3].octets);

    }
    StatsSetUI64(tv, dispatch_fwd.pkts, hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[2].pkts
                                        + hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[4].pkts);
    StatsSetUI64(tv, dispatch_fwd.byte, hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[2].octets
                                        + hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[4].octets);

    total_stats.current_packets = hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[0].pkts
                                + hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[1].pkts
                                + hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[2].pkts
                                + hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[3].pkts;

    StatsSetUI64(tv, total_counters.pkts, total_stats.current_packets);

    total_stats.current_bytes = hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[0].octets
                              + hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[1].octets
                              + hStat.u.query_v3.data.adapter.aAdapters[0].color.aColor[2].octets;
...
static void *NapatechStatsLoop(void *arg)
{
    ...
    total_counters.drop = StatsRegisterCounter("napa_total.overflow_drop", tv);
    ...
    if (enable_stream_stats) {
		for (int i = 0; i < stream_cnt; ++i) {
			...
			stream_counters[i].drop = StatsRegisterCounter(drop_buf, tv);
		}
    }
	...
	StatsSetUI64(tv, total_counters.drop, 0);
    ...
    if (bypass_supported) {
		...
		if (enable_stream_stats) {
			for (int i = 0; i < stream_cnt; ++i) {
				...
				StatsSetUI64(tv, stream_counters[i].drop, 0);
			}
		}
...
#define PORTS_SPEC_SIZE 64
    ...
    if (NapatechUseHWBypass()) {
        SCLogInfo("Listening on the following Napatech port-pairs:");
    }
    ...
    TAILQ_FOREACH(port, &ntports->head, next)
    {
        ...
        if (NapatechUseHWBypass()) {
        	...
        	if (strchr(port->val, '-')) {
				...
				ports_spec.first[iteration] = atoi(copystr);
				ports_spec.second[iteration] = atoi(strchr(copystr, '-') + 1);
			    ...
			    SCLogInfo("    %d - %d", ports_spec.first[iteration], ports_spec.second[iteration]);
				...
				SCLogError(SC_ERR_NAPATECH_PARSE_CONFIG,
						"When using hardware flow bypass ports must be specified as segment port-pairs. E.g. ports: [0-1, 0-2]");
				exit(EXIT_FAILURE);
            }
#endif
			...
			if (strncmp(port->val, "all", 3) == 0) {
				...
				ports_spec.first[iteration] = ports_spec.second[iteration] = atoi(port->val);
    ...
    if (bypass_supported) {
		...
		char ports_a[64];
		char ports_b[64];
	    memset(ports_a, 0, sizeof(ports_a));
	    memset(ports_b, 0, sizeof(ports_b));

//	    if (is_inline) {
			for (pair = 0; pair < iteration; ++pair) {
				char port_str[4];

				snprintf(port_str, sizeof(port_str), "%s%u ", pair == 0 ? "" : ",", ports_spec.first[pair]);
				strlcat(ports_a, port_str, sizeof(ports_a));

				snprintf(port_str, sizeof(port_str), "%s%u ", pair == 0 ? "" : ",", ports_spec.second[pair]);
				strlcat(ports_b, port_str, sizeof(ports_b));
			}

		/* This is the assign for dropping traffic */
		snprintf(ntpl_cmd, sizeof (ntpl_cmd),
		...
		NapatechSetFilter(hconfig, ntpl_cmd);
		 ...
		 snprintf(ntpl_cmd, sizeof (ntpl_cmd),
				"assign[priority=1;streamid=drop;colormask=0x3]=(Layer3Protocol==IPV6)and(port==%s)and(Key(KDEF%u,KeyID=%u)==%u)",
				ports_a,
				NAPATECH_KEYTYPE_IPV6,
				NAPATECH_KEYTYPE_IPV6,
				NAPATECH_FLOWTYPE_DROP);
        ...
        if (strcmp(ports_a, ports_b) != 0) {
            snprintf(ntpl_cmd, sizeof (ntpl_cmd),
                    "assign[priority=1;streamid=drop;colormask=0x3]=(Layer3Protocol==IPV6)and(port==%s)and(Key(KDEF%u,KeyID=%u,fieldaction=swap)==%u)",
                    ports_b,
                    NAPATECH_KEYTYPE_IPV6,
                    NAPATECH_KEYTYPE_IPV6,
                    NAPATECH_FLOWTYPE_DROP);
