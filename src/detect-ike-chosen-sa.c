...
#define PARSE_REGEX                                                                                \
    "^\\s*(encryption_algorithm|hash_algorithm|authentication_method|group_description|\
...
static int DetectEngineInspectIkeChosenSaGeneric(ThreadVars *tv, DetectEngineCtx *de_ctx,
...
void DetectIkeChosenSaRegister(void)
{
    ...
    DetectAppLayerInspectEngineRegister("ike.chosen_sa_attribute", ALPROTO_IKE, SIG_FLAG_TOCLIENT,
            1, DetectEngineInspectIkeChosenSaGeneric);
...
static int IKEChosenSaParserTest(void)
{
    ...
    FAIL_IF(strcmp(de->sa_type, "hash_algorithm") != 0);
