...
static int DetectEngineInspectIkeChosenSaGeneric(ThreadVars *tv,
        DetectEngineCtx *de_ctx, DetectEngineThreadCtx *det_ctx,
        const Signature *s, const SigMatchData *smd,
        Flow *f, uint8_t flags, void *alstate,
        void *txv, uint64_t tx_id);

static int DetectIkeChosenSaMatch (DetectEngineThreadCtx *, Flow *,
...
void DetectIkeChosenSaRegister (void)
{
    ...
    sigmatch_table[DETECT_AL_IKE_CHOSEN_SA].url = "/rules/ike-keywords.html#ike-chosen_sa_attribute";
    ...
    DetectAppLayerInspectEngineRegister("ike.chosen_sa_attribute",
            ALPROTO_IKE, SIG_FLAG_TOCLIENT, 2,
            DetectEngineInspectIkeChosenSaGeneric);
 ...
 * \retval 1 match.
 */
static int DetectIkeChosenSaMatch(DetectEngineThreadCtx *det_ctx,
...
static DetectIkeChosenSaData *DetectIkeChosenSaParse (const char *rawstr)
{
    ...
    if (ret < 3 || ret > 5) {
        SCLogError(SC_ERR_PCRE_MATCH, "pcre match for ike.chosen_sa_attribute failed, should be: <sa_attribute>=<type>, but was: %s; error code %d", rawstr, ret);
        goto error;
    }
...
static int IKEChosenSaParserTest (void)
{
    ...
    de = DetectIkeChosenSaParse("hash_algorithm=2");
