...
static AppLayerReturn SMBTCPParseRequest(Flow *f, void *state,
        ...
        void *local_data, const uint8_t flags)
{
    ...
    if (input == NULL && input_len > 0) {
        ...
        AppLayerReturn res = rs_smb_parse_request_tcp(f, state, pstate,
...
static AppLayerReturn SMBTCPParseResponse(Flow *f, void *state,
        AppLayerParserState *pstate, const uint8_t *input, uint32_t input_len,
        void *local_data, const uint8_t flags)
{
    ...
    if (input == NULL && input_len > 0) {
        ...
        AppLayerReturn res = rs_smb_parse_response_tcp(f, state, pstate,
                input, input_len, local_data, flags);
        SCReturnStruct(res);
    }
}
