                            ...
                            uint32_t decompress_depth, uint32_t compress_depth)
{
    ...
    if (compressed_data == NULL) {
        SCLogError(SC_ERR_MEM_ALLOC, "Can't allocate memory for compressed_data");
        exit(EXIT_FAILURE);
    }
    ...
    if (decompressed_data == NULL) {
        SCLogError(SC_ERR_MEM_ALLOC, "Can't allocate memory for decompressed_data");
        exit(EXIT_FAILURE);
    }
        ...
        compression_type == FILE_FLASH_ZLIB_COMPRESSION)
    {
        ...
        /* To prevent overflow during decompression, it's better to pass
         * decompressed_swf_len instead of decompressed_data_len. */
        r = FileDecompressFlashZlibData(compressed_data, compressed_data_len,
                                        decompressed_data, decompressed_swf_len);
               ...
               compression_type == FILE_FLASH_LZMA_COMPRESSION)
    {
        ...
        ptmp = SCRealloc(compressed_data, compressed_data_len);
        if (ptmp == NULL) {
            SCFree(compressed_data);
            compressed_data = NULL;
            SCLogError(SC_ERR_MEM_ALLOC, "Can't realloc memory for compressed_data");
            exit(EXIT_FAILURE);
        }
        compressed_data = ptmp;
        /* put lzma properties */
        memcpy(compressed_data, buf + 12, 5);
        /* put lzma end marker */
        memset(compressed_data + 5, 0xFF, 8);
        /* put compressed data */
        memcpy(compressed_data + 13, buf + offset, compressed_data_len - 13);
        /* To prevent overflow during decompression, it's better to pass
         * decompressed_swf_len instead of decompressed_data_len. */
        r = FileDecompressFlashLzmaData(compressed_data, compressed_data_len,
                                        decompressed_data, decompressed_swf_len);
        if (r == FILE_FLASH_DECOMP_NOK)
            goto out;
    } else {
    ...
    if (new_buffer == NULL) {
        SCLogError(SC_ERR_MEM_ALLOC, "Can't alloc memory for the buffer");
        exit(EXIT_FAILURE);
    }
