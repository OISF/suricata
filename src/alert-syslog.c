...
static TmEcode AlertSyslogIPv6(ThreadVars *tv, const Packet *p, void *data)
{
    ...
    PrintInetIPv6(
            (const void *)GET_IPV6_SRC_ADDR(p), srcip, sizeof(srcip), ast->file_ctx->shorten_ipv6);
    PrintInetIPv6(
            (const void *)GET_IPV6_DST_ADDR(p), dstip, sizeof(dstip), ast->file_ctx->shorten_ipv6);
