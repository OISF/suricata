...
static int GetODPNumThreads(const char *iface_name)
{
    ...
    if (node == NULL) {
        ...
        ret = ConfGetChildValueWithDefault(if_root, if_default, "threads", &threads_str);
	if (ret != 0) {
                max_workers = atol(threads_str);
    ...
    SCLogInfo("No odp.threads in config, using 1 ODP worker thread per interface");
...
static void *ParseODPConfig(const char *iface_name)
{
    ...
    if (ODP_PKTIO_INVALID == pktio) {
        SCLogError(SC_ERR_RUNMODE, "Error: pktio create failed for %s", iface_name);
	SCFree(conf);
        return NULL;
    }
    ...
    if (odp_pktin_queue_config(pktio, &pktin_param)) {
        SCLogError(SC_ERR_RUNMODE, "Error: queue_config failed for %s", iface_name);
    ...
    if (odp_pktio_start(pktio)) {
        SCLogError(SC_ERR_RUNMODE, "Error: unable to start pktio for %s", iface_name);
	odp_pktio_close(pktio);
	SCFree(conf);
        return NULL;
    }
...
int RunModeIdsODPWorkers(void)
{
    ...
    SCLogInfo("Using odp.interface=%s", live_dev);
