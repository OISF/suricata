 ...
 *
 * Implements support for tls_cert keyword.
 */
...
/**
 * \brief Registration function for keyword: tls_cert
 */
void DetectTlsCertRegister(void)
{
    ...
    sigmatch_table[DETECT_AL_TLS_CERT].Free = NULL;
    ...
    DetectAppLayerInspectEngineRegister2("tls_cert", ALPROTO_TLS,
            SIG_FLAG_TOCLIENT, TLS_STATE_CERT_READY,
            DetectEngineInspectBufferGeneric, GetData);

    DetectAppLayerMpmRegister2("tls_cert", SIG_FLAG_TOCLIENT, 2,
            PrefilterGenericMpmRegister, GetData, ALPROTO_TLS,
            TLS_STATE_CERT_READY);

    DetectBufferTypeSetDescriptionByName("tls_cert", "TLS certificate");

    g_tls_cert_buffer_id = DetectBufferTypeGetByName("tls_cert");
}

/**
 * \brief This function setup the tls_cert modifier keyword
 *
...
/**
 * \test Test that a signature containing tls_cert is correctly parsed
 *       and that the keyword is registred.
 */
static int DetectTlsCertTest01(void)
{
    ...
    sm = de_ctx->sig_list->sm_lists[g_tls_cert_buffer_id];
    FAIL_IF_NULL(sm);
 ...
 */
static int DetectTlsCertTest02(void)
{
    /* client hello */
...
{
#ifdef UNITTESTS
    UtRegisterTest("DetectTlsCertTest01", DetectTlsCertTest01);
    UtRegisterTest("DetectTlsCertTest02", DetectTlsCertTest02);
#endif /* UNITTESTS */
}
