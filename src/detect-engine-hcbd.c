...
/**
 */
static uint8_t *DetectEngineHCBDGetBufferForTX(uint64_t tx_id,
                                               DetectEngineCtx *de_ctx,
                                               DetectEngineThreadCtx *det_ctx,
                                               Flow *f, HtpState *htp_state,
                                               uint8_t flags,
                                               uint32_t *buffer_len, uint8_t *status)
{
    *status = APP_STAGE_NOT_DONE;
    ...
    *buffer_len = det_ctx->hcbd[index].buffer_len;
    ...
    if (AppLayerGetProgress(ALPROTO_HTTP, tx, 0) > TX_PROGRESS_REQ_BODY)
        *status = APP_STAGE_DONE;
    return buffer;
}
