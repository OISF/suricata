...
static int GetIPInformations(Packet *p, char* srcip, Port* sp, char* dstip, Port* dp, int ipproto)
{
    if ((PKT_IS_TOSERVER(p))) {
        switch (ipproto) {
            case AF_INET:
                PrintInet(AF_INET, (const void *) GET_IPV4_SRC_ADDR_PTR(p), srcip, 46);
                PrintInet(AF_INET, (const void *) GET_IPV4_DST_ADDR_PTR(p), dstip, 46);
                break;
            case AF_INET6:
                PrintInet(AF_INET6, (const void *) GET_IPV6_SRC_ADDR(p), srcip, 46);
                PrintInet(AF_INET6, (const void *) GET_IPV6_DST_ADDR(p), dstip, 46);
                break;
            default:
                return 0;
        }
        ...
        *dp = p->dp;
        ...
        switch (ipproto) {
            case AF_INET:
                PrintInet(AF_INET, (const void *) GET_IPV4_DST_ADDR_PTR(p), srcip, 46);
                PrintInet(AF_INET, (const void *) GET_IPV4_SRC_ADDR_PTR(p), dstip, 46);
                break;
            case AF_INET6:
                PrintInet(AF_INET6, (const void *) GET_IPV6_DST_ADDR(p), srcip, 46);
                PrintInet(AF_INET6, (const void *) GET_IPV6_SRC_ADDR(p), dstip, 46);
                break;
            default:
                return 0;
        }
...
static void LogTlsLogPem(Packet *p, SSLState *state, LogTlsFileCtx *log, int ipproto)
{
#define PEMHEADER "-----BEGIN CERTIFICATE-----\n"
#define PEMFOOTER "-----END CERTIFICATE-----\n"
    //Loging pem certificate
    ...
    fp = fopen(filename, "w");
    if (fp != NULL) {
        char srcip[46], dstip[46], timebuf[64];
        Port sp, dp;
        CreateTimeString(&p->ts, timebuf, sizeof(timebuf));
        if(!GetIPInformations(p, srcip, &sp, dstip, &dp, ipproto))
            goto end_fp;
        fprintf(fp, "TIME:              %s\n", timebuf);
        if (p->pcap_cnt > 0) {
        ...
        fprintf(fp, "TLS SUBJECT:       %s\n"
                    "TLS ISSUERDN:      %s\n"
                    "TLS FINGERPRINT:   %s\n",
                state->curr_connp->cert0_subject,
                state->curr_connp->cert0_issuerdn,
                state->curr_connp->cert0_fingerprint);
...
static TmEcode LogTlsLogIPWrapper(ThreadVars *tv, Packet *p, void *data, PacketQueue *pq, PacketQueue *postpq, int ipproto)
{
    ...
    if (ssl_state->curr_connp->cert_log_flag & SSL_TLS_LOG_PEM) {
        LogTlsLogPem(p, ssl_state, hlog, ipproto);
    }
