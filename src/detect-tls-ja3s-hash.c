 ...
 *
 * Implements support for ja3s_hash keyword.
 */
...
/**
 * \brief Registration function for keyword: ja3s_hash
 */
void DetectTlsJa3SHashRegister(void)
{
    sigmatch_table[DETECT_AL_TLS_JA3S_HASH].name = "ja3s_hash";
    sigmatch_table[DETECT_AL_TLS_JA3S_HASH].desc = "content modifier to match the JA3S hash buffer";
    sigmatch_table[DETECT_AL_TLS_JA3S_HASH].url = DOC_URL DOC_VERSION "/rules/ja3-keywords.html#ja3s_hash";
    sigmatch_table[DETECT_AL_TLS_JA3S_HASH].Match = NULL;
    sigmatch_table[DETECT_AL_TLS_JA3S_HASH].Setup = DetectTlsJa3SHashSetup;
    sigmatch_table[DETECT_AL_TLS_JA3S_HASH].Free = NULL;
    ...
    DetectAppLayerInspectEngineRegister2("ja3s_hash", ALPROTO_TLS, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectBufferGeneric, GetData);

    DetectAppLayerMpmRegister2("ja3s_hash", SIG_FLAG_TOCLIENT, 2,
            PrefilterGenericMpmRegister, GetData, ALPROTO_TLS, 0);

    DetectBufferTypeSetDescriptionByName("ja3s_hash", "TLS JA3S hash");

    DetectBufferTypeRegisterSetupCallback("ja3s_hash",
            DetectTlsJa3SHashSetupCallback);

    DetectBufferTypeRegisterValidateCallback("ja3s_hash",
            DetectTlsJa3SHashValidateCallback);

    g_tls_ja3s_hash_buffer_id = DetectBufferTypeGetByName("ja3s_hash");
}

/**
 * \brief this function setup the ja3s_hash modifier keyword used in the rule
 *
...
{
    DetectBufferSetActiveList(s, g_tls_ja3s_hash_buffer_id);
                                               ...
                                               const char **sigerror)
{
    ...
    for ( ; sm != NULL; sm = sm->next)
    {
        ...
        if (cd->flags & DETECT_CONTENT_NOCASE) {
            *sigerror = "ja3s_hash should not be used together with "
