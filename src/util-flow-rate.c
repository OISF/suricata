...
void FlowRateRegisterFlowStorage(void)
{
    ...
    track_flow = ConfNodeLookupChild(root, "rate-tracking") != NULL ? true : false;
    if (!track_flow)
        return;

    ConfNode *node = ConfGetNode("flow.rate-tracking");
...
FlowRateStore *FlowRateStoreInit(void)
{
    ...
    for (int i = 0; i < 2; i++) {
        frs->dir[i].size = flow_rate_config.interval.secs;
...
static inline void FlowRateClearSumTill(
...
void FlowRateStoreUpdate(FlowRateStore *frs, SCTime_t p_ts, uint32_t pkt_len, int direction)
{
    ...
    if ((frs->dir[direction].last_idx == frs->dir[direction].size - 1) &&
        ...
        if (idx != 0) {
            /* Update the sum */
            FlowRateClearSumTill(frs, 0, idx, direction);
    ...
    if (p_ts.secs - start_ts.secs < frs->dir[direction].size) {
        SCLogDebug("Packet in the ts size window");
        if (idx > frs->dir[direction].last_idx + 1) {
            ...
            SCLogDebug("Revisiting the ring after a full round");
        ...
        SCLogDebug("Packet outside the ring buffer size window");
    ...
    SCLogDebug("Byte sum for dir %d: %" PRIu64, direction, frs->dir[direction].sum);
    /* In any case, update the last seen timestamp */
    frs->dir[direction].last_ts = p_ts;
}

bool FlowRateIsExceeding(ThreadVars *tv, DecodeThreadVars *dtv, FlowRateStore *frs, int direction)
{
