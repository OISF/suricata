...
/* HTTP */
static int PrefilterMpmHTTPFiledataRegister(DetectEngineCtx *de_ctx,
...
static inline InspectionBuffer *HttpServerBodyXformsGetDataCallback(
        DetectEngineThreadCtx *det_ctx,
        const DetectEngineTransforms *transforms,
        const int list_id, InspectionBuffer *base_buffer)
{
...
static InspectionBuffer *HttpServerBodyGetDataCallback(DetectEngineThreadCtx *det_ctx,
        ...
        void *txv, const int list_id, const int base_id)
{
    SCEnter();
...
static int DetectEngineInspectBufferHttpBody(DetectEngineCtx *de_ctx,
        ...
        const Signature *s, Flow *f, uint8_t flags, void *alstate, void *txv, uint64_t tx_id)
{
    bool eof = (AppLayerParserGetStateProgress(f->proto, f->alproto, txv, flags) > engine->progress);
    const InspectionBuffer *buffer = HttpServerBodyGetDataCallback(det_ctx,
            engine->v2.transforms, f, flags, txv, engine->sm_list, engine->sm_list_base);
 ...
 */
static void PrefilterTxHTTPFiledata(DetectEngineThreadCtx *det_ctx,
        const void *pectx,
        ...
        const uint64_t idx, const uint8_t flags)
{
    ...
    InspectionBuffer *buffer = HttpServerBodyGetDataCallback(det_ctx,
            ctx->transforms, f, flags, txv, list_id, ctx->base_list_id);
    if (buffer == NULL)
        return;
        ...
        const DetectBufferMpmRegistery *mpm_reg, int list_id)
{
    ...
    return PrefilterAppendTxEngine(de_ctx, sgh, PrefilterTxHTTPFiledata,
...
static inline InspectionBuffer *FiledataWithXformsGetDataCallback(
        DetectEngineThreadCtx *det_ctx,
        const DetectEngineTransforms *transforms,
        const int list_id, int local_file_id, InspectionBuffer *base_buffer, const bool first)
{
...
static InspectionBuffer *FiledataGetDataCallback(
        DetectEngineThreadCtx *det_ctx,
        const DetectEngineTransforms *transforms,
...
{
    SCEnter();
    SCLogDebug("starting: list_id %d base_id %d first %s", list_id, base_id, first?"true":"false");
    ...
    if (base_id != list_id && buffer->inspect != NULL) {
        ...
        return FiledataWithXformsGetDataCallback(det_ctx, transforms, list_id, local_file_id, buffer, first);
    }
    if (!first && buffer->inspect != NULL) {
    ...
    if (list_id != base_id) {
        ...
        InspectionBuffer *tbuffer = FiledataWithXformsGetDataCallback(det_ctx, transforms, list_id, local_file_id, buffer, first);
        ...
        Flow *f, uint8_t flags, void *alstate, void *txv, uint64_t tx_id)
{
    ...
    for (; file != NULL; file = file->next) {
        ...
        InspectionBuffer *buffer = FiledataGetDataCallback(det_ctx,
            transforms, f, flags, file, engine->sm_list, engine->sm_list_base, local_file_id, false);
        if (buffer == NULL)
            continue;
        ...
        const uint64_t idx, const uint8_t flags)
{
    ...
    if (ffc != NULL) {
        ...
        for (; file != NULL; file = file->next) {
            ...
            InspectionBuffer *buffer = FiledataGetDataCallback(det_ctx,
                    ctx->transforms, f, flags, file, list_id, ctx->base_list_id, local_file_id, true);
            if (buffer == NULL)
                continue;
