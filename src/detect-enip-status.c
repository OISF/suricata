...
static int DetectEnipStatusSetup(DetectEngineCtx *de_ctx, Signature *s, const char *rulestr)
{
    ...
    sm->type = DETECT_ENIPSTATUS;
    sm->ctx = (void *)du32;

    SigMatchAppendSMToList(s, sm, g_enip_status_id);
    SCReturnInt(0);
}
...
{
    uint32_t status;
    if (!rs_enip_get_status(txv, flags, &status))
        SCReturnInt(0);
...
void DetectEnipStatusRegister(void)
{
    ...
    DetectAppLayerInspectEngineRegister2("enip.status", ALPROTO_ENIP, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectGenericList, NULL);
    DetectAppLayerInspectEngineRegister2("enip.status", ALPROTO_ENIP, SIG_FLAG_TOCLIENT, 0,
            DetectEngineInspectGenericList, NULL);
