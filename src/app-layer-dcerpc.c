...
static uint16_t DCERPCTCPProbe(Flow *f, uint8_t direction,
...
static int DCERPCRegisterPatternsForProtocolDetection(void)
{
    if (AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_DCERPC,
                "|05 00|", 2, 0, STREAM_TOSERVER, DCERPCTCPProbe, 0, 0) < 0)
    {
        return -1;
    }
    if (AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_DCERPC,
                "|05 00|", 2, 0, STREAM_TOCLIENT, DCERPCTCPProbe, 0, 0) < 0) // TODO min and max depth set to zerp, is this right?
    {
        return -1;
    }
