...
port-pairs. With traffic recieved from one port it is transmitted out the 
...
- napa_total.pkts - The total of packets recieved by the card.

- napa_total.byte - The total count of bytes received by the card.
 
- napa_total.overflow_drop - The number of packets that were dropped because 
...
- napa_bypass.active_flows: 
  ...
  The number of flows actively programed on the hardware to be forwared or dropped.

- napa_bypass.total_flows:

  The total count of flows programed since the application started. 
...
- napa<streamid>.pkts: The number of packets recieved by the stream.

- napa<streamid>.bytes: The total bytes received by the stream.

- napa<streamid>.drop: The number of packets dropped from this stream due to buffer overflow conditions.
...
These are the Napatech options available in the suricata configuration file::
  ...
  napatech:
    ...
    # One of the ports will recieve inbound traffic for the network and the other 
