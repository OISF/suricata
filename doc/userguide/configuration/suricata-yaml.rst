...
RFB can have some strings whose maximum length according to the RFC is 4GB.
...
An event ``rfb.too_long_string`` will be emitted when a string exceeds the limit. The default is 4 KB.

::

      rfb:
        max-string-length: 4 KB
