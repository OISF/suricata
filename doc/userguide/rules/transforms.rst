...
and the original buffer content is preserved. This means content keywords following a failed
``pcrexform`` match against the original (un-transformed) data, providing pass-through semantics.
``absent: error_or`` or ``absent: must_error`` can be used to detect when the pattern fails;
``absent: must_succeed`` can be used to ensure content keywords only run against successfully-transformed data.
...
.. note:: When ``from_base64`` encounters data it cannot decode (e.g., in ``strict`` mode
   ...
   and the original buffer content is preserved. Content keywords following a failed decode
   therefore match against the original (un-decoded) data. ``absent: error_or`` or
   ``absent: must_error`` can be used to detect when a decode fails; ``absent: must_succeed``
   can be used to ensure content keywords only run against successfully-decoded data.
