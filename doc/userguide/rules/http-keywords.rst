...
The ``http.host.raw`` buffer matches on HTTP host name content but does not
have more than the standard normalization performed on the buffer contents.
(see :ref:`http.host`)
...
of a HTTP1 or HTTP2 request.

It is possible to use any of the :doc:`payload-keywords` with the
``http.request_header`` keyword.

For HTTP2, the header name and value get concatenated by ": " (colon and space).
The colon and space are commonly noted with the hexadecimal format `|3a 20|`
within signatures.

To detect if a http2 header name contains a ":" (colon), the keyword
:ref:`http2.header_name` can be used.

Example HTTP1 Request::
...
The body of a HTTP responses can be very large, therefore the response body is
...
.. note:: If a HTTP body is using gzip or deflate, ``file.data`` will match on
