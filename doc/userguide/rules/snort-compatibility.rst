...
and Snort and that apply to rules and rule writing.
...
-  Snort will normalize to a single space between the colon at the end of
   the header name and header value if there is at least one space
   character (0x20 only so not 0x90) immediately after the colon.  This means
   that, unlike Suricata, if there is no space after the colon before the
   header value, the content of the header line will remain unchanged in
...
-  HTTP file extraction added in Suricata version 1.2; SMTP file extraction added in Suricata version 3.0.
