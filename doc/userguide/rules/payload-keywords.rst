...
.. note:: The ``error_or`` option requires the buffer to have a transform that can fail
   (e.g., ``from_base64``). Using ``error_or`` on a buffer without such a transform will
   cause a rule loading error.
