...
.. note:: this list isn't fully comprehensive, in the sense that each item has
   ...
   priority is write (highest) > write + read > read (lowest).
...
.. container:: example-rule
    ...
    pass tls $HOME_NET any -> $EXTERNAL_NET any (tls.sni; content:"some.\
    content"; startswith; nocase; endswith; msg:"matching TLS allowed";\
    priority:1; flow:to_server, established; content:"hypothetical"; sid:5;\
    rev:1; gid:10000003;)
...
+--------------------+-------------------+
| Signature ID (sid) | Internal ID (iid) |
+====================+===================+
|  2                 |  0                |
+--------------------+-------------------+
|  5                 |  1                |
...
Some other notes regarding these rules and their assigned internal priority (iid):

  - For the sake of illustrating, rules with sids 1 and 4 are identical, but for
    their sids. In such a case, the internal priority is based on sid, so the
    rule with lower sid will have the higher internal priority.
  - Signatures with sids 2 and 5 are another example: sid 5 ends up with a "lower"
    priority, in this case, because its sid comes after sid 2. ``content`` alone
    doesn't affect a rule's internal priority.
