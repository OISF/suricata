...
const char *RunModeIdsPfringPluginGetDefaultMode(void)
{
    return "workers";
}

void RunModeIdsPfringPluginRegister(int slot)
{
    RunModeRegisterNewRunMode(slot, "autofp",
                              ...
                              "detect thread",
                              RunModeIdsPfringPluginAutoFp);
    RunModeRegisterNewRunMode(slot, "single",
                              "Single threaded pfring mode",
                              RunModeIdsPfringPluginSingle);
    RunModeRegisterNewRunMode(slot, "workers",
                              "Workers pfring mode, each thread does all"
                              " tasks from acquisition to logging",
                              RunModeIdsPfringPluginWorkers);
    return;
}

static void PfringPluginDerefConfig(void *conf)
{
    PfringPluginIfaceConfig *pfp = (PfringPluginIfaceConfig *)conf;
 ...
 *
 * \return a PfringPluginIfaceConfig corresponding to the interface name
 */
static void *OldParsePfringPluginConfig(const char *iface)
{
    const char *threadsstr = NULL;
    PfringPluginIfaceConfig *pfconf = SCMalloc(sizeof(*pfconf));
    ...
    pfconf->DerefFunc = PfringPluginDerefConfig;
 ...
 * \return a PfringPluginIfaceConfig corresponding to the interface name
 */
static void *ParsePfringPluginConfig(const char *iface)
{
    const char *threadsstr = NULL;
    ConfNode *if_root;
    ...
    memset(pfconf, 0, sizeof(PfringPluginIfaceConfig));
    strlcpy(pfconf->iface, iface, sizeof(pfconf->iface));
    pfconf->threads = 1;
    pfconf->cluster_id = 1;
...
static int PfringPluginConfigGetThreadsCount(void *conf)
{
    PfringPluginIfaceConfig *pfp = (PfringPluginIfaceConfig *)conf;
    return pfp->threads;
}
...
static int GetDevAndParser(const char **live_dev, ConfigIfaceParserFunc *parser)
{
    ...
    if (PfringConfLevel() > PFRING_CONF_V1) {
        *parser = ParsePfringPluginConfig;
    } else {
        SCLogInfo("Using 1.0 style configuration for pfring");
        *parser = OldParsePfringPluginConfig;
...
int RunModeIdsPfringPluginAutoFp(void)
{
    SCEnter();
...
/* We include only if pfring is enabled */
    ...
    ret = RunModeSetLiveCaptureAutoFp(tparser,
                              PfringPluginConfigGetThreadsCount,
                              "ReceivePfringPlugin",
                              "DecodePfringPlugin", thread_name_autofp,
                              live_dev);
    if (ret != 0) {
    ...
    SCLogInfo("RunModeIdsPfringPluginAutoFp initialised");

    return 0;
}

int RunModeIdsPfringPluginSingle(void)
{
    SCEnter();
...
/* We include only if pfring is enabled */
    ...
    ret = RunModeSetLiveCaptureSingle(tparser,
                              PfringPluginConfigGetThreadsCount,
                              "ReceivePfringPlugin",
                              "DecodePfringPlugin", thread_name_single,
                              live_dev);
    if (ret != 0) {
        SCLogError(SC_ERR_RUNMODE, "Runmode start failed");
        exit(EXIT_FAILURE);
    ...
    SCLogInfo("RunModeIdsPfringPluginSingle initialised");

    return 0;
}

int RunModeIdsPfringPluginWorkers(void)
{
    SCEnter();
...
/* We include only if pfring is enabled */
    ...
    ret = RunModeSetLiveCaptureWorkers(tparser,
                              PfringPluginConfigGetThreadsCount,
                              "ReceivePfringPlugin",
                              "DecodePfringPlugin", thread_name_workers,
                              live_dev);
    if (ret != 0) {
        SCLogError(SC_ERR_RUNMODE, "Runmode start failed");
        exit(EXIT_FAILURE);
    ...
    SCLogInfo("RunModeIdsPfringPluginWorkers initialised");
