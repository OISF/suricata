...
// Addition of JsonBuilderWrapper to look like a rust app-layer
// even if we must use C API for JsonBuilder (because of its rust repr)

use super::template::TemplateTransaction;
use std::ffi::CString;
use suricata::cast_pointer;
use suricata::jsonbuilder::JsonError;
use suricata_sys::jsonbuilder::{JsonBuilder, jb_set_string, jb_close, jb_open_object};

use std;

// syntax sugar around C API of JsonBuilder to feel like a normal app-layer in log_template
pub struct JsonBuilderWrapper {
    inner: *mut JsonBuilder,
}

impl JsonBuilderWrapper {
    fn close(&mut self) -> Result<(), JsonError> {
        if unsafe { !jb_close(self.inner) } {
    ...
    fn open_object(&mut self, key: &str) -> Result<(), JsonError> {
        ...
        if unsafe { !jb_open_object(self.inner, keyc.as_ptr()) } {
            return Err(JsonError::Memory);
        }
        Ok(())
    }
    fn set_string(&mut self, key: &str, val: &str) -> Result<(), JsonError> {
        ...
        if unsafe { !jb_set_string(self.inner, keyc.as_ptr(), valc.as_ptr()) } {
            return Err(JsonError::Memory);
        }
        Ok(())
    }
...
fn log_template(tx: &TemplateTransaction, js: &mut JsonBuilderWrapper) -> Result<(), JsonError> {
...
pub(super) unsafe extern "C" fn template_logger_log(
    ...
    let mut js = JsonBuilderWrapper{inner: js};
