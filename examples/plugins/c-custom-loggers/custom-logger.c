...
static int CustomFlowLogger(ThreadVars *tv, void *thread_data, Flow *f)
{
    ...
    if ((SCFlowGetFlags(f) & FLOW_DIR_REVERSED) == 0) {
        if (SCFlowIsIPv4(f)) {
            PrintInet(AF_INET, (const void *)&(SCFlowGetSourceAddressPtr(f)->addr_data32[0]),
                    src_ip, sizeof(src_ip));
            PrintInet(AF_INET, (const void *)&(SCFlowGetDestinationAddressPtr(f)->addr_data32[0]),
                    dst_ip, sizeof(dst_ip));
        } else if (SCFlowIsIPv6(f)) {
            PrintInet(AF_INET6, (const void *)&(SCFlowGetSourceAddressPtr(f)->address), src_ip,
                    sizeof(src_ip));
            PrintInet(AF_INET6, (const void *)&(SCFlowGetDestinationAddressPtr(f)->address), dst_ip,
                    sizeof(dst_ip));
        }
        ...
        if (SCFlowIsIPv4(f)) {
            ...
            PrintInet(AF_INET6, (const void *)&(SCFlowGetDestinationAddressPtr(f)->address), src_ip,
                    sizeof(src_ip));
            PrintInet(AF_INET6, (const void *)&(SCFlowGetSourceAddressPtr(f)->address), dst_ip,
                    sizeof(dst_ip));
        }
