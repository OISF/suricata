...
static int CustomFlowLogger(ThreadVars *tv, void *thread_data, Flow *f)
{
    ...
    if ((SCFlowGetFlags(f) & FLOW_DIR_REVERSED) == 0) {
        if (SCFlowIsIPv4(f)) {
            PrintInet(AF_INET, SCFlowGetSourceAddress(f), src_ip, sizeof(src_ip));
            PrintInet(AF_INET, SCFlowGetDestinationAddress(f), dst_ip, sizeof(dst_ip));
        } else if (SCFlowIsIPv6(f)) {
            PrintInet(AF_INET6, SCFlowGetSourceAddress(f), src_ip, sizeof(src_ip));
            PrintInet(AF_INET6, SCFlowGetDestinationAddress(f), dst_ip, sizeof(dst_ip));
        }
        sp = SCFlowGetSourcePort(f);
        dp = SCFlowGetDestinationPort(f);
    } else {
        if (SCFlowIsIPv4(f)) {
            PrintInet(AF_INET, SCFlowGetDestinationAddress(f), src_ip, sizeof(src_ip));
            PrintInet(AF_INET, SCFlowGetSourceAddress(f), dst_ip, sizeof(dst_ip));
        } else if (SCFlowIsIPv6(f)) {
            PrintInet(AF_INET6, SCFlowGetDestinationAddress(f), src_ip, sizeof(src_ip));
            PrintInet(AF_INET6, SCFlowGetSourceAddress(f), dst_ip, sizeof(dst_ip));
        }
