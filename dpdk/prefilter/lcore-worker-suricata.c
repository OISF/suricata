...
static void PktsReceiveIPS(struct lcore_values *lv)
{
    ...
    pkts_to_inspect_cnt2 =
            MbufsBypassSort(pkts + pkt_count1, pkt_count2, &pkts_to_inspect[pkts_to_inspect_cnt1],
                    &lv->fke_arr[pkts_to_inspect_cnt1], lv->tmp_ring_bufs, lv->rings_cnt, false);
    ...
    pkts_to_bypass_cnt2 = PktsBypassSort(&pkts_to_inspect[pkts_to_inspect_cnt1],
            ...
            &lv->fke_arr[pkts_to_inspect_cnt1], false);
...
void ThreadSuricataExitStats(struct lcore_values *lv)
{
    ...
    Log().info("Lcore %d MSGS: received %lu sent %lu failed msg enqueues %lu mempool putbacks %lu "
               "adds %lu soft deletes %lu evicts %lu updates %lu not found %lu",
