...
#[no_mangle]
pub unsafe extern "C" fn SCSha256Update(hasher: &mut SCSha256, bytes: *const u8, len: u32, bytes_hashed: *mut u32) {
...
#[no_mangle]
pub unsafe extern "C" fn SCSha1Update(hasher: &mut SCSha1, bytes: *const u8, len: u32, bytes_hashed: *mut u32) {
    update(&mut hasher.0, bytes, len, bytes_hashed);
}
...
#[no_mangle]
pub unsafe extern "C" fn SCMd5Update(hasher: &mut SCMd5, bytes: *const u8, len: u32, bytes_hashed: *mut u32) {
    update(&mut hasher.0, bytes, len, bytes_hashed);
}
...
unsafe fn update<D: Digest>(digest: &mut D, bytes: *const u8, mut len: u32, bytes_hashed: *mut u32) {
    if g_hash_byte_limit > 0 && !bytes_hashed.is_null() {
        let bytes_remaining = g_hash_byte_limit - *bytes_hashed;
