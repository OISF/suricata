...
#[no_mangle]
pub unsafe extern "C" fn register_ntp_parsers() {
    ...
    if AppLayerProtoDetectConfProtoDetectionEnabled(ip_proto_str.as_ptr(), parser.name) != 0 {
        ...
        if alproto == ALPROTO_FAILED || alproto == ALPROTO_UNKNOWN {
            SCLogDebug!("Error registering protocol detection for NTP.");
            return;
        }
        // store the allocated ID for the probe function
        ALPROTO_NTP = alproto;
        if AppLayerParserConfParserEnabled(ip_proto_str.as_ptr(), parser.name) != 0 {
            let alproto = AppLayerRegisterParser(&parser);
            if alproto == ALPROTO_FAILED || alproto == ALPROTO_UNKNOWN {
                SCLogDebug!("Error registering parser for NTP.");
                return;
            }
        }
    } else {
