...
impl HandshakeParams {
    ...
    pub fn cipher_suites(&self) -> &[TlsCipherSuiteID] {
        &self.ciphersuites
    }

    pub fn cipher_suites_len(&self) -> usize {
        self.ciphersuites.len()
    }
    ...
    pub fn extensions(&self) -> &[TlsExtensionType] {
        &self.extensions
    }
...
#[no_mangle]
pub unsafe extern "C" fn SCHandshakeAddALPN(hs: &mut HandshakeParams, alpn: *const c_char, len: u16) {
...
// IMPORTANT: When calling this, the user MUST call SCFreeCStringArray() afterwards to free the
// memory
#[no_mangle]
pub unsafe extern "C" fn SCHandshakeGetALPNs(hs: &mut HandshakeParams, out: *mut CStringArray) -> bool {
    if out.is_null() {
        return false;
    }

    if hs.alpns.is_empty() {
        return false;
    }

    let mut data = Vec::with_capacity(hs.alpns.len());
    for alpn in &hs.alpns {
        data.push(CStringData {
            data: alpn.as_ptr(),
            len: alpn.len(),
        });
    }
...
#[no_mangle]
pub unsafe extern "C" fn SCFreeCStringArray(array: CStringArray) {
    if !array.items.is_null() {
        let _ = Vec::from_raw_parts(array.items as *mut CStringData, array.nr, array.nr);
    }
}
...
mod tests {
    ...
    fn test_add_cipher_suite_filters_grease() {
        ...
        assert_eq!(hs.cipher_suites().len(), 0);
    }
    ...
    fn test_add_cipher_suite_accepts_normal() {
        ...
        assert_eq!(hs.cipher_suites(), &[TlsCipherSuiteID(0x1301)]);
    }
    ...
    fn test_add_cipher_suite_len_tracking() {
        ...
        hs.add_cipher_suite(TlsCipherSuiteID(0x1a1a)); // GREASE
        assert_eq!(hs.cipher_suites().len(), 2);
    }
