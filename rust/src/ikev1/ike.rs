...
use crate::ikev1::ikev1::{handle_ikev1, IkeV1Header};
use crate::ikev1::ikev2::{handle_ikev2, IKEV2Event, IKEV2ConnectionState};
...
pub struct IKEV1State {
    tx_id: u64,
    pub transactions: Vec<IKEV1Transaction>,
...
impl IKEV1State{
    ...
    pub fn new_tx(&mut self) -> IKEV1Transaction {
    ...
    fn set_progress(&mut self) {
        ...
        if self.ikev1_container.client_key_exchange.len() > 0 {
            progress = 3;
        }
        if self.ikev1_container.server_key_exchange.len() > 0 {
            progress = 4;
        }

        // todo add again
        if let Some(transaction) = self.get_tx(self.tx_id - 1) {
            if transaction.hdr.ikev1_header.encrypted_payloads {
                // for encrypted payloads we limit progress to 1
                progress = 1;
            }
            else if progress == 1 && transaction.hdr.spi_responder.is_some() {
...
#[no_mangle]
pub extern "C" fn rs_ikev1_state_get_events(tx: *mut std::os::raw::c_void)
...
#[no_mangle]
pub extern "C" fn rs_ikev1_state_get_event_info_by_id(event_id: std::os::raw::c_int,
...
#[no_mangle]
pub extern "C" fn rs_ikev1_state_get_event_info(event_name: *const std::os::raw::c_char,
