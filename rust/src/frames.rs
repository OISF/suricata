...
impl Frame {
    ...
    pub fn set_len(&self, flow: *const Flow, dir: i32, len: i64) {
        unsafe {
            AppLayerFrameSetLengthById(flow, dir, self.frame_id, len);
        };
    }

    pub fn set_tx(&self, flow: *const Flow, dir: i32, tx_id: u64) {
        unsafe {
            AppLayerFrameSetTxIdById(flow, dir, self.frame_id, tx_id);
        };
    }

    pub fn add_event(&self, flow: *const Flow, dir: i32, event: u8) {
        unsafe {
            AppLayerFrameAddEventById(flow, dir, self.frame_id, event);
        };
    }
}
