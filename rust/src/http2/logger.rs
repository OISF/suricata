...
use super::parser;
use crate::dns::log::{rs_dns_do_log_answer, rs_dns_log_json_answer, rs_dns_log_json_query};
    ...
    if tx.dns_request_tx.is_some() || tx.dns_response_tx.is_some() {
        ...
        if let Some(dtx) = &tx.dns_request_tx {
            ...
            for i in 0..0xFFFF {
                ...
                if !rs_dns_log_json_query(dtx, i, 0xFFFFFFFFFFFFFFFF, &mut jsa) {
                    break;
                }
                jsa.close()?;
        ...
        if let Some(dtx) = &tx.dns_response_tx {
            if rs_dns_do_log_answer(dtx, 0xFFFFFFFFFFFFFFFF) {
                // logging at root of dns object
                rs_dns_log_json_answer(dtx, 0xFFFFFFFFFFFFFFFF, js);
            }
        }
        js.close()?; // dns
    }
