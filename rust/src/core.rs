...
pub type SCLogMessageFunc = extern fn(
...
pub type DetectEngineStateFreeFunc = extern fn(state: *mut DetectEngineState);

pub type AppLayerParserTriggerRawStreamReassemblyFunc =
    extern fn(flow: *const Flow, direction: i32);
pub type AppLayerDecoderEventsSetEventRawFunc =
    extern fn(events: *mut *mut AppLayerDecoderEvents, event: u8);

pub type AppLayerDecoderEventsFreeEventsFunc = extern fn(events: *mut *mut AppLayerDecoderEvents);

pub enum SuricataStreamingBufferConfig {}

pub type SCFileOpenFileWithId = extern fn(
...
pub type SCFileCloseFileById = extern fn(
    file_container: &FileContainer,
    track_id: u32,
    data: *const u8,
    data_len: u32,
...
pub type SCFileAppendDataById =
    extern fn(file_container: &FileContainer, track_id: u32, data: *const u8, data_len: u32) -> i32;
pub type SCFileAppendGAPById =
    extern fn(file_container: &FileContainer, track_id: u32, data: *const u8, data_len: u32) -> i32;
pub type SCFilePrune = extern fn(file_container: &FileContainer);
pub type SCFileContainerRecycle = extern fn(file_container: &FileContainer);

pub type SCFileSetTx = extern fn(file: &FileContainer, tx_id: u64);
...
pub struct SuricataContext {
    ...
    pub AppLayerRegisterParser: extern fn(
        parser: *const crate::applayer::RustParser,
        alproto: AppProto,
    ) -> std::os::raw::c_int,
}
...
#[no_mangle]
pub extern fn rs_init(context: &'static mut SuricataContext) {
    init_ffi(context);
}
...
pub fn sc_app_layer_decoder_events_free_events(events: *mut *mut AppLayerDecoderEvents) {
    unsafe {
        if let Some(c) = SC {
