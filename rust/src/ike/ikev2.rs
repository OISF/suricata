...
pub fn handle_ikev2(mut state: &mut IKEState, current: &[u8], isakmp_header: IsakmpHeader, direction: u8) -> AppLayerResult {
    let hdr = IkeV2Header {
    ...
    match parse_ikev2_payload_list(current, hdr.next_payload) {
        Ok((_,Ok(ref p))) => {
            for payload in p {
                ...
                state.ikev2_container.connection_state = state.ikev2_container.connection_state.advance(payload);
                if let Some(tx) = state.transactions.last_mut() {
                    // borrow back tx to update it
                    tx.payload_types.ikev2_payload_types.append(&mut payload_types);
                    tx.errors = errors;
                    tx.notify_types.append(&mut notify_types);

                    if direction == STREAM_TOCLIENT && state.ikev2_container.server_transforms.len() > 0 {
                        tx.hdr.ikev2_transforms.clear();
                        for transforms in &state.ikev2_container.server_transforms {
                            let mut proposal = Vec::new();
                            transforms.iter().for_each(|t|
                                match *t {
                                    IkeV2Transform::Encryption(ref e) => proposal.push(IkeV2Transform::Encryption(*e)),
                                    IkeV2Transform::Auth(ref e) => proposal.push(IkeV2Transform::Auth(*e)),
                                    IkeV2Transform::PRF(ref e) => proposal.push(IkeV2Transform::PRF(*e)),
                                    IkeV2Transform::DH(ref e) => proposal.push(IkeV2Transform::DH(*e)),
                                    IkeV2Transform::ESN(ref e) => proposal.push(IkeV2Transform::ESN(*e)),
                                    _ => (),
                                });
