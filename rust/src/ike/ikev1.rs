...
impl Ikev1ParticipantData {
    ...
    pub fn update(&mut self, key_exchange: &String, nonce: &String, vendor_ids: &Vec<String>, transforms: &Vec<Vec<SaAttribute>>) {
...
pub fn handle_ikev1(state: &mut IKEState, current: &[u8], isakmp_header: IsakmpHeader, direction: u8) -> AppLayerResult {
            ...
            Ok((rem, payload_list)) => {
                ...
                if direction == STREAM_TOSERVER {
                    state.ikev1_container.client.update(
                        &tx.hdr.ikev1_header.key_exchange,
                        &tx.hdr.ikev1_header.nonce,
