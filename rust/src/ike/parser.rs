...
pub fn parse_security_association(
    ...
    do_parse!(
        ...
        data: cond!(domain_of_interpretation == 1 && i.len() >= 8, take!(i.len() - 8)) >>
        (
...
pub fn parse_proposal(
    ...
    do_parse!(
        ...
        spi: take!(spi_size) >>
        payload_data: cond!((i.len() as i16 - 4) - spi_size as i16 >= 0, take!((i.len() as u16 - 4) - spi_size as u16)) >>
        (
            ProposalPayload {
                proposal_number,
                proposal_type,
...
pub fn parse_payload<'a>(
    ...
    key_exchange: &mut Vec<u8>,
    nonce: &mut Vec<u8>,
    transforms: &mut Vec<Vec<SaAttribute>>,
    vendor_ids: &mut Vec<String>,
