...
use std;
use super::ike::{IKETransaction, IKEState, IkeHeaderWrapper};
...
fn log_common_header(hdr: &IkeHeaderWrapper, jb: &mut JsonBuilder) -> Result<(), JsonError> {
    jb.set_uint("version_major", hdr.maj_ver as u64)?;
    jb.set_uint("version_minor", hdr.min_ver as u64)?;
    jb.set_string("init_spi", &hdr.spi_initiator)?;
    jb.set_string("resp_spi", &hdr.spi_responder)?;
...
fn log_ikev1(state: &IKEState, tx: &IKETransaction, flags: u32, jb: &mut JsonBuilder) -> Result<(), JsonError> {
    ...
    log_common_header(&tx.hdr, jb)?;
    if let Some(payload_types) = &tx.payload_types.ikev1_payload_types {
        jb.open_array("contained_payload_types")?;
        for pt in payload_types {
            jb.append_string(pt.to_string().as_str())?;
        }
        jb.close()?; // contained_payload_types

        if (flags & LOG_EXTENDED) == LOG_EXTENDED {
            jb.open_array("contained_payload_types_verbose")?;
            for pt in payload_types {
                append_payload_type_extended(jb, pt)?;
            }
            jb.close()?; // contained_payload_types_verbose
        }
    ...
    if let Some(exchange_type) = tx.hdr.ikev1_header.exchange_type {
        ...
        if (flags & LOG_EXTENDED) == LOG_EXTENDED {
            match exchange_type {
                1 => jb.set_string("exchange_type_verbose", ExchangeType::Base.to_string().as_str())?,
                2 => jb.set_string("exchange_type_verbose", ExchangeType::IdentityProtection.to_string().as_str())?,
                3 => jb.set_string("exchange_type_verbose", ExchangeType::AuthenticationOnly.to_string().as_str())?,
                4 => jb.set_string("exchange_type_verbose", ExchangeType::Aggressive.to_string().as_str())?,
                5 => jb.set_string("exchange_type_verbose", ExchangeType::Informational.to_string().as_str())?,
                6 => jb.set_string("exchange_type_verbose", ExchangeType::Transaction.to_string().as_str())?,
                32 => jb.set_string("exchange_type_verbose", ExchangeType::QuickMode.to_string().as_str())?,
                33 => jb.set_string("exchange_type_verbose", ExchangeType::NewGroupMode.to_string().as_str())?,
                _ => jb.set_string("exchange_type_verbose", ExchangeType::None.to_string().as_str())?
            };
        }
    }
    ...
    if !tx.hdr.ikev1_header.encrypted_payloads {
        ...
        if state.ikev1_container.client.key_exchange.len() > 0 {
            jb.set_string("key_exchange_payload", &state.ikev1_container.client.key_exchange)?;
        ...
        for client_transform in &state.ikev1_container.client.transforms {
            jb.start_object()?;
            add_attributes(client_transform, flags, jb)?;
            jb.close()?;
        }
        jb.close()?; // proposals
        jb.close()?; // client
        ...
        if state.ikev1_container.server.key_exchange.len() > 0 {
            jb.set_string("key_exchange_payload", &state.ikev1_container.server.key_exchange)?;
        ...
        for server_transform in &state.ikev1_container.server.transforms {
            ...
            jb.open_object("chosen_proposal")?;
            add_attributes(server_transform, flags, jb)?;
            jb.close()?;
            index += 1;
        }
        jb.close()?; // server
