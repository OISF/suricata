...
use crate::enip::parser::{
    ...
    CIP_MULTIPLE_SERVICE, ENIP_CIP_PATH_ATTR_16BIT, ENIP_CIP_PATH_ATTR_8BIT,
...
fn enip_cip_has_attribute(cipdir: &CipDir, attr: u16) -> std::os::raw::c_int {
    if let CipDir::Request(req) = cipdir {
        ...
        if let EnipCipRequestPayload::GetAttributeList(ga) = &req.payload {
...
pub unsafe extern "C" fn rs_enip_get_status(
    ...
    let tx = cast_pointer!(tx, EnipTransaction);
    match enip_get_status(tx, direction.into()) {
        Some(x) => {
            *value = x;
            return true;
        }
...
#[no_mangle]
pub unsafe extern "C" fn rs_enip_get_state(
    tx: *mut std::os::raw::c_void, value: *mut u8,
) -> bool {
    let tx = cast_pointer!(tx, EnipTransaction);
    if let Some(ref response) = tx.response {
        if let EnipPayload::ListIdentity(lip) = &response.payload {
