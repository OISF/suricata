        ...
        match parser::parse_enip_pdu(input) {
            Ok((_, pdu)) => {
                let _pdu = Frame::new(
                    ...
                    EnipFrameType::EnipHeader as u8,
                );
                let _pdu = Frame::new(
                    flow,
                    &stream_slice,
                    ...
                    EnipFrameType::EnipPayload as u8,
                );
                if let parser::EnipPayload::Cip(c) = &pdu.payload {
                    for item in c.items.iter() {
                        if let parser::EnipItemPayload::Data(_d) = &item.payload {
                            let _pdu = Frame::new(
                                flow,
                                &stream_slice,
                                &input[item.start_offset..],
                ...
                } else if let Some(tx) = self.find_request(&pdu) {
                    if pdu.invalid {
                        tx.tx_data.set_event(EnipEvent::InvalidPdu as u8);
                    }
...
fn response_matches_request(req: &parser::EnipPdu, resp: &parser::EnipPdu) -> bool {
    ...
    if req.header.session != resp.header.session
        && req.header.cmd != parser::ENIP_CMD_REGISTER_SESSION
    {
    ...
    if let parser::EnipPayload::Cip(c1) = &req.payload {
        if let parser::EnipPayload::Cip(c2) = &resp.payload {
            ...
            if c1.items.len() >= 2 && c2.items.len() >= 2 && c1.items[1].item_type == parser::ENIP_ITEM_TYPE_CONNECTED_DATA && c2.items[1].item_type == parser::ENIP_ITEM_TYPE_CONNECTED_DATA {
