...
use crate::core::{
    AppProto, Flow, ALPROTO_FAILED, ALPROTO_UNKNOWN, IPPROTO_TCP, IPPROTO_UDP, STREAM_TOCLIENT,
    STREAM_TOSERVER,
};
use nom7 as nom;
use std;
        ...
        Ok((rem, header)) => {
            ...
            match header.cmd {
                ...
                parser::ENIP_CMD_LIST_INTERFACES => {
                    if !parser::parse_enip_list_interfaces(rem).is_ok() {
                        return false;
                    }
                }
...
unsafe extern "C" fn rs_enip_probing_parser_udp(
...
unsafe extern "C" fn rs_enip_probing_parser_tcp(
    _flow: *const Flow, _direction: u8, input: *const u8, input_len: u32, _rdir: *mut u8,
) -> AppProto {
    // Need at least 24 bytes.
    if input.is_null() {
...
unsafe extern "C" fn rs_enip_state_free(state: *mut c_void) {
    std::mem::drop(Box::from_raw(state as *mut EnipState));
}

unsafe extern "C" fn rs_enip_state_tx_free(state: *mut c_void, tx_id: u64) {
    let state = cast_pointer!(state, EnipState);
    state.free_tx(tx_id);
}

unsafe extern "C" fn rs_enip_parse_request_udp(
...
unsafe extern "C" fn rs_enip_parse_response_udp(
    _flow: *const Flow, state: *mut c_void, _pstate: *mut c_void, stream_slice: StreamSlice,
    _data: *const c_void,
) -> AppLayerResult {
    let state = cast_pointer!(state, EnipState);
...
unsafe extern "C" fn rs_enip_parse_request_tcp(
...
unsafe extern "C" fn rs_enip_parse_response_tcp(
    _flow: *const Flow, state: *mut c_void, pstate: *mut c_void, stream_slice: StreamSlice,
    _data: *const c_void,
) -> AppLayerResult {
...
unsafe extern "C" fn rs_enip_tx_get_alstate_progress(tx: *mut c_void, _direction: u8) -> c_int {
...
pub unsafe extern "C" fn rs_enip_register_parsers() {
    ...
    let mut parser = RustParser {
        ...
        ipproto: IPPROTO_UDP,
        probe_ts: Some(rs_enip_probing_parser_udp),
        probe_tc: Some(rs_enip_probing_parser_udp),
        min_depth: 0,
        max_depth: 16,
        state_new: rs_enip_state_new,
        state_free: rs_enip_state_free,
        tx_free: rs_enip_state_tx_free,
        parse_ts: rs_enip_parse_request_udp,
        parse_tc: rs_enip_parse_response_udp,
    ...
    parser.probe_ts = Some(rs_enip_probing_parser_tcp);
    parser.probe_tc = Some(rs_enip_probing_parser_tcp);
    parser.parse_ts = rs_enip_parse_request_tcp;
    parser.parse_tc = rs_enip_parse_response_tcp;
