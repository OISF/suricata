...
fn is_base64_char(b: u8) -> bool {
    nom7::character::is_alphanumeric(b) || b"+/=".contains(&b)
}

fn parse_base64_message(input: &[u8]) -> IResult<&[u8], String> {
    let (input, value) = nom7::combinator::map_res(
        nom7::bytes::complete::take_while(is_base64_char),
        std::str::from_utf8,
    )(input)?;
    let (input, _) = tag("\r\n")(input)?;
