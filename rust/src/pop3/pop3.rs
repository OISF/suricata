            ...
            match POP3_PARSER.parse(start, Direction::ToClient) {
                Ok((rem, Some(msg))) => {
                    if let InnerMessage::Response(mut response) = msg.inner {
                        ...
                        if response.status == sawp_pop3::Status::OK && tx.command.is_some() {
...
unsafe extern "C" fn rs_pop3_tx_get_alstate_progress(tx: *mut c_void, direction: u8) -> c_int {
    let tx = cast_pointer!(tx, POP3Transaction);
    if direction == Direction::ToServer as u8 {
        (tx.command.is_some() || tx.complete) as c_int
    } else {
