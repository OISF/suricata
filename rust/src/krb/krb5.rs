...
use kerberos_parser::krb5::{
    EncryptionType, ErrorCode, KrbError, MessageType, PrincipalName, Realm,
};
...
unsafe extern "C" fn krb5_state_get_tx(
...
unsafe extern "C" fn krb5_state_get_tx_count(state: *mut std::os::raw::c_void) -> u64 {
    let state = cast_pointer!(state, KRB5State);
    state.tx_id
}

unsafe extern "C" fn krb5_state_tx_free(state: *mut std::os::raw::c_void, tx_id: u64) {
    let state = cast_pointer!(state, KRB5State);
    state.free_tx(tx_id);
}

pub extern "C" fn krb5_tx_get_alstate_progress(
...
unsafe extern "C" fn krb5_probing_parser(
...
unsafe extern "C" fn krb5_probing_parser_tcp(
    ...
    match be_u32(slice) as IResult<&[u8], u32> {
        Ok((rem, record_mark)) => {
            ...
            return krb5_probing_parser(_flow, direction, rem.as_ptr(), rem.len() as u32, rdir);
        }
        Err(Err::Incomplete(_)) => {
            return ALPROTO_UNKNOWN;
        }
