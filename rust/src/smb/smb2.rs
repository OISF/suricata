        ...
        Ok((_, rd)) => {
            ...
            let found = if let Some(tx) = state.get_file_tx_by_fuid_with_open_file(&file_guid, Direction::ToClient) {
                if let Some(SMBTransactionTypeData::FILE(ref mut tdf)) = tx.type_data {
                    ...
                    if max_queue_size != 0 && tdf.file_tracker.get_inflight_size() + rd.len as u64 > max_queue_size as u64 {
        ...
        Ok((_, wr)) => {
            ...
            let found = if let Some(tx) = state.get_file_tx_by_fuid_with_open_file(&file_guid, Direction::ToServer) {
                if let Some(SMBTransactionTypeData::FILE(ref mut tdf)) = tx.type_data {
                    ...
                    if max_queue_size != 0 && tdf.file_tracker.get_inflight_size() + wr.wr_len as u64 > max_queue_size as u64 {
