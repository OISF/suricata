        ...
        Ok((_, rd)) => {
            ...
            if !found {
                ...
                if is_pipe && is_dcerpc {
                    ...
                    let tx = match state.new_file_tx(&file_guid, &file_name, Direction::ToClient) {
                        Some(tx) => tx,
                        None => return,
                    };
...
pub fn smb2_write_request_record(state: &mut SMBState, r: &Smb2Record, nbss_remaining: u32)
{
    ...
    if smb2_create_new_tx(r.command) {
        ...
        let tx = match state.new_generic_tx(2, r.command, tx_key) {
            Some(tx) => tx,
            None => return,
        };
        ...
        Ok((_, wr)) => {
            ...
            if !found {
                ...
                if is_pipe && is_dcerpc {
                    ...
                    let tx = match state.new_file_tx(&file_guid, &file_name, Direction::ToServer) {
                        Some(tx) => tx,
                        None => return,
                    };
                ...
                Ok((_, rd)) => {
                    ...
                    match rd.data {
                        Smb2SetInfoRequestData::RENAME(ref ren) => {
                            ...
                            let tx = match state.new_rename_tx(rd.guid.to_vec(), oldname, newname) {
                                Some(tx) => tx,
                                None => return,
                            };
                            tx.hdr = tx_hdr;
                        ...
                        Smb2SetInfoRequestData::DISPOSITION(ref dis) => {
                            ...
                            let tx = match state.new_setfileinfo_tx(fname, rd.guid.to_vec(), rd.class as u16, rd.infolvl as u16, dis.delete) {
                                Some(tx) => tx,
                                None => return,
                            };
                            tx.hdr = tx_hdr;
        ...
        SMB2_COMMAND_NEGOTIATE_PROTOCOL => {
            match parse_smb2_request_negotiate_protocol(r.data) {
                Ok((_, rd)) => {
                    ...
                    if !found {
                        let tx = match state.new_negotiate_tx(2) {
                            Some(tx) => tx,
                            None => return,
                        };
        ...
        SMB2_COMMAND_CREATE => {
            match parse_smb2_request_create(r.data) {
                Ok((_, cr)) => {
                    ...
                    let tx_hdr = SMBCommonHdr::from2(r, SMBHDR_TYPE_GENERICTX);
                    let tx = match state.new_create_tx(cr.data,
