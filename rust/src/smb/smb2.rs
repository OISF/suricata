        ...
        Ok((_, rd)) => {
            ...
            if !found {
                ...
                if share_name.len() == 0 && !is_pipe {
                    ...
                    if smb_dcerpc_probe(rd.data) {
                        ...
                        if !is_dcerpc {
                            state
                                .guid2name_map
                                .insert(file_guid.to_vec(), b"suricata::dcerpc".to_vec());
                        }
                        is_pipe = true;
                        is_dcerpc = true;
                    } else {
                ...
                if is_pipe && is_dcerpc {
                    ...
                    let (tx, files, flags) =
                        state.new_file_tx(&file_guid, &file_name, STREAM_TOCLIENT);
                    if let Some(SMBTransactionTypeData::FILE(ref mut tdf)) = tx.type_data {
                        let file_id: u32 = tx.id as u32;
                        if offset < tdf.file_tracker.tracked {
                            set_event_fileoverlap = true;
                    ...
                    tx.hdr = SMBCommonHdr::new(SMBHDR_TYPE_HEADER, r.session_id, r.tree_id, 0);
        ...
        Ok((_, wr)) => {
            ...
            if !found {
                ...
                if is_pipe && is_dcerpc {
                    ...
                    let (tx, files, flags) =
                        state.new_file_tx(&file_guid, &file_name, STREAM_TOSERVER);
                    if let Some(SMBTransactionTypeData::FILE(ref mut tdf)) = tx.type_data {
                        let file_id: u32 = tx.id as u32;
                        if wr.wr_offset < tdf.file_tracker.tracked {
                            set_event_fileoverlap = true;
...
pub fn smb2_request_record<'b>(state: &mut SMBState, r: &Smb2Record<'b>) {
    ...
    let have_tx = match r.command {
        ...
        SMB2_COMMAND_NEGOTIATE_PROTOCOL => match parse_smb2_request_negotiate_protocol(r.data) {
            Ok((_, rd)) => {
        ...
        SMB2_COMMAND_TREE_CONNECT => match parse_smb2_request_tree_connect(r.data) {
            Ok((_, tr)) => {
        ...
        SMB2_COMMAND_CREATE => match parse_smb2_request_create(r.data) {
            Ok((_, cr)) => {
                ...
                let tx_hdr = SMBCommonHdr::from2(r, SMBHDR_TYPE_GENERICTX);
                let tx = state.new_create_tx(&cr.data.to_vec(), cr.disposition, del, dir, tx_hdr);
        ...
        SMB2_COMMAND_CLOSE => {
            match parse_smb2_request_close(r.data) {
                Ok((_, cd)) => {
                    let found_ts =
                        match state.get_file_tx_by_fuid(&cd.guid.to_vec(), STREAM_TOSERVER) {
                            Some((tx, files, flags)) => {
                                if !tx.request_done {
                    ...
                    let found_tc =
                        match state.get_file_tx_by_fuid(&cd.guid.to_vec(), STREAM_TOCLIENT) {
                            Some((tx, files, flags)) => {
                                if !tx.request_done {
                                    if let Some(SMBTransactionTypeData::FILE(ref mut tdf)) =
                                        tx.type_data
