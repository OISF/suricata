                                ...
                                if pd.loi == 1013 { // set disposition info
                                    match parse_trans2_request_data_set_file_info_disposition(rd.data_blob) {
                                        Ok((_, disp)) => {
                                            ...
                                            let Some(tx) = state.new_setpathinfo_tx(pd.oldname,
                                                    rd.subcmd, pd.loi, disp.delete) else {
                                                return;
                                            };
                                            tx.hdr = tx_hdr;
                                            tx.request_done = true;
                                ...
                                if pd.loi == 1013 { // set disposition info
                                    match parse_trans2_request_data_set_file_info_disposition(rd.data_blob) {
                                        Ok((_, disp)) => {
                                            ...
                                            let Some(tx) = state.new_setfileinfo_tx(filename, pd.fid.to_vec(),
                                                    rd.subcmd, pd.loi, disp.delete) else {
                                                return;
                                            };
                                            tx.hdr = tx_hdr;
                                    ...
                                    match parse_trans2_request_data_set_file_info_rename(rd.data_blob) {
                                        Ok((_, ren)) => {
                                            ...
                                            let Some(tx) = state.new_rename_tx(pd.fid.to_vec(), oldname, newname) else {
                                                return;
                                            };
                                            tx.hdr = tx_hdr;
                                            tx.request_done = true;
        ...
        SMB1_COMMAND_NT_CREATE_ANDX => {
            match parse_smb_create_andx_request_record(&r.data[*andx_offset-SMB1_HEADER_SIZE..], r) {
                Ok((_, cr)) => {
                    ...
                    let tx_hdr = SMBCommonHdr::from1(r, SMBHDR_TYPE_GENERICTX);
                    let Some(tx) = state.new_create_tx(&cr.file_name,
                            cr.disposition, del, dir, tx_hdr) else {
                        return;
                    };
