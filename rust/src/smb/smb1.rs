...
fn smb1_close_file(state: &mut SMBState, fid: &Vec<u8>, direction: u8)
{
        ...
        SMB1_COMMAND_CLOSE => {
            match parse_smb1_close_request_record(r.data) {
                Ok((_, cd)) => {
                    ...
                    smb1_close_file(state, &fid, STREAM_TOSERVER);
                },
                _ => {
                    events.push(SMBEvent::MalformedData);
                },
    ...
    let have_tx = match r.command {
        ...
        SMB1_COMMAND_CLOSE => {
            ...
            if let Some(fid) = fid {
                ...
                smb1_close_file(state, &fid, STREAM_TOCLIENT);
            }
            false
        },
        SMB1_COMMAND_TRANS => {
    ...
    match result {
        Ok((_, rd)) => {
            ...
            if r.command == SMB1_COMMAND_WRITE_AND_CLOSE {
                ...
                smb1_close_file(state, &file_fid, STREAM_TOSERVER);
            }
        },
        _ => {
            events.push(SMBEvent::MalformedData);
