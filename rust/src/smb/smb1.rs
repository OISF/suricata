...
pub fn smb1_check_tx(cmd: u8) -> bool {
    match cmd {
        SMB1_COMMAND_READ_ANDX | SMB1_COMMAND_WRITE_ANDX | SMB1_COMMAND_TRANS => false,
        _ => true,
    }
}
...
fn smb1_request_record_one<'b>(
    state: &mut SMBState, r: &SmbRecord<'b>, command: u8, andx_offset: &mut usize,
) -> u32 {
    ...
    let have_tx = match command {
                        ...
                        match parse_trans2_request_params_set_path_info(rd.setup_blob) {
                            Ok((_, pd)) => {
                                ...
                                if pd.loi == 1013 {
                                    ...
                                    match parse_trans2_request_data_set_file_info_disposition(
                                        rd.data_blob,
                                    ) {
                                        Ok((_, disp)) => {
                                            SCLogDebug!(
                                                "TRANS2 SET_FILE_INFO DATA DISPOSITION DONE {:?}",
                                                disp
                                            );
                                            let tx_hdr =
                                        ...
                                        Err(nom::Err::Error(_e)) | Err(nom::Err::Failure(_e)) => {
                                            SCLogDebug!(
                                                "TRANS2 SET_FILE_INFO DATA DISPOSITION ERROR {:?}",
                                                _e
                                            );
                                            events.push(SMBEvent::MalformedData);
                                            false
                                    ...
                                    match parse_trans2_request_data_set_path_info_rename(
                                        rd.data_blob,
                                    ) {
                                        Ok((_, ren)) => {
                                            SCLogDebug!(
                                                "TRANS2 SET_PATH_INFO DATA RENAME DONE {:?}",
                                                ren
                                            );
                                            let tx_hdr =
                                                SMBCommonHdr::from1(r, SMBHDR_TYPE_GENERICTX);
                                        ...
                                        Err(nom::Err::Incomplete(_n)) => {
                                            SCLogDebug!(
                                                "TRANS2 SET_PATH_INFO DATA RENAME INCOMPLETE {:?}",
                                                _n
                                            );
                                            events.push(SMBEvent::MalformedData);
                                            false
                                        ...
                                        Err(nom::Err::Error(_e)) | Err(nom::Err::Failure(_e)) => {
                                            SCLogDebug!(
                                                "TRANS2 SET_PATH_INFO DATA RENAME ERROR {:?}",
                                                _e
                                            );
                                            events.push(SMBEvent::MalformedData);
                                            false
                                ...
                                if pd.loi == 1013 {
                                    ...
                                    match parse_trans2_request_data_set_file_info_disposition(
                                        ...
                                        Ok((_, disp)) => {
                                            ...
                                            let filename =
                                                match state.guid2name_map.get(&frankenfid) {
                                                    Some(n) => n.to_vec(),
                                    ...
                                    match parse_trans2_request_data_set_file_info_rename(
                                        rd.data_blob,
                                    ) {
                                        Ok((_, ren)) => {
                                            SCLogDebug!(
                                                "TRANS2 SET_FILE_INFO DATA RENAME DONE {:?}",
                                                ren
                                            );
                                            let tx_hdr =
                                                SMBCommonHdr::from1(r, SMBHDR_TYPE_GENERICTX);
                                        ...
                                        Err(nom::Err::Incomplete(_n)) => {
                                            SCLogDebug!(
                                                "TRANS2 SET_FILE_INFO DATA RENAME INCOMPLETE {:?}",
                                                _n
                                            );
                                            events.push(SMBEvent::MalformedData);
                                            false
                                        ...
                                        Err(nom::Err::Error(_e)) | Err(nom::Err::Failure(_e)) => {
                                            SCLogDebug!(
                                                "TRANS2 SET_FILE_INFO DATA RENAME ERROR {:?}",
                                                _e
                                            );
                                            events.push(SMBEvent::MalformedData);
                                            false
        ...
        SMB1_COMMAND_NT_CREATE_ANDX => {
            match parse_smb_create_andx_request_record(
                &r.data[*andx_offset - SMB1_HEADER_SIZE..],
                r,
            ) {
                Ok((_, cr)) => {
        ...
        SMB1_COMMAND_NT_CANCEL | SMB1_COMMAND_TRANS2_SECONDARY | SMB1_COMMAND_LOCKING_ANDX => {
...
pub fn smb1_request_record<'b>(state: &mut SMBState, r: &SmbRecord<'b>) -> u32 {
    ...
    loop {
        ...
        if smb1_command_is_andx(command) {
            if let Ok((_, andx_hdr)) =
                smb1_parse_andx_header(&r.data[andx_offset - SMB1_HEADER_SIZE..])
            {
...
fn smb1_response_record_one<'b>(
    ...
    let have_tx = match command {
        ...
        SMB1_COMMAND_TREE_CONNECT_ANDX => {
            ...
            match parse_smb_connect_tree_andx_response_record(
                &r.data[*andx_offset - SMB1_HEADER_SIZE..],
            ) {
                Ok((_, tr)) => {
                    let name_key = SMBCommonHdr::from1(r, SMBHDR_TYPE_TREE);
        ...
        SMB1_COMMAND_NT_CREATE_ANDX => {
            ...
            if r.nt_status == SMB_NTSTATUS_SUCCESS {
                match parse_smb_create_andx_response_record(
                    &r.data[*andx_offset - SMB1_HEADER_SIZE..],
                ) {
                    Ok((_, cr)) => {
                        SCLogDebug!("Create AndX {:?}", cr);
    ...
    match parse_smb_trans_request_record(r.data, r) {
        Ok((_, rd)) => {
            ...
            if rd.pipe != None {
                ...
                state
                    .ssn2vec_map
                    .insert(SMBCommonHdr::from1(r, SMBHDR_TYPE_GUID), pipe.fid.to_vec());
    ...
    match parse_smb_trans_response_record(r.data) {
        Ok((_, rd)) => {
            ...
            if r.nt_status == SMB_NTSTATUS_BUFFER_OVERFLOW {
                let key =
                    SMBHashKeyHdrGuid::new(SMBCommonHdr::from1(r, SMBHDR_TYPE_TRANS_FRAG), fid);
                SCLogDebug!(
...
pub fn smb1_write_request_record<'b>(
    state: &mut SMBState, r: &SmbRecord<'b>, andx_offset: usize, command: u8,
) {
    let mut events: Vec<SMBEvent> = Vec::new();
        ...
        Ok((_, rd)) => {
            ...
            if !found {
                ...
                if is_pipe {
                    ...
                    let (tx, files, flags) =
                        state.new_file_tx(&file_fid, &file_name, STREAM_TOSERVER);
                    if let Some(SMBTransactionTypeData::FILE(ref mut tdf)) = tx.type_data {
                        let file_id: u32 = tx.id as u32;
                        SCLogDebug!("FID {:?} found at tx {}", file_fid, tx.id);
                        if rd.offset < tdf.file_tracker.tracked {
        ...
        match parse_smb_read_andx_response_record(&r.data[andx_offset - SMB1_HEADER_SIZE..]) {
            Ok((_, rd)) => {
                ...
                let (offset, file_fid) = match state.ssn2vecoffset_map.remove(&fid_key) {
                    ...
                    None => {
                        SCLogDebug!(
                            "SMBv1 READ response: reply to unknown request: left {} {:?}",
                            rd.len - rd.data.len() as u32,
                            rd
                        );
                ...
                if !is_pipe {
                    ...
                    if !found {
                        let (tx, files, flags) =
                            state.new_file_tx(&file_fid, &file_name, STREAM_TOCLIENT);
                        if let Some(SMBTransactionTypeData::FILE(ref mut tdf)) = tx.type_data {
                            let file_id: u32 = tx.id as u32;
                            SCLogDebug!("FID {:?} found at tx {}", file_fid, tx.id);
                            if offset < tdf.file_tracker.tracked {
...
fn smb1_response_record_generic<'b>(
    state: &mut SMBState, r: &SmbRecord<'b>, events: Vec<SMBEvent>,
) {
    let tx_key = SMBCommonHdr::from1(r, SMBHDR_TYPE_GENERICTX);
