...
fn test_parse_smb1_write_andx_request_record_padding() {
    // In the origin packet, data_length == bcc == 20, if we use a proxy to change data_length to 17,
    // Windows still accept it and write 17 bytes to file
    ...
    assert_eq!(record.data, b"ABCDEFGHIJKLMNOPQ"); // Windows server write 17 bytes to file
}

#[test]
fn test_parse_smb1_write_andx_request_record_negative_offset() {
    // In the origin packet, data_offset == 0x3f, if we use a proxy to change data_offset to 0x3c,
    // Windows still accept it as offset which cause bcc be written to file
    let data = hex::decode("0eff000000014000000000ff00000008001400000014003c000000000014004142434445464748494a4b4c4d4e4f5051520a0a").unwrap();
    let result = parse_smb1_write_andx_request_record(&data, SMB1_HEADER_SIZE);
    let record = result.unwrap().1;
    assert_eq!(record.offset, 0);
    assert_eq!(record.len, 20);
    assert_eq!(record.fid, &[0x01, 0x40]);
    assert_eq!(record.data.len(), 20);
    assert_eq!(record.data, b"\x00\x14\x00ABCDEFGHIJKLMNOPQ");

    // if data_offset < 0x3C, Windows return a error
    let data = hex::decode("0eff000000014000000000ff00000008001400000014003b000000000014004142434445464748494a4b4c4d4e4f5051520a0a").unwrap();
    let result = parse_smb1_write_andx_request_record(&data, SMB1_HEADER_SIZE);
