...
) -> IResult<&[u8], Smb1WriteRequestRecord<'_>> {
    let origin_i = i;
...
pub fn parse_smb_trans_request_record_params(
    ...
    (
        ...
        Option<SmbPipeProtocolRecord<'_>>,
    ),
    SmbError,
> {
    let (i, wct) = le_u8.parse(i)?;
...
pub fn parse_smb_trans_response_error_record(
    i: &[u8],
) -> IResult<&[u8], SmbRecordTransResponse<'_>> {
    let (i, _wct) = le_u8.parse(i)?;
    let (i, bcc) = le_u16.parse(i)?;
...
pub fn parse_smb_trans_response_regular_record(
    i: &[u8],
) -> IResult<&[u8], SmbRecordTransResponse<'_>> {
    let (i, wct) = le_u8.parse(i)?;
    let (i, _total_param_cnt) = le_u16.parse(i)?;
...
pub fn smb1_session_setup_parse_wct13<'a>(
    r: &'a SmbRecord, blob: &'a [u8],
) -> (&'a [u8], SessionSetupRequest) {
...
pub fn parse_smb_setup_andx_record<'a>(
    i: &'a [u8], r: &'a SmbRecord,
) -> IResult<&'a [u8], SmbRecordSetupAndX<'a>> {
    let (i, wct) = le_u8.parse(i)?;
    if wct == 12 {
        ...
        let record = SmbRecordSetupAndX {
            sec_blob,
            request_host: smb1_session_setup_request_host_info(r, i),
        };
        Ok((i, record))
    } else if wct == 13 {
        let (i, _skip1) = take(14_usize).parse(i)?;
...
pub fn parse_smb_response_setup_andx_record(
    i: &[u8],
) -> IResult<&[u8], SmbResponseRecordSetupAndX<'_>> {
    let (i, wct) = peek(le_u8).parse(i)?;
    match wct {
...
pub fn parse_smb_read_andx_request_record(
    i: &[u8],
) -> IResult<&[u8], SmbRequestReadAndXRecord<'_>> {
    let (i, wct) = le_u8.parse(i)?;
    let (i, _andx_command) = le_u8.parse(i)?;
    let (i, _) = take(1_usize).parse(i)?; // reserved
    let (i, _andx_offset) = le_u16.parse(i)?;
...
pub fn parse_smb_read_andx_response_record(
    i: &[u8],
) -> IResult<&[u8], SmbResponseReadAndXRecord<'_>> {
    let (i, wct) = le_u8.parse(i)?;
    let (i, _andx_command) = le_u8.parse(i)?;
    let (i, _) = take(1_usize).parse(i)?; // reserved
    let (i, _andx_offset) = le_u16.parse(i)?;
    ...
    let (i, bcc) = le_u16.parse(i)?;
    let (i, _padding) = cond(bcc > data_len_low, |b| take(bcc - data_len_low)(b)).parse(i)?; // TODO figure out how this works with data_len_high
    let (i, _padding_evasion) = cond(data_offset > 36 + 2 * (wct as u16), |b| {
        take(data_offset - (36 + 2 * (wct as u16)))(b)
    })
    .parse(i)?;
...
pub fn parse_trans2_request_params_set_file_info(
    i: &[u8],
) -> IResult<&[u8], Trans2RecordParamSetFileInfo<'_>> {
    let (i, fid) = take(2_usize).parse(i)?;
...
pub fn parse_trans2_request_data_set_file_info_rename(
    i: &[u8],
) -> IResult<&[u8], Trans2RecordParamSetFileInfoRename<'_>> {
...
pub fn parse_trans2_request_data_set_path_info_rename(
    i: &[u8],
) -> IResult<&[u8], Trans2RecordParamSetPathInfoRename<'_>> {
    let (i, replace) = le_u8.parse(i)?;
    let (i, _reserved) = take(3_usize).parse(i)?;
    let (i, _root_dir) = take(4_usize).parse(i)?;
    let (i, newname_len) = le_u32.parse(i)?;
...
pub fn parse_smb_create_andx_response_record(
    i: &[u8],
) -> IResult<&[u8], SmbResponseCreateAndXRecord<'_>> {
    let (i, wct) = le_u8.parse(i)?;
    let (i, _andx_command) = le_u8.parse(i)?;
    let (i, _) = take(1_usize).parse(i)?; // reserved
    let (i, _andx_offset) = le_u16.parse(i)?;
