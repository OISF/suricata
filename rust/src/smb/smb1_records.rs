...
pub fn parse_smb1_write_andx_request_record(i : &[u8], andx_offset: usize) -> IResult<&[u8], Smb1WriteRequestRecord> {
    ...
    let (file_offset, data_pad_offset) = if let Some(ho) = high_offset {
        // TODO: throw error when negative occurs
        ((ho as u64) << 32 | offset as u64, data_offset-ax-0x20)
    } else {
        (offset as u64, data_offset-ax-0x1c)
    };
