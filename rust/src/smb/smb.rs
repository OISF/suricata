        ...
        while cur_i.len() > 0 { // min record size
            match parse_nbss_record(cur_i) {
                Ok((rem, ref nbss_hdr)) => {
                    ...
                    let _nbss_pdu = Frame::new_ts(flow, stream_slice, cur_i, nbss_hdr.length as i64 + 4, SMBFrameType::NBSS as u8);
                    SCLogDebug!("NBSS PDU frame {:?}", _nbss_pdu);
                    let _nbss_hdr_frame = Frame::new_ts(flow, stream_slice, cur_i, 4 as i64, SMBFrameType::NBSSHdr as u8);
                    SCLogDebug!("NBSS HDR frame {:?}", _nbss_hdr_frame);
                    let _nbss_data_frame = Frame::new_ts(flow, stream_slice, &cur_i[4..], nbss_hdr.length as i64, SMBFrameType::NBSSData as u8);
                    ...
                    if nbss_hdr.message_type == NBSS_MSGTYPE_SESSION_MESSAGE {
                        ...
                        match parse_smb_version(nbss_hdr.data) {
                            Ok((_, ref smb)) => {
                                ...
                                SCLogDebug!("SMB {:?}", smb);
                                if smb.version == 0xff_u8 { // SMB1
                                    let smb_pdu = Frame::new_ts(flow, stream_slice, nbss_hdr.data, nbss_hdr.length as i64, SMBFrameType::SMB1 as u8);
                                    ...
                                    while nbss_data.len() > 0 {
                                        let smb_pdu = Frame::new_ts(flow, stream_slice, nbss_hdr.data, nbss_hdr.length as i64, SMBFrameType::SMB2 as u8);
                                        SCLogDebug!("SMB PDU frame {:?}", smb_pdu);
    ...
    pub fn parse_tcp_data_tc_partial<'b>(&mut self, flow: *const Flow, stream_slice: &StreamSlice, input: &'b[u8]) -> usize
    {
        ...
        match parse_nbss_record_partial(input) {
            Ok((output, ref nbss_part_hdr)) => {
                ...
                let _nbss_pdu = Frame::new_tc(flow, stream_slice, input, nbss_part_hdr.length as i64 + 4, SMBFrameType::NBSS as u8);
                SCLogDebug!("NBSS PDU frame {:?}", _nbss_pdu);
                ...
                let _nbss_data_frame = Frame::new_tc(flow, stream_slice, &input[4..], nbss_part_hdr.length as i64, SMBFrameType::NBSSData as u8);
            ...
            match parse_nbss_record(cur_i) {
                Ok((rem, ref nbss_hdr)) => {
                    ...
                    let _nbss_pdu = Frame::new_tc(
                            flow, stream_slice, cur_i, nbss_hdr.length as i64 + 4, SMBFrameType::NBSS as u8);
                    SCLogDebug!("NBSS PDU frame {:?}", _nbss_pdu);
                    let _nbss_hdr_frame = Frame::new_tc(
                            flow, stream_slice, cur_i, 4 as i64, SMBFrameType::NBSSHdr as u8);
                    SCLogDebug!("NBSS HDR frame {:?}", _nbss_hdr_frame);
                    let _nbss_data_frame = Frame::new_tc(
                            flow, stream_slice, &cur_i[4..], nbss_hdr.length as i64, SMBFrameType::NBSSData as u8);
                    SCLogDebug!("NBSS DATA frame {:?}", _nbss_data_frame);
