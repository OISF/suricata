...
use crate::flow::{flow_get_flags, flow_get_last_time, flow_get_ports, Flow, FLOW_DIR_REVERSED};
use crate::frames::*;
...
impl SMBState {
    ...
    fn add_nbss_ts_frames(
        &mut self, flow: *mut Flow, stream_slice: &StreamSlice, input: &[u8], nbss_len: i64,
...
unsafe extern "C" fn smb_parse_request_tcp(
    flow: *mut Flow, state: *mut ffi::c_void, _pstate: *mut AppLayerParserState,
...
extern "C" fn smb_parse_request_tcp_gap(state: &mut SMBState, input_len: u32) -> AppLayerResult {
...
extern "C" fn smb_parse_response_tcp_gap(state: &mut SMBState, input_len: u32) -> AppLayerResult {
...
unsafe extern "C" fn smb_probe_begins_tcp(
...
// probing parser
// return 1 if found, 0 is not found
unsafe extern "C" fn c_smb_probe_tcp(
    _f: *const Flow, flags: u8, input: *const u8, len: u32, rdir: *mut u8,
) -> AppProto {
    if len < MIN_REC_SIZE as u32 || input.is_null() {
        return ALPROTO_UNKNOWN;
...
unsafe extern "C" fn smb_state_get_tx_count(state: *mut ffi::c_void) -> u64 {
    let state = cast_pointer!(state, SMBState);
    SCLogDebug!("smb_state_get_tx_count: returning {}", state.tx_id);
    return state.tx_id;
}

unsafe extern "C" fn smb_state_get_tx(state: *mut ffi::c_void, tx_id: u64) -> *mut ffi::c_void {
    let state = cast_pointer!(state, SMBState);
...
unsafe extern "C" fn smb_state_tx_free(state: *mut ffi::c_void, tx_id: u64) {
    let state = cast_pointer!(state, SMBState);
    SCLogDebug!("freeing tx {}", tx_id);
    state.free_tx(tx_id);
}

unsafe extern "C" fn smb_tx_get_alstate_progress(tx: *mut ffi::c_void, direction: u8) -> i32 {
...
unsafe extern "C" fn smb_get_tx_data(tx: *mut std::os::raw::c_void) -> *mut AppLayerTxData {
    let tx = cast_pointer!(tx, SMBTransaction);
...
unsafe extern "C" fn smb3_probe_tcp(
...
pub unsafe extern "C" fn SCRegisterSmbParser() {
    ...
    if SCAppLayerProtoDetectConfProtoDetectionEnabled(ip_proto_str.as_ptr(), parser.name) != 0 {
        ...
        SCLogConfig!(
            ...
            cfg_max_read_size(),
            cfg_max_read_queue_cnt(),
            cfg_max_read_queue_size()
        );
        SCLogConfig!(
            "write: max record size: {}, max queued chunks {}, max queued size {}",
            cfg_max_write_size(),
            cfg_max_write_queue_cnt(),
            cfg_max_write_queue_size()
        );
