...
pub extern "C" fn rs_smb_parse_request_tcp(
    flow: &mut Flow, state: &mut SMBState, _pstate: *mut std::os::raw::c_void, input: *const u8,
    input_len: u32, _data: *mut std::os::raw::c_void, flags: u8,
) -> AppLayerResult {
...
#[no_mangle]
pub extern "C" fn rs_smb_probe_tcp(direction: u8, input: *const u8, len: u32, rdir: *mut u8) -> i8 {
...
#[no_mangle]
pub extern "C" fn rs_smb_state_get_tx_count(state: &mut SMBState) -> u64 {
    SCLogDebug!("rs_smb_state_get_tx_count: returning {}", state.tx_id);
    return state.tx_id;
}

#[no_mangle]
pub extern "C" fn rs_smb_state_get_tx(state: &mut SMBState, tx_id: u64) -> *mut SMBTransaction {
...
pub extern "C" fn rs_smb_state_get_tx_iterator(
    state: &mut SMBState, min_tx_id: u64, istate: &mut u64,
...
#[no_mangle]
pub extern "C" fn rs_smb_state_tx_free(state: &mut SMBState, tx_id: u64) {
    SCLogDebug!("freeing tx {}", tx_id as u64);
    state.free_tx(tx_id);
}

#[no_mangle]
pub extern "C" fn rs_smb_tx_get_alstate_progress(tx: &mut SMBTransaction, direction: u8) -> u8 {
...
#[no_mangle]
pub extern "C" fn rs_smb_get_tx_data(tx: *mut std::os::raw::c_void) -> *mut AppLayerTxData {
...
pub extern "C" fn rs_smb_state_set_tx_detect_state(
    tx: &mut SMBTransaction, de_state: &mut DetectEngineState,
) {
...
#[no_mangle]
pub extern "C" fn rs_smb_state_truncate(state: &mut SMBState, direction: u8) {
    if (direction & STREAM_TOSERVER) != 0 {
        state.trunc_ts();
    } else {
        state.trunc_tc();
...
#[no_mangle]
pub extern "C" fn rs_smb_state_get_event_info_by_id(
