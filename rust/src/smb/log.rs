    ...
    match tx.type_data {
        Some(SMBTransactionTypeData::SESSIONSETUP(ref x)) => {
            if let Some(ref ntlmssp) = x.ntlmssp {
                let mut jsd = JsonBuilder::new_object();
            ...
            if let Some(ref ticket) = x.krb_ticket {
                ...
                for sname in ticket.sname.name_string.iter() {
                    jsa.append_string(&sname)?;
                }
                jsa.close()?;
                jsd.set_object("snames", &jsa)?;
                jsd.close()?;
                jsb.set_object("kerberos", &jsd)?;
            }
        ...
        Some(SMBTransactionTypeData::NEGOTIATE(ref x)) => {
            if x.smb_ver == 1 {
                ...
                for d in &x.dialects {
                    ...
                    jsa.append_string(&dialect)?;
                }
                jsa.close()?;
                jsb.set_object("client_dialects", &jsa)?;
            } else if x.smb_ver == 2 {
                let mut jsa = JsonBuilder::new_array();
        ...
        Some(SMBTransactionTypeData::DCERPC(ref x)) => {
            ...
            if x.req_set {
                match x.req_cmd {
                    DCERPC_TYPE_REQUEST => {
                        jsd.set_uint("opnum", x.opnum as u64)?;
                        let mut req = JsonBuilder::new_object();
                        req.set_uint("frag_cnt", x.frag_cnt_ts as u64)?;
                        req.set_uint("stub_data_size", x.stub_data_ts.len() as u64)?;
                        jsd.set_object("req", &req)?;
                    },
                    DCERPC_TYPE_BIND => {
                        match state.dcerpc_ifaces {
                            Some(ref ifaces) => {
                                ...
                                for i in ifaces {
                                    ...
                                    if i.acked {
                                        jso.set_uint("ack_result", i.ack_result as u64)?;
                                        jso.set_uint("ack_reason", i.ack_reason as u64)?;
                                    }
                                    jso.close()?;
                                    jsa.append_object(&jso)?;
                                }
                                jsa.close()?;
                                jsd.set_object("interfaces", &jsa)?;
                            },
                            _ => {},
                        }
                    },
            ...
            if x.res_set {
                match x.res_cmd {
                    DCERPC_TYPE_RESPONSE => {
                        let mut res = JsonBuilder::new_object();
                        res.set_uint("frag_cnt", x.frag_cnt_tc as u64)?;
                        res.set_uint("stub_data_size", x.stub_data_tc.len() as u64)?;
                        res.close()?;
                        jsd.set_object("res", &res)?;
                    },
                    // we don't handle BINDACK w/o BIND
                    _ => {},
                }
            }
            jsd.set_uint("call_id", x.call_id as u64)?;
            jsd.close()?;
            jsb.set_object("dcerpc", &jsd)?;
        }
