...
pub fn smb_write_dcerpc_record<'b>(
    state: &mut SMBState, vercmd: SMBVerCmdStat, hdr: SMBCommonHdr, data: &'b [u8],
) -> bool {
...
fn dcerpc_response_handle<'b>(tx: &mut SMBTransaction, vercmd: SMBVerCmdStat, dcer: &DceRpcRecord) {
...
pub fn smb_read_dcerpc_record<'b>(
    state: &mut SMBState, vercmd: SMBVerCmdStat, hdr: SMBCommonHdr, guid: &[u8], indata: &'b [u8],
) -> bool {
    let (_, ntstatus) = vercmd.get_ntstatus();
