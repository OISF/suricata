...
use crate::core::SC;
use crate::core::{self, AppLayerEventType, AppProto, DetectEngineState, Direction, Flow};
...
pub type ParseFn = unsafe extern fn(
    flow: *const Flow,
    state: *mut c_void,
    pstate: *mut c_void,
...
pub type StateAllocFn = extern fn(*mut c_void, AppProto) -> *mut c_void;
pub type StateFreeFn = unsafe extern fn(*mut c_void);
pub type StateTxFreeFn = unsafe extern fn(*mut c_void, u64);
pub type StateGetTxFn = unsafe extern fn(*mut c_void, u64) -> *mut c_void;
pub type StateGetTxCntFn = unsafe extern fn(*mut c_void) -> u64;
pub type StateGetProgressFn = unsafe extern fn(*mut c_void, u8) -> c_int;
pub type GetEventInfoFn =
    unsafe extern fn(*const c_char, *mut c_int, *mut AppLayerEventType) -> c_int;
pub type GetEventInfoByIdFn =
    unsafe extern fn(c_int, *mut *const c_char, *mut AppLayerEventType) -> i8;
pub type LocalStorageNewFn = extern fn() -> *mut c_void;
pub type LocalStorageFreeFn = extern fn(*mut c_void);
pub type GetTxFilesFn = unsafe extern fn(*mut c_void, *mut c_void, u8) -> AppLayerGetFileState;
pub type GetTxIteratorFn = unsafe extern fn(
    ipproto: u8,
    alproto: AppProto,
    state: *mut c_void,
    min_tx_id: u64,
...
pub type GetTxDataFn = unsafe extern fn(*mut c_void) -> *mut AppLayerTxData;
pub type GetStateDataFn = unsafe extern fn(*mut c_void) -> *mut AppLayerStateData;
pub type ApplyTxConfigFn = unsafe extern fn(*mut c_void, *mut c_void, c_int, AppLayerTxConfig);
pub type TruncateFn = unsafe extern fn(*mut c_void, u8);
pub type GetFrameIdByName = unsafe extern fn(*const c_char) -> c_int;
pub type GetFrameNameById = unsafe extern fn(u8) -> *const c_char;
...
extern {
    pub fn AppLayerProtoDetectPPRegister(
        ipproto: u8, portstr: *const c_char, alproto: AppProto, min_depth: u16, max_depth: u16,
        dir: u8, pparser1: ProbeFn, pparser2: ProbeFn,
    );
    pub fn AppLayerProtoDetectPPParseConfPorts(
        ipproto_name: *const c_char, ipproto: u8, alproto_name: *const c_char, alproto: AppProto,
        min_depth: u16, max_depth: u16, pparser_ts: ProbeFn, pparser_tc: ProbeFn,
    ) -> i32;
    pub fn AppLayerProtoDetectPMRegisterPatternCS(
        ipproto: u8, alproto: AppProto, pattern: *const c_char, depth: u16, offset: u16,
        direction: u8,
    ) -> c_int;
    pub fn AppLayerProtoDetectPMRegisterPatternCSwPP(
        ipproto: u8, alproto: AppProto, pattern: *const c_char, depth: u16, offset: u16,
        direction: u8, ppfn: ProbeFn, pp_min_depth: u16, pp_max_depth: u16,
    ) -> c_int;
    pub fn AppLayerProtoDetectConfProtoDetectionEnabled(
        ipproto: *const c_char, proto: *const c_char,
    ) -> c_int;
    pub fn AppLayerProtoDetectConfProtoDetectionEnabledDefault(
        ipproto: *const c_char, proto: *const c_char, default: bool,
    ) -> c_int;
...
pub unsafe extern fn state_get_tx_iterator<S: State<Tx>, Tx: Transaction>(
...
pub trait AppLayerFrameType {
    ...
    unsafe extern fn ffi_id_from_name(name: *const std::os::raw::c_char) -> i32
    where
        Self: Sized,
    {
        if name.is_null() {
    ...
    extern fn ffi_name_from_id(id: u8) -> *const std::os::raw::c_char
    where
        Self: Sized,
    {
        Self::from_u8(id)
