...
pub struct AppLayerTxData {
    ...
    /// progress tracked yet. Special values 254 (APP_LAYER_TX_INSPECTED)
    /// and 255 (APP_LAYER_TX_SKIP_INSPECT) are used to mark direction
    /// as complete.
...
impl AppLayerTxData {
    ...
    pub fn for_direction(direction: Direction) -> Self {
        let (detect_progress_ts, detect_progress_tc, updated_ts, updated_tc) = match direction {
            Direction::ToServer => (0, APP_LAYER_TX_SKIP_INSPECT, true, false),
            Direction::ToClient => (APP_LAYER_TX_SKIP_INSPECT, 0, false, true),
        };
        Self {
            config: AppLayerTxConfig::new(),
            logged: LoggerFlags::new(),
...
pub const APP_LAYER_TX_SKIP_INSPECT: u8 = 255u8;
