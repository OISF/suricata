...
const QUIC_MAX_CRYPTO_FRAG_LEN: usize = 65535;
            ...
            match frame {
                ...
                Frame::CryptoFrag(frag) => {
                    ...
                    if to_server {
                        ...
                        if frag.data.len() < QUIC_MAX_CRYPTO_FRAG_LEN {
                            self.frag_ts = frag.data.clone();
                        } else {
                            frag_long = true;
                        }
                    } else if frag.data.len() < QUIC_MAX_CRYPTO_FRAG_LEN {
                        self.frag_tc = frag.data.clone();
                    } else {
                        frag_long = true;
                    }
                }
            ...
            match QuicHeader::from_bytes(buf, DEFAULT_DCID_LEN) {
                Ok((rest, header)) => {
                    ...
                    let past_frag = if to_server {
                        std::mem::swap(&mut self.frag_ts, &mut frag);
                        &frag
                    } else {
                        std::mem::swap(&mut self.frag_tc, &mut frag);
                        &frag
                    };
                    match QuicData::from_bytes(framebuf, past_frag) {
                        Ok(data) => {
