...
pub fn is_type_dir_mismatch(t: u8, dir: Direction) -> bool {
    match dir {
        Direction::ToClient => {
            if t == DCERPC_TYPE_REQUEST ||
               t == DCERPC_TYPE_BIND ||
               t == DCERPC_TYPE_ALTER_CONTEXT {
                return true;
            }
        }
        Direction::ToServer => {
            if t == DCERPC_TYPE_RESPONSE ||
               t == DCERPC_TYPE_BINDACK ||
               t == DCERPC_TYPE_ALTER_CONTEXT_RESP {
                return true;
            }
        }
    }
...
impl DCERPCState {
    ...
    pub fn handle_input_data(&mut self, input: &[u8], direction: Direction) -> AppLayerResult {
        ...
        let hdr_type = if self.header.is_some() {
                            self.get_hdr_type().unwrap_or(DCERPC_TYPE_UNKNOWN)
                        } else { DCERPC_TYPE_UNKNOWN };
        if (self.header.is_none() ||
            is_type_dir_mismatch(hdr_type, direction)) && !cur_i.is_empty() {
