...
pub extern "C" fn dcerpc_probe_tcp(_f: *const core::Flow, direction: u8, input: *const u8,
...
fn register_pattern_probe() -> i8 {
    unsafe {
        if AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP as u8, 9, // TODO add a constant for alproto dcerpc
                                                     b"|05 00|\0".as_ptr() as *const std::os::raw::c_char, 2, 0,
                                                     core::STREAM_TOSERVER, dcerpc_probe_tcp, 0, 0) < 0 {
            SCLogInfo!("AppLayerProtoDetectPMSignature TS FAILED");
            return -1;
        }
        if AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP as u8, 9,
                                                     b"|05 00|\0".as_ptr() as *const std::os::raw::c_char, 2, 0,
                                                     core::STREAM_TOCLIENT, dcerpc_probe_tcp, 0, 0) < 0 {
            SCLogInfo!("AppLayerProtoDetectPMSignature TC FAILED");
            return -1;
        }
    }
...
pub unsafe extern "C" fn rs_dcerpc_register_parser() {
    ...
    let parser = RustParser {
        ...
        default_port: default_port.as_ptr(),  // TODO should be an option
        ipproto: IPPROTO_TCP,
        probe_ts: None, // it works on particular port probing
        probe_tc: None,
        min_depth: 0,
        max_depth: 16,
