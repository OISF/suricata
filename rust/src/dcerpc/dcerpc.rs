    ...
    pub fn parse_dcerpc_header(&mut self, input: &[u8]) -> i32 {
        match parser::dcerpc_parse_header(input) {
            Ok((leftover_bytes, header)) => {
                ...
                println!("fraglen: {:?}", header.frag_length);
                println!("authlen: {:?}", header.auth_length);
    ...
    pub fn dcerpc_parse(&mut self, input: &[u8], direction: u8) -> AppLayerResult {
        ...
        let buffer = match direction {
            core::STREAM_TOSERVER => {
                ...
                println!("buffer_ts: {:?}", self.buffer_ts.len());
        ...
        if self.bytes_consumed < 16 && input_len > 0 {
            ...
            println!("bytes_consumed: {:?}", self.bytes_consumed);
        }
        ...
        match self.get_hdr_type() {
            Some(x) => match x {
                DCERPC_TYPE_BIND | DCERPC_TYPE_ALTER_CONTEXT => {
                    println!("buffer len before bind parsing: {:?}, parsed: {:?}", buffer.len(), parsed);
                ...
                DCERPC_TYPE_BINDACK | DCERPC_TYPE_ALTER_CONTEXT_RESP => {
                    println!("buffer len befor bindack parsing: {:?}, parsed: {:?}", buffer.len(), parsed);
                ...
                DCERPC_TYPE_REQUEST => {
                    println!("buffer len befor request parsing: {:?}, parsed: {:?}", buffer.len(), parsed);
                ...
                DCERPC_TYPE_RESPONSE => {
                    println!("buffer len befor response parsing: {:?}, parsed: {:?}", buffer.len(), parsed);
...
mod tests {
    ...
    pub fn test_parse_dcerpc() {
        ...
        let mut state = DCERPCState::new();
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&dcerpcrequest, core::STREAM_TOSERVER));
    ...
    pub fn test_parse_bind_pdu() {
        ...
        let mut state = DCERPCState::new();
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bind1, core::STREAM_TOSERVER));
        //assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bind2, core::STREAM_TOSERVER));
    }

    #[test]
    // C test 6
    pub fn test_parse_bind_fragmented() {
        let bind1: &[u8] = &[
        ...
        if let Some(ref bind) = state.dcerpcbind {
            ...
            assert_eq!(732, state.bytes_consumed);
        }
    }
    ...
    pub fn test_parse_bind_frag_2()
    {
        ...
        let mut state = DCERPCState::new();
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&request1, core::STREAM_TOSERVER));
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&request2, core::STREAM_TOSERVER));
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&request3, core::STREAM_TOSERVER));
        if let Some(ref req) = state.dcerpcrequest {
    ...
    pub fn test_parse_dcerpc_frag()
    {
        ...
        let mut state = DCERPCState::new();
        assert_eq!(AppLayerResult::err(), state.dcerpc_parse(&fault, core::STREAM_TOSERVER));
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&request1, core::STREAM_TOSERVER));
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&request2, core::STREAM_TOSERVER));
        if let Some(ref req) = state.dcerpcrequest {
    ...
    #[test]
    // C test 11
    pub fn test_parse_dcerpc_frag2()
    {
        let request1: &[u8] = &[
            0x05, 0x00, 0x00, 0x03, 0x10, 0x00, 0x00, 0x00,
        ...
        let mut state = DCERPCState::new();
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&request1, core::STREAM_TOSERVER));
        // TODO ASK about this, 36 is fraglen in req1 and the number of bytes
        //assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&request2, core::STREAM_TOSERVER));
        //assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&request3, core::STREAM_TOSERVER));
    }
    ...
    pub fn test_parse_dcerpc_back_frag()
    {
        ...
        let mut state = DCERPCState::new();
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bind_ack1, core::STREAM_TOSERVER));
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bind_ack2, core::STREAM_TOSERVER));
    }
    ...
    pub fn test_parse_bind_pdu_ctx_id_non_zero()
    {
        ...
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bindbuf, core::STREAM_TOSERVER));
        if let Some(ref bind) = state.dcerpcbind {
            let bind_uuid = &bind.uuid_list[0].uuid;
            assert_eq!(1, bind.uuid_list.len());
            assert_eq!(cmp::Ordering::Equal, bind_uuid.iter()
                        .zip(expected_uuid)
                        .map(|(x, y)| x.cmp(y))
    ...
    #[test]
    // Check for another endless loop with bind pdus
    ...
    #[test]
    // Check for another endless loop with bind_ack pdus
    pub fn test_parse_bindack_pdu_infinite_loop()
    {
        ...
        let mut state = DCERPCState::new();
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bind_ack, core::STREAM_TOSERVER));
    }
    ...
    pub fn test_parse_bindack_internal_ids()
    {
        ...
        let mut state = DCERPCState::new();
        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bind1, core::STREAM_TOSERVER));
        assert_eq!(AppLayerResult::err(), state.dcerpc_parse(&bind_ack1, core::STREAM_TOSERVER));
//        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bind2, core::STREAM_TOSERVER));
//        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bind_ack2, core::STREAM_TOSERVER));
//        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bind3, core::STREAM_TOSERVER));
//        assert_eq!(AppLayerResult::ok(), state.dcerpc_parse(&bind_ack3, core::STREAM_TOSERVER));
        if let Some(ref back) = state.dcerpcback {
            assert_eq!(0, back.accepted_uuid_list.len());
        }
    }
}
