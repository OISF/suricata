...
impl DCERPCState {
    ...
    /// * Failure: -1 in case of Incomplete data.
    ...
    pub fn process_header(&mut self, input: &[u8]) -> i32 {
        match parser::parse_dcerpc_header(input) {
            ...
            Err(nom::Err::Error(_)) => {
                SCLogDebug!("EoF reached with the data");
                -1
            }
            Err(_) => {
    ...
    pub fn handle_common_stub(&mut self, input: &[u8], bytes_consumed: i32, dir: u8) -> i32 {
        ...
        self.padleft = fraglen - DCERPC_HDR_LEN - bytes_consumed as u16; // bytes processed so far = header length
    ...
    pub fn handle_input_data(&mut self, input: &[u8], direction: u8) -> AppLayerResult {
        ...
        if self.data_needed_for_dir != direction && buffer.len() != 0 {
            println!("DCERPC FAILED check data_needed_for_dir");
            return AppLayerResult::err();
        }
        ...
        // else return ok and the buffer stays empty
...
mod tests {
    ...
    #[test]
    pub fn test_parse_dcerpc_frag3() {
        let request1: &[u8] = &[
