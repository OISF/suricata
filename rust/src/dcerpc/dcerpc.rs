...
impl DCERPCState {
    ...
    pub fn parse_dcerpc_header(&mut self, input: &[u8]) -> i32 {
        match parser::dcerpc_parse_header(input) {
            Ok((_leftover_bytes, header)) => {
                ...
                {
                    println!("DCERPC Header did not validate.");
                    return -1;
                }
    ...
    pub fn process_common_stub(&mut self, input: &[u8]) -> i32 {
        ...
        println!("parse_request: padleft: {:?}, input_left: {:?}\n", self.padleft, input_left);
    ...
    pub fn dcerpc_parse(&mut self, input: &[u8], input_len: u32) -> i32 {
        let mut parsed = 0;
        let mut retval = 0;
        if self.bytes_consumed < 16 && input_len > 0 {
            parsed = self.parse_dcerpc_header(&input);
            if parsed == -1 {
                return -1;
            }
        ...
        match pkt_type {
            DCERPC_TYPE_BIND | DCERPC_TYPE_ALTER_CONTEXT => {
                retval = self.parse_dcerpc_bind(&input[parsed as usize..]);
                if retval == -1 {
                    return -1;
                }
            }
            DCERPC_TYPE_BINDACK | DCERPC_TYPE_ALTER_CONTEXT_RESP => {
                retval = self.parse_bind_ack(&input[parsed as usize..]);
                if retval == -1 {
                    return -1;
                }
            }
            DCERPC_TYPE_REQUEST => {
                retval = self.parse_request(&input[parsed as usize..]);
                if retval == -1 {
                    return -1;
                }
            },
            DCERPC_TYPE_RESPONSE => {
                ...
                retval = self.process_common_stub(&input[parsed as usize..]);
                if retval == -1 {
                    return -1;
                }
...
pub extern "C" fn rs_dcerpc_parse_response(
    ...
    // TODO figure out how to deal with the last arg here
    if input_len > 0 {
        if input != std::ptr::null_mut() {
            let buf = unsafe { std::slice::from_raw_parts(input, input_len as usize) };
            //return state.parse_dcerpc(buf);
...
#[no_mangle]
pub extern "C" fn rs_dcerpc_get_tx_detect_flags() {}
...
mod tests {
    ...
    pub fn test_parse_dcerpc() {
        ...
        let mut state = DCERPCState::new();
        assert_eq!(1, state.dcerpc_parse(&dcerpcrequest, dcerpcrequest.len() as u32));
    ...
    #[test]
    pub fn test_dcerpc_fragmented_bind_pdu() {
        let dcerpcbind1: &[u8] = &[
        ...
        let mut state = DCERPCState::new();
        assert_eq!(1, state.dcerpc_parse(dcerpcbind1, dcerpcbind1.len() as u32));
        if let Some(ref bind) = state.dcerpcbind {
            assert_eq!(16, bind.numctxitems);
            assert_eq!(16, bind.uuid_list.len());
        }
        assert_eq!(1, state.dcerpc_parse(dcerpcbind2, dcerpcbind2.len() as u32));
    }
}
