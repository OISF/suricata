...
impl DCERPCUDPState {
    ...
    fn evaluate_serial_no(&mut self) -> u16 {
        ...
        let mut cursor_serial_no = Cursor::new(vec![serial_hi, serial_lo]);
        if endianness & 0x10 == 0 {
            serial_no = cursor_serial_no.read_u16::<BigEndian>().unwrap();
        } else {
            serial_no = cursor_serial_no.read_u16::<LittleEndian>().unwrap();
        }
        serial_no
    }
