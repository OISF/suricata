...
impl DCERPCUDPState {
    ...
    pub fn update_tx_id(&mut self, serial_no: u16) {
        let mut tx_for_serial_no = self.find_tx(serial_no);
        if let Some(ref mut tx) = tx_for_serial_no {
    ...
    pub fn handle_input_data(&mut self, input: &[u8]) -> AppLayerResult {
        ...
        let tx = DCERPCTransaction::new(self.tx_id, serial_no as u32, endianness);
