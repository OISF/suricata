...
#[no_mangle]
pub extern "C" fn scjs_new_object() -> *mut JsonBuilder {
    let boxed = Box::new(JsonBuilder::new_object());
    Box::into_raw(boxed)
}

#[no_mangle]
pub unsafe extern "C" fn scjs_free(js: &mut JsonBuilder) {
    let _: Box<JsonBuilder> = std::mem::transmute(js);
}

#[no_mangle]
pub unsafe extern "C" fn scjs_open_object(js: &mut JsonBuilder, key: *const c_char) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn scjs_open_array(js: &mut JsonBuilder, key: *const c_char) -> bool {
    if let Ok(s) = CStr::from_ptr(key).to_str() {
...
#[no_mangle]
pub unsafe extern "C" fn scjs_set_string(
    js: &mut JsonBuilder,
    key: *const c_char,
    val: *const c_char,
) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn scjs_set_object(
    js: &mut JsonBuilder,
    key: *const c_char,
    val: &mut JsonBuilder,
) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn scjs_add_string(js: &mut JsonBuilder, val: *const c_char) -> bool {
    if let Ok(val) = CStr::from_ptr(val).to_str() {
...
#[no_mangle]
pub unsafe extern "C" fn scjs_add_uint(js: &mut JsonBuilder, val: u64) -> bool {
    return js.add_uint(val).is_ok();
}

#[no_mangle]
pub unsafe extern "C" fn scjs_set_uint(js: &mut JsonBuilder, key: *const c_char, val: u64) -> bool {
    if let Ok(key) = CStr::from_ptr(key).to_str() {
...
#[no_mangle]
pub unsafe extern "C" fn scjs_close(js: &mut JsonBuilder) -> bool {
    js.close().is_ok()
}

#[no_mangle]
pub unsafe extern "C" fn scjs_tostring(js: &mut JsonBuilder) -> *mut c_char {
...
#[no_mangle]
pub unsafe extern "C" fn scjs_to_cstring(js: &mut JsonBuilder, len: &mut usize) -> *mut c_char {
    if let Ok(s) = CString::new(js.buf.as_bytes()) {
        *len = js.buf.len();
        s.into_raw()
    } else {
