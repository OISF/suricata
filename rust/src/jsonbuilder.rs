...
impl JsonBuilder {
    ...
    pub fn set_bool(&mut self, key: &str, val: bool) -> Result<&mut Self, JsonError> {
        ...
        self.buf.push_str("\":");
        match val {
            true => self.buf.push_str("true"),
            false => self.buf.push_str("false"),
        }
        Ok(self)
    }
...
#[no_mangle]
pub extern "C" fn jb_len(js: &mut JsonBuilder) -> usize {
    js.len()
}
...
/// Returns the JsonBuilder buffer as a C-string, optionally storing the length
/// in an output pointer.
#[no_mangle]
