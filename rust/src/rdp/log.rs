...
fn x224_req_to_json(js: &Json, x224: &X224ConnectionRequest) {
    ...
    if let Some(ref req) = x224.negotiation_request {
        if !req.flags.is_empty() {
            ...
            if req
                .flags
                .contains(NegotiationRequestFlags::REDIRECTED_AUTHENTICATION_MODE_REQUIRED)
            {
                flags.array_append_string("redirected_authentication_mode_required");
            }
            if req
                .flags
                .contains(NegotiationRequestFlags::CORRELATION_INFO_PRESENT)
            {
        ...
        match &from_server {
            NegotiationFromServer::Response(ref resp) => {
                if !resp.flags.is_empty() {
                    ...
                    if resp
                        .flags
                        .contains(NegotiationResponseFlags::EXTENDED_CLIENT_DATA_SUPPORTED)
                    {
                        flags.array_append_string("extended_client_data");
                    }
                    if resp.flags.contains(
                        NegotiationResponseFlags::DYNVC_GFX_PROTOCOL_SUPPORTED,
                    ) {
                    ...
                    if resp
                        .flags
                        .contains(NegotiationResponseFlags::RESTRICTED_ADMIN_MODE_SUPPORTED)
                    {
                        flags.array_append_string("restricted_admin");
                    }
                    if resp.flags.contains(
                        NegotiationResponseFlags::REDIRECTED_AUTHENTICATION_MODE_SUPPORTED,
                    ) {
                ...
                match resp.protocol {
                    Protocol::ProtocolRdp => {
                        js.set_string("protocol", "rdp");
                    }
                    Protocol::ProtocolSsl => {
                        js.set_string("protocol", "ssl");
                    }
                    Protocol::ProtocolHybrid => {
                        js.set_string("protocol", "hybrid");
                    }
                    Protocol::ProtocolRdsTls => {
                        js.set_string("protocol", "rds_tls");
                    }
                    Protocol::ProtocolHybridEx => {
                        js.set_string("protocol", "hybrid_ex");
                    }
                }
            }
                    ...
                    client.early_capability_flags
                {
                    if !early_capability_flags.is_empty() {
                        ...
                        if early_capability_flags
                            .contains(EarlyCapabilityFlags::RNS_UD_CS_SUPPORT_STATUSINFO_PDU)
                        {
                            flags.array_append_string("support_statusinfo_pdu");
                        }
                        if early_capability_flags
                            .contains(EarlyCapabilityFlags::RNS_UD_CS_STRONG_ASYMMETRIC_KEYS)
                        {
                        ...
                        if early_capability_flags
                            .contains(EarlyCapabilityFlags::RNS_UD_CS_VALID_CONNECTION_TYPE)
                        {
                            flags.array_append_string("valid_connection_type");
                        }
                        if early_capability_flags
                            .contains(EarlyCapabilityFlags::RNS_UD_CS_SUPPORT_MONITOR_LAYOUT_PDU)
                        {
                            flags.array_append_string("support_monitor_layout_pdu");
                        }
                        if early_capability_flags
                            .contains(EarlyCapabilityFlags::RNS_UD_CS_SUPPORT_NETCHAR_AUTODETECT)
                        {
                            flags.array_append_string("support_netchar_autodetect");
                        }
                        if early_capability_flags
                            .contains(EarlyCapabilityFlags::RNS_UD_CS_SUPPORT_DYNVC_GFX_PROTOCOL)
                        {
                            flags.array_append_string("support_dynvc_gfx_protocol");
                        }
                        if early_capability_flags
                            .contains(EarlyCapabilityFlags::RNS_UD_CS_SUPPORT_DYNAMIC_TIME_ZONE)
                        {
                            flags.array_append_string("support_dynamic_time_zone");
                        }
                        if early_capability_flags
                            .contains(EarlyCapabilityFlags::RNS_UD_CS_SUPPORT_HEARTBEAT_PDU)
                        {
