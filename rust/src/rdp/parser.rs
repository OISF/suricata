...
pub fn millimeters_to_opt(x: u32) -> Option<u32> {
    if x >= 10 && x <= 10_000 {
        Some(x)
    } else {
        None
...
/// rdp-spec, section 2.2.1.3.2 Client Core Data
pub fn desktop_scale_to_opt(x: u32) -> Option<u32> {
    if x >= 100 && x <= 500 {
        Some(x)
    } else {
        None
...
/// rdp-spec, section 2.2.1.3.2 Client Core Data
pub fn device_scale_to_opt(x: u32) -> Option<u32> {
...
pub fn parse_t123_tpkt(input: &[u8]) -> IResult<&[u8], T123Tpkt> {
    let (i1, _version) =
        try_parse!(input, verify!(be_u8, |x| x == TpktVersion::T123 as u8));
    let (i2, _reserved) = try_parse!(i1, be_u8);
    // less u8, u8, u16
    let (i3, sz) = try_parse!(i2, map_opt!(be_u16, |x: u16| x.checked_sub(4)));
    let (i4, data) = try_parse!(i3, take!(sz));
...
/// rdp-spec, section 2.2.1.1
pub fn parse_x224_connection_request(
    input: &[u8],
) -> IResult<&[u8], X224ConnectionRequest> {
    let (i1, length) = try_parse!(input, verify!(be_u8, |x| x != 0xff)); // 0xff is reserved
    let (i2, cr_cdt) = try_parse!(
        i1,
        bits!(tuple!(
    ...
    let (i3, dst_ref) = try_parse!(i2, verify!(be_u16, |x| x == 0));
    let (i4, src_ref) = try_parse!(i3, be_u16);
    let (i5, class_options) = try_parse!(
        i4,
        bits!(tuple!(
    ...
    let (i6, sz) = try_parse!(i5, expr_opt!(length.checked_sub(6)));
...
pub fn parse_x224_connection_request_class_0(
    input: &[u8],
) -> IResult<&[u8], X224ConnectionRequest> {
...
// rdp-spec, section 2.2.1.1.1
named!(pub parse_rdp_cookie<RdpCookie>,
    do_parse!(
        _key: verify!(
            take!(8),
            |x| x == "Cookie: ".as_bytes())
...
// rdp-spec, section 2.2.1.1.1
named!(pub parse_negotiation_request<NegotiationRequest>,
    do_parse!(
        _typ: verify!(
            le_u8,
...
/// rdp-spec, section 2.2.1.2
/// x.224-spec, section 13.3
pub fn parse_x224_connection_confirm(
    input: &[u8],
    ...
    let (i7, negotiation_from_server) = {
        if sz > 0 {
            let (i7, data) = try_parse!(i6, take!(sz));
...
pub fn parse_x224_connection_confirm_class_0(
    input: &[u8],
) -> IResult<&[u8], X224ConnectionConfirm> {
...
// rdp-spec, section 2.2.1.1.1
named!(pub parse_negotiation_response<NegotiationResponse>,
    do_parse!(
        _typ: verify!(
            le_u8,
...
// rdp-spec, section 2.2.1.1.1
named!(pub parse_negotiation_failure<NegotiationFailure>,
    do_parse!(
        _typ: verify!(
            le_u8,
...
/// x224-spec, section 13.7
pub fn parse_x223_data_class_0(input: &[u8]) -> IResult<&[u8], X223Data> {
    let (i1, _length) = try_parse!(input, verify!(be_u8, |x| x == 2));
    let (i2, _dt_x_roa) = try_parse!(
        i1,
        bits!(tuple!(
    ...
    let (i3, _eot) = try_parse!(i2, verify!(be_u8, |x| x == 0x80));
...
/// rdp-spec, section 2.2.1.3.2
pub fn parse_mcs_connect(input: &[u8]) -> IResult<&[u8], McsConnectRequest> {
    let (i1, _ber_type) = try_parse!(
        input,
        verify!(
            le_u8,
    ...
    let (i2, _t125_type) = try_parse!(
        i1,
        verify!(le_u8, |x| x == T125Type::T125TypeMcsConnectRequest as u8)
    );

    // skip to, and consume, H.221 client-to-server key
    let (i3, _skipped) = try_parse!(i2, take_until_and_consume!("Duca"));

    let (i4, data) = try_parse!(i3, length_data!(parse_per_length_determinant));
...
/// rdp-spec, section 2.2.1.3.2
pub fn parse_cs_client_core_data(
    input: &[u8],
) -> IResult<&[u8], CsClientCoreData> {
    let (i1, _typ) =
        try_parse!(input, verify!(le_u16, |x| x == CsType::Core as u16));
    // less u16, u16
    let (i2, sz) = try_parse!(i1, map_opt!(le_u16, |x: u16| x.checked_sub(4)));
    let (i3, data) = try_parse!(i2, take!(sz));
    let (j1, version) =
        try_parse!(data, map!(le_u32, num::FromPrimitive::from_u32));
    let (j2, desktop_width) = try_parse!(j1, le_u16);
    let (j3, desktop_height) = try_parse!(j2, le_u16);
    let (j4, color_depth) =
        try_parse!(j3, map!(le_u16, num::FromPrimitive::from_u16));
    let (j5, sas_sequence) =
        try_parse!(j4, map!(le_u16, num::FromPrimitive::from_u16));
    let (j6, keyboard_layout) = try_parse!(j5, le_u32);
    let (j7, client_build) =
        try_parse!(j6, map!(le_u32, windows::build_number_to_os));
    let (j8, client_name) =
        try_parse!(j7, map_res!(take!(32), le_slice_to_string));
    let (j9, keyboard_type) =
        try_parse!(j8, map!(le_u32, num::FromPrimitive::from_u32));
    let (j10, keyboard_subtype) = try_parse!(j9, le_u32);
    let (j11, keyboard_function_key) = try_parse!(j10, le_u32);
    let (j12, ime_file_name) =
        try_parse!(j11, map_res!(take!(64), le_slice_to_string));
...
/// rdp-spec, section 2.2.1.3.4
pub fn parse_cs_net(input: &[u8]) -> IResult<&[u8], CsNet> {
    let (i1, _typ) =
        try_parse!(input, verify!(le_u16, |x| x == CsType::Net as u16));
    // less _typ (u16), this length indicator (u16), count (u32)
    let (i2, sz) = try_parse!(i1, map_opt!(le_u16, |x: u16| x.checked_sub(8)));
    ...
    for _index in 0..count {
        ...
        let (j1, name) =
            try_parse!(remainder, map_res!(take!(8), utf7_slice_to_string));
        channels.push(name);
...
// cf. rdp-spec, section 2.2.1.3.4
named!(pub parse_cs_unknown<CsUnknown>,
    do_parse!(
        typ: map_opt!(
            le_u16,
            |x| {
...
// rdp-spec, section 2.2.1.4
named!(pub parse_mcs_connect_response<McsConnectResponse>,
    do_parse!(
        _ber_type: verify!(
            le_u8,
            // BER: 0b01=application, 0b1=non-primitive, 0b11111
...
mod tests_negotiate_incomplete_49350 {
    ...
    fn test_t123_incomplete() {
        ...
        assert_eq!(
            // fails: try_parse!(i2, map_opt!(be_u16, |x: u16| x.checked_sub(4)));
    ...
    fn test_x224_incomplete() {
        ...
        assert_eq!(
            // fails: try_parse!(i5, expr_opt!(length.checked_sub(6)));
...
mod tests_core_incomplete_49350 {
    ...
    fn test_x223_incomplete() {
        ...
        assert_eq!(
            // fails: try_parse!(i2, verify!(be_u8, |x| x == 0x80));
    ...
    fn test_connect_incomplete() {
        ...
        assert_eq!(
            // fails: try_parse!(i3, length_data!(parse_per_length_determinant));
