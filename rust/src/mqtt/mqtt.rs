...
impl MQTTEvent {
    fn from_i32(value: i32) -> Option<MQTTEvent> {
        match value {
            0 => Some(MQTTEvent::MissingConnect),
            1 => Some(MQTTEvent::MissingPublish),
            2 => Some(MQTTEvent::MissingSubscribe),
            3 => Some(MQTTEvent::MissingUnsubscribe),
            4 => Some(MQTTEvent::DoubleConnect),
            5 => Some(MQTTEvent::UnintroducedMessage),
            6 => Some(MQTTEvent::InvalidQosLevel),
            7 => Some(MQTTEvent::MissingMsgId),
            8 => Some(MQTTEvent::UnassignedMsgtype),
            _ => None,
        }
    }
}
        ...
        match msg.op {
            MQTTOperation::CONNECT(ref conn) => {
                ...
                if let Some(tx) = self.get_tx_by_pkt_id(MQTT_CONNECT_PKT_ID) {
                    &mut MQTTState::set_event(tx, MQTTEvent::DoubleConnect);
                } else {
...
pub extern "C" fn rs_mqtt_state_get_event_info_by_id(event_id: std::os::raw::c_int,
                                                      ...
                                                      -> i8
{
    if let Some(e) = MQTTEvent::from_i32(event_id as i32) {
        let estr = match e {
