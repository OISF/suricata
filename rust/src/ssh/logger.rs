...
fn log_ssh(tx: &SSHTransaction) -> Option<Json> {
    ...
    if tx.cli_hdr.protover.len() > 0 {
        ...
        cjs.set_string(
            "proto_version",
            &String::from_utf8_lossy(&tx.cli_hdr.protover),
        );
        if tx.cli_hdr.swver.len() > 0 {
            cjs.set_string(
                "software_version",
                &String::from_utf8_lossy(&tx.cli_hdr.swver),
            );
        }
        js.set("client", cjs);
    }
    if tx.srv_hdr.protover.len() > 0 {
        ...
        sjs.set_string(
            "proto_version",
            &String::from_utf8_lossy(&tx.srv_hdr.protover),
        );
        if tx.srv_hdr.swver.len() > 0 {
            sjs.set_string(
                "software_version",
                &String::from_utf8_lossy(&tx.srv_hdr.swver),
            );
        }
        js.set("server", sjs);
    }
