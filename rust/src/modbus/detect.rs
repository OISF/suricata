...
lazy_static! {
    static ref ACCESS_RE: Regex = Regex::new("^\\s*\"?\\s*access\\s*(read|write)\\s*(discretes|coils|input|holding)?(?:,\\s*address\\s+([<>]?\\d+)(?:<>(\\d+))?(?:,\\s*value\\s+([<>]?\\d+)(?:<>(\\d+))?)?)?\\s*\"?\\s*$").unwrap();
    static ref FUNC_RE: Regex = Regex::new(
        "^\\s*\"?\\s*function\\s*(!?[A-z0-9]+)(?:,\\s*subfunction\\s+(\\d+))?\\s*\"?\\s*$"
    )
    .unwrap();
    static ref UNIT_RE: Regex =
            Regex::new("^\\s*\"?\\s*unit\\s+([<>]?\\d+)(?:<>(\\d+))?(?:,\\s*(.*))?\\s*\"?\\s*$")
                .unwrap();
}

#[derive(Debug, PartialEq)]
pub struct DetectModbus {
...
#[no_mangle]
pub unsafe extern "C" fn rs_parse_access(c_arg: *const c_char) -> *mut c_void {
    if c_arg.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn rs_parse_function(c_arg: *const c_char) -> *mut c_void {
    if c_arg.is_null() {
        return std::ptr::null_mut();
    }
    if let Ok(arg) = CStr::from_ptr(c_arg).to_str() {
...
#[no_mangle]
pub unsafe extern "C" fn rs_parse_unit_id(c_arg: *const c_char) -> *mut c_void {
    if c_arg.is_null() {
        return std::ptr::null_mut();
    }
    if let Ok(arg) = CStr::from_ptr(c_arg).to_str() {
...
pub unsafe extern "C" fn rs_modbus_free(ptr: *mut c_void) {
    if !ptr.is_null() {
        let _ = Box::from_raw(ptr as *mut DetectModbus);
    }
}
...
#[no_mangle]
pub extern "C" fn rs_modbus_inspect(tx: &ModbusTransaction, modbus: &DetectModbus) -> u8 {
    let req = match &tx.request {
        Some(r) => r,
        None => return 0,
    };

    if let Some(unit_id) = &modbus.unit_id {
        if !check_match(unit_id, req.unit_id.into()) {
            return 0;
        }
    }
    ...
    if let Some(access_type) = &modbus.access_type {
        ...
        if rd_wr_access == AccessType::NONE
            || !req.access_type.intersects(rd_wr_access)
            || (access_func != AccessType::NONE && !req.access_type.intersects(access_func))
        {
            return 0;
        }

        return inspect_data(req, modbus) as u8;
    }

    if let Some(category) = modbus.category {
        return u8::from(req.category.intersects(category));
    }

    match &modbus.function {
        Some(func) if func == &req.function.code => match modbus.subfunction {
            Some(subfunc) => {
                if let Data::Diagnostic { func, data: _ } = &req.data {
...
/// not it is a match
fn inspect_data(request: &Message, modbus: &DetectModbus) -> bool {
    let sig_address = if let Some(sig_addr) = &modbus.address {
        ...
        // not it is a match
        if let Some(req_addr) = request.get_address_range() {
    ...
    if let Some(value) = request.get_write_value_at_address(&sig_address) {
...
fn parse_access(access_str: &str) -> Result<DetectModbus, ()> {
...
fn parse_function(func_str: &str) -> Result<DetectModbus, ()> {
    ...
    let mut modbus: DetectModbus = Default::default();
...
fn parse_unit_id(unit_str: &str) -> Result<DetectModbus, ()> {
