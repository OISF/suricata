...
use sawp_modbus::{self, AccessType, ErrorFlags, Message};
...
impl ModbusTransaction {
    ...
    fn set_events_from_flags(&mut self, flags: &ErrorFlags) {
                    ...
                    match direction {
                        Direction::ToServer | Direction::Unknown => {
                            match self.find_response_and_validate(&mut msg) {
                                Some(tx) => {
                                    tx.set_events_from_flags(&msg.flags);
                                    tx.request = Some(msg);
                                }
                                None => {
...
mod tests {
    ...
    fn invalid_length_request() {
        ...
        assert_eq!(msg.flags, ErrorFlags::DATA_LENGTH);
    }
    ...
    fn exception_code_invalid() {
        ...
        assert_eq!(msg.flags, ErrorFlags::EXC_CODE);
    }
