...
use sawp_modbus::{self, Message};

pub const REQUEST_FLOOD: u64 = 500; // Default unreplied Modbus requests are considered a flood
...
impl ModbusState {
    ...
    pub fn find_request_and_validate(&mut self, resp: &mut Message) -> Option<&mut ModbusTransaction> {
        for tx in &mut self.transactions {
    ...
    /// transaction, if it exists
    pub fn find_response_and_validate(&mut self, req: &mut Message) -> Option<&mut ModbusTransaction> {
        for tx in &mut self.transactions {
    ...
    pub fn new_tx(&mut self) -> ModbusTransaction {
        self.tx_id += 1;
        let mut tx = ModbusTransaction::new(self.tx_id);

        // Check flood limit
        if REQUEST_FLOOD != 0 && self.tx_id > REQUEST_FLOOD {
    ...
    pub fn free_tx(&mut self, tx_id: u64) {
        if let Some(index) = self.transactions.iter().position(|tx| tx.id == tx_id + 1) {
            ...
            // Check flood limit
            if self.givenup && REQUEST_FLOOD != 0 && self.tx_id < REQUEST_FLOOD {
                self.givenup = false;
            }
        }
    }
    ...
    pub fn parse(&mut self, input: &[u8], direction: Direction) -> AppLayerResult {
        ...
        while rest.len() > 0 {
            match MODBUS_PARSER.parse(rest, direction.clone()) {
                Ok((input, Some(mut msg))) => {
                    match direction {
                        Direction::ToServer | Direction::Unknown => match self.find_response_and_validate(&mut msg) {
                            Some(tx) => {
                                tx.request = Some(msg);
                            }
                            None => {
                ...
                Ok((input, None)) => {
                    return AppLayerResult::incomplete((rest.len() - input.len()) as u32, 1)
                }
                Err(SawpError {
                    kind: SawpErrorKind::Incomplete(nom_6::Needed::Size(needed)),
                }) => {
                ...
                Err(SawpError {
                    kind: SawpErrorKind::Incomplete(nom_6::Needed::Unknown),
                }) => return AppLayerResult::incomplete((input.len() - rest.len()) as u32, 1),
...
pub extern "C" fn rs_modbus_parse_request(
    ...
    if input_len == 0 {
        if unsafe { AppLayerParserStateIssetFlag(pstate, APP_LAYER_PARSER_EOF) } > 0 {
...
#[no_mangle]
pub extern "C" fn rs_modbus_state_progress_completion_status(
    _direction: u8,
) -> std::os::raw::c_int {
    1
}
...
mod tests {
    use super::*;
    use sawp_modbus::{
        Data, Diagnostic, DiagnosticSubfunction, ErrorFlags, Exception, ExceptionCode,
        FunctionCode, Read, Write,
    };
    ...
    fn invalid_length_request() {
        let mut state = ModbusState::new();
        assert_eq!(
            AppLayerResult::incomplete(15, 2),
        ...
        assert_eq!(msg.flags, ErrorFlags::DATA_VALUE);
    }
    ...
    fn exceeded_length_request() {
        let mut state = ModbusState::new();
        assert_eq!(
            AppLayerResult::incomplete(0, 65523),
