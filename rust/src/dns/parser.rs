    ...
    loop {
        ...
        if len == 0x00 {
            ...
            match length_data(be_u8)(pos) as IResult<&[u8], _> {
                ...
                _ => {
                    return Err(nom::Err::Error(error_position!(pos, ErrorKind::OctDigit)));
                }
            }
    ...
    for _ in 0..count {
        match do_parse!(
            ...
            Ok((rem, val)) => {
                ...
                let result: IResult<&'a [u8], Vec<DNSRData>> = do_parse!(
                    data,
                    rdata: many_m_n!(1, n, complete!(call!(dns_parse_rdata, message, rrtype)))
                        >> (rdata)
                );
                match result {
                    Ok((_, rdatas)) => {
...
pub fn dns_parse_response<'a>(slice: &'a [u8]) -> IResult<&[u8], DNSResponse> {
    do_parse!(
        ...
        header: dns_parse_header
            >> queries: count!(call!(dns_parse_query, slice), header.questions as usize)
            >> answers: call!(dns_parse_answer, slice, header.answer_rr as usize)
            >> authorities: call!(dns_parse_answer, slice, header.authority_rr as usize)
            >> (DNSResponse {
                header: header,
                queries: queries,
                answers: answers,
...
fn dns_parse_rdata_mx<'a>(input: &'a [u8], message: &'a [u8]) -> IResult<&'a [u8], DNSRData> {
    ...
    do_parse!(
        input,
        be_u16 >> name: call!(dns_parse_name, message) >> (DNSRData::MX(name))
    )
}
...
fn dns_parse_rdata_txt<'a>(input: &'a [u8]) -> IResult<&'a [u8], DNSRData> {
    do_parse!(
        input,
        len: be_u8 >> txt: take!(len) >> (DNSRData::TXT(txt.to_vec()))
    )
}
...
pub fn dns_parse_request<'a>(input: &'a [u8]) -> IResult<&[u8], DNSRequest> {
    do_parse!(
        input,
        header: dns_parse_header
            >> queries: count!(call!(dns_parse_query, input), header.questions as usize)
            >> (DNSRequest {
                header: header,
                queries: queries,
            })
