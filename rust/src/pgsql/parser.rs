...
// Possible values: ERROR, FATAL, or PANIC// TODO Address the fact that the non-localizable version of Severity field has different values, in case of a notice
...
do_parse!(
    data: many_till!(call!(parse_error_response_field), tag!("\0"))
    >> (data.0)
));
...
// TODO remember that the Severity field has different values, in case of a Notice response
named!(
