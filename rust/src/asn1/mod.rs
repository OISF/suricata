...
impl Asn1 {
    ...
    fn check(&self, ad: &DetectAsn1Data) -> Result<Option<Asn1Check>, Asn1CheckError> {
        for obj in &self.0 {
            let res = Asn1::check_object_recursive(obj, ad, ad.max_frames)?;
            if res.is_some() {
                return Ok(res);
            }
        }
    ...
    fn from_slice(input: &'static [u8], ad: &DetectAsn1Data) -> Result<Asn1, Asn1DecodeError>{
        ...
        while !rest.is_empty() {
            if results.len() >= ad.max_frames {
                return Err(Asn1DecodeError::MaxFrames);
            }

            let res = parse_ber_recursive(rest, ad.max_frames);
...
#[no_mangle]
pub(crate) unsafe extern "C" fn rs_asn1_checks(ptr: *const Asn1, ad_ptr: *const DetectAsn1Data) -> u8 {
