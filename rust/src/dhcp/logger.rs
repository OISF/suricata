...
impl DHCPLogger {
    ...
    pub fn log(
        &self,
        tx: &DHCPTransaction,
    ) -> Result<Option<JsonBuilder>, Box<dyn std::error::Error>> {
        if !self.do_log(tx) {
            return Ok(None);
        }
        ...
        js.set_string("client_mac", &format_addr_hex(&header.clienthw.to_vec()))?;
        ...
        for option in options {
            ...
            match &option.option {
                ...
                &DHCPOptionWrapper::Generic(ref option) => match code {
                    ...
                    DHCP_OPT_TYPE => {
                        self.log_opt_type(&mut js, option)?;
                    }
                    ...
                    DHCP_OPT_PARAMETER_LIST => {
                        if self.extended {
                            self.log_opt_parameters(&mut js, option)?;
                        }
                    }
                    DHCP_OPT_DNS_SERVER => {
                        if self.extended {
                            self.log_opt_dns_server(&mut js, option)?;
                        }
                    }
                    DHCP_OPT_ROUTERS => {
                        if self.extended {
                            self.log_opt_routers(&mut js, option)?;
                        }
                    }
                    _ => {}
                },
...
pub extern "C" fn rs_dhcp_logger_log(
    ...
    if let Ok(Some(js)) = logger.log(tx) {
        Box::into_raw(Box::new(js))
    } else {
        std::ptr::null_mut()
    }
}
