...
fn extract_parsed_email_from_response(
    response: &ImapMessage, command: Vec<u8>, direction: u8,
    ...
    {
        if let Some(email_data) = fetch.get_email() {
...
fn response_body_truncated(response: &ImapMessage) -> bool {
    matches!(
        &response.message,
        ImapMessageType::Untagged {
            fetch_data: Some(fetch),
            ..
        } if fetch.body_truncated
    )
}

fn update_tx_with_response_email(
    tx: &mut ImapTransaction, response: &ImapMessage, command: Vec<u8>, direction: u8,
) {
    ...
    if tx.parsed_email.is_none() {
        if let Some((parsed_email, too_many)) =
            extract_parsed_email_from_response(response, command, direction)
        {
            if too_many {
    ...
    if response_body_truncated(response) {
...
impl ImapState {
    ...
    fn parse_request(&mut self, flow: *mut Flow, stream_slice: StreamSlice) -> AppLayerResult {
        ...
        if self.request_gap {
            if imap_parse_message(input).is_err() {
                return AppLayerResult::ok();
            }
        ...
        while !start.is_empty() {
            ...
            if should_consume_literal {
                if let Some(ref mut literal) = self.pending_literal {
                    let bytes_needed = literal.remaining() as usize;
                    if start.len() >= bytes_needed {
                        let is_single_chunk = literal.bytes_consumed == 0;
                        literal.consume_chunk(&start[..bytes_needed], IMAP_MAX_BODY_SIZE);
                        let literal_truncated = literal.truncated;
                        ...
                        let res = email.as_ref().map(|e| {
                            let cmd = if let Some(tx) =
                                self.transactions.iter().rev().find(|tx| !tx.complete)
                            {
                                extract_command_from_requests(&tx.requests)
                            } else {
                                Vec::new()
                            };
                            build_parsed_email(e, cmd, STREAM_TOSERVER)
                        });
                        ...
                        if let Ok((remaining, _)) = crlf::<_, Error<_>>.parse(start) {
                            start = remaining;
                        }
                        continue;
                    } else {
                        literal.consume_chunk(start, IMAP_MAX_BODY_SIZE);
                        return AppLayerResult::ok();
                    }
                }
            }
                ...
                Ok((rem, request)) => {
                    ...
                    if let Some((size, is_plus)) = setup_literal {
                        self.pending_literal =
                            Some(LiteralInfo::new(size, is_plus, IMAP_MAX_BODY_SIZE));
                    }
                }
                ...
                Ok((rem, response)) => {
                    ...
                    if let Some(ref tag) = response.tag {
                        if let Some(tx) = self.find_request(tag) {
                            ...
                            let cmd = extract_command_from_requests(&tx.requests);
                            update_tx_with_response_email(tx, &response, cmd, STREAM_TOCLIENT);
...
fn register_pattern_probe() -> i8 {
    unsafe {
        ...
        // Depth 17 = max 6-char tag + space + len("CAPABILITY").
        if SCAppLayerProtoDetectPMRegisterPatternCI(
            IPPROTO_TCP,
            ALPROTO_IMAP,
            b" CAPABILITY\0".as_ptr() as *const c_char,
            17,
            0,
