...
static IMAP_MAX_HEADERS: usize = 512;
...
) -> (ImapParsedEmail, bool, bool) {
    let body_too_large = email.email_body.len() > IMAP_MAX_BODY_SIZE;
    let body = if body_too_large {
        email.email_body[..IMAP_MAX_BODY_SIZE].to_vec()
    } else {
                ...
                if let Some(ref mut literal) = self.pending_literal {
                    ...
                    if start.len() >= bytes_needed {
                        ...
                        if let Some(tx) = self.transactions.iter_mut().rev().find(|tx| !tx.complete)
                        {
                            ...
                            if let Some((parsed_email, too_many, body_too_large)) = res {
                                if too_many {
                    ...
                    if let Some(ref tag) = response.tag {
                        if let Some(tx) = self.find_request(tag) {
                            ...
                            if tx.parsed_email.is_none() {
                                ...
                                let res = extract_parsed_email_from_response(
                                    &response,
                                    cmd,
                                    STREAM_TOCLIENT,
                                );
