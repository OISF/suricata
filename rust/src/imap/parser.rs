...
fn parse_response_data_with_literals(i: &[u8]) -> IResult<&[u8], (Vec<u8>, Vec<LiteralContext>)> {
    ...
    loop {
        ...
        if let Some((prefix, literal_size)) = detect_trailing_literal(line_content) {
            ...
            let open = result.iter().filter(|&&c| c == b'(').count();
            let close = result.iter().filter(|&&c| c == b')').count();

            if open > 0 && open == close {
                return Ok((after_crlf, (result, literal_ctxs)));
            }

            if after_crlf.is_empty() || (open == 0 && close == 0) {
                return Ok((after_crlf, (result, literal_ctxs)));
            }
...
mod tests {
    ...
    fn test_parse_email_headers_simple() {
        ...
        assert_eq!(headers.get("from"), Some(&vec!["sender@example.com".to_string()]));
        assert_eq!(
            headers.get("to"),
    ...
    fn test_parse_email_headers_folded() {
        ...
        assert_eq!(
            headers.get("subject"),
            Some(&vec!["This is a very long subject that spans multiple lines".to_string()])
        );
        assert_eq!(rem, b"\r\nBody");
    }
    ...
    fn test_parse_email_content() {
        ...
        assert_eq!(result.headers.get("subject"), Some(&vec!["Hello".to_string()]));
