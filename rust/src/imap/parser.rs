...
use nom8::branch::alt;
use nom8::bytes::complete::{tag, tag_no_case, take, take_till, take_while, take_while1};
use nom8::character::complete::{char, crlf, digit1, space0, space1};
use nom8::combinator::{map, map_res, opt, value};
...
pub(super) const IMAP_MAX_BODY_SIZE: usize = 10 * 1024 * 1024;
pub(super) const IMAP_MAX_HEADERS: usize = 512;
pub(super) const IMAP_MAX_LINE_SIZE: usize = 8 * 1024;
...
impl FetchData {
    pub fn get_email(&self) -> Option<EmailData> {
        let mut email: Option<EmailData> = None;
        let mut header_count = 0usize;
        for part in &self.body_parts {
            if let Some(em) = &part.email {
                match email {
                    None => {
                    ...
                    Some(ref mut m) => {
                        ...
                        if !em.email_body.is_empty() {
                            m.email_body.extend_from_slice(&em.email_body);
                        }
                    }
                }
            }
...
impl LiteralInfo {
    pub fn new(size: u32, is_literal_plus: bool, retain_limit: usize) -> Self {
        Self {
            size,
            is_literal_plus,
            bytes_consumed: 0,
            buffer: Vec::with_capacity(std::cmp::min(size as usize, retain_limit)),
            truncated: size as usize > retain_limit,
        }
    }
    ...
    pub fn consume_chunk(&mut self, chunk: &[u8], retain_limit: usize) -> usize {
        let consumed = std::cmp::min(chunk.len(), self.remaining() as usize);
        let chunk = &chunk[..consumed];
        let retain_left = retain_limit.saturating_sub(self.buffer.len());
        ...
        self.bytes_consumed = self.bytes_consumed.saturating_add(consumed as u32);
        consumed
    }
}
...
fn parse_response_data_with_literals(
    i: &[u8], retain_limit: usize,
) -> IResult<&[u8], (Vec<u8>, Vec<LiteralContext>)> {
    ...
    let mut literal_retain_left = retain_limit;

    loop {
        let (after_line, line_content) = take_till(is_line_ending).parse(rem)?;
        ...
        if let Some((prefix, literal_size)) = detect_trailing_literal(line_content) {
            ...
            let (after_crlf, _) = crlf.parse(after_line)?;

            let (after_literal, literal_data): (&[u8], &[u8]) =
                take(literal_size as usize).parse(after_crlf)?;
            let retain_len = std::cmp::min(literal_data.len(), literal_retain_left);
            let retained_literal = literal_data[..retain_len].to_vec();
            literal_retain_left -= retain_len;
            result.extend_from_slice(&retained_literal);

            literal_ctxs.push(LiteralContext {
                prefix: curr_prefix.clone(),
                literal_data: retained_literal,
                truncated: retain_len < literal_data.len(),
            });
...
fn parse_untagged_response(i: &[u8]) -> IResult<&[u8], ImapMessage> {
    ...
    let (seq_number, keyword, rem) = if first_token.iter().all(|c| c.is_ascii_digit()) {
        let (i, _) = space1.parse(i)?;
        let (i, kw) = take_while1(|c: u8| c.is_ascii_alphanumeric()).parse(i)?;
    ...
    let (i, data, fetch_data, raw_line, line_truncated) = if is_fetch {
        let first_line_end = i.iter().position(|&c| c == b'\r').unwrap_or(i.len());
        let raw_len = start.len() - i.len() + first_line_end;
        let (mut raw_line, line_truncated) = cap_raw_line(&start[..raw_len]);

        let (i, (data, literal_ctxs)) = parse_response_data_with_literals(i, IMAP_MAX_BODY_SIZE)?;

        // When FETCH spans multiple lines due to literals, raw_line only
        // captures the first line and misses the closing parenthesis.
        if !literal_ctxs.is_empty()
            && raw_line.len() < IMAP_MAX_LINE_SIZE
            && raw_line.last() != Some(&b')')
        {
            raw_line.push(b')');
        }
        ...
        let raw_len = start.len() - i.len() - 2;
        let (raw_line, line_truncated) = cap_raw_line(&start[..raw_len]);
        (i, data, None, raw_line, line_truncated)
    };
...
pub fn parse_literal_specifier(i: &[u8]) -> IResult<&[u8], (u32, bool)> {
    let (i, _) = char('{').parse(i)?;
    let (i, size_bytes) = digit1.parse(i)?;
...
pub fn parse_email_content(i: &[u8]) -> IResult<&[u8], EmailData> {
    let (rem, parsed_headers) = parse_email_headers(i)?;
    let headers_len = i.len() - rem.len();
    let (body, _) = crlf.parse(rem)?;
...
mod tests {
    ...
    fn test_merged_email_header_and_text() {
        let i = b"* 1 FETCH (BODY[HEADER] {24}\r\nFrom: test@example.com\r\n BODY[TEXT] {12}\r\nHello World!)\r\n";
        let (_, msg) = parse_response(i).unwrap();
        match msg.message {
            ImapMessageType::Untagged { fetch_data, .. } => {
                let fetch = fetch_data.unwrap();
                let merged = fetch.get_email().expect("Should have merged email");
                assert_eq!(
