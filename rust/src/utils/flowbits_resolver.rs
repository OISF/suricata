...
pub unsafe extern "C" fn SCCreateNodeEdgeDirectedGraph(
    graph: *mut c_void, iid: u32, ntype: bool, write_cmd: bool, cmd: u8, sig_nidx: u32, sid: u32,
) -> i64 {
    let g = &mut *(graph as *mut StableDiGraph<SCGNode, u8>);

    let node_idx;
    if let Some(nidx) = get_or_create_node(g, iid, ntype, sid) {
        node_idx = nidx;
    } else {
    ...
    let sidx = NodeIndex::from(sig_nidx);
    ...
    if write_cmd {
        ...
        match g.try_update_edge(sidx, node_idx, cmd) {
            ...
            Ok(e) => {
                SCLogNotice!("Created an edge from {:?} -> {:?}", sid, node_idx);
                let g = &mut *(graph as *mut StableDiGraph<SCGNode, u8>);
        ...
        if cmd == 3 { /* isnotset is an independent READ command, leave it out of the graph */
            /* return any positive value to indicate there was no error */
            return 1;
        }
        match g.try_update_edge(node_idx, sidx, cmd) {
            /* edge from flowbit to signature */
            Ok(_) => {
                SCLogNotice!("Created an edge from {:?} -> {:?}", node_idx, sidx);
            }
            Err(GraphError::EdgeIxLimit) => {
                SCLogError!("Error adding edge; Graph is at full capacity");
                return -2;
...
fn check_cycle_update_graph(graph: &mut StableDiGraph<SCGNode, u8>, cur_e: EdgeIndex) -> i8
{
    ...
    for scc in sccs {
        SCLogNotice!("Current scc: {:?}", scc);
        scc_set = scc.iter().copied().collect();
        if scc.len() <= 2 {
            if scc.len() == 2 {
                debug_validate_bug_on!(!(graph[scc[0]].ntype ^ graph[scc[1]].ntype));
                /* Remove self cycle. */
                let flowbit_edge = graph.edge_indices().find(|&edge_idx| {
                    if let Some((src, tgt)) = graph.edge_endpoints(edge_idx) {
                        // Check if edge is within SCC and has target weight
                        scc_set.contains(&src) &&
                        scc_set.contains(&tgt) &&
                        graph.edge_weight(edge_idx) == Some(&4)
                    } else {
                        false
                    }
                });
                if let Some(e) = flowbit_edge {
                    SCLogNotice!("Found the flowbit edge causing cycle. Removing now..");
                    graph.remove_edge(e);
                }
            }
            continue;  // skip single SCCs
        }
            ...
            if let Some((src, tgt)) = graph.edge_endpoints(edge_idx) {
                if scc_set.contains(&src) && scc_set.contains(&tgt) {
                    if let Some(weight) = graph.edge_weight(edge_idx) {
                        ...
                        SCLogNotice!("edge_wts updated to: {:?}", edge_wts);
                    }
                }
            }
        }
    ...
    let mut deduped_wts: Vec<u8> = edge_wts.into_iter().filter(|x| seen.insert(*x)).collect();
    deduped_wts.retain(|&x| (x != 3 && x != 4)); // add assert for 3
    SCLogNotice!("deduped_wts: {:?}", deduped_wts);

    /* This means that the cycle was created by edges of varying weights */
    if deduped_wts.len() > 1 {
        graph.remove_edge(cur_e);
        return 0;
    } else if deduped_wts.is_empty() {
        /* There must have been self loop. This is because invalid flowbit combinations on the same
         * flowbit are not rejected. */
        SCLogNotice!("Non-performant flowbit combinations were found");
        return 0;
    }
    ...
    let sids: Vec<_> = scc_set.into_iter().filter(|x| !graph[*x].ntype).map(|a| (graph[a].sid)).collect();
    SCLogNotice!("Cyclic dependency found between flowbits from signatures: {:?}", sids);
    return -1;
}
...
fn get_or_create_node(
    g: &mut StableDiGraph<SCGNode, u8>, iid: u32, ntype: bool, sid: u32,
) -> Option<NodeIndex> {
...
fn bfs_tree_dag(g: &mut StableDiGraph<SCGNode, u8>, sorted_sid_list: &mut [u32]) -> i8 {
    ...
    let nidx;
    if let Some(idx) = get_or_create_node(g, u32::MAX, false, u32::MAX) {
        nidx = idx;
    } else {
        SCLogError!("Error adding node; Graph is at full capacity");
        return -1;
