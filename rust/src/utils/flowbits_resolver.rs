...
/// Function to create and empty directed Graph
#[no_mangle]
...
pub unsafe extern "C" fn SCCreateNodeEdgeDirectedGraph(
    ...
    if ntype {
        ...
        if cmd {
            ...
            match g.try_update_edge(sidx, node_idx, ()) {
                ...
                Ok(_) => {
                    SCLogDebug!("Created an edge from {:?} -> {:?}", sidx, node_idx);
                }
    ...
    SCLogDebug!("node count: {:?}", g.node_count());
...
fn normalize_graph(g: &mut StableDiGraph<SCGNode, ()>) {
    ...
    for (from, to) in map_new_edges {
        /* Don't create an edge between a node and itself, that creates a cycle */
        if from != to && g.find_edge(from, to).is_none() {
            SCLogDebug!("map_new_edges -- from: {:?}; to: {:?}", from, to);
            g.add_edge(from, to, ());
        }
    }

    for node in fb_nodes_list {
        SCLogDebug!("Removing node {:?}", node);
...
fn bfs_tree_dag(g: &mut StableDiGraph<SCGNode, ()>, sorted_sid_list: &mut [u32]) -> i8 {
