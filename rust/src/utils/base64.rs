...
pub fn decode_rfc4648(decoder: &mut Decoder, input: &[u8], output: &mut [u8], decoded_bytes: &mut u32, max_decoded: u32) -> Result<()>
{
    let mut i = input;
    let mut offset = 0;
    let mut stop = false;
...
/// Only called by the from_base64 transform from C
pub fn decode_rfc2045_c(decoder: &mut Decoder, input: &[u8], output: &mut [u8], decoded_bytes: &mut u32) -> Result<()>
{
    let mut i = input;
    let mut offset = 0;
    while !i.is_empty() {
...
/// Called by the pure rust implementation of MIME parser
