...
pub enum Base64Mode {
     ...
     * */
    RFC2045, /* SPs are allowed during transfer but must be skipped by Decoder */
    Strict,
...
pub fn decode_rfc4648(decoder: &mut Decoder, input: &[u8], max_decoded: u32) -> Result<Vec<u8>> {
    let mut i = input;
    let mut maxlen = (((decoder.nb as usize + i.len()) * 3) / 4) as u32;
    ...
    while !i.is_empty() {
        while decoder.nb < 4 && !i.is_empty() {
            if base64_map(i[0]).is_ok() || i[0] == b'=' {
                ...
                while decoder.nb < 4 && offset < maxlen as usize {
