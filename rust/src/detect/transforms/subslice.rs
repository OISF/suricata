...
static mut G_BOOL_TRUNCATE: bool = true;
...
fn subslice_do_parse(i: &str) -> Option<DetectTransformSubsliceData> {
    ...
    match parts.as_slice() {
        [offset] => {
            ...
            Some(DetectTransformSubsliceData { offset, nbytes: None })
        }
        [offset, nbytes] => {
            let offset = offset.parse::<isize>().ok()?;
            let nbytes = nbytes.parse::<isize>().ok()?;
            if nbytes == 0 {
                return None;
            }
...
fn check_truncate() -> bool {
    unsafe {
        return G_BOOL_TRUNCATE;
    }
}
...
fn subslice_apply<'a>(data: &'a [u8], ctx: &DetectTransformSubsliceData) -> Option<&'a [u8]> {
    ...
    let end = match ctx.nbytes {
        ...
        Some(e) if e > len && !check_truncate() => return None,
        Some(e) => e + start,
    };
    ...
    if start >= data.len() || end > data.len() && !check_truncate() {
        return None;
    }
...
pub unsafe extern "C" fn DetectTransformSubsliceRegister() {
    ...
    G_BOOL_TRUNCATE = conf_get_bool("subslice.truncate");
}
...
mod tests {
    ...
    fn test_subslice_parse_invalid() {
        ...
        assert!(subslice_do_parse("0").is_none());
