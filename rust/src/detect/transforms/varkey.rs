...
pub(super) struct VarKeyLocation {
...
pub(super) fn var_key_bytes<'a>(input: &'a [u8], loc: &VarKeyLocation) -> Option<&'a [u8]> {
...
fn parse_var_loc(input: &str) -> IResult<&str, VarKeyLocation> {
    ...
    Ok((rest, VarKeyLocation { nbytes, offset }))
}
...
/// `nbytes` is zero.
pub(super) fn parse_var_spec(s: &str) -> Option<VarKeyLocation> {
    match parse_var_loc(s) {
...
mod tests {
    ...
    #[test]
    fn test_parse_var_spec_zero_nbytes() {
        assert_eq!(parse_var_spec("0 0"), None);
    }
