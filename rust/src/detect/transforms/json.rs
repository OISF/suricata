        ...
        match state {
            ...
            JsonParseState::Escape => {
                ...
                match i {
                    ...
                    b'"' | b'\\' => {
                        // keep required escapes for \x22 and \x5c
                        // see rfc 8259 definition of unescaped
                        output[nb] = b'\\';
                        nb += 1;
                        output[nb] = i;
                        nb += 1;
                ...
                if n < 3 {
                    ...
                    if enc < 0x80 {
                        match enc {
                            ...
                            0x00..=0x1F => {
                                // keep unicode escape for these control characters, as they are not valid in JSON strings
                                output[nb] = b'\\';
                                nb += 1;
                                output[nb] = b'u';
                                nb += 1;
                        ...
                        nb += 1;
                    } else if !(0xd800..=0xdbff).contains(&enc) {
...
mod tests {
    ...
    fn test_json_decode_transform() {
        ...
        buf = Vec::new();
        buf.extend_from_slice(b"\"keep escape\\tindent\"");
        let mut out = vec![0; buf.len()];
        let nb = json_decode_transform_do(&buf, &mut out);
        assert_eq!(&out[..nb as usize], b"\"keep escape\\tindent\"");
        ...
        buf = Vec::new();
        buf.extend_from_slice(b"\"unescape \\/ slash\"");
        let mut out = vec![0; buf.len()];
        let nb = json_decode_transform_do(&buf, &mut out);
        assert_eq!(&out[..nb as usize], b"\"unescape / slash\"");
        ...
        buf = Vec::new();
        buf.extend_from_slice(b"\"truncated \\u000");
        let mut out = vec![0; buf.len()];
        let nb = json_decode_transform_do(&buf, &mut out);
        assert_eq!(&out[..nb as usize], b"\"truncated \\u000");
    }
}
