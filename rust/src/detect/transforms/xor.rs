...
/// Parsed key specifier — either a hex string or an inline buffer location.
...
fn parse_key_part(s: &str) -> Option<XorKeySpec> {
    ...
    Some(XorKeySpec::Hex(s.to_string()))
}
...
unsafe extern "C" fn xor_setup(
    ...
    let key_source = match parsed.key_spec {
        XorKeySpec::Hex(ref s) => match try_parse_hex_key(s) {
        ...
        XorKeySpec::Var(loc) => {
            if loc.nbytes == 0 {
                SCLogError!("XOR transform: var key byte count must be at least 1");
                return -1;
            }
    ...
    let id_buf: Vec<u8> = match &key_source {
...
fn xor_transform_do(input: &[u8], output: &mut [u8], key: &[u8], xor_offset: usize) {
    ...
    for ((inp, out), k) in input[xor_offset..]
        .iter()
...
mod tests {
    ...
    fn test_parse_hex_key() {
        ...
        assert_eq!(r.key_spec, XorKeySpec::Hex("0a0DC8ff".into()));
    ...
    fn test_parse_offset_hex() {
        ...
        assert_eq!(r.key_spec, XorKeySpec::Hex("0d0ac8ff".into()));
    ...
    // Build an id_buf for a variable key the same way xor_setup does.
    ...
    // Build an id_buf for a static key the same way xor_setup does.
