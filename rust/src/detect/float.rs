...
mod tests {
    ...
    fn test_detect_parse_valid() {
        let (_, val) = detect_parse_float::<f64>("1.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 1.0, FloatCore::epsilon());
        // Default mode is equality
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeEqual);

        let (_, val) = detect_parse_float::<f32>("1.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 1.0, FloatCore::epsilon());
        // Default mode is equality
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeEqual);
        ...
        let (_, val) = detect_parse_float::<f64>(">= 1.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 1.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeGte);

        let (_, val) = detect_parse_float::<f64>("<= 1.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 1.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeLte);
        ...
        let (_, val) = detect_parse_float::<f64>("!= 1.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 1.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeNe);

        let (_, val) = detect_parse_float::<f64>("37.0-42.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 37.0, FloatCore::epsilon());
        assert_float_absolute_eq!(val.arg2, 42.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeRange);
        let (_, val) = detect_parse_float::<f64>("!37.0-42.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 37.0, FloatCore::epsilon());
        assert_float_absolute_eq!(val.arg2, 42.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeNegRg);
    }
    ...
    fn test_detect_match_valid() {
        ...
        // Default mode is equality
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeEqual);
        assert_eq!(true, detect_match_float(&val, 1.0));

        let (_, val) = detect_parse_float::<f64>("> 1.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 1.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeGt);
        assert_eq!(true, detect_match_float(&val, 1.1));
        assert_eq!(false, detect_match_float(&val, 1.0));

        let (_, val) = detect_parse_float::<f64>(">= 1.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 1.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeGte);
        assert_eq!(true, detect_match_float(&val, 1.0));
        assert_eq!(true, detect_match_float(&val, 1.5));
        assert_eq!(false, detect_match_float(&val, 0.5));

        let (_, val) = detect_parse_float::<f64>("<= 1.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 1.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeLte);
        assert_eq!(true, detect_match_float(&val, 1.0));
        assert_eq!(true, detect_match_float(&val, 0.5));
        assert_eq!(false, detect_match_float(&val, 1.5));

        let (_, val) = detect_parse_float::<f64>("< 1.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 1.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeLt);
        assert_eq!(true, detect_match_float(&val, 0.9));
        assert_eq!(false, detect_match_float(&val, 1.0));
        ...
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeEqual);
        assert_eq!(true, detect_match_float(&val, 1.0));
        assert_eq!(false, detect_match_float(&val, 0.9));
        assert_eq!(false, detect_match_float(&val, 1.1));
        ...
        assert_float_absolute_eq!(val.arg2, 42.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeRange);
        assert_eq!(true, detect_match_float(&val, 37.1));
        assert_eq!(true, detect_match_float(&val, 41.9));
        assert_eq!(false, detect_match_float(&val, 35.0));
        assert_eq!(false, detect_match_float(&val, 43.0));

        let (_, val) = detect_parse_float::<f64>("!37.0-42.0").unwrap();
        assert_float_absolute_eq!(val.arg1, 37.0, FloatCore::epsilon());
        assert_float_absolute_eq!(val.arg2, 42.0, FloatCore::epsilon());
        assert_eq!(val.mode, DetectFloatMode::DetectFloatModeNegRg);
        assert_eq!(true, detect_match_float(&val, 37.0));
        assert_eq!(true, detect_match_float(&val, 42.0));
        assert_eq!(true, detect_match_float(&val, 35.0));
        assert_eq!(true, detect_match_float(&val, 43.0));
        assert_eq!(false, detect_match_float(&val, 37.1));
        assert_eq!(false, detect_match_float(&val, 41.9));
    }
    ...
    fn do_match_test(val: &str, arg1: f64, arg1_cmp: f64, arg2: f64, mode: DetectFloatMode) {
        ...
        unsafe {
            ...
            assert_float_absolute_eq!((*val).arg1, arg1, FloatCore::epsilon());
            assert_float_absolute_eq!((*val).arg2, arg2, FloatCore::epsilon());
