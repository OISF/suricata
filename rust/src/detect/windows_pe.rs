...
pub(crate) fn parse_section_headers(data: &[u8], pe_offset: usize, num_sections: u16) -> Vec<SectionInfo> {
    ...
    if ctx_ref.needs_deep_parse {
        ...
        if let Some(ref needles) = ctx_ref.import_dlls {
            ...
            if needles.len() <= 4 {
                ...
                let import_set: HashSet<&str> =
                    cached_imports.iter().map(|s| s.as_str()).collect();
                for needle in needles.iter() {
        ...
        if let Some(ref needle) = ctx_ref.section_name {
            let found = sections.iter().any(|s| {
                section_name_str(&s.name).eq_ignore_ascii_case(needle.as_str())
            });
            if !found {
                return 0;
            }
