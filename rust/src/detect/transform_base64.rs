...
fn get_mode_value(value: &str) -> Result<DetectBase64Mode, ()> {
    let res = match value {
        "relax" => DetectBase64Mode::ModeRelax,
...
mod tests {
    ...
    fn test_parser_invalid() {
        ...
        assert!(parse_transform_base64("bytes 4, offset 70000, mode strict, mode relax").is_err());
    }
    ...
    fn test_parser_parse_partial_valid() {
        ...
        tbd.mode = DetectBase64Mode::ModeRelax;
        tbd.offset = 0;
        tbd.nbytes = 0;
        let (_, val) = parse_transform_base64("mode relax").unwrap();
        assert_eq!(val, tbd);
    }
    ...
    fn test_parser_parse_valid() {
        ...
        valid_test(
            "bytes 4, offset 3933, mode relax",
            4,
            "",
            3933,
            "",
            DetectBase64Mode::ModeRelax,
