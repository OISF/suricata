...
pub const TRANSFORM_FROM_BASE64_MODE_DEFAULT: scbase64::Base64Mode = scbase64::Base64Mode::RFC4648;
...
fn get_mode_value(value: &str) -> Option<scbase64::Base64Mode> {
    let res = match value {
        "rfc4648" => Some(scbase64::Base64Mode::RFC4648),
        "rfc2045" => Some(scbase64::Base64Mode::RFC2045),
        "strict" => Some(scbase64::Base64Mode::Strict),
        _ => None,
    };
...
fn parse_transform_base64(
    ...
    for value in values {
        ...
        match name {
            ...
            _ => {
                return Err(make_error(format!("unknown scbase64 keyword: {}", name)));
            }
        };
    }
