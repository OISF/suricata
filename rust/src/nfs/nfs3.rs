 ...
 * NFS3Transaction where the can be looked up per handle as part of the
 * Transaction lookup.
 */
...
pub struct NFS3Files {
    pub files_ts: SuricataFileContainer,
    pub files_tc: SuricataFileContainer,
    pub flags_ts: u16,
    pub flags_tc: u16,
}
...
impl NFS3Files {
    pub fn new() -> NFS3Files {
        NFS3Files {
            files_ts:SuricataFileContainer::default(),
            files_tc:SuricataFileContainer::default(),
            flags_ts:0,
            flags_tc:0,
        }
    }
    ...
    pub fn get(&mut self, direction: u8) -> (&mut SuricataFileContainer, u16)
    {
...
impl NFS3State {
    ...
    fn new_file_tx(&mut self, file_handle: &Vec<u8>, file_name: &Vec<u8>, direction: u8)
        -> (&mut NFS3Transaction, &mut SuricataFileContainer, u16)
    {
        let mut tx = self.new_tx();
    ...
    fn get_file_tx_by_handle(&mut self, file_handle: &Vec<u8>, direction: u8)
        -> Option<(&mut NFS3Transaction, &mut SuricataFileContainer, u16)>
    {
        ...
        let tcp_buffer = match self.tcp_buffer_ts.len() {
            ...
            _ => {
                ...
                if self.tcp_buffer_ts.len() + i.len() > 1000000 {
                    ...
                    SCLogNotice!("parse_tcp_data_ts: TS buffer exploded {} {}", self.tcp_buffer_ts.len(), i.len()); //return 1;
                };
    ...
    fn getfiles(&mut self, direction: u8) -> * mut SuricataFileContainer {
        ...
        if direction == STREAM_TOCLIENT {
            &mut self.files.files_tc as *mut SuricataFileContainer
        } else {
            &mut self.files.files_ts as *mut SuricataFileContainer
        }
    }
...
#[no_mangle]
pub extern "C" fn rs_nfs3_getfiles(direction: u8, ptr: *mut NFS3State) -> * mut SuricataFileContainer {
