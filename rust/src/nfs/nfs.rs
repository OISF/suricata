...
{
    let state = cast_pointer!(state, NFSState);
    let file_flags = FileFlowToFlags(f, Direction::ToServer as u8);
    rs_nfs_setfileflags(Direction::ToServer as u8, state, file_flags);
...
{
    let state = cast_pointer!(state, NFSState);
    let file_flags = FileFlowToFlags(f, Direction::ToClient as u8);
    rs_nfs_setfileflags(Direction::ToClient as u8, state, file_flags);
    SCLogDebug!("parsing {} bytes of response data", input_len);
    let buf = std::slice::from_raw_parts(input, input_len as usize);
        ...
        1 => {
            ...
            match nfs_probe(slice, adirection.into()) {
                1 => {
                    ...
                    if direction & (Direction::ToServer as u8 | Direction::ToClient as u8) != adirection {
                        *rdir = adirection;
                    }
                    ALPROTO_NFS
                },
