...
impl NFSState {
    ...
    fn setfileflags(&mut self, direction: u8, flags: u16) {
        ...
        if direction == 1 {        // TODO change toclient
...
pub extern "C" fn rs_nfs_udp_state_get_event_info_by_id(_event_id: std::os::raw::c_int,
                                              event_name: *mut *const std::os::raw::c_char,
                                              event_type: *mut AppLayerEventType)
                                             -> i8
{
    unsafe {
        *event_name = b"NFS UDP event name (generic)\0".as_ptr() as *const std::os::raw::c_char;
        ...
        1 => {
            ...
            match nfs_probe(slice, adirection) {
                1 => {
                    ...
                    return unsafe { ALPROTO_NFS };
                },
                0 => { return ALPROTO_UNKNOWN },
...
pub extern "C" fn rs_nfs_probe(_f: *const Flow,
    ...
    -> AppProto
{
    if len < NFS_MIN_FRAME_LEN as u32 {
        return ALPROTO_UNKNOWN;
    }
    ...
    match nfs_probe(slice, direction) {
        1 => { return unsafe { ALPROTO_NFS } },
        -1 => { return unsafe { ALPROTO_FAILED } },
...
#[no_mangle]
pub extern "C" fn rs_nfs_udp_probe_ts(_f: *const Flow,
                               _direction: u8,
                               input: *const u8,
                               len: u32,
                               _: *mut u8)
...
#[no_mangle]
pub extern "C" fn rs_nfs_udp_probe_tc(_f: *const Flow,
                               _direction: u8,
                               input: *const u8,
                               len: u32,
                               _: *mut u8)
...
pub unsafe extern "C" fn rs_nfs_udp_register_parser() {
    ...
    let parser = RustParser {
        ...
        default_port: default_port.as_ptr(),
        ipproto: IPPROTO_UDP,
        probe_ts: Some(rs_nfs_udp_probe_ts),
        probe_tc: Some(rs_nfs_udp_probe_tc),
        min_depth: 0,
        max_depth: 16,
        state_new: rs_nfs_state_new,
        state_free: rs_nfs_state_free,
        ...
        get_events: Some(rs_nfs_state_get_events),
        get_eventinfo: Some(rs_nfs_state_get_event_info),
        get_eventinfo_byid : Some(rs_nfs_udp_state_get_event_info_by_id),
        localstorage_new: None,
        localstorage_free: None,
        get_files: Some(rs_nfs_getfiles),
        get_tx_iterator: Some(rs_nfs_state_get_tx_iterator),
    ...
    ) != 0
    {
        ...
        if AppLayerProtoDetectPPParseConfPorts(ip_proto_str.as_ptr(), IPPROTO_UDP as u8,
                parser.name, ALPROTO_NFS, 0, NFS_MIN_FRAME_LEN,  // TODO generic flag name
                rs_nfs_probe_ms, rs_nfs_probe_ms) == 0 {
            ...
            AppLayerProtoDetectPPRegister(IPPROTO_UDP as u8,
                default_port.as_ptr(), ALPROTO_NFS, 0,
                NFS_MIN_FRAME_LEN, STREAM_TOSERVER,
                    rs_nfs_udp_probe_ts, rs_nfs_udp_probe_tc);
        }
        if AppLayerParserConfParserEnabled(
            ip_proto_str.as_ptr(),
            parser.name,
