...
{
    let tx = cast_pointer!(tx, NFSTransaction);
    if Direction::from(direction) == Direction::ToServer && tx.request_done {
        SCLogDebug!("TOSERVER progress 1");
        return 1;
    } else if Direction::from(direction) == Direction::ToClient && tx.response_done {
        ...
        len: u32, rdir: *mut u8) -> AppProto
{
    ...
    match nfs_probe_dir(slice, &mut adirection) {
        1 => {
            if Direction::from(adirection) == Direction::ToServer {
