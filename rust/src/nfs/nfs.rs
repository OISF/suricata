...
impl NFSState {
    ...
    pub fn mark_response_tx_done(
        ...
        // First try to find a regular (non-file) transaction
        if let Some(mytx) = self.get_tx_by_xid(xid) {
            ...
            // Also check for file transactions (e.g., WRITE) that may not have response_done set yet
            if let Some(mytx) = self.transactions.iter_mut().find(|tx| tx.is_file_tx && tx.xid == xid && !tx.response_done) {
                mytx.tx_data.updated_tc = true;
                mytx.tx_data.updated_ts = true;
                mytx.response_done = true;
                mytx.rpc_response_status = rpc_status;
                ...
                SCLogDebug!(
                    "process_reply_record: file tx ID {} XID {:04X} REQUEST {} RESPONSE {}",
                    mytx.id,
                    mytx.xid,
                    mytx.request_done,
                    mytx.response_done
                ...
                //SCLogNotice!("process_reply_record: not TX found for XID {}", xid);
            }
        }
    }
            ...
            Some(tx) => {
                if let Some(NFSTransactionTypeData::FILE(ref mut tdf)) = tx.type_data {
                    ...
                    if is_last {
                        tdf.file_last_xid = r.hdr.xid;
                        tx.is_last = true;
                        // Don't set response_done here - wait for response to set it
                        tx.is_file_closed = true;
                    }
                    true
                } else {
