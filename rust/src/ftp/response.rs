...
pub struct FTPResponseLine {
    code: *mut c_char,     // Response code as a string (may be null)
...
fn parse_response_line(input: &str) -> Option<FTPResponseLine> {
    ...
    Some(FTPResponseLine {
        code: c_code.into_raw(),
        response: c_response.into_raw(),
        length: response.len(),
        truncated: false,
        total_size,
    })
...
pub unsafe extern "C" fn SCFTPFreeResponseLine(response: *mut FTPResponseLine) {
    ...
    if !response.code.is_null() {
        let _ = CString::from_raw(response.code);
    }

    if !response.response.is_null() {
        let _ = CString::from_raw(response.response);
    }
}
...
mod tests {
    ...
    fn test_parse_valid_response() {
        ...
        let code_cstr = unsafe { CStr::from_ptr(parsed.code) };
        let code_str = code_cstr.to_str().unwrap();
        assert_eq!(code_str, "220");
        let response_cstr = unsafe { CStr::from_ptr(parsed.response) };
