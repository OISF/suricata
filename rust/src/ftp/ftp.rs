...
lazy_static! {
    static ref FTP_COMMANDS: Vec<FtpCommand> = vec![
        ...
        FtpCommand::new("MAX", FtpRequestCommand::FTP_COMMAND_MAX),
    ];
}
    ...
    if let Some(val) = conf_get("app-layer.protocols.ftp.memcap") {
        if let Ok(v) = get_memval(val) {
            *memcap = v;
            ...
            SCLogWarning!("Invalid value {} for ftp.memcap; defaulting to {}", val, *memcap);
        }
    }
    if let Some(val) = conf_get("app-layer.protocols.ftp.max-tx") {
        if let Ok(v) = val.parse::<u32>() {
            *max_tx = v;
            ...
            SCLogWarning!("Invalid value {} for ftp.max-tx; defaulting to {}", val, *max_tx);
        }
    }
    ...
    if let Some(val) = conf_get("app-layer.protocols.ftp.max-line-length") {
        if let Ok(v) = get_memval(val) {
            *max_line_len = v as u32;
            ...
            SCLogWarning!("Invalid value {} for ftp.max-line-length; defaulting to {}", val, *max_line_len);
        }
    }
}
