...
fn parse_bytemath(input: &str) -> IResult<&str, DetectByteMathData, RuleParseError<&str>> {
    ...
    // Result is required; rvalue can be a variable or scalar value
    byte_math.result = CString::new(tmp_result).unwrap().into_raw();
    if (byte_math.flags & DETECT_BYTEMATH_FLAG_RVALUE_VAR) != 0 {
        byte_math.rvalue_str = CString::new(tmp_rvalue).unwrap().into_raw();
    }
...
pub unsafe extern "C" fn ScByteMathFree(ptr: *mut DetectByteMathData) {
    if !ptr.is_null() {
        ...
        if !bmd.result.is_null() {
            let _ = CString::from_raw(bmd.result as *mut c_char);
        }
        if !bmd.rvalue_str.is_null() {
            let _ = CString::from_raw(bmd.rvalue_str as *mut c_char);
        }
    }
}
