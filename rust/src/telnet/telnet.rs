...
pub struct TelnetState {
    ...
    request_specific_frame: i64,
    /// either control or data frame
    response_specific_frame: i64,
...
impl TelnetState {
    pub fn new() -> Self {
        Self {
            ...
            response_gap: false,
            request_frame: 0,
            request_specific_frame: 0,
            response_frame: 0,
            response_specific_frame: 0,
    ...
    fn parse_request(&mut self, flow: *const Flow, stream_slice: &StreamSlice, input: &[u8]) -> AppLayerResult {
        ...
        let mut start = input;
        while start.len() > 0 {
            if self.request_frame == 0 {
                let frame = Frame::new_ts(flow, stream_slice, start, -1 as i64, TelnetFrameType::Pdu as u8);
                self.request_frame = frame.frame_id;
            }
            if self.request_specific_frame == 0 {
                if let Ok((_, is_ctl)) = parser::peek_message_is_ctl(start) {
                    ...
                    self.request_specific_frame = f.frame_id;
                }
            }
            match parser::parse_message(start) {
                Ok((rem, request)) => {
                    ...
                    if self.request_frame > 0 {
                        frame_set_len(flow, 0, self.request_frame, consumed as i64);
                        self.request_frame = 0;
                    }
                    if self.request_specific_frame > 0 {
                        frame_set_len(flow, 0, self.request_specific_frame, consumed as i64);
                        self.request_specific_frame = 0;
                    }
    ...
    fn parse_response(&mut self, flow: *const Flow, stream_slice: &StreamSlice, input: &[u8]) -> AppLayerResult {
        ...
        while start.len() > 0 {
            if self.response_frame == 0 {
                let frame = Frame::new_tc(flow, stream_slice, start, -1 as i64, TelnetFrameType::Pdu as u8);
                self.response_frame = frame.frame_id;
            }
            if self.response_specific_frame == 0 {
                if let Ok((_, is_ctl)) = parser::peek_message_is_ctl(start) {
                    let frame = if is_ctl {
                    ...
                    self.response_specific_frame = frame.frame_id;
                }
            }
            ...
            match r {
                Ok((rem, response)) => {
                    ...
                    if self.response_frame > 0 {
                        frame_set_len(flow, 1, self.response_frame, consumed as i64);
                        self.response_frame = 0;
                    }
                    if self.response_specific_frame > 0 {
                        frame_set_len(flow, 1, self.response_specific_frame, consumed as i64);
                        self.response_specific_frame = 0;
                    }
                    ...
                    if let parser::TelnetMessageType::Data(d) = response {
                        match self.state {
                            ...
                            TelnetProtocolState::PasswdRecv => {
                                let message = std::str::from_utf8(&d).unwrap().to_string();
