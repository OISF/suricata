...
impl<T> List<T> {
    ...
    pub(crate) fn get_last(&self) -> Option<&T> {
    ...
    /// mutable reference.
    pub(crate) fn get_last_mut(&mut self) -> Option<&mut T> {
    ...
    pub(crate) fn replace(&mut self, idx: usize, value: T) -> Result<(), HtpStatus> {
    ...
    pub(crate) fn remove(&mut self, idx: usize) -> Result<(), HtpStatus> {
        if idx < self.elements.len() {
