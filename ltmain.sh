	      ...
	      if func_win32_import_lib_p "$dlprefile"; then
	        ...
	        $opt_dry_run || {
	          ...
	          eval "$NM $dlprefile 2>/dev/null | $global_symbol_pipe |
	            $SED -e '/I __imp/d' -e 's/I __nm_/D /;s/_nm__//' >> '$nlist.tmp'"
		  # work around bug in msys gcc; puts __nm_ symbols in import lib even for functions
	          eval "cat '$nlist.tmp' | sed -n -e '/^D /p' | sort > '$nlist.tmpD'"
	          eval "cat '$nlist.tmp' | sed -n -e '/^T /p' | sort > '$nlist.tmpT'"
	          eval "join -j 2 -o 1.1,1.2 '$nlist.tmpD' '$nlist.tmpT' | sed -e 's|^|/^|' -e 's|\$|/d|' > '$nlist.tmp.sed'"
		  eval "cat '$nlist.tmp' | sed -f '$nlist.tmp.sed' > '$nlist'"
	        }
