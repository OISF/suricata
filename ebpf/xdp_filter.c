...
static int __always_inline filter_vxlan(struct xdp_md *ctx, void *data, __u64 nh_off, void *data_end)
{
    __u16 vlan0 = 0;
    __u16 h_proto;
...
static int __always_inline filter_ipv4(struct xdp_md *ctx, void *data, __u64 nh_off, void *data_end, __u16 vlan0, __u16 vlan1)
{
    ...
    if (iph->protocol == IPPROTO_GRE) {
        ...
        if (data + nh_off + sizeof(struct iphdr) + sizeof(struct udphdr) + sizeof(struct vxlanhdr) + sizeof(struct ethhdr) <= data_end) {
            struct udphdr * uh = (struct udphdr *)(data + nh_off + sizeof(struct iphdr));
            if (uh->dest == VXLAN_PORT) {
                struct vxlanhdr * vh = (struct vxlanhdr *)(data + nh_off + sizeof(struct iphdr) + sizeof(struct udphdr));
